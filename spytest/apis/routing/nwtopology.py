...
class TOPOLOGY:
    ...
    def get_dut_loopback_ip_list(dut, addr_family, vrf=''):
        ...
        lpbk_ip_list = []
        for link_name, ip_data in nw_topo[dut][addr_family]['link'].items():
            if ip_data['type'] == 'LBK' :
                if 'ip' in ip_data.keys():
    ...
    def setup_base_testbed_topology():
        ...
        for from_dut_idx, from_dut in enumerate(nw_topo['dut_list'], start = 1):
    ...
    def find_spine_leaf_topo_in_dut_list(spine_list=[], leaf_list=[], save_path='yes'):
        ...
        for spine_dut in spine_list :
            ...
            if spine_path['found'] :
                ...
                for seg_idx, seg_data in spine_path['segment'].items():
    ...
    def find_fullmesh_topo_in_dut_list(dut_list=[], start_dut='', save_path='yes'):
        ...
        for segt_idx, segt_data in found_path['segment'].items():
    ...
    def get_topology_segment_links(segment_data, to_dut='', vrf=''):
        ...
        segment_links = []
        for link_idx, link_data in segment_data.items():
    ...
    def show_topo_path(path):
        ...
        if 'loopbacks' in path.keys():
            ...
            for dut, lbk_list in path['loopbacks'].items() :
                ...
                for lbk_name, lbk_data in lbk_list.items():
    ...
    def config_all_vrfs(config='yes'):
        ...
        for dut in dut_list :
            ...
            for vrf_name, vrf_data in nw_topo[dut]['vrf'].items():
                if vrf_name != 'default' :
