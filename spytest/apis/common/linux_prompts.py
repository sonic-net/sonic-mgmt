...
class Prompts(object):
    ...
    def __init__(self, model=None, logger=None, normal_user_mode=None):
        ...
        self.oyaml = None
        model = "linux" if not model else re.sub("_(ssh|terminal)$", "", model)
        ...
        # self.normal_user_prompt_default = self.normal_user_prompt_tmpl
        self.normal_user_prompt_default = None
    ...
    def get_normal_user_prompts(self, index):
        prompts = [self._get_prompt_for_mode(self.normal_user_mode, index=index)]
    ...
    def update_with_hostname_pattern(self, pattern, username, hostname, index=None):
        if re.search(r"{username}", pattern):
            pattern = re.sub(r"{username}", username, pattern)
        if re.search(r"{}", pattern):
            pattern = re.sub(r"{}", hostname, pattern)
        return pattern

    def update_with_hostname(self, username, hostname, index=None):
        retval = bool(not self.hostname or self.hostname == hostname)
        self.hostname = hostname
        for pattern in self.patterns:
            if isinstance(self.patterns[pattern], dict):
                self.update_with_hostname(username, hostname, self.patterns[pattern])
                continue
            self.patterns[pattern] = self.update_with_hostname_pattern(self.patterns[pattern], username, hostname, index)
            if index is not None:
                self.patterns["{}--{}".format(pattern, index)] = self.patterns[pattern]
        if self.normal_user_prompt_default:
            self.normal_user_prompt_default = self.update_with_hostname_pattern(self.normal_user_prompt_default, username, hostname, index)
        return retval
    ...
    def _get_prompt_for_mode(self, mode, ifname_type=None, index=0):
        ifname_type = self._check_ifname_type(ifname_type)
    ...
    def get_prompt_for_mode(self, mode, ifname_type=None, index=0, role=None):
        if mode == self.normal_user_mode:
            prompts = []
            if self.normal_user_prompt_default:
                prompts.append(self.normal_user_prompt_default)
            prompts.extend(self.get_normal_user_prompts(index))
            return "|".join(prompts)
        return self._get_prompt_for_mode(mode, ifname_type, index)
    ...
    def check_move_for_parent_of_from_mode(self, prompt, mode, **kwargs):
        if mode == "fp-dhcp6s-pool-config" or mode == "fp-dhcp-pool-config":
            if "pool_name" not in self.stored_values:
                self.stored_values["pool_name"] = kwargs["pool_name"]
                return False
            else:
                if self.stored_values["pool_name"] != kwargs["pool_name"]:
                    self.stored_values["pool_name"] = kwargs["pool_name"]
                    return True

        if mode == "fp-intf-config":
            prompt2 = prompt.replace("\\", "")
        ...
        # REMOVE-ME
        if mode == "fp-intf-config":
            if "interface" not in self.stored_values:
                self.stored_values["interface"] = kwargs["interface"]
                return False
            else:
                if self.stored_values["interface"] != kwargs["interface"]:
                    self.stored_values["interface"] = kwargs["interface"]
                    return True
            return True
        # REMOVE-ME
