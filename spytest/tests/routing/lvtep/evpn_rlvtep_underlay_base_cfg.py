...
from evpn_rlvtep import enable_debugs, disable_debugs
from evpn_rlvtep import setup_vxlan, setup_l2vni, setup_l3vni
from evpn_rlvtep import setup_mclag, setup_5549_mlag_underlay
from evpn_rlvtep import evpn_dict, retry_api, get_num_of_bfd_sessions_up
...
def setup_evpn_5549():
    ...
    for intf1,intf2 in zip(data.spine1_port_list1,data.spine2_port_list1):
       if evpn_dict['cli_mode'] == 'click':
           ...
           dict2 = {'local_as':data.bgp_spine_local_as[1],'config':'yes','config_type_list':["neighbor","connect"],'remote_as':'external','neighbor':intf2, \
               'addr_family':"ipv6",'connect':"1"}
           parallel.exec_parallel(True,data.spine_nodes_list,bgp_obj.config_bgp,[dict1,dict2])
    ...
    for port_lst1 in [data.leafs_spine1_port_lst1, data.leafs_spine2_port_lst1]:
       if evpn_dict['cli_mode'] == 'click':
          dict = {}
          for i in range(0,4):
              dict[i] ={'local_as':data.bgp_leaf_local_as[i],'config':'yes','config_type_list':["remote-as"],'remote_as':'external', \
                  'interface':port_lst1[i]}
          dict_lst = [dict[x] for x in range(0,4)]
          parallel.exec_parallel(True,data.leaf_nodes_list,bgp_obj.config_bgp,dict_lst)
          dict = {}
          for i in range(0,4):
    ...
    for j,port_lst1 in zip(range(0,2),[data.leafs_spine1_po_lst1, data.leafs_spine2_po_lst1]):
       if evpn_dict['cli_mode'] == 'click':
          ...
          dict = {}
          for i in range(0,4):
              dict[i] = {'local_as':data.bgp_leaf_local_as[i], 'config': 'yes',  'config_type_list':["neighbor","connect"], 'remote_as': data.bgp_spine_local_as[j], \
              'addr_family':"ipv6",'neighbor' : port_lst1[i],'connect':"1"}
          dict_lst = [dict[x] for x in range(0,4)]
          parallel.exec_parallel(True,data.leaf_nodes_list,bgp_obj.config_bgp,dict_lst)
    ...
    for ipv6_1,ipv6_2,j in zip(data.leaf_spine1_ipv6_list, data.leaf_spine2_ipv6_list,range(0,4)):
       dict1 = {'local_as':data.bgp_spine_local_as[0], 'config': 'yes',  'config_type_list':["neighbor"], 'remote_as':data.bgp_leaf_local_as[j], 'neighbor' : ipv6_1,'addr_family':"ipv6"}
       dict2 = {'local_as':data.bgp_spine_local_as[1], 'config': 'yes',  'config_type_list':["neighbor"], 'remote_as':data.bgp_leaf_local_as[j], 'neighbor' : ipv6_2,'addr_family':"ipv6"}
       parallel.exec_parallel(True,data.spine_nodes_list,bgp_obj.config_bgp,[dict1,dict2])
       ...
       dict1 = {'local_as':data.bgp_spine_local_as[0], 'config': 'yes', 'config_type_list':["advertise_all_vni"], 'remote_as':data.bgp_leaf_local_as[j], 'neighbor':ipv6_1}
       dict2 = {'local_as':data.bgp_spine_local_as[1], 'config': 'yes', 'config_type_list':["advertise_all_vni"], 'remote_as':data.bgp_leaf_local_as[j], 'neighbor':ipv6_2}
       parallel.exec_parallel(True,data.spine_nodes_list,evpn_api.config_bgp_evpn,[dict1,dict2])

    for ipv6_lst1,j in zip([data.spine1_ipv6_list,data.spine2_ipv6_list],range(0,2)):
       dict = {}
       for i in range(0,4):
           dict[i] = {'local_as':data.bgp_leaf_local_as[i], 'config': 'yes', 'config_type_list':["neighbor"], 'remote_as':data.bgp_spine_local_as[j], 'neighbor' : ipv6_lst1[i],'addr_family':"ipv6"}
       dict_lst = [dict[x] for x in range(0,4)]
       parallel.exec_parallel(True,data.leaf_nodes_list,bgp_obj.config_bgp,dict_lst)
       ...
       dict = {}
       for i in range(0,4):
           dict[i] = {'local_as':data.bgp_leaf_local_as[i], 'config': 'yes', 'config_type_list':["advertise_all_vni"], 'remote_as': data.bgp_spine_local_as[j], 'neighbor':ipv6_lst1[i]}
       dict_lst = [dict[x] for x in range(0,4)]
       parallel.exec_parallel(True,data.leaf_nodes_list,evpn_api.config_bgp_evpn,dict_lst)
    ...
    parallel.exec_parallel(True, evpn_dict["mlag_node_list"],
                           ipfeature.configure_loopback, [input,input])
    ...
    for i, dut, port in zip(range(0,4), data.leaf_nodes_list, data.leafs_spine1_port_lst1):
        ...
        if len(x[i]) == 0:
            st.error("########## IPv6 link local address not enabled for dut {} and port {} ##########".format(dut,port))
...
def debug_cmds2():
    ...
    ip.show_ip_route(vars.D3, "ipv4","vtysh", None)
    ip.show_ip_route(vars.D3, "ipv6","vtysh", None)
    ip.show_ip_route(vars.D6, "ipv4","vtysh", None)
    ip.show_ip_route(vars.D6, "ipv6","vtysh", None)


def config_evpn_lvtep_5549():
    setup_evpn_5549()
    setup_5549_mlag_underlay()
    setup_vxlan()
    setup_l2vni()
    setup_l3vni()
    setup_mclag()
