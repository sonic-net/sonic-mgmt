...
def test_ft_arp_entry_link_failure():
    ...
    # Verify dynamic arp entries
    st.log("Verifying the arp entries on the DUT.")
    if not poll_wait(arp_obj.verify_arp,15,dut1,data.t1d1_ip_addr,data.t1d1_mac_addr,vars.D1T1P1,cli_type=data.cli_type):
        st.report_fail("ARP_entry_dynamic_entry_fail", data.t1d1_ip_addr, dut1)
    st.log("Verifying the arp entries on the DUT")
    if not poll_wait(arp_obj.verify_arp,15,dut1,data.t2d1_ip_addr,data.t2d1_mac_addr,vars.D1T1P2,data.vlan_1,cli_type=data.cli_type):
        st.report_fail("ARP_entry_dynamic_entry_fail", data.t2d1_ip_addr, dut1)
