...
def evpn_underlay_hooks(request):
    ...
    api_list = [[create_stream],[create_evpn_config]]
...
@pytest.mark.cli
def test_FtOpSoRoEvpnRouterCli311(request):
    success = True
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterCli311")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterCli311")


@pytest.mark.ft
def test_FtOpSoRoEvpnRouterFt3211():
    success = True
    st.log("###Verify EVPN neighborship by removing and adding back a neighbor###")
    st.log("###remove a neighbor {} from leaf1###".format(evpn_dict["spine1"]["loop_ip_list"][0]))
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][0], neighbor=evpn_dict["spine1"]["loop_ip_list"][0],
         config_type_list=["activate"],remote_as=evpn_dict['spine1']['local_as'],config="no",local_as=evpn_dict['leaf1']['local_as'])

    st.log("###verify EVPN neighbor {} not present in leaf1###".format(evpn_dict["spine1"]["loop_ip_list"][0]))
    if Evpn.verify_bgp_l2vpn_evpn_summary(dut=evpn_dict["leaf_node_list"][0],
                                       neighbor=[evpn_dict["spine1"]["loop_ip_list"][0]]):
        st.error("EVPN neighbor {} is present which is NOT expected".format(evpn_dict["spine1"]["loop_ip_list"][0]))
        success=False
    else:
        st.log("EVPN neighbor {} not present as expected".format(evpn_dict["spine1"]["loop_ip_list"][0]))

    st.wait(15)
    st.log("###verify BGP neighbor {} in leaf1###".format(evpn_dict["spine1"]["loop_ip_list"][0]))
    if not ip_bgp.check_bgp_session(dut=evpn_dict["leaf_node_list"][0],
                    nbr_list=[evpn_dict["spine1"]["loop_ip_list"][0], evpn_dict["spine2"]["loop_ip_list"][0]],
                    state_list=['Established'] * 2):
        st.error("BGP neighbors are NOT in established state in leaf1")
        success = False
    else:
        st.log("BGP neighbors are in established state in leaf1 as expected")

    st.log("###add back the removed neighbor {} in leaf1###".format(evpn_dict["spine1"]["loop_ip_list"][0]))
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][0], neighbor=evpn_dict["spine1"]["loop_ip_list"][0],
         config_type_list=["activate"],remote_as=evpn_dict['spine1']['local_as'],config="yes",local_as=evpn_dict['leaf1']['local_as'])
    st.wait(5)
    st.log("###verify EVPN neighbor {} is present in leaf1###".format(evpn_dict["spine1"]["loop_ip_list"][0]))
    if not Evpn.verify_bgp_l2vpn_evpn_summary(dut=evpn_dict["leaf_node_list"][0],
                neighbor=[evpn_dict["spine1"]["loop_ip_list"][0]],updown=["up"]):
        st.error("EVPN neighbor {} is NOT present in leaf1".format(evpn_dict["spine1"]["loop_ip_list"][0]))
        success=False
    else:
        st.log("EVPN neighbor {} is up as expected".format(evpn_dict["spine1"]["loop_ip_list"][0]))

    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3211")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3211")


@pytest.mark.ft
def test_FtOpSoRoEvpnRouterFt3212(Ft32214_fixture):
    success = True
    ...
    st.log("### remove loopback interface {} from leaf3### ".format("Loopback2"))
    if evpn_dict['cli_mode'] == "klish":
        ip.delete_ip_interface(evpn_dict["leaf_node_list"][2],"Loopback2", evpn_dict["leaf3"]["loop_ip_list"][1], '32')
    ip.configure_loopback(dut=evpn_dict["leaf_node_list"][2],loopback_name="Loopback2",config="no")
    ...
    st.log("### add back the removed loopback interface {} in leaf3 ###".format("Loopback2"))
    if evpn_dict['cli_mode'] == "click":
        ip.configure_loopback(dut=evpn_dict["leaf_node_list"][2], loopback_name="Loopback2")
    if st.get_ui_type() in ["rest-put","rest-patch"]:
        ip.configure_loopback(dut=evpn_dict["leaf_node_list"][2], loopback_name="Loopback2")
    ip.config_ip_addr_interface(dut=evpn_dict["leaf_node_list"][2], interface_name="Loopback2",
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3212")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3212")

def test_FtOpSoRoEvpnRouterFt3242():
    success = True

    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test case")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    ...
    if not Evpn.verify_vxlan_evpn_remote_mac_id(dut=evpn_dict["leaf_node_list"][3],
                                ...
                                vlan=evpn_dict["leaf3"]["l3_vni_name_list"][0],identifier="all",
    ...
    '''
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3242")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3242")

def test_FtOpSoRoEvpnRouterFt3243():
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3243")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3243")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3243")

def test_FtOpSoRoEvpnRouterFt3241():
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3241")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    ip.create_static_route(evpn_dict["leaf_node_list"][2],static_ip=evpn_dict["ipv4_static_route"],
                           interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],vrf="Vrf1")

    ip.create_static_route(evpn_dict["leaf_node_list"][2],static_ip=evpn_dict["ipv6_static_route"],
                           interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],family="ipv6",vrf="Vrf1")
    ...
    ip.delete_static_route(evpn_dict["leaf_node_list"][2],next_hop="",static_ip=evpn_dict["ipv4_static_route"],
                           interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],vrf="Vrf1")

    ip.delete_static_route(evpn_dict["leaf_node_list"][2],next_hop="",static_ip=evpn_dict["ipv6_static_route"],
                           interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],family="ipv6",vrf="Vrf1")

    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3241")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3241")

def test_FtOpSoRoEvpnRouterFt3222_5():
    global vars
    tc_list = ["test_FtOpSoRoEvpnRouterFt3222","test_FtOpSoRoEvpnRouterFt3223","test_FtOpSoRoEvpnRouterFt3224",
                         "test_FtOpSoRoEvpnRouterFt3225","test_FtOpSoRoEvpnRouterFt3226"]
    tc_list_summary = ["Test VxLAN by removing and adding back RD for L2 VNI VLAN","Test VxLAN by modifying RD for L2 VNI VLAN",
                         "Test VxLAN by removing and adding back import RT for L2 VNI VLAN",
                         "Test VxLAN by configuring wrong import RT for L2 VNI VLAN",
                         "Test VxLAN by removing and adding back export RT for L2 VNI VLAN"]
    success = True

    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3222_5")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][2], config_type_list=["vni"],
                         vni=evpn_dict["leaf1"]['tenant_l2_vlan_list'][0],vni_rd="23:23",
    ...
    st.log("Step 2: Remove the manual L2 RD and verify the manual L2 RD is not received in remote node")
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][2], config_type_list=["vni"],
                         vni=evpn_dict["leaf1"]['tenant_l2_vlan_list'][0], vni_rd="23:23",
                         config="no", local_as=evpn_dict["leaf3"]['local_as'])

    Evpn.clear_bgp_evpn(evpn_dict["leaf_node_list"][2], "*", soft_dir="out")
    Evpn.clear_bgp_evpn(evpn_dict["leaf_node_list"][3], "*", soft_dir="in")

    if retry_api_false(Evpn.verify_bgp_l2vpn_evpn_route,evpn_dict["leaf_node_list"][3], rd='23:23',retry_count=2,delay=1):
        st.log(" Test Case Step 2 PASSED - Remove the manual entry and verify the manual L2 RD not found")
    else:
        st.log("Evpn routes with L2 rd 23:23 is still present after removing the manual L2 RD {}".format(evpn_dict["leaf_node_list"][2]))
        st.error("Testcase: {}, Summary: {} , result: Failed".format(tc_list[1], tc_list_summary[1]))
        success = False

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][2], config_type_list=["vni"],
                         vni=evpn_dict["leaf1"]['tenant_l2_vlan_list'][0], vni_unconfig="yes",
                         config="no", local_as=evpn_dict["leaf3"]['local_as'])

    st.log("Step 3a: Change the L2 Route-target to 23:23 and verify the same is reflected in the evpn routes")

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][2], config_type_list=["vni"],
                         vni=evpn_dict["leaf1"]['tenant_l2_vlan_list'][0], vni_both_rt="23:23",
                         config="yes", local_as=evpn_dict["leaf3"]['local_as'])

    if retry_api(Evpn.verify_bgp_l2vpn_evpn_vni_id,evpn_dict["leaf_node_list"][2],rt=['23:23','23:23'],
                   vni=evpn_dict["leaf1"]['tenant_l2_vlan_list'][0],retry_count=2,delay=1):
        st.log(" Test Case Step 3 PASSED - Change the L2 Route-target to 23:23 & verify through evpn routes")
    else:
        st.log("Evpn routes with route-target 23:23 is not present after adding it manually on {}".format(
            evpn_dict["leaf_node_list"][2]))
        st.error("Testcase: {}, Summary: {} , result: Failed".format(tc_list[2], tc_list_summary[2]))
        success = False

    st.log("Step 3b: Verify VxLAN tunnel status in leaf4 to leaf 3 is up since L2 RT mis-match but L3 RT is matched")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][3],
        ...
        st.log("VxLAN tunnel is not UP in leaf4 when L2 RT mis-match but L3 RT is matched")
        st.error("Testcase: {}, Summary: {} , result:Failed".format(tc_list[3], tc_list_summary[3]))
        success = False
    else:
        st.log(" Test Case Step 4 PASSED - Verify VxLAN tunnel status is incorrect when L2 RT mis-mtach but L3 RT is matched")

    st.log("Step 4a: Change the L3 Route-target to 50:50 and verify the same is reflected in the evpn routes")
    ...
    if retry_api(Evpn.verify_bgp_l2vpn_evpn_route_detail_type_prefix,evpn_dict["leaf_node_list"][3],
                         ...
                         rmac=evpn_dict["leaf_base_mac_list"][evpn_dict["leaf_node_list"][2]],retry_count=2,delay=1):
        st.log(" Test Case Step 3 PASSED - Change the L3 Route-target to 50:50 & verify through evpn routes")
    else:
        st.error("Testcase: {}, Summary: {} , result:Failed".format(tc_list[2], tc_list_summary[2]))
        success = False

    st.log("Step 4b: Verify VxLAN tunnel is deleted when L2 RT and L3 RT both are mis-macthed")
    ...
    st.log("Step 5: Revert back the L3 Route-target to auto and verify the same is reflected in the evpn routes")
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][2], config_type_list=["vrf_rd_rt"],vrf_name="Vrf1",
    ...
    if retry_api(Evpn.verify_bgp_l2vpn_evpn_route_detail_type_prefix,evpn_dict["leaf_node_list"][3],
                         ...
                         rvtep=evpn_dict["leaf3"]["loop_ip_list"][1],vni_id=evpn_dict["leaf3"]["l3_vni_list"][0],
                         rmac=evpn_dict["leaf_base_mac_list"][evpn_dict["leaf_node_list"][2]],retry_count=2,delay=1):
        st.log(" Test Case Step 5 PASSED - Remove the L3 Route-target 50:50 & verify auto L3 RT through evpn routes")
    else:
        st.log(" Test Case Step 5 FAILED - Remove the L3 Route-target 50:50 & verify auto L3 RT through evpn routes")
        st.error("Testcase: {}, Summary: {} , result: Failed".format(tc_list[2], tc_list_summary[2]))
        success = False

    st.log("Step 5a: Revert back the L2 Route-target to auto and verify the manual L2 RT is not reflected in the evpn routes")
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][2], config_type_list=["vni"],
                         vni=evpn_dict["leaf1"]['tenant_l2_vlan_list'][0], vni_both_rt="23:23",
                         config="yes", local_as=evpn_dict["leaf3"]['local_as'],vni_unconfig="yes")

    if retry_api_false(Evpn.verify_bgp_l2vpn_evpn_vni_id,evpn_dict["leaf_node_list"][2],rt=['23:23','23:23'],
                         vni=evpn_dict["leaf1"]['tenant_l2_vlan_list'][0],retry_count=2,delay=1):
        st.log("Step 5a PASSED - Verified RT 23:23 is not present after manual L2 RT removed")
    else:
        st.log("Step 5a Evpn routes with route-target 23:23 is present even after reverting back to auto RT on {}".format(
            evpn_dict["leaf_node_list"][2]))
        st.error("Testcase: {}, Summary: {} , result:Failed".format(tc_list[2], tc_list_summary[2]))
        success = False
    ...
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][3],
                                           ...
                                           rem_vtep_list=[evpn_dict["leaf3"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up'] * 1):
        st.log("Step 5b VxLAN tunnel is down even when auto L2 RT and auto L3 RT is restored")
        st.error("Testcase: {}, Summary: {} , result:Failed".format(tc_list[2], tc_list_summary[2]))
        success = False
    else:
        st.log(" Step 5b PASSED - Verify VxLAN tunnel is up with auto L2 and L3 RT")

    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt3222_5")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt3222_5")

'''
def test_FtOpSoRoEvpnRouterFt3221():
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3221")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt3221")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt3221")
'''

def test_FtOpSoRoEvpnRouterFt3244_4(Ft32214_fixture):
    tc_list = ["test_FtOpSoRoEvpnRouterFt3244","test_FtOpSoRoEvpnRouterFt3245","test_FtOpSoRoEvpnRouterFt3246",
                         "test_FtOpSoRoEvpnRouterFt3247"]
    ...
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3244_4")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    Evpn.verify_bgp_l2vpn_evpn_route_detail_type_prefix(dut=evpn_dict["leaf_node_list"][3],
                         prefix="[5]:[0]:[24]:["+ipv4_nw[0]+"]",rd=evpn_dict["leaf3"]["l3_vni_ip_list"][0],
    ...
    if not Evpn.verify_bgp_l2vpn_evpn_route_detail_type_prefix(dut=evpn_dict["leaf_node_list"][3],
                         prefix="[5]:[0]:[24]:["+ipv4_nw[0]+"]",rd=evpn_dict["leaf3"]["l3_vni_ip_list"][0],rt="50:50",
                         rvtep=evpn_dict["leaf3"]["loop_ip_list"][1],vni_id=evpn_dict["leaf3"]["l3_vni_list"][0],
                         rmac=evpn_dict["leaf_base_mac_list"][evpn_dict["leaf_node_list"][2]]):
    ...
    st.log("Step 4: Verify VxLAN tunnel status in leaf4 to leaf 3 is down since the route-target is not matching")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][3],
                         src_vtep=evpn_dict["leaf4"]["loop_ip_list"][1],
                         rem_vtep_list=[evpn_dict["leaf3"]["loop_ip_list"][1]],
                         exp_status_list=['oper_up'] * 1):
        st.log("VxLAN tunnel is not up - L3 RT mis-match but L3 RT matches")
        st.error("Testcase: {}, Summary: {} , result:Failed".format(tc_list[2], tc_list_summary[2]))
        success = False
    else:
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt3244_4")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt3244_4")

def test_FtOpSoRoEvpnRouterFt3229_10(Ft32215_fixture):
    global vars

    st.log("### Test VxLAN by clearing EVPN neighbor ###")
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3229_10")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    output = filter_and_select(output, {"updown"}, match={"neighbor" : evpn_dict["spine1"]["loop_ip_list"][0]})
    ...
    st.log("clear the EVPN table soft-in in Leaf3")
    Evpn.clear_bgp_evpn(vars.D5, "*", soft_dir="in")
    Evpn.clear_bgp_evpn(vars.D5, "*", soft_dir="out")
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt3229_10")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt3229_10")

def test_FtOpSoRoEvpnRouterFt3214(Ft3214_fixture):
    hdrMsg("TC ID: FtOpSoRoEvpnRouterFt3214; TC SUMMARY : Test VxLAN by creating vlan after VxLAN map for that VLAN")
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3214")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    if verify_traffic():
        st.log("traffic verification passed on new vlan")
    else:
        success=False
        st.error("traffic verification failed on new vlan")
    current_stream_dict["stream"] = stream_dict["l2_3214"]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3214")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3214")
...
@pytest.mark.ft
def test_FtOpSoRoEvpnRouterFt32211(Ft32211_fixture):
    global vars
    hdrMsg("TC ID: test_FtOpSoRoEvpnRouterFt32211; TC SUMMARY : Test VxLAN by removing redistribute connected from BGP")
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32211")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    ############################################################################################
    hdrMsg(" \n####### Delete network command in bgp ##############\n")
    ############################################################################################
    Bgp.config_bgp(dut=evpn_dict["leaf_node_list"][3], local_as=evpn_dict["leaf4"]["local_as"], config_type_list=["network"],network=evpn_dict["leaf4"]["loop_ip_list"][1]+'/32',config='no')
    ...
    ############################################################################################
    hdrMsg(" \n####### Reconfigure network command in bgp ##############\n")
    ############################################################################################
    Bgp.config_bgp(dut=evpn_dict["leaf_node_list"][3], local_as=evpn_dict["leaf4"]["local_as"], config_type_list=["network"],network=evpn_dict["leaf4"]["loop_ip_list"][1]+'/32',config='yes')
    ...
    current_stream_dict["stream"] = [stream_dict["all"]]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32211")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32211")
...
@pytest.mark.ft
def test_FtOpSoRoEvpnRouterFt32214(Ft32214_fixture):
    global vars
    hdrMsg("TC ID: FtOpSoRoEvpnRouterFt32214; TC SUMMARY : Test VxLAN by deleting and adding back BGP EVPN address family")
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32214")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    ############################################################################################
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], neighbor=evpn_dict["spine1"]["loop_ip_list"][0],
                         config_type_list=["activate"], config="no", local_as=evpn_dict['leaf4']['local_as'],
                         remote_as=evpn_dict['spine1']['local_as'])
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], neighbor=evpn_dict["spine2"]["loop_ip_list"][0],
                         config_type_list=["activate",], config="no", local_as=evpn_dict['leaf4']['local_as'],
                         remote_as=evpn_dict['spine2']['local_as'])
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], neighbor=evpn_dict["spine2"]["loop_ip_list"][0],
                         config_type_list=['advertise_all_vni'], config="no", local_as=evpn_dict['leaf4']['local_as'],
                         remote_as=evpn_dict['spine2']['local_as'])
    st.wait(3)
    ...
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], config='yes', local_as=evpn_dict["leaf4"]["local_as"],
        config_type_list=["activate"],remote_as=evpn_dict['spine1']['local_as'],
        neighbor=evpn_dict["spine1"]["loop_ip_list"][0])

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], config='yes', local_as=evpn_dict["leaf4"]["local_as"],
        config_type_list=["advertise_all_vni"],remote_as=evpn_dict['spine1']['local_as'],
        neighbor=evpn_dict["spine1"]["loop_ip_list"][0])

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], config='yes', local_as=evpn_dict["leaf4"]["local_as"],
        config_type_list=["activate"],remote_as=evpn_dict['spine2']['local_as'],
        neighbor=evpn_dict["spine2"]["loop_ip_list"][0])

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], config='yes', local_as=evpn_dict["leaf4"]["local_as"],
        config_type_list=["advertise_all_vni"],remote_as=evpn_dict['spine2']['local_as'],
        neighbor=evpn_dict["spine2"]["loop_ip_list"][0])
    st.wait(3)
    ...
    ############################################################################################
    Evpn.verify_bgp_l2vpn_evpn_summary(dut=evpn_dict["leaf_node_list"][3], neighbor=[evpn_dict["spine1"]["loop_ip_list"][0], evpn_dict["spine2"]["loop_ip_list"][0]],updown=["up", "up"],identifier=evpn_dict["leaf4"]["loop_ip_list"][0])
    ...
    current_stream_dict["stream"] = stream_dict["all"]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32214")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32214")
...
def test_FtOpSoRoEvpnRouterFt32218(Ft32218_fixture):
    global vars
    hdrMsg("TC ID: FtOpSoRoEvpnRouterFt32218; TC SUMMARY : Test VxLAN by shutting down the links towards downstream")
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32218")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                       src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                       rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                       exp_status_list=['oper_up']):
    ...
    mac_list = filter_mac_list(vars.D5, tg_dict['dut_6_mac_pattern'])
    if len(mac_list) == 0:
        st.log("PASS: Remote macs are not installed in D5 as expected.")
    else:
    ...
    ############################################################################################
    if verify_traffic():
        st.log("PASS: traffic verification passed after trigger")
    else:
        success = False
        st.error("FAIL: traffic verification failed after trigger")
        debug_traffic(evpn_dict["leaf_node_list"][2], evpn_dict["leaf_node_list"][3])
    current_stream_dict["stream"] = stream_dict["l2_32218"]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32218")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32218")
...
@pytest.mark.ft
def test_FtOpSoRoEvpnRouterFt32219_2(Ft32219_fixture):
    hdrMsg("TC ID: FtOpSoRoEvpnRouterFt32219; TC SUMMARY : Test VxLAN for static mac;\
             TC ID: FtOpSoRoEvpnRouterFt32221; TC SUMMARY : Test VxLAN for static MAC behavior across leaf nodes;")

    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32219_2")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    start_traffic(action="stop", stream_han_list=stream_dict["l2_32219_1"])
    st.log("start traffic from first tgen port of DUT5 and DUT6")
    ...
    start_traffic(action="stop", stream_han_list=stream_dict["l2_32219_2"])
    st.log("start traffic from first tgen port of DUT5 and DUT6")
    start_traffic(stream_han_list=stream_dict["l2_32219_1"])
    ...
    if verify_traffic():
        ...
        st.error("FAIL: Traffic verification failed for static mac")
        debug_traffic(evpn_dict["leaf_node_list"][2],evpn_dict["leaf_node_list"][3])
    current_stream_dict["stream"] = stream_dict["l2_32219_1"]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32219")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32219")
...
def Ft32219_fixture(request,evpn_underlay_hooks):
    ...
    if current_stream_dict.get("stream",None):
        start_traffic(action='stop',stream_han_list=current_stream_dict["stream"])
        current_stream_dict.pop("stream")
    utils.exec_all(True, [[Mac.clear_mac, dut] for dut in evpn_dict["leaf_node_list"]])
...
def test_FtOpSoRoEvpnRouterFt32410_4(Ft32215_fixture):
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32410_4")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    hdrMsg("Covering TCs:FtOpSoRoEvpnRouterFt32410,FtOpSoRoEvpnRouterFt32411")
    hdrMsg("Covering TCs:FtOpSoRoEvpnRouterFt32413,FtOpSoRoEvpnRouterFt32414")
    hdrMsg("FtOpSoRoEvpnRouterFt32410 -Verify best route export to EVPN routing table")
    ...
    ip.delete_static_route(evpn_dict["leaf_node_list"][2],next_hop="",static_ip=evpn_dict["ipv6_static_route"],
                         interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],family="ipv6",vrf="Vrf1")
    ip.create_static_route(evpn_dict["leaf_node_list"][2],static_ip="123.1.1.0/25",
                         interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],vrf="Vrf1")
    ip.create_static_route(evpn_dict["leaf_node_list"][2],static_ip="1230::/98",
                         interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],family="ipv6",vrf="Vrf1")

    hdrMsg("FtOpSoRoEvpnRouterFt32414 - Verify the new best route is exported via EVPN")
    ...
    hdrMsg("FtOpSoRoEvpnRouterFt32413 -Verify local VRF route is preferred over the imported route")

    st.log("### Step 7: Verify same ipv4 and ipv6 route locally in Leaf 4 ###")
    ip.create_static_route(evpn_dict["leaf_node_list"][3],static_ip="123.1.1.0/25",
                                interface="Vlan"+evpn_dict['leaf4']["tenant_l3_vlan_list"][0],vrf="Vrf1")
    ip.create_static_route(evpn_dict["leaf_node_list"][3],static_ip="1230::/98",
                                interface="Vlan"+evpn_dict['leaf4']["tenant_l3_vlan_list"][0],family="ipv6",vrf="Vrf1")
    ...
    ip.delete_static_route(evpn_dict["leaf_node_list"][3],next_hop="",static_ip="123.1.1.0/25",
                                interface="Vlan"+evpn_dict['leaf4']["tenant_l3_vlan_list"][0],vrf="Vrf1")

    ip.delete_static_route(evpn_dict["leaf_node_list"][3],next_hop="",static_ip="1230::/98",
                                interface="Vlan"+evpn_dict['leaf4']["tenant_l3_vlan_list"][0],family="ipv6",vrf="Vrf1")

    st.log("### Step 8: Remove static ipv4 and ipv6 route under vrf Vrf1 in Leaf 3 ###")
    ip.delete_static_route(evpn_dict["leaf_node_list"][2],next_hop="",static_ip="123.1.1.0/25",
                                interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],vrf="Vrf1")

    ip.delete_static_route(evpn_dict["leaf_node_list"][2],next_hop="",static_ip="1230::/98",
                                interface="Vlan"+evpn_dict['leaf3']["tenant_l3_vlan_list"][0],family="ipv6",vrf="Vrf1")

    hdrMsg("FtOpSoRoEvpnRouterFt32411 -Verify flap of BGP VRF routes")
    ...
    st.log("### Step 11: No shutdown the TG link to flap tenant routes in Leaf 4 ###")
    ...
    for i in range(10):
        ipv4_entry = ip.fetch_ip_route(dut=evpn_dict["leaf_node_list"][2],
                                ...
                                shell="vtysh",family="ipv4",select=None,match={"ip_address":evpn_dict["leaf3"]["l3_tenant_ip_net"][0]})
        if len(ipv4_entry) == 0:
            st.wait(1, "The IPv4 route entry {} still not exist in show ip route".format(evpn_dict["leaf3"]["l3_tenant_ip_net"][0]))
            if i == 9:
    ...
    st.log("Start L3 IPv4 and IPv6 traffic from Leaf 3 to Leaf 4 for new L3 VNI")
    tg_dict["tg"].tg_arp_control(handle=stream_dict["l3_v4host_1"], arp_target='all')
    tg_dict["tg"].tg_arp_control(handle=stream_dict["l3_v4host_2"], arp_target='all')
    tg_dict["tg"].tg_arp_control(handle=stream_dict["l3_v6host_1"], arp_target='all')
    tg_dict["tg"].tg_arp_control(handle=stream_dict["l3_v6host_2"], arp_target='all')
    ...
    st.log("Step 12: Verify L3 traffic from Leaf 3 to Leaf 4 after L3 VNI change")
    if verify_traffic():
    ...
    current_stream_dict["stream"] = stream_dict["l3"]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32410_4")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32410_4")


def test_FtOpSoRoEvpnRouterFt3248_2(Ft3248_fixture):
    hdrMsg("FtOpSoRoEvpnRouterFt3248_2 - Test change/deletion of L3 VNI associated with VRF/AF")
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3248_2")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    st.log("Step 1: Verify L3 traffic from Leaf 3 to Leaf 4 after L3 VNI change")
    if verify_traffic():
        st.log("traffic verification passed after L3 VNI config changes")
    else:
        success=False
    ...
    hdrMsg("Remove Current L3 VNI config - to verify test case id FtOpSoRoEvpnRouterFt3249")
    ...
    hdrMsg("Add New L3 VNI config - to verify test case id FtOpSoRoEvpnRouterFt3248")
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3248_2")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3248_2")


def test_FtOpSoRoEvpnRouterFt32220(Ft32215_fixture):
    global vars

    tg = tg_dict['tg']
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32220")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt32220")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt32220")


def test_FtOpSoRoEvpnRouterFt3213(Ft32215_fixture):
    global vars
    success = True
    if evpn_dict['cli_mode'] != "klish":
        hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3213")
        if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt3213")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt3213")

def test_FtOpSoRoEvpnRouterFt32421_3(Ft32215_fixture):
    global vars
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32421_3")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    tc_list = ["test_FtOpSoRoEvpnRouterFt32421","test_FtOpSoRoEvpnRouterFt32422","test_FtOpSoRoEvpnRouterFt32423"]
    ...
    hdrMsg(" STEP:2  Remove both L2vni config from all the nodes")
    ...
    dict = {'evpn_type_3_prefix': "[3]:[0]:[32]:[" + ipv4_nw_1 + "]", 'status_code': "*>",
            'next_hop': evpn_dict["leaf3"]["loop_ip_list"][1]}
    if  not utils_obj.retry_parallel(Evpn.verify_bgp_l2vpn_evpn_route_type_multicast, dut_list=[dut],
    ...
    st.log("Verify Ipv4 traffic only after removing L2vni & Ipv6 L3vni prefix routes from Leaf3 to Leaf4")
    ...
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], config_type_list=["vrf_vni"],
                         vrf_name=evpn_dict["leaf3"]["vrf_name_list"][0],local_as=evpn_dict['leaf4']['local_as'],
                         l3_vni_id=evpn_dict["leaf3"]["l3_vni_list"][0], vtep_name=evpn_dict["leaf4"]["vtepName"])
    ...
    if Evpn.verify_bgp_l2vpn_evpn_route_type_multicast(dut=evpn_dict["leaf_node_list"][3],
        ...
        st.log("As expected IMR route is not present after adding the L3 VNI config")

    st.log("Verify Ipv4 traffic & verify only after removing flapping the L2vni &  L3vni prefix routes from Leaf3 to Leaf4")
    ...
    st.log("Verify Ipv6 traffic only after removing L2vni & Ipv4 L3vni prefix routes from Leaf3 to Leaf4")
    ...
    st.log("Verify Ipv6 traffic & verify only after removing flapping the L2vni &  Ipv4 L3vni prefix routes from Leaf3 to Leaf4")
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt32421_3")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt32421_3")

def test_FtOpSoRoEvpnRouterFt32412(Ft32215_fixture):
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32412")
    if not retry_api(Evpn.verify_vxlan_tunnel_status, evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up'], delay=2, retry_count=3):
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    hdrMsg("FtOpSoRoEvpnRouterFt32412 - Verify importing Routes via EVPN")
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32412")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32412")



def test_FtOpSoRoEvpnRouterFt32217_2(Ft32217_fixture):
    global vars
    hdrMsg("TC ID: FtOpSoRoEvpnRouterFt32217; TC SUMMARY : Test VxLAN for MAC aging and clear MAC \
            TC ID: FtOpSoRoEvpnRouterFt32226; TC SUMMARY : Test Remote macs are not aged out")
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32217_2")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    current_stream_dict["stream"] = stream_dict["l2_32218"]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32217_2")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32217_2")
...
def test_FtOpSoRoEvpnRouterFt32216(Ft32215_fixture):
    global vars
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32216")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                       src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                       rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                       exp_status_list=['oper_up']):
    ...
    if not retry_api(Evpn.verify_vxlan_evpn_remote_mac_id, vars.D6, vni=evpn_dict["leaf4"]["tenant_l2_vlan_list"][0],
                     vlan=evpn_dict["leaf4"]["tenant_l2_vlan_name_list"][0],
                     rvtep=evpn_dict["leaf3"]["loop_ip_list"][1], type="dynamic",
                     identifier=evpn_dict["leaf3"]["loop_ip_list"][1], mac=evpn_dict["leaf3"]["tenant_mac_l2_colon"],
                     retry_count=3, delay=2):
        st.log("########## Mac {} not present in leaf4 as expected, passed "
               "##########".format(evpn_dict["leaf3"]["tenant_mac_l2_colon"]))
    else:
        success = False
        st.error("########## FAIL: Mac {} still present in leaf4, failed "
               "##########".format(evpn_dict["leaf3"]["tenant_mac_l2_colon"]))
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt32216")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt32216")

def test_FtOpSoRoEvpnRouterFt32426(Ft32215_fixture):
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32426")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    hdrMsg("FtOpSoRoEvpnRouterFt32426 - Verify traffic with duplicate ip across VRF")
    ...
    st.log("Step 2: Send IPv4 and IPv6 L3 traffic b/w L3 to L4 using duplicate ip")
    ...
    start_traffic(stream_han_list=stream_dict["l3_32426"])
    ...
    start_traffic(action="stop",stream_han_list=stream_dict["l3_32426"])
    ...
    if not ip.verify_ip_route(dut=evpn_dict["leaf_node_list"][3],
        ...
        st.log("Step 3 PASSED - Verify Vrf1 Leaf 4 tenant IPv4 prefix route in Vrf2")
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32426")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32426")

def test_FtOpSoRoEvpnRouterFt32417(Ft32215_fixture):
    global vars
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32417")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    hdrMsg("FtOpSoRoEvpnRouterFt32417 - Verify asymmetric IRB Forwarding in default VRF")
    ...
    start_traffic(stream_han_list=stream_dict["l3_32417"], port_han_list=[tg_dict['d5_tg_ph1'],tg_dict['d6_tg_ph2']])
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32417")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32417")


def test_FtOpSoRoEvpnRouterFt32215(Ft32215_fixture):
    global vars
    hdrMsg("TC ID: FtOpSoRoEvpnRouterFt32215; TC SUMMARY : Test VxLAN by deleting and adding back router BGP ")
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32215")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    hdrMsg(" \n####### Delete BGP in leaf4 ##############\n")
    ############################################################################################
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3],  config='remove_vni', vrf_name=evpn_dict["leaf4"][
        "vrf_name_list"][0],l3_vni_id=evpn_dict["leaf4"]["l3_vni_list"][0],config_type_list=["vrf_vni"],
    ...
    ############################################################################################
    hdrMsg(" \n####### Verify tunnel to leaf4 gets deleted in leaf3 ##############\n")
    ############################################################################################

    vtep_list = Evpn.get_tunnel_list(vars.D5)
    if evpn_dict["leaf4"]["loop_ip_list"][1] in vtep_list:
        st.error("FAIL: Tunnel to leaf4 still not deleted." )
        success=False
    else:
        st.log("PASS: Tunnel to leaf4 deleted")
    ...
    ############################################################################################
    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], config='yes',local_as=evpn_dict["leaf4"]["local_as"],
        vrf_name=evpn_dict["leaf4"]["vrf_name_list"][0],l3_vni_id=evpn_dict["leaf4"]["l3_vni_list"][0],
        config_type_list=["vrf_vni"], vtep_name=evpn_dict["leaf4"]["vtepName"])

    Bgp.config_bgp(dut=evpn_dict["leaf_node_list"][3],config='yes', local_as=evpn_dict["leaf4"]["local_as"], config_type_list=["neighbor","ebgp_mhop","update_src"],remote_as=evpn_dict['leaf4']['rem_as_list'][0],ebgp_mhop='2',neighbor=evpn_dict["spine1"]["loop_ip_list"][0],redistribute='connected',update_src=evpn_dict['leaf4']['loop_ip_list'][0],router_id=evpn_dict['leaf4']['loop_ip_list'][0])
    Bgp.config_bgp(dut=evpn_dict["leaf_node_list"][3],config='yes', local_as=evpn_dict["leaf4"]["local_as"], config_type_list=["neighbor", "ebgp_mhop","update_src"], remote_as=evpn_dict['leaf4']['rem_as_list'][1],ebgp_mhop='2',neighbor=evpn_dict["spine2"]["loop_ip_list"][0],redistribute='connected',update_src=evpn_dict['leaf4']['loop_ip_list'][0],router_id=evpn_dict['leaf4']['loop_ip_list'][0])
    Bgp.config_bgp(dut=evpn_dict["leaf_node_list"][3],config='yes', local_as=evpn_dict["leaf4"]["local_as"], config_type_list=["network"],network=evpn_dict["leaf4"]["loop_ip_list"][1]+'/32')

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], config='yes',local_as=evpn_dict["leaf4"]["local_as"],
        remote_as=evpn_dict["spine1"]["local_as"],
        config_type_list=["activate"], neighbor=evpn_dict["spine1"]["loop_ip_list"][0])

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3], config='yes',local_as=evpn_dict["leaf4"]["local_as"],
        remote_as=evpn_dict["spine2"]["local_as"],
        config_type_list=["activate"], neighbor=evpn_dict["spine2"]["loop_ip_list"][0])

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3],config='yes',local_as=evpn_dict["leaf4"]["local_as"],
        config_type_list=["advertise_all_vni"], neighbor=evpn_dict["spine1"]["loop_ip_list"][0])

    Evpn.config_bgp_evpn(dut=evpn_dict["leaf_node_list"][3],  config='yes', local_as=evpn_dict["leaf4"]["local_as"],
        config_type_list=["advertise_all_vni"], neighbor=evpn_dict["spine2"]["loop_ip_list"][0])
    ...
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                    ...
                                    evpn_dict["leaf4"]["loop_ip_list"][1],evpn_dict["leaf1"]["loop_ip_list"][1]],
                                    exp_status_list=['oper_up'] * 3):
        st.error("VxLAN tunnel is NOT UP in leaf3")
        success=False
    else:
        st.log("VxLAN tunnel is UP as expected in leaf3")
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32215")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32215")
...
@pytest.fixture(scope="function")
def Ft32223_fixture(request, evpn_underlay_hooks):
    utils.exec_all(True, [[arp.clear_arp_table, evpn_dict["leaf_node_list"][2]],
                          [arp.clear_arp_table, evpn_dict["leaf_node_list"][3]]])
    yield

    hdrMsg("### CLEANUP for 32223 ###")
    if current_stream_dict.get("stream",None):
        start_traffic(action='stop',stream_han_list=current_stream_dict["stream"])
        current_stream_dict.pop("stream")


def test_FtOpSoRoEvpnRouterFt32223(Ft32223_fixture):
    global vars
    ...
    st.log("########## check which spine routes the traffic ##########")
    output = ip.show_ip_route(dut=vars.D5)
    output = filter_and_select(output, ["ip_address"],{"ip_address": evpn_dict["leaf4"]["loop_ip_list"][1]+"/32",
                                             "nexthop": evpn_dict["spine1"]["loop_ip_list"][0]})
    ...
    output1 = filter_and_select(output1, ["ip_address"],{"ip_address": evpn_dict["leaf4"]["loop_ip_list"][1]+"/32",
                                             "nexthop": evpn_dict["spine2"]["loop_ip_list"][0]})
    if output:
        st.log("########## traffic is routed via Spine1 ##########")
        ...
        output3 = filter_and_select(output3, ["ip_address"],
                                    ...
                                    "nexthop": evpn_dict["spine2"]["loop_ip_list"][0]})
        if output3:
            st.log("########## FAIL: routes are NOT pointing to other spine2, failed ##########")
        else:
            st.log("########## routes are pointing to other spine2, passed ##########")
    elif output1:
        st.log("########## traffic is routed via Spine2 ##########")
        shut_dut = evpn_dict["leaf_node_list"][2]
        ...
        output3 = filter_and_select(output3, ["ip_address"],
                                    ...
                                    "nexthop": evpn_dict["spine1"]["loop_ip_list"][0]})
        if output3:
            st.log("########## FAIL: routes are NOT pointing to other spine1, failed ##########")
        else:
            st.log("########## routes are pointing to other spine1 now, passed ##########")
    else:
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt32223")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt32223")
...
def test_FtOpSoRoEvpnRouterFt32212(Ft32212_fixture):
    global vars
    ...
    if output:
        st.log("########## traffic is routed via Spine1 ##########")
        shut_dut = evpn_dict["leaf_node_list"][2]
        shut_neigh = evpn_dict["spine1"]["loop_ip_list"][0]
        st.log("########## clear BGP neighbor {} in leaf3 ##########".format(shut_neigh))
        bgp.clear_ip_bgp_vtysh(dut=shut_dut,value=shut_neigh)
    elif output1:
        st.log("########## traffic is routed via Spine2 ##########")
        shut_dut = evpn_dict["leaf_node_list"][2]
        shut_neigh = evpn_dict["spine2"]["loop_ip_list"][0]
        st.log("########## clear BGP neighbor {} in leaf3 ##########".format(shut_neigh))
        bgp.clear_ip_bgp_vtysh(dut=shut_dut, value=shut_neigh)
    else:
        st.log("########## FAIL: route entry to {} not exists ########"
                 "##".format(evpn_dict["leaf4"]["loop_ip_list"][1] + "/32"))
    retry_api(ip_bgp.check_bgp_session, evpn_dict["bgp_node_list"][4],
              nbr_list=[evpn_dict["spine1"]["loop_ip_list"][0],
                        evpn_dict["spine2"]["loop_ip_list"][0]],
              state_list=['Established'] * 2, retry_count=10, delay=2)
    retry_api(ip_bgp.check_bgp_session, evpn_dict["bgp_node_list"][5],
              nbr_list=[evpn_dict["spine1"]["loop_ip_list"][0],
                        evpn_dict["spine2"]["loop_ip_list"][0]],
              state_list=['Established'] * 2, retry_count=10, delay=2)
    if verify_traffic():
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt32212")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt32212")
...
def Ft32213_fixture(request, evpn_underlay_hooks):
    ...
    bgp.config_bgp(dut=evpn_dict["leaf_node_list"][2], local_as=evpn_dict["leaf3"]["local_as"],
               config='yes', config_type_list=["max_path_ebgp"], max_path_ebgp='2')
    utils.exec_all(True, [[arp.clear_arp_table, evpn_dict["leaf_node_list"][2]],
                          [arp.clear_arp_table, evpn_dict["leaf_node_list"][3]]])
    yield
    ...
    st.log("restore max path config")
    bgp.config_bgp(dut=evpn_dict["leaf_node_list"][2], local_as=evpn_dict["leaf3"]["local_as"],
               config='no', config_type_list=["max_path_ebgp"], max_path_ebgp='2')
    if current_stream_dict.get("stream",None):
        start_traffic(action='stop',stream_han_list=current_stream_dict["stream"])
        current_stream_dict.pop("stream")


def test_FtOpSoRoEvpnRouterFt32213(Ft32213_fixture):
    global vars

    tg = tg_dict['tg']
    success = True
    st.log("########## Test VxLAN by clearing BGP session with max path more than one ##########")
    ...
    st.log("########## verify traffic received in first tgen port of Leaf3 and Leaf4 ##########")
    if verify_traffic():
        st.log("########## traffic verification passed before clearing BGP session with maxpath more than"
               " one ##########")
    else:
        success = False
    ...
    st.log("########## clear BGP neighbor {} in leaf3 ##########".format(evpn_dict["spine1"]["loop_ip_list"][0]))
    bgp.clear_ip_bgp_vtysh(dut=evpn_dict["leaf_node_list"][2],value=evpn_dict["spine1"]["loop_ip_list"][0])
    retry_api(ip_bgp.check_bgp_session, evpn_dict["leaf_node_list"][2],
              nbr_list=[evpn_dict["spine1"]["loop_ip_list"][0],
                        evpn_dict["spine2"]["loop_ip_list"][0]],
              state_list=['Established'] * 2, retry_count=10, delay=2)
    if verify_traffic():
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt32213")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt32213")


def test_FtOpSoRoEvpnRouterFt32427_2():
    global vars
    success = True
    tc_list = ["test_FtOpSoRoEvpnRouterFt32427","test_FtOpSoRoEvpnRouterFt32428"]
    ...
    hdrMsg("TC ID: test_FtOpSoRoEvpnRouterFt32426_2; TC SUMMARY : Verify slow path Forwarding Scenarios over IP Fabric")
    ipv4_nw = evpn_dict["leaf3"]["l3_vni_ip_net"][0].split("/")
    ipv6_nw = evpn_dict["leaf3"]["l3_vni_ipv6_net"][0].split("/")


    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before proceeding with the test case test_FtOpSoRoEvpnRouterFt32426_2")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                       src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                       rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                       exp_status_list=['oper_up']):
        st.error("VXLAN tunnel is NOT UP in leaf3 before starting the test case test_FtOpSoRoEvpnRouterFt32426_2")
        success = False
    else:
        st.log("VXLAN tunnel is UP as expected in leaf1")
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32427_2")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32427_2")

def test_FtOpSoRoEvpnRouterFt32416(Ft32215_fixture):
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32416")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    hdrMsg("FtOpSoRoEvpnRouterFt32416 - Verify symmetric IRB Forwarding")
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32416")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32416")


def test_FtOpSoRoEvpnRouterFt3261(Ft32211_fixture):
    global vars

    tg = tg_dict['tg']
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3261")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3261")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3261")

'''
def test_FtOpSoRoEvpnRouterFt3281_3(Ft3281_fixture):

    global vars
    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt3281_3")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    tc_list = ["test_FtOpSoRoEvpnRouterFt3281","test_FtOpSoRoEvpnRouterFt32111","test_FtOpSoRoEvpnRouterFt32114"]
    ...
    intf_list = evpn_dict["leaf3"]["pch_intf_list"] + evpn_dict["leaf3"]["intf_list_spine"][2:4] + evpn_dict["leaf3"]["intf_list_spine"][6:8]
    ############################################################################################
    ...
    if num_of_paths <= 3:
        ...
        st.log('PASS: Traffic is load balanced as expected')
        st.report_tc_pass("test_FtOpSoRoEvpnRouterFt32111","tc_passed")
    ...
    if num_of_paths <= 4:
        ...
        st.report_tc_pass("test_FtOpSoRoEvpnRouterFt32114","tc_passed")
    ...
    if num_of_paths != 3:
        ...
        st.log('PASS: Traffic flowing through only 3 paths as expected after maxpath change.')
    ...
    if timer2 < timer1:
        ...
        success=False
        st.error("EVPN neighbors failed to reset and re-establish the connection")
    ...
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt3281")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt3281")
...
@pytest.mark.ft
def test_FtOpSoRoEvpnRouterFt32227(Ft32227_fixture):
    global vars
    hdrMsg("TC ID: FtOpSoRoEvpnRouterFt32227; TC SUMMARY : Test VxLAN by removing and adding back VLAN to VNI mapping which causes tunnel flap")

    success = True
    hdrMsg(" STEP:1  Make sure the Vxlan tunnel is up before starting the test_FtOpSoRoEvpnRouterFt32227")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
    ...
    for i in range(1,4):
        vtep_list = Evpn.get_tunnel_list(vars.D5)
        if evpn_dict["leaf4"]["loop_ip_list"][1] in vtep_list:
            st.log("######### tunnel towards {} still exists; retry : {} ###"
                   "#####".format(evpn_dict["leaf4"]["loop_ip_list"][1],i))
            st.wait(5)
        else:
            result = True
            break
    if result:
        st.log("PASS: tunnel towards {} not exists in leaf3 as expected".format(evpn_dict["leaf4"]["loop_ip_list"][1]))
    else:
        st.error("FAIL: tunnel towards {} exists in leaf3 even after L2VNI got"
                 " removed in leaf4".format(evpn_dict["leaf4"]["loop_ip_list"][1]))
        success = False
    ...
    for i in range(1,4):
        vtep_list = Evpn.get_tunnel_list(vars.D6)
        if evpn_dict["leaf3"]["loop_ip_list"][1] in vtep_list:
            st.log("######### non zero tunnel exists; retry : {} ########".format(i))
            st.wait(5)
        else:
            result = True
            break
    if result:
        st.log("PASS: as expected NO tunnels exists in leaf4")
    else:
        st.error("FAIL: some tunnels exists even after removing L2VNI from leaf4; expected 0")
        success = False
    ...
    for i in range(1,4):
        ...
        if evpn_dict["leaf4"]["loop_ip_list"][1] not in vtep_list:
            st.log("######### tunnel towards {} not found; retry : {} ###"
                   "#####".format(evpn_dict["leaf4"]["loop_ip_list"][1],i))
            st.wait(5)
        else:
            result = True
            break
    if result:
        st.log("PASS: tunnel towards {} exists in leaf3 as expected".format(evpn_dict["leaf4"]["loop_ip_list"][1]))
    else:
        st.error("FAIL: tunnel towards {} NOT exists in leaf3".format(evpn_dict["leaf4"]["loop_ip_list"][1]))
        success = False
    ...
    for i in range(1,4):
        ...
        if evpn_dict["leaf3"]["loop_ip_list"][1] not in vtep_list:
            st.log("######### expecting 1 tunnel; retry : {} ########".format(i))
            st.wait(5)
        else:
            result = True
            break
    if result:
        st.log("PASS: number of tunnel is 1 as expected in leaf4")
    else:
        st.error("FAIL: number of tunnels is NOT 1 in leaf4")
        success = False
    ...
    ############################################################################################
    hdrMsg(" \n####### Step 10 - Create L2 streams with multiple vlan ids ##############\n")
    ############################################################################################
    ############################################################################################
    ...
    current_stream_dict["stream"] = stream_dict["l2"]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFt32227")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFt32227")
...
@pytest.mark.ft
def test_FtOpSoRoEvpnRouterFtCeta31598(Ft32214_fixture):
    success = True
    techsupport_not_gen = True
    hdrMsg("### TC Summary: Verify VxLAN tunnel status and traffic across add and remove of duplicate VTEP IP in user Vrf ###")
    hdrMsg("### step 1 verify VXLAN tunnel status in Leaf 3 and Leaf 4 ###")
    if not retry_api(Evpn.verify_vxlan_tunnel_status,evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up'],retry_count=3, delay=5):
        st.error("step 1 FAIL: VXLAN tunnel is NOT UP in leaf3 towards Leaf4 before starting the test case")
        debug_vxlan_cmds(evpn_dict["leaf_node_list"][2],evpn_dict["leaf_node_list"][3])
        success = False
    else:
        st.log("step 1 PASS: VXLAN tunnel is UP in leaf3 towards Leaf4 before starting the test case")

    if retry_api(Evpn.verify_vxlan_tunnel_status, evpn_dict["leaf_node_list"][3],
                 src_vtep=evpn_dict["leaf4"]["loop_ip_list"][1],
                 rem_vtep_list=[evpn_dict["leaf3"]["loop_ip_list"][1]], exp_status_list=['oper_up'],
                 retry_count=3, delay=5):
        st.log("########## step 2 PASS: VXLAN tunnel is UP in leaf4 towards Leaf3 before starting the test case ##########")
    else:
        success = False
        st.error("##### step 2 FAIL: VXLAN tunnel is NOT UP in leaf4 towards Leaf3 before starting the test case #####")

    start_traffic(stream_han_list=stream_dict["l2"])
    st.wait(5, "waiting after starting traffic")

    hdrMsg("step 3 Verify L2 traffic b/w Leaf3 and Leaf4 with base underlay config")
    if verify_traffic(tx_port=tg_dict["d5_tg_port1"], rx_port=tg_dict["d6_tg_port1"], tx_ratio=1, rx_ratio=1):
        st.log("##### step 3 traffic verification passed #####")
    else:
        success=False
        st.error("########## step 3 traffic verification failed ##########")

    if (not success) and techsupport_not_gen:
        techsupport_not_gen = False
        basic.get_techsupport(filename='FtOpSoRoEvpnRouterFtBplusMr1')

    hdrMsg("### step 4 Add static route in user vrf Vrf1 having same VTEP IP network ###")
    input1={"next_hop":evpn_dict["leaf3"]["l3_vni_name_list"][0],"static_ip":evpn_dict["leaf4"]["loop_ip_list"][1]+"/32", \
        "family":"ipv4","vrf":"Vrf1"}
    input2={"next_hop":evpn_dict["leaf4"]["l3_vni_name_list"][0],"static_ip":evpn_dict["leaf3"]["loop_ip_list"][1]+"/32", \
        "family":"ipv4","vrf":"Vrf1"}
    parallel.exec_parallel(True, evpn_dict["leaf_node_list"][2:4], ip.create_static_route,[input1,input2])

    hdrMsg("### step 5 Verify route check for the newly added non default Vrf route entry ###")
    if not ip.verify_ip_route(dut=evpn_dict["leaf_node_list"][2],
                                vrf_name=evpn_dict["leaf3"]["vrf_name_list"][0],
                                shell="",family="ipv4",
                                interface=evpn_dict["leaf3"]["l3_vni_name_list"][0],
                                ip_address=evpn_dict["leaf4"]["loop_ip_list"][1]+"/32",
                                type="S"):
        st.error("step 5 FAIL: Verify Leaf3 tenant IPv4 route in Vrf1 having same route entry as VTEP IP of Leaf4")
        success = False
    else:
        st.log("step 5 PASS: Verify Leaf3 tenant IPv4 route in Vrf1 having same route entry as VTEP IP of Leaf4")

    if not ip.verify_ip_route(dut=evpn_dict["leaf_node_list"][3],
                                vrf_name=evpn_dict["leaf4"]["vrf_name_list"][0],
                                shell="",family="ipv4",
                                interface=evpn_dict["leaf3"]["l3_vni_name_list"][0],
                                ip_address=evpn_dict["leaf3"]["loop_ip_list"][1]+"/32",
                                type="S"):
        st.error("step 6 FAIL: Verify Leaf 4 tenant IPv4 route in Vrf1 having same route entry as VTEP IP of Leaf 3")
        success = False
    else:
        st.log("step 6 PASS: Verify Leaf 4 tenant IPv4 route in Vrf1 having same route entry as VTEP IP of Leaf 3")

    hdrMsg("### step 7 verify VXLAN tunnel status in Leaf 3 and Leaf 4 ###")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        st.error("step 7 FAIL: VXLAN tunnel is NOT UP in leaf3 towards Leaf4")
        debug_vxlan_cmds(evpn_dict["leaf_node_list"][2],evpn_dict["leaf_node_list"][3])
        success = False
    else:
        st.log("step 7 PASS: VXLAN tunnel is UP in leaf3 towards Leaf4")

    if Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][3],
                 src_vtep=evpn_dict["leaf4"]["loop_ip_list"][1],
                 rem_vtep_list=[evpn_dict["leaf3"]["loop_ip_list"][1]], exp_status_list=['oper_up']):
        st.log("##### step 8 PASS: VXLAN tunnel in leaf3 is Up towards leaf4 as expected #####")
    else:
        success = False
        st.error("########## step 8 FAIL: VXLAN tunnel in leaf3 is NOT Up towards leaf4 ##########")

    hdrMsg("step 9 Verify L2 traffic b/w Leaf3 and Leaf4 with duplicate VTEP IP across VRF")
    if verify_traffic(tx_port=tg_dict["d5_tg_port1"], rx_port=tg_dict["d6_tg_port1"], tx_ratio=1, rx_ratio=1):
        st.log("##### step 9 traffic verification passed #####")
    else:
        success = False
        debug_traffic(evpn_dict["leaf_node_list"][2],evpn_dict["leaf_node_list"][3])
        st.error("########## step 9 traffic verification failed ##########")

    if (not success) and techsupport_not_gen:
        techsupport_not_gen = False
        basic.get_techsupport(filename='FtOpSoRoEvpnRouterFtBplusMr1')

    hdrMsg("### step 10 Remove the newly added static route in Leaf3 and Leaf4 for user vrf Vrf1 ###")
    input1={"next_hop":evpn_dict["leaf3"]["l3_vni_name_list"][0],"static_ip":evpn_dict["leaf4"]["loop_ip_list"][1]+"/32", \
        "family":"ipv4","vrf":"Vrf1"}
    input2={"next_hop":evpn_dict["leaf4"]["l3_vni_name_list"][0],"static_ip":evpn_dict["leaf3"]["loop_ip_list"][1]+"/32", \
        "family":"ipv4","vrf":"Vrf1"}
    parallel.exec_parallel(True, evpn_dict["leaf_node_list"][2:4], ip.delete_static_route,[input1,input2])

    hdrMsg("### step 11 Verify the underlay route for VTEP IP rechability in Leaf 3 towards Leaf 4 and vice versa ###")
    if ip.verify_ip_route(dut=evpn_dict["leaf_node_list"][2],shell="",family="ipv4",
                                nexthop=evpn_dict["spine1"]["loop_ip_list"][0],
                                ip_address=evpn_dict["leaf4"]["loop_ip_list"][1]+"/32",
                                type="B"):
        st.log("step 11 PASS: Verify Leaf3 underlay route to reach Leaf4 VTEP IP in default vrf")
    elif ip.verify_ip_route(dut=evpn_dict["leaf_node_list"][2],shell="",family="ipv4",
                                nexthop=evpn_dict["spine2"]["loop_ip_list"][0],
                                ip_address=evpn_dict["leaf4"]["loop_ip_list"][1]+"/32",
                                type="B"):
        st.log("step 11 PASS: Verify Leaf3 underlay route to reach Leaf4 VTEP IP in default vrf")
    else:
        st.error("step 11 FAIL: Verify Leaf3 underlay route to reach Leaf4 VTEP IP in default vrf")
        success = False

    if ip.verify_ip_route(dut=evpn_dict["leaf_node_list"][3],shell="",family="ipv4",
                                nexthop=evpn_dict["spine1"]["loop_ip_list"][0],
                                ip_address=evpn_dict["leaf3"]["loop_ip_list"][1]+"/32",
                                type="B"):
        st.log("step 12 PASS: Verify Leaf4 underlay route to reach Leaf3 VTEP IP in default vrf")
    elif ip.verify_ip_route(dut=evpn_dict["leaf_node_list"][3],shell="",family="ipv4",
                                nexthop=evpn_dict["spine2"]["loop_ip_list"][0],
                                ip_address=evpn_dict["leaf3"]["loop_ip_list"][1]+"/32",
                                type="B"):
        st.log("step 12 PASS: Verify Leaf4 underlay route to reach Leaf3 VTEP IP in default vrf")
    else:
        st.error("step 12 FAIL: Verify Leaf4 underlay route to reach Leaf3 VTEP IP in default vrf")
        success = False

    hdrMsg("### step 13 verify VXLAN tunnel status in Leaf 3 and Leaf 4 ###")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        st.error("step 13 FAIL: VXLAN tunnel in Leaf3 towards Leaf4 is not Up")
        debug_vxlan_cmds(evpn_dict["leaf_node_list"][2],evpn_dict["leaf_node_list"][3])
        success = False
    else:
        st.log("step 13 PASS: VXLAN tunnel in Leaf3 towards Leaf4 is Up")

    if Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][3],
                 src_vtep=evpn_dict["leaf4"]["loop_ip_list"][1],
                 rem_vtep_list=[evpn_dict["leaf3"]["loop_ip_list"][1]], exp_status_list=['oper_up']):
        st.log("##### step 14 PASS: VXLAN tunnel in Leaf4 towards Leaf3 is Up #####")
    else:
        success = False
        st.error("########## step 14 FAIL: VXLAN tunnel in Leaf4 towards Leaf3 is not Up #####")

    hdrMsg("step 15 Verify L2 traffic b/w Leaf3 and Leaf4 after removing duplicate VTEP host route from user VRF")
    if verify_traffic(tx_port=tg_dict["d5_tg_port1"], rx_port=tg_dict["d6_tg_port1"], tx_ratio=1, rx_ratio=1):
        st.log("##### step 15 traffic verification passed #####")
    else:
        success=False
        debug_traffic(evpn_dict["leaf_node_list"][2],evpn_dict["leaf_node_list"][3])
        st.error("########## step 15 traffic verification failed ##########")
    ...
    current_stream_dict["stream"] = stream_dict["l2"]
    if success:
        st.report_pass("test_case_id_passed","test_FtOpSoRoEvpnRouterFtCeta31598")
    else:
        st.report_fail("test_case_id_failed","test_FtOpSoRoEvpnRouterFtCeta31598")


'''
def test_FtOpSoRoEvpnRouterFt3411_2(Ft32215_fixture):
    global vars
    success = True
    ...
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
        ...
        st.log("VXLAN tunnel is UP as expected on all the leaf nodes")
    tc_list = ["test_FtOpSoRoEvpnRouterFt3411","test_FtOpSoRoEvpnRouterFt3412"]
    ...
    if verify_traffic():
        ...
        success = False
        st.error("######## traffic verification failed before the start of the test case ########")

    Bgp.config_bgp_graceful_restart(vars.D1, user_command='preserve-fw-state',
                                    local_asn=evpn_dict["spine1"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D2, user_command='preserve-fw-state',
                                    local_asn=evpn_dict["spine2"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D3, user_command='preserve-fw-state',
                                    local_asn=evpn_dict["leaf1"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D4, user_command='preserve-fw-state',
                                    local_asn=evpn_dict["leaf2"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D5, user_command='preserve-fw-state', local_asn=evpn_dict["leaf3"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D6, user_command='preserve-fw-state', local_asn=evpn_dict["leaf4"]["local_as"],
                                    config='add')

    Bgp.config_bgp_graceful_restart(vars.D1,
                                    local_asn=evpn_dict["spine1"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D2,
                                    local_asn=evpn_dict["spine2"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D3,
                                    local_asn=evpn_dict["leaf1"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D4,
                                    local_asn=evpn_dict["leaf2"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D5, local_asn=evpn_dict["leaf3"]["local_as"],
                                    config='add')
    Bgp.config_bgp_graceful_restart(vars.D6, local_asn=evpn_dict["leaf4"]["local_as"],
                                    config='add')
    ...
    hdrMsg("Verify the tunnel is Up after the warm reboot")
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
                                           src_vtep=evpn_dict["leaf3"]["loop_ip_list"][1],
                                           rem_vtep_list=[evpn_dict["leaf4"]["loop_ip_list"][1]],
                                           exp_status_list=['oper_up']):
        ...
        st.log("VXLAN tunnel is UP as expected in leaf3 after warm reboot")
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt3411_2")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt3411_2")


def test_FtOpSoRoEvpnRouterFt3431_6(Ft3431_fixture):
    global vars
    success = True
    testcase_id = 'Ft3431_6'

    tc_list = ["test_FtOpSoRoEvpnRouterFt3431", "test_FtOpSoRoEvpnRouterFt3441", "test_FtOpSoRoEvpnRouterFt3442",
               "test_FtOpSoRoEvpnRouterFt3443", "test_FtOpSoRoEvpnRouterFt3445", "test_FtOpSoRoEvpnRouterFt3446" ]
    ...
    if not Evpn.verify_vxlan_tunnel_status(dut=evpn_dict["leaf_node_list"][2],
        ...
        st.error("VXLAN tunnel is NOT UP in leaf3 before starting the test case")
        success = False
        output = ip_obj.show_ip_route(vars.D5)
        output = bgp.show_bgp_ipv4_summary_vtysh(vars.D5)
        output = Evpn.fetch_evpn_neigh_output(vars.D5)
    else:
        st.log("VXLAN tunnel is UP as expected in leaf1")
    ...
    if retry_api(verify_mac_count, vars.D6, mac_count=40000,retry_count=4, delay=40):
        ...
        st.report_tc_pass("FtOpSoRoEvpnRouterFt3441", "tc_passed")
    else:
        success = False
    ...
    if retry_api(verify_mac_count, vars.D5, mac_count=40000,retry_count=4, delay=40):
        ...
        st.report_tc_pass("FtOpSoRoEvpnRouterFt3442", "tc_passed")
    else:
        success = False
    ...
    hdrMsg("Perform config reload on dut5")
    ...
    if not retry_api(Evpn.verify_vxlan_tunnel_status, evpn_dict["leaf_node_list"][3],
        ...
        st.report_tc_pass("FtOpSoRoEvpnRouterFt3431", "tc_passed")

    ###########################################################
    hdrMsg("Step 5 : Find the number of macs in Leaf3 and remote leaf node leaf4 after the trigger.")
    ###########################################################

    if retry_api(verify_mac_count, vars.D6, mac_count=40000, retry_count=4, delay=80):
        st.log("########## MAC count verification passed in D6 after the trigger #######")
        st.report_tc_pass("FtOpSoRoEvpnRouterFt3443", "tc_passed")
    else:
        success = False
        st.error("########## FAIL: MAC count verification failed in D6 after the trigger #######")

    if retry_api(verify_mac_count, vars.D5, mac_count=40000, retry_count=4, delay=80):
        st.log("########## MAC count verification passed in D5 after the trigger #######")
        st.report_tc_pass("FtOpSoRoEvpnRouterFt3445", "tc_passed")
    else:
        success = False
    ...
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoRoEvpnRouterFt3431_6")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoRoEvpnRouterFt3431_6")
