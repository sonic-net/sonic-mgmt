...
def test_ft_static_twicenat():
    ...
    nat_stats = natapi.poll_for_nat_statistics(vars.D1, protocol=data.proto_all, src_ip=data.tw_global_ip_addr, dst_ip=data.out_ip_addr_l[3])
    if not nat_stats:
        st.error("Received empty list, nat statistics not updated")
        util_nat_debug_fun()
    ...
    if not int(nat_stats[0]['packets']) == (int(data.pkt_count)):
        util_nat_debug_fun()
        st.report_fail("twicenat_translation_failed_in_packet")
    result = natapi.poll_for_nat_statistics(vars.D1, protocol=data.proto_all, src_ip=data.in1_ip_addr_h[3], dst_ip=data.tw_test_ip_addr)
    if not result:
        st.error("Received empty list, nat statistics not updated")
        util_nat_debug_fun()
        st.report_fail("twicenat_translation_failed_in_packet")
...
def nat_dut_config():
    ...
    natapi.config_nat_feature(vars.D1)
