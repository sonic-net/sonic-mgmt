...
def vrrp_base_config():
    ...
    if not result:
        st.error(err)
        utils.exec_all(True,[[st.generate_tech_support,data.dut1,'Lag_issue_dut1'],[st.generate_tech_support,data.dut2,'Lag_issue_dut2']])
    ...
    result = verify_bgp()
    if not result:
        utils.exec_all(True,[[st.generate_tech_support,data.dut1,'bgp_issue_dut1'],[st.generate_tech_support,data.dut4,'bgp_issue_dut4_01']])
        utils.exec_all(True,[[st.generate_tech_support,data.dut2,'bgp_issue_dut2'],[st.generate_tech_support,data.dut4,'bgp_issue_dut4_02']])
        return False
    ###################################################
