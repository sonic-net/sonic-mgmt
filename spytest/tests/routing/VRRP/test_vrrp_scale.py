##############################################################################
#Script Title : VRRPv2 Scale Test
...
from vrrp_scale_vars import *
from vrrp_scale_vars import data
from vrrp_utils_scale import *
...
def test_vrrp_scale_001(prologue_epilogue):
    tc_list = ["FtOpSoRoVrrpSc0001","FtOpSoRoVrrpSc0002","FtOpSoRoVrrpSc0003","FtOpSoRoVrrpSc0004","FtOpSoRoVrrpSc0005"]
    ...
    ###########################################################
    #data.tg1.tg_traffic_control(action='run', port_handle=data.tg_handles[0])
    run_traffic()

    ###########################################################
    hdrMsg("Step T2 : Verify Traffic for scale config for all VRID")
    ############################################################
    result = verify_tg_traffic_rate(data.tg1,data.tg2,data.tgd3_ports,data.tgd4_ports)
    if result is False:
        err = "Testcase {} Traffic check failed with after delete/configure VRRP {} and flap Vlan".format(tc_list[0],vrrp_ip_list[0][0])
        st.error(err);tc_result=False;err_list.append(err)

    ###########################################################
    hdrMsg("Stop Traffic for VRRP instance scale config")
    ###########################################################
    #data.tg1.tg_traffic_control(action='stop', port_handle=data.tg_handles[0])
    run_traffic(action='stop')

    ###########################################################
    hdrMsg("Step T3 : Delete/Configure VRRP sessions {} on dut1(Master) and verify vrrp master backup role".format(vrid_list[0:int(vrrp_sessions/2)]))
    ############################################################
    ...
    ###########################################################
    hdrMsg("Step T4 : Shutdown/No-shutdown VRRP enabled LAG member port in master node dut1 and check new vrrp role")
    ############################################################
    ...
    ###########################################################
    hdrMsg("Step T5 : Verify Traffic for scale VRRP instances with new VRRP master ")
    ############################################################
    result = verify_tg_traffic_rate(data.tg1,data.tg2,data.tgd3_ports,data.tgd4_ports)
    if result is False:
        err = "Testcase {} Traffic check failed with after delete/configure VRRP {} and flap Vlan".format(tc_list[0],vrrp_ip_list[0][0])
    ...
    ###########################################################################################
    hdrMsg("Step T6: Config save and fast boot DUT" )
    ###########################################################################################
    ...
    ###########################################################
    hdrMsg("Step T7 : Verify VRRP Master/Backup election for all {} configured sessions after dut1 fast reload".format(vrrp_sessions))
    ############################################################
    ...
    ###########################################################
    hdrMsg("Step T8 : Verify Traffic for scale VRRP instances after dut1 fast reload ")
    ############################################################
    result = verify_tg_traffic_rate(data.tg1,data.tg2,data.tgd3_ports,data.tgd4_ports)
    if result is False:
        err = "Testcase {} Traffic check failed with after delete/configure VRRP {} and flap Vlan".format(tc_list[0],vrrp_ip_list[0][0])
