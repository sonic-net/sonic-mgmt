...
def config_vlan(config='yes'):
    if config == 'yes' :
        ...
        utils.exec_all(True,[[vlan_api.config_vlan_range,data.dut1,'{} {}'.format(dut1_vlans[0],dut1_vlans[-1])],
                             [vlan_api.create_vlan,data.dut2,'{}'.format(dut2_vlans[0])],
                             [vlan_api.create_vlan,data.dut3,'{}'.format(dut3_vlans[0])],
        ...
        utils.exec_all(True,[[vlan_api.add_vlan_member,data.dut1,'{}'.format(vrrp_vlans[0]), lag_intf_list[0],True],
                         [vlan_api.add_vlan_member,data.dut2,'{}'.format(vrrp_vlans[0]), lag_intf_list[1],True],
                         [vlan_api.add_vlan_member,data.dut3,'{}'.format(vrrp_vlans[0]), lag_intf_list[0],True]])
        vlan_api.add_vlan_member(data.dut3, '{}'.format(vrrp_vlans[0]),lag_intf_list[1],True)
        ...
        vlan_api.add_vlan_member(data.dut4,'{}'.format(dut4_vlans[4]),data.d4tg_ports)

        ######################################################################
        #hdrMsg("Vlan-Config: Configure on dut3 dut3_tg_port as trunk port with member vlans {}".format(vrrp_vlans))
        ######################################################################

        vlan_api.add_vlan_member(data.dut3,'{}'.format(vrrp_vlans[0]),data.d3tg_ports)

    else:
        ##########################################################################
        hdrMsg("Vlan-Deconfig: Remove Vlan members  on all DUTs")
        ##########################################################################
        utils.exec_all(True,[[vlan_api.delete_vlan_member,data.dut1,'{}'.format(vrrp_vlans[0]), lag_intf_list[0],'True'],
                         [vlan_api.delete_vlan_member,data.dut2,'{}'.format(vrrp_vlans[0]), lag_intf_list[1],'True'],
                         [vlan_api.delete_vlan_member,data.dut3,'{}'.format(vrrp_vlans[0]), lag_intf_list[0],'True']])
        vlan_api.delete_vlan_member(data.dut3, '{}'.format(vrrp_vlans[0]),lag_intf_list[1],'True')
        ...
        vlan_api.delete_vlan_member(data.dut4, '{}'.format(dut4_vlans[4]), data.d4tg_ports)
        vlan_api.delete_vlan_member(data.dut3, '{}'.format(vrrp_vlans[0]), data.d3tg_ports)
        ##########################################################################
        ...
        utils.exec_all(True,[[vlan_api.config_vlan_range,data.dut1,'{} {}'.format(dut1_vlans[0],dut1_vlans[-1]),'del'],
                             [vlan_api.delete_vlan,data.dut2,'{}'.format(dut2_vlans[0])],
                             [vlan_api.delete_vlan,data.dut3,'{}'.format(dut3_vlans[0])],
...
def config_vrf(config='yes'):
    if config == 'yes' :
        ##################################################################
        hdrMsg("VRF-Config: Configure VRF on dut1 ,dut2 and dut4 ")
        ##################################################################
        dict1 = {'vrf_name':vrrp_vrf, 'config': 'yes'}
        parallel.exec_parallel(True, [data.dut1, data.dut2, data.dut4], vrf_api.config_vrf, [dict1, dict1,dict1])
        #vrf_api.config_vrf(data.dut1, vrf_name =vrrp_vrf, config = 'yes')
        #vrf_api.config_vrf(data.dut2, vrf_name =vrrp_vrf, config = 'yes')
        #vrf_api.config_vrf(data.dut4, vrf_name =vrrp_vrf, config = 'yes')

        hdrMsg("bind-VRF-to-interface:  on dut1 ,dut2 and dut4")
        for vrfint in dut1_vlan_intf+[data.d1d4_ports[3]]:
            vrf_api.bind_vrf_interface(data.dut1,vrf_name = vrrp_vrf, intf_name =vrfint,skip_error='True')

        for vrfint in dut2_vlan_intf+[data.d2d4_ports[3]]:
            vrf_api.bind_vrf_interface(data.dut2,vrf_name = vrrp_vrf, intf_name =vrfint,skip_error='True')

        for vrfint in dut4_vlan_intf+[data.d4d1_ports[3],data.d4d2_ports[3]]:
            vrf_api.bind_vrf_interface(data.dut4,vrf_name = vrrp_vrf, intf_name =vrfint,skip_error='True')


    else:
        hdrMsg("unbind-VRF-to-interface:  on dut1 ,dut2 and dut4")
        for vrfint in dut1_vlan_intf+[data.d1d4_ports[3]]:
            vrf_api.bind_vrf_interface(data.dut1,vrf_name = vrrp_vrf, intf_name =vrfint,config = 'no',skip_error='True')

        for vrfint in dut2_vlan_intf+[data.d2d4_ports[3]]:
            vrf_api.bind_vrf_interface(data.dut2,vrf_name = vrrp_vrf, intf_name =vrfint,config = 'no',skip_error='True')

        for vrfint in dut4_vlan_intf+[data.d4d1_ports[3],data.d4d2_ports[3]]:
            vrf_api.bind_vrf_interface(data.dut4,vrf_name = vrrp_vrf, intf_name =vrfint,config = 'no',skip_error='True')

        hdrMsg("Remove VRF-Config: Remove VRF configu on dut1 ,dut2 and dut4 ")
        dict1 = {'vrf_name':vrrp_vrf, 'config': 'no'}
        parallel.exec_parallel(True, [data.dut1, data.dut2, data.dut4], vrf_api.config_vrf, [dict1, dict1,dict1])
        #vrf_api.config_vrf(data.dut1, vrf_name =vrrp_vrf, config = 'no')
        #vrf_api.config_vrf(data.dut2, vrf_name =vrrp_vrf, config = 'no')
        #vrf_api.config_vrf(data.dut4, vrf_name =vrrp_vrf, config = 'no')
...
def config_ip(config='yes'):
    ...
    hdrMsg("IP-config: On DUT4 (TOR) {} connected routes on vlans {} with addresses {} "
           "respectively ".format(config_str,dut4_vlan_intf[-1],dut4_tg_ip_list))
    ##########################################################################

    api_name(data.dut4,dut4_vlan_intf[-1],dut4_tg_ip_list[0],mask)
    ...
    if config == 'yes':
        arp.add_static_arp(data.dut4, tg_dest_ip_list[0], tg_dest_mac_list[0], interface="Vlan{}".format(dut4_vlans[-1]))
        mac_api.config_mac(data.dut4,tg_dest_mac_list[0],dut4_vlans[-1],data.d4tg_ports)
    else:
        arp.delete_static_arp(data.dut4,tg_dest_ip_list[0])
        mac_api.delete_mac(data.dut4,tg_dest_mac_list[0],dut4_vlans[-1])
...
def config_bgp(config='yes'):
    if config == 'yes':
        ...
        ##########################################################################
        dict1 = {'local_as':dut1_as,'router_id':dut1_router_id,'config_type_list':['router_id'],'vrf_name':vrrp_vrf}
        dict2 = {'local_as':dut2_as,'router_id':dut2_router_id,'config_type_list':['router_id'],'vrf_name':vrrp_vrf}
        dict3 = {'local_as': dut4_as, 'router_id': dut4_router_id, 'config_type_list': ['router_id'],'vrf_name':vrrp_vrf}
        ...
        for nbr_1,nbr_3 in zip(dut4_1_ip_list,dut1_4_ip_list):
           dict1 = {'local_as':dut1_as,'peergroup':peer_v4_1,'config_type_list':['peergroup'],'remote_as':dut4_as,'neighbor':nbr_1,'vrf_name':vrrp_vrf}
           dict2 = {'local_as':dut4_as,'peergroup': peer_v4_1, 'config_type_list': ['peergroup'], 'remote_as': dut1_as,'neighbor': nbr_3,'vrf_name':vrrp_vrf}
        ...
        for nbr_1,nbr_3 in zip(dut4_2_ip_list,dut2_4_ip_list):
           dict1 = {'local_as':dut2_as,'peergroup':peer_v4_2,'config_type_list':['peergroup'],'remote_as':dut4_as,'neighbor':nbr_1,'vrf_name':vrrp_vrf}
           dict2 = {'local_as':dut4_as,'peergroup': peer_v4_2, 'config_type_list': ['peergroup'], 'remote_as': dut2_as,'neighbor': nbr_3,'vrf_name':vrrp_vrf}
        ...
        for route in dut4_route_list:
            bgp_api.config_bgp(data.dut4,local_as=dut4_as,config_type_list=['network'],network='{}/{}'.format(route,mask),vrf_name=vrrp_vrf)
    else:
        ##########################################################################
        hdrMsg("BGP-Deconfig: Delete BGP routers globally from all DUTs")
        ##########################################################################
        dict1 = {'local_as':dut1_as,'config_type_list': ["removeBGP"], 'removeBGP': 'yes', 'config': 'no','vrf_name':vrrp_vrf}
        dict2 = {'local_as':dut2_as,'config_type_list': ["removeBGP"], 'removeBGP': 'yes', 'config': 'no','vrf_name':vrrp_vrf}
        dict3 = {'local_as':dut4_as,'config_type_list': ["removeBGP"], 'removeBGP': 'yes', 'config': 'no','vrf_name':vrrp_vrf}
        parallel.exec_parallel(True, [data.dut1, data.dut2, data.dut4], bgp_api.config_bgp, [dict1, dict2, dict3])
...
def config_tgen(config='yes'):
    if config == 'yes':
        ...
        down_link_stream = data.tg2.tg_traffic_config(mac_src=tg2_src_mac, mac_dst=data.D4_tg_mac, rate_pps=traffic_rate, \
                                                 mode='create', port_handle=data.tg_handles[1],
                                                 transmit_mode='continuous',l2_encap='ethernet_ii_vlan', vlan="enable", vlan_id=dut4_vlans[0],
                                                 l3_protocol='ipv4', ip_src_addr=tg_dest_ip_list[0] \
        ...
        data.stream_details[data.stream_handles['downstream']] = "Downstream IPv4 Traffic: SRC-MAC: {}" \
                                                                 "Dest-MAC: {} Vlan-ID: {} SRC-IP:{} " \
                                                                 "DEST-IP: {} ".format(tg2_src_mac,data.D4_tg_mac,dut4_vlans[0],tg_dest_ip_list[0],vrrp_ip_list[0][2])
...
def config_vrrp(config='yes'):
    if config == 'yes':
        ##############################################################################
        hdrMsg("Configure 1-VRRP sessions with dut1 Master and dut2 backup ")
        ##############################################################################
...
def verify_vrrp(traffic_check='no',dut_check='no',check_backup='no',summary='no',retry_count=5,delay=1):
    ...
    if summary == 'no':
        for vrid,intf,vmac,vip in zip(vrid_list,dut1_vlan_intf,vmac_list_1,vip_list):
        ...
        if traffic_check == 'yes':
            ###########################################################
            hdrMsg("Verify for VRRP sessions {} ,Master DUT {} forwards data traffic".format(vrid_list, data.dut1))
            ############################################################
            result = verify_traffic(master_dut=data.dut1, backup_dut=data.dut2, vrid=vrid_list,dut_check=dut_check)
            if result is False:
                return False
        '''
        for vrid,intf,vmac,vip in zip(vrid_list[vrrp_sessions/2:],dut1_vlan_intf[vrrp_sessions/2:],vmac_list_1[vrrp_sessions/2:],vip_list[vrrp_sessions/2:]):
        ...
        if traffic_check == 'yes':
            ###########################################################
            hdrMsg("Verify for VRRP sessions {} ,Master DUT {} forwards data traffic".format(vrid_list[vrrp_sessions/2:],data.dut2))
            ############################################################
            result = verify_traffic(master_dut=data.dut2, backup_dut=data.dut1, vrid=vrid_list[vrrp_sessions/2:], dut_check=dut_check)
            if result is False:
                return False
        '''
    else:
        ################################################################
        hdrMsg("Verify DUT1 is Master for VRIDs {} and vice versa on dut2".format(vrid_list[0]))
        ################################################################
        role_list_1 = ['Master'] * (vrrp_sessions)
        role_list_2 = ['Backup'] * (vrrp_sessions)
...
def run_traffic(stream_handle=None,action='start'):
    if stream_handle is None:
        stream_handle = [data.stream_handles['vrrp_{}'.format(id)] for id in vrid_list]
    else:
...
def verify_bgp():
    ...
    ############################################################
    result = retry_api(ip_bgp.check_bgp_session,data.dut1,nbr_list=dut4_1_ip_list,state_list=['Established']*len(dut4_1_ip_list),vrf_name=vrrp_vrf)
    if result is False:
    ...
    ############################################################
    result = retry_api(ip_bgp.check_bgp_session,data.dut2,nbr_list=dut4_2_ip_list,state_list=['Established']*len(dut4_2_ip_list),vrf_name=vrrp_vrf)
    if result is False:
...
def debug_vrrp():
    ...
    def vrrp_dut1():
        ip_api.show_ip_route(data.dut1, vrf_name =vrrp_vrf)
        ...
        check_mac(data.dut2,vrrp_vlans,vmac_list,[lag_intf_list[1]]*len(vrrp_vlans))
        check_mac(data.dut3,vlan_list,vmac_list,lag_intf_list)
                        
    def vrrp_dut2():
        ip_api.show_ip_route(data.dut2, vrf_name =vrrp_vrf)
