...
from utilities.common import poll_wait, filter_and_select
...
def sflow_module_hooks(request):
    ...
    [output, exceptions] = exec_all(True, [[interface_status_show, vars.D1, [vars.D1T1P1, vars.D1D2P1, vars.D1D2P2]], [interface_status_show, vars.D2, [vars.D2T1P1, vars.D2D1P1, vars.D2D1P2]]])
    ...
    get_params()
    [_, exceptions] = exec_all(True, [[tg_init], [sflow_module_prolog]], first_on_main=True)
    ensure_no_exception(exceptions)
    yield
    sflow_module_epilog()
...
def sflow_module_prolog():
    ...
    [_, exceptions] = exec_foreach(True, data.dut_list, config_routing_interfaces, params)
    ensure_no_exception(exceptions)
...
def test_ft_sflow_sampling_v4_sFlow_collector():
    ...
    if report_flag:
        ...
        exceptions = exec_all(True, [[show_interface_counters_detailed, vars.D1, vars.D1T1P1], [show_interface_counters_detailed, vars.D2, vars.D2T1P1]])[1]
        ensure_no_exception(exceptions)
        exceptions = exec_all(True, [[show_interface_counters_detailed, vars.D1, vars.D1D2P1], [show_interface_counters_detailed, vars.D2, vars.D2D1P1]])[1]
        ensure_no_exception(exceptions)
...
def test_ft_sflow_sampling_v6_sFlow_collector():
    ...
    if capture_result:
        # st.report_fail("sflow_negative_verifcation_failed")
        report_flag, module_flag = 1, 1
    if report_flag:
        sflow.psample_stats(vars.D2, ['psample_cb'])
        exceptions = exec_all(True, [[show_interface_counters_detailed, vars.D1, vars.D1T1P1], [show_interface_counters_detailed, vars.D2, vars.D2T1P1]])[1]
...
def test_ft_sflow_sampling_sFlow_collector_management_port():
    ...
    sflow.add_del_collector(vars.D2, collector_name=data.collector_name_2, ip_address=sflow_collector_ip,
                            port_number=None, action="add")
    poll_wait(ipfeature.ping, 3, vars.D2, sflow_collector_ip, family="ipv4")
    exceptions = exec_all(True, [[clear_interface_counters, vars.D1],[clear_interface_counters, vars.D2]])[1]
    ensure_no_exception(exceptions)
...
def test_ft_sflow_collector_port_poch():
    ...
    if not poll_wait(sflow.verify_config, 30, vars.D2, data=collector_data):
        poch_config_remove()
        st.report_fail("failed_to_config_sflow")
    exceptions = exec_all(True, [[clear_interface_counters, vars.D1],[clear_interface_counters, vars.D2]])[1]
    ensure_no_exception(exceptions)
