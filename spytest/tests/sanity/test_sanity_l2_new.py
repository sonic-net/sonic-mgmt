# experiments to see if we can improve the performance
import pytest
...
base_line_final_result = {
    'test_base_line2_portchannel_create_delete': 'NA',
    'test_base_line2_random_link_flap_portchannel': 'NA',
    'test_base_line2_l2_taggged_forwarding_with_portchannel': 'NA',
    'test_base_line2_vlan_port_association': 'NA',
    'test_base_line2_port_move_from_vlan_a_to_vlan_b': 'NA',
    'test_base_line2_vlan_create_delete_and_mac_learning_with_bum': 'NA',
    'test_base_line2_mac_move_single_vlan': 'NA',
    'test_base_line2_mac_move_across_vlans': 'NA'
}

data = SpyTestDict()
data.vlan_id = str(random_vlan_list()[0])
data.portChannelName = "PortChannel5"
data.portChannelCreated = False
...
def check_ports_up_dut_0(dut, vars, topology):
...
def check_ports_up_dut(dut, vars, topology):
    all_ports = topology[dut]["ports"] + [topology[dut]["TGports"]]
    if intf_obj.verify_interface_status(dut, all_ports, 'oper', 'up'):
        return True
    intf_obj.interface_shutdown(dut, all_ports)
    st.wait(data.post_link_shutdown_wait_time)
    intf_obj.interface_noshutdown(dut, all_ports)
    if intf_obj.verify_interface_status(dut, all_ports, 'oper', 'up'):
        return True
    st.error('some of the interfaces {} are down'.format(all_ports))
    return False

def check_ports_up(vars, topology):
    [retvals, _ ] = st.exec_each(topology.keys(), check_ports_up_dut, vars, topology)
    if not all(retvals):
        st.report_fail("msg", "failure veriying oper status of ports with shut and no shut")

def check_vlan_pc_config_dut(dut, vars, topology, tagged):

    tg_pc_ports = [topology[dut]["TGports"], data.portChannelName]

    vlan_obj.clear_vlan_configuration(dut, thread=False)
    if not data.portChannelCreated:
    ...
    vlan_obj.add_vlan_member(dut, data.vlan_id, tg_pc_ports, tagged)

    if tagged:
        if not vlan_obj.verify_vlan_config(dut, data.vlan_id, tagged=tg_pc_ports):
            st.error("vlan tagged member fail on port {},vlan {}".format(tg_pc_ports, data.vlan_id))
            return False
    else:
        if not vlan_obj.verify_vlan_config(dut, data.vlan_id, untagged=tg_pc_ports):
            st.error("vlan untagged member fail on port {},vlan {}".format(tg_pc_ports, data.vlan_id))
            return False
    return True

def check_vlan_pc_config(vars, topology, tagged):
    [retvals, _ ] = st.exec_each(topology.keys(), check_vlan_pc_config_dut, vars, topology, tagged)
    if not all(retvals):
        st.report_fail("msg", "failed to verify vlan and port channel")
    data.portChannelCreated = True

def check_pc_status_dut(dut, vars, topology):
...
def check_pc_status(vars, topology):
    [retvals, _ ] = st.exec_each(topology.keys(), check_pc_status_dut, vars, topology)
    if not all(retvals):
        st.report_fail("msg", "failure veriying port channel status")
    st.wait(data.wait_post_port_channel_up)

def check_traffic(vars, topology, tg1, stream_list, traffic_details):
...
def check_mac_table_dut(dut, vars, topology):
    ...
    if not mac_obj.verify_mac_address(dut, data.vlan_id, complete_mac_address_list):
        st.error("mac_address_verification_fail")
        return False
    return True

def check_mac_table(vars, topology):
    [retvals, _ ] = st.exec_each(topology.keys(), check_mac_table_dut, vars, topology)
    if not all(retvals):
        st.report_fail("msg", "failure veriying MAC table")

def check_pc_create_delete_dut(dut, vars, topology):
    portchannel_list = []
    for i in range(6, data.no_of_port_channel_create_and_delete+6):
        portchannel_obj.create_portchannel(dut, "PortChannel00{}".format(i))
        portchannel_list.append("PortChannel00{}".format(i))
    dut_portchannel_list = len(portchannel_obj.get_portchannel_list(dut))
    assert int(dut_portchannel_list) == int(data.no_of_port_channel_create_and_delete+1), \
        st.log("port channel count {} verification fail on port channel {}".format
               (data.no_of_port_channel_create_and_delete+1, dut_portchannel_list))
    portchannel_obj.delete_portchannel(dut, portchannel_list)
    dut_portchannel_list = len(portchannel_obj.get_portchannel_list(dut))
    if not int(dut_portchannel_list) == 1:
        st.error("port channel count {} verification fail on port channel {}".format(1, dut_portchannel_list), dut=dut)
        return False
    return True

def check_pc_create_delete(vars, topology):
    [retvals, _ ] = st.exec_each(topology.keys(), check_pc_create_delete_dut, vars, topology)
    if not all(retvals):
        st.report_fail("msg", "failure veriying port channel create and delete")

def do_link_flap(vars):
    for _ in range(1, 2):
...
@pytest.mark.sanity_new
def test_base_line2_portchannel_create_delete():
    ...
    st.exec_each(topology.keys(), intf_obj.clear_interface_counters)
    ...
    if sub_test_1:
        ...
        try:
            ...
            base_line_final_result['test_base_line2_portchannel_create_delete'] = 'Pass'
        except Exception as e:
            st.log(e)
            base_line_final_result['test_base_line2_portchannel_create_delete'] = 'Fail'

        st.log("#"*90)
        st.log("############# Sub test 1: test_base_line2_portchannel_create_delete - END  ################")
        st.log("#"*90)

    if sub_test_2 and base_line_final_result['test_base_line2_portchannel_create_delete'] == 'Pass':
        st.log("#"*90)
        st.log("############# Sub test 2: test_base_line2_random_link_flap_portchannel  - START  ################")
        st.log("#"*90)
        try:
            ...
            base_line_final_result['test_base_line2_random_link_flap_portchannel'] = 'Pass'
        except Exception as e:
            st.log(e)
            base_line_final_result['test_base_line2_random_link_flap_portchannel'] = 'Fail'

        st.log("#"*90)
        st.log("############# Sub test 2: test_base_line2_random_link_flap_portchannel  - END  ################")
        st.log("#"*90)

    if sub_test_3 and base_line_final_result['test_base_line2_portchannel_create_delete'] == 'Pass':
        st.log("#"*90)
        st.log("############# Sub test 3: test_base_line2_l2_taggged_forwarding_with_portchannel - START  "
               "################")
        st.log("#"*90)
        try:
            ...
            base_line_final_result['test_base_line2_l2_taggged_forwarding_with_portchannel'] = 'Pass'
        except Exception as e:
            st.log(e)
            base_line_final_result['test_base_line2_l2_taggged_forwarding_with_portchannel'] = 'Fail'

        st.log("#"*90)
        st.log("############# Sub test 3: test_base_line2_l2_taggged_forwarding_with_portchannel - END "
               " ################")
        st.log("#"*90)

    if sub_test_4 and base_line_final_result['test_base_line2_portchannel_create_delete'] == 'Pass':
        st.log("#"*90)
        st.log("############# Sub test 4: test_base_line2_vlan_port_association - START  ################")
        st.log("#"*90)
        try:
            ...
            base_line_final_result['test_base_line2_vlan_port_association'] = 'Pass'
        except Exception as e:
            st.log(e)
            base_line_final_result['test_base_line2_vlan_port_association'] = 'Fail'

        st.log("#"*90)
        st.log("############# Sub test 4: test_base_line2_vlan_port_association - END  ################")
        st.log("#"*90)

    if sub_test_5 and base_line_final_result['test_base_line2_portchannel_create_delete'] == 'Pass':
        st.log("#"*90)
        st.log("############# Sub test 5: test_base_line2_port_move_from_vlan_a_to_vlan_b - START  ################")
        st.log("#"*90)
        try:
            st.log("Config Vlan Untagged and Port-channel on both DUTs")
            check_vlan_pc_config(vars, topology, False)
            ...
            base_line_final_result['test_base_line2_port_move_from_vlan_a_to_vlan_b'] = 'Pass'
        except Exception as e:
            st.log(e)
            base_line_final_result['test_base_line2_port_move_from_vlan_a_to_vlan_b'] = 'Fail'

        st.log("#"*90)
        st.log("############# Sub test 5: test_base_line2_port_move_from_vlan_a_to_vlan_b - END  ################")
        st.log("#"*90)

    # Result printing
    st.log(sprint_vtable(['Test', 'Result'],
                         [[k, v] for k, v in filter_and_select([base_line_final_result],
                                                               ['test_base_line2_portchannel_create_delete',
                                                                'test_base_line2_random_link_flap_portchannel',
                                                                'test_base_line2_l2_taggged_forwarding_with_portchannel',
                                                                'test_base_line2_vlan_port_association',
                                                                'test_base_line2_port_move_from_vlan_a_to_vlan_b']
                                                               )[0].items()], 0))
    ...
    out = base_line_final_result['test_base_line2_portchannel_create_delete']
    if out == 'Pass':
...
@pytest.mark.sanity_new
@pytest.mark.depends("test_base_line2_portchannel_create_delete")
def test_base_line2_random_link_flap_portchannel():
    out = base_line_final_result['test_base_line2_random_link_flap_portchannel']
    if out == 'Pass':
        st.report_pass("test_case_passed")
    elif out == 'Fail':
        st.report_fail("test_case_failed")
...
@pytest.mark.sanity_new
@pytest.mark.depends("test_base_line2_portchannel_create_delete")
def test_base_line2_l2_taggged_forwarding_with_portchannel():
    out = base_line_final_result['test_base_line2_l2_taggged_forwarding_with_portchannel']
    if out == 'Pass':
        st.report_pass("test_case_passed")
    elif out == 'Fail':
        st.report_fail("test_case_failed")
...
@pytest.mark.sanity_new
@pytest.mark.depends("test_base_line2_portchannel_create_delete")
def test_base_line2_vlan_port_association():
    out = base_line_final_result['test_base_line2_vlan_port_association']
    if out == 'Pass':
        st.report_pass("test_case_passed")
    elif out == 'Fail':
        st.report_fail("test_case_failed")
...
@pytest.mark.sanity_new
@pytest.mark.depends("test_base_line2_portchannel_create_delete")
def test_base_line2_port_move_from_vlan_a_to_vlan_b():
    out = base_line_final_result['test_base_line2_port_move_from_vlan_a_to_vlan_b']
    if out == 'Pass':
        st.report_pass("test_case_passed")
    elif out == 'Fail':
        st.report_fail("test_case_failed")
...
@pytest.mark.sanity_new
def test_base_line2_vlan_create_delete_and_mac_learning_with_bum():
    ...
    if sub_test_8:
        st.log("#"*90)
        st.log("############# Sub test 8: test_base_line2_vlan_create_delete_and_mac_learning_with_bum - START  "
               "################")
        st.log("#"*90)
        try:
            ...
            base_line_final_result['test_base_line2_vlan_create_delete_and_mac_learning_with_bum'] = 'Pass'
        except Exception as e:
            st.log(e)
            base_line_final_result['test_base_line2_vlan_create_delete_and_mac_learning_with_bum'] = 'Fail'

        st.log("#"*90)
        st.log("############# Sub test 8: test_base_line2_vlan_create_delete_and_mac_learning_with_bum - END "
               " ################")
        st.log("#"*90)

    if sub_test_10 and base_line_final_result['test_base_line2_vlan_create_delete_and_mac_learning_with_bum'] == 'Pass':
        st.log("#"*90)
        st.log("############# Sub test 10: test_base_line2_mac_move_single_vlan - START  ################")
        st.log("#"*90)
        try:
            ...
            st.log("Traffic test passed..")
            base_line_final_result['test_base_line2_mac_move_single_vlan'] = 'Pass'
        except Exception as e:
            st.log(e)
            base_line_final_result['test_base_line2_mac_move_single_vlan'] = 'Fail'

        st.log("#"*90)
        st.log("############# Sub test 10: test_base_line2_mac_move_single_vlan - END  ################")
        st.log("#"*90)

    if sub_test_11 and base_line_final_result['test_base_line2_vlan_create_delete_and_mac_learning_with_bum'] == 'Pass'\
            and base_line_final_result['test_base_line2_mac_move_single_vlan'] == 'Pass':
        st.log("#"*90)
        st.log("############# Sub test 11: test_base_line2_mac_move_across_vlans - START  ################")
        st.log("#"*90)
        try:
            ...
            base_line_final_result['test_base_line2_mac_move_across_vlans'] = 'Pass'
        except Exception as e:
            st.log(e)
            base_line_final_result['test_base_line2_mac_move_across_vlans'] = 'Fail'

        st.log("#"*90)
        st.log("############# Sub test 11: test_base_line2_mac_move_across_vlans - END  ################")
        st.log("#"*90)
        ...
        sprint_vtable(['Test', 'Result'],
                      [[k, v] for k, v in
                       filter_and_select([base_line_final_result],
                                         ['test_base_line2_vlan_create_delete_and_mac_learning_with_bum',
                                          'test_base_line2_mac_move_single_vlan',
                                          'test_base_line2_mac_move_across_vlans']
                                         )[0].items()], 0))
    ...
    out = base_line_final_result['test_base_line2_vlan_create_delete_and_mac_learning_with_bum']
    if out == 'Pass':
        st.report_pass("test_case_passed")
    elif out == 'Fail':
        st.report_fail("test_case_failed")
...
@pytest.mark.sanity_new
@pytest.mark.depends("test_base_line2_vlan_create_delete_and_mac_learning_with_bum")
def test_base_line2_mac_move_single_vlan():
    out = base_line_final_result['test_base_line2_mac_move_single_vlan']
    if out == 'Pass':
        st.report_pass("test_case_passed")
    elif out == 'Fail':
        st.report_fail("test_case_failed")
...
@pytest.mark.sanity_new
@pytest.mark.depends("test_base_line2_vlan_create_delete_and_mac_learning_with_bum")
def test_base_line2_mac_move_across_vlans():
    out = base_line_final_result['test_base_line2_mac_move_across_vlans']
    if out == 'Pass':
        st.report_pass("test_case_passed")
    elif out == 'Fail':
        st.report_fail("test_case_failed")
...
@pytest.mark.sanity_new
def test_base_line2_sflow_config_global_enable_disable_klish():
    """
