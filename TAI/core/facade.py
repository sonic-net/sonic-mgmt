...
class PlatformAdapter:
    ...
    Usage:
        ...
        qos.apply_scheduler("SCHEDULER|scheduler.0", {'type': 'DWRR'})
    ...
    def read_dbal_field(self, table_name: str, field_name: str) -> Optional[str]:
        """
        Read a single field from a DNX dbal table.

        Q3D returns the raw string value. Non-DNX platforms (XGS/Tomahawk) inherit
        the base no-op and always return None.
        ...
        Returns:
            Raw string value, or None on non-DNX platforms or read failure.
        """
        qos = self._get_or_create_adapter(QoSAdapter)
        return qos.read_dbal_field(table_name, field_name)

    def apply_dbal_field(self, table_name: str, field_name: str, value: Any) -> bool:
        """
        Write a single field to a DNX dbal table.

        Q3D issues the bcmcmd. Non-DNX platforms (XGS/Tomahawk) inherit the
        base no-op which returns True without doing anything.
        ...
        Returns:
            True on success or non-DNX platform, False if the bcmcmd failed.
        """
        qos = self._get_or_create_adapter(QoSAdapter)
        return qos.apply_dbal_field(table_name, field_name, value)
    ...
    def get_pg_profile(self, pg_number: int, interface: str, exact: bool = False):
        ...
        Returns:
            (profile_name, pg_key) or (None, None) if not found
        """
        qos = self._get_or_create_adapter(QoSAdapter)
