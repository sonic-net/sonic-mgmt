...
class ThriftAdapter(AdapterBase):
    ...
    def get_pg_counters(self, test_case: Any, src_port_id: int, dst_port_id: int,
        ...
        ingress = ([recv_counters[c] for c in test_case.ingress_counters])[0]
        egress = ([xmit_counters[c] for c in test_case.egress_counters])[0]
    ...
    def check_rx_drop(self, recv_delta: list, ingress_counters: list):
        ...
        Returns:
            (True, reason_str) if drops occurred above margin, (False, '') otherwise
        """
        margin = self.get_ingress_drop_margin()
        for cntr in self.get_active_ingress_drop_counters(ingress_counters):
    ...
    def check_tx_drop(self, xmit_delta: list, egress_counters: list):
        ...
        call site when asserting that no drops should have occurred.
        ...
        Returns:
            (True, reason_str) if drops occurred, (False, '') otherwise
        """
    ...
    def check_pfc_triggered(self, recv_delta: list, pg: int):
        ...
        Returns (True, '') when PFC fired.  Use ``not ok`` at the call site
        when asserting that PFC should NOT have triggered.

        Same logic for all platforms — pg counter index is platform-invariant.
        ...
        Returns:
            (True, '') if PFC triggered, (False, reason_str) otherwise
        """
        if recv_delta[pg] <= 0:
            return False, 'pg={} delta={}'.format(pg, recv_delta[pg])
        return True, ''
