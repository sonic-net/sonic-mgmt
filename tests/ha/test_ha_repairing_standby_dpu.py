...
def _update_ha_set_with_replacement_dpu(
    ...
    # Keep preferred_vdpu_id pointing to the surviving (primary) DPU
    ...
    for duthost in duthosts:
        if len(duthosts) > 2 and duthost == duthosts[1]:
            # Skip programming the old standby DPU's switch if it's different from the replacement's switch
            continue
...
def test_ha_repairing_standby_dpu(
    localhost,
    duthosts,
    dpuhosts,
    ptfhost,
    ...
    """
    Test replacement of the standby DPU in a live HA set.

    Workflow:
      1. Trigger planned shutdown on the standby DPU (dpuhosts[1]) that is
         being removed from the HA set.
      2. Update the HA set on all switches so that the old standby vDPU is
      ...
      5. Verify that the primary DPU remains active and the replacement DPU
         reaches the standby state.
    """
    ha_set_id = "haset0_0"
    ...
    new_vdpu_id = f"vdpu1_{dpuhosts[2].dpu_index}"
    ...
    try:
        # ------------------------------------------------------------------
        # Step 1: Planned shutdown — remove standby DPU from the HA set
        # ------------------------------------------------------------------
        logger.info("Step 1: Triggering planned shutdown on standby DPU")
        ...
        pytest_assert(verify_ha_state(duthosts[1], standby_vdpu_key, "dead"),
                      "Standby DPU did not reach dead state after planned shutdown")
        pytest_assert(verify_ha_state(duthosts[0], primary_vdpu_key, "standalone"),
                      "Primary HA state is not standalone")

        logger.info("Standby DPU is in dead state, Primary DPU is in standalone state")

        # ------------------------------------------------------------------
        # Step 2: Update HA set on all switches — replace old standby with new DPU
        # ------------------------------------------------------------------
        old_vdpu_id = f"vdpu1_{dpuhosts[1].dpu_index}"
        logger.info(
            f"Step 2: Updating HA set to replace '{old_vdpu_id}' with "
        ...
        # Verify the new standby DPU is in dead state (as expected right after joining)
        pytest_assert(
            verify_ha_state(replacement_duthost, new_vdpu_key, "dead"),
            "New standby DPU did not reach dead state after joining the HA set"
        )
        logger.info("New standby DPU is in dead state as expected after HA set update")

        # ------------------------------------------------------------------
        # Step 3: Program all ENIs on the new (replacement) DPU
        # ------------------------------------------------------------------
        replacement_dpuhost = dpuhosts[2]
        logger.info(f"HA: Step 3: Programming ENIs on the replacement DPU: {replacement_duthost.hostname}")
        ...
        # ------------------------------------------------------------------
        logger.info("Step 4: Activating the new standby DPU")
        pytest_assert(
            activate_secondary_dash_ha(
                localhost, replacement_duthost, ptfhost, new_vdpu_key, "activate_role",
                owner=ha_owner
            ),
            f"Failed to activate HA on replacement standby DPU ({new_vdpu_key})"
        )
        logger.info("Replacement standby DPU reached standby state")
        ...
        pytest_assert(
            verify_ha_state(duthosts[0], primary_vdpu_key, "active"),
            "Primary DPU HA state is not active after standby replacement"
        )
        pytest_assert(
            verify_ha_state(replacement_duthost, new_vdpu_key, "standby"),
            "Replacement standby DPU HA state is not standby"
        )

        logger.info(
            "Standby DPU replacement test completed successfully: "
            f"primary DPU '{primary_vdpu_key}' is active, "
            f"replacement standby DPU '{new_vdpu_key}' is standby"
        )
    finally:
        stop_event.set()
