...
class GNMIEnvironment(object):
    def __init__(self, my_duthost):
        self.work_dir = "/tmp/_my_gnmi_dir/"
        ...
        # self.enable_zmq = my_duthost.shell("netstat -na | grep -w 8100", module_ignore_errors=True)['rc'] == 0
...
def generate_gnmi_cert(localhost, my_duthost):
    """
    Args:
        localhost: fixture for localhost
        my_duthost: fixture for my_duthost
    ...
    create_ext_conf(my_duthost.mgmt_ip, env.work_dir+"extfile.cnf")
...
def apply_gnmi_cert(my_duthost, ptfhost):
    """
    ...
    my_duthost.copy(src=env.work_dir+env.gnmi_ca_cert, dest=env.gnmi_cert_path)
...
def recover_gnmi_cert(localhost, my_duthost, skip_cert_cleanup):
    """
...
def gnmi_set(my_duthost, ptfhost, delete_list, update_list, replace_list):
    """
...
def gnmi_get(my_duthost, ptfhost, path_list):
    """
...
def apply_messages(
    ...
    write_gnmi_files(localhost, my_duthost, ptfhost, env, delete_list, update_list, max_updates_in_single_cmd)
    time.sleep(wait_after_apply)


def apply_gnmi_file(localhost, my_duthost, ptfhost, dest_path=None, config_json=None,
    ...
    if dest_path:
        ...
        ret = my_duthost.shell(dut_command)
...
def write_gnmi_files(localhost, my_duthost, ptfhost, env, delete_list, update_list, max_updates_in_single_cmd):
    ...
    if delete_list:
        ...
        for delete_list in delete_list_group:
            gnmi_set(my_duthost, ptfhost, delete_list, [], [])
    if update_list:
        update_list_group = _devide_list(update_list)
        for update_list in update_list_group:
            gnmi_set(my_duthost, ptfhost, [], update_list, [])
