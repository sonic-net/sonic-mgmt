...
class SonicDockerManager:
    ...
    Common usage:
        1. Execute command on `syncd` for single-asic and `syncd0` for multi asic
            ...
            duthost.containers().bgp()[0].exec("echo hello-world")

        2. Execute command on `syncd` for single-asic and `syncd0`, `syncd1`, ...`syncdN` for multi asic
        in PARALLEL

            duthost.containers().bgp().exec("echo hello-world")

        3. Execute command only on `syncd1`

            duthost.containers().bgp(1).exec("echo hello-world")
            # or
            duthost.containers().bgp(namespace="asic1").exec("echo hello-world")
            # or
            duthost.containers().bgp(asic_index=1).exec("echo hello-world")
        ...
        def __getattr__(self, attr):
            try:
                ...
                def _handle(*args, **kwargs):
                    ...
                    with SafeThreadPoolExecutor(max_workers=len(self)) as executor:
                        for docker in self:
                            result[self.index(docker)] = executor.submit(
    ...
    def __getattr__(self, attr):
        ...
        if attr in object.__getattribute__(self, "containers"):
            def _handle(asic_index=None, namespace=None):
                ...
                if namespace:
                    asic_index = int(re.search(r"(\d+)$", namespace).group())
    ...
    def exec(self, container_name, cmd, shell=False, **kwargs):
        exec_cmd = self.duthost.command if not shell else self.duthost.shell
