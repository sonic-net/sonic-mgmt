...
class CiscoHost(AnsibleHostBase):
    ...
    def __getattr__(self, module_name):
        ...
        # Note: 'copy' is intentionally NOT listed here -- CiscoHost defines
        # its own copy() method (SFTP via Paramiko) because IOS XR doesn't
        # support the stock Ansible copy/net_put modules. Python's attribute
        # lookup resolves cisco_host.copy to that explicit method before
        # __getattr__ is consulted, so listing it here would be dead.
        allowed_modules = ['shell', 'command', 'file', 'fetch', 'stat', 'template']
    ...
    def copy(self, src=None, dest=None, content=None, **kwargs):
        ...
        if content is not None:
            # Create a temporary file with the content. NamedTemporaryFile creates
            # the file on disk immediately, so mark it for cleanup before any write
            # that could raise (e.g., disk full, encoding error). Otherwise a write
            # failure would leak the file because the finally block below skips
            # cleanup when temp_file_created is False.
                    ...
                    try:
                        ...
                        if idle_time > idle_timeout:
                            logger.debug(
                                ...
                                f"(>{idle_timeout}s idle timeout) - command appears complete")
                            break
