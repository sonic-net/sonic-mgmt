...
class AttributeManager:
    ...
    def build_port_attributes(
        ...
        """Build resolved attributes for the selected categories.

        Args:
            dut_name: Selected DUT hostname.
            platform: Selected DUT platform.
            hwsku: Selected DUT HWSKU.
            categories: Optional iterable of category names. ``None`` preserves
                the existing all-category behavior.
            category_validators: Optional mapping of category name to a
                callable accepting ``(kind, path, meta, body)``. Validators run
                for every supplied shard before resolution, including shards
                that do not match the selected DUT's ports.
        """
        selected_categories = None if categories is None else set(categories)
        discovered_categories = self._discover_categories()
        ...
        validators = category_validators or {}
        for category_name, category_dir in categories:
            ...
            if validator:
                for kind, path, meta, body in shards:
                    try:
                        validator(kind, path, meta, body)
                    except AttributeMergeError:
                        raise
                    except Exception as e:
                        raise AttributeMergeError(
                            f"Category '{category_name}' validation failed for {path}: {e}"
                        ) from e
