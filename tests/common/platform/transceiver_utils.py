...
def is_cmis_version_supported(cmis_version, min_required_version=5.0):
    """
...
def get_supported_available_optical_interfaces(eeprom_infos, parsed_presence, min_cmis_version=5.0):
    ...
    @return: list: List of available optical interface names that meet all criteria
    """
    ...
    for port_name, eeprom_info in eeprom_infos.items():
        ...
        if "SFP EEPROM detected" not in eeprom_info[port_name]:
            continue
        media_technology = eeprom_info.get("Media Interface Technology", "COPPER").upper()
        if "COPPER" in media_technology:
            continue
        cmis_version = eeprom_info.get("CMIS Revision", "0")
        if not is_cmis_version_supported(cmis_version, min_cmis_version):
