...
scan_dbs = {
        ...
        "app-db": {
            ...
            "keys_skip_val_comp": {
                ...
                "post3",
                "main",
                # SerDes driver-strength values that portmgrd re-applies from
                # media-settings asynchronously after a port is re-added via
                # `config apply-patch`. They may transiently differ from the
                # pre-patch snapshot, so exclude them from the comparison
                # (same rationale as the pre/post/main fields above).
                "idriver",
                "ipredriver"
            }
        },
