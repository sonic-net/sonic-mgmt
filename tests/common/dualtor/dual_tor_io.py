...
class DualTorIOV2:
    """Class to conduct I/O verification on dualtor."""

    @staticmethod
    def _calculate_test_network(vlan_interface_ip):
        """Calculate valid range for T1 src/dst addresses."""
        # for vlan interface ip as 192.168.0.0/21, the test network will be 192.168.8.0/21
        vlan_interface = ipaddress.ip_interface(vlan_interface_ip)
        vlan_interface_network = vlan_interface.network
        vlan_interface_network_supernet = vlan_interface_network.supernet()
        test_network = list(vlan_interface_network_supernet.address_exclude(vlan_interface_network))[0]
        return str(test_network.network_address), test_network.num_addresses

    def __init__(
        self, cable_type, activehosts, ptfhost, ptfadapter, tbinfo, io_ready,
        standbyhost=None, tor_vlan_port=None, send_interval=0.01
    ):
        self.cable_type = cable_type
        self.tor_vlan_intf = tor_vlan_port
        self.activehosts = activehosts
        self.standbyhost = standbyhost
        self.ptfadapter = ptfadapter
        self.ptfhost = ptfhost
        self.tbinfo = tbinfo
        self.io_ready_event = io_ready
        ...
        # assume vlan configs are the same on both ToRs
        first_active_tor_config = self.activehosts[0].get_running_config_facts()
        vlan_table = first_active_tor_config["VLAN"]
        vlan_name = list(vlan_table.keys())[0]
        vlan_interfaces = first_active_tor_config["VLAN_INTERFACE"]
        vlan_interface_ipv4 = [_ for _ in vlan_interfaces if isinstance(ipaddress.ip_interface(_), ipaddress.IPv4Interface)][0]
        self.vlan_mac = vlan_table[vlan_name]["mac"]

        self.src_addr, self.n_hosts = self._calculate_test_network(vlan_interface_ipv4)

        self.activehosts_mg_facts = [_.get_extended_minigraph_facts(self.tbinfo) for _ in self.activehosts]

        # get device port to ptf port mapping for each active ToR
        self.tor_to_ptf_intf_maps = {activehost.hostname: mg_facts for activehost, mg_facts in zip(self.activehosts, self.activehosts_mg_facts)}

        # get the mapping from ptf ports that connected to portchannels to the corresponding device mac
        self.pc_ptf_intfs_to_tor_lladdrs = {}
        self.active_host_lladdrs = {}
        for activehost, mg_facts in zip(self.activehosts, self.activehosts_mg_facts):
            self.active_host_lladdrs[activehost.hostname] = activehost.facts["router_mac"]
            portchannel_info = mg_facts['minigraph_portchannels']
            for pc in portchannel_info.values():
                for member in pc['members']:
                    self.pc_ptf_intfs_to_tor_lladdrs[self.tor_to_ptf_intf_maps[activehost.hostname][member]] = activehost.facts["router_mac"]

        self.mux_cable_table = first_active_tor_config["MUX_CABLE"]
        self.mux_port_to_ptf_intf_map = {}
        self.test_interfaces = {}
        self.ptf_intf_to_server_ip_map = {}

        first_active_tor_to_ptf_intf_map = self.tor_to_ptf_intf_maps[self.activehosts[0].hostname]
        for mux_port, port_config in natsorted(self.mux_cable_table.items()):
            if port_config.get("cable_type", CableType.default_type) == self.cable_type:
                self.test_interfaces.append(mux_port)
                ptf_intf = first_active_tor_to_ptf_intf_map[mux_port]
                self.mux_port_to_ptf_intf_map[mux_port] = ptf_intf
                server_ip = str(port_config['server_ipv4'].split("/")[0])
                self.ptf_intf_to_server_ip_map[ptf_intf] = server_ip
        
        self._configure_arp_responder()

        logger.info("Cable type: %s", self.cable_type)
        logger.info("Mux ports: %s".format(str(self.mux_cable_table)))
        logger.info("Selected testing interfaces: %s", self.test_interfaces)

        self.time_to_listen = 300.0
        self.sniff_time_incr = 0
        self.send_interval = max(send_interval, 0.0035)
    ...
    def _configure_arp_responder(self):
        """
        @summary: Generate ARP responder configuration using vlan_host_map.
        Copy this configuration to PTF and restart arp_responder
        """
    ...
    def start_io_test(self, traffic_generator=None):
        ...
        # start and later join the sender and sniffer threads
        self.send_and_sniff(
            sender=self.traffic_sender_thread,
            sniffer=self.traffic_sniffer_thread
        )
    ...
    def generate_from_t1_to_server(self):
        ...
        if self.tor_vlan_intf:
            # If destination VLAN intf is specified,
            # use only the connected server
            ptf_port = self.mux_port_to_ptf_intf_map[self.tor_vlan_intf]
            server_ip_list = [
                self.ptf_intf_to_server_ip_map[ptf_port]
            ]
        else:
        ...
        logger.info("-"*20 + "T1 to server packet" + "-"*20)
        logger.info("PTF source intf: random")
        logger.info(
            "Ethernet address: dst: {} src: {}".format(
                list(self.active_host_lladdrs.values()),
                'random'
            )
        )
        logger.info("IP address: dst: {} src: random".format('all' if len(server_ip_list) > 1 else server_ip_list[0]))
        logger.info("TCP port: dst: {}".format(TCP_DST_PORT))
        logger.info("DUT mac: {}".format(str(self.active_host_lladdrs)))
        logger.info("VLAN mac: {}".format(self.vlan_mac))
        logger.info("-"*50)
        ...
        for i in range(self.packets_per_server):
            for server_ip in server_ip_list:
                packet = tcp_tx_packet_orig.copy()
                ptf_src_intf = random.choice(
                    self.pc_ptf_intfs_to_tor_lladdrs.keys()
                )
                eth_src = self.ptfadapter.dataplane.get_mac(
                    0, ptf_src_intf
                )
                eth_dst = self.pc_ptf_intfs_to_tor_lladdrs[ptf_src_intf]
                packet[scapyall.Ether].src = eth_src
                packet[scapyall.Ether].dst = eth_dst
                packet[scapyall.IP].src = self.random_host_ip()
                packet[scapyall.IP].dst = server_ip
                payload = str(i) + payload_suffix
                packet.load = payload
        ...
        self.sent_pkt_dst_mac = list(self.active_host_lladdrs.values())
        self.received_pkt_src_mac = [self.vlan_mac]
    ...
    def generate_from_server_to_t1(self):
        ...
        for i in range(self.packets_per_server):
            for vlan_intf in vlan_src_intfs:
                ptf_src_intf = self.mux_port_to_ptf_intf_map[vlan_intf]
                server_ip = self.ptf_intf_to_server_ip_map[ptf_src_intf]
                eth_src = ptf_intf_to_mac_map[ptf_src_intf]
                payload = str(i) + payload_suffix
                packet = tcp_tx_packet_orig.copy()
        ...
        self.sent_pkt_dst_mac = [self.vlan_mac]
        self.received_pkt_src_mac = list(self.active_host_lladdrs.values())
    ...
    def examine_flow(self):
        ...
        filtered_packets = [ pkt for pkt in self.all_packets if
            ...
            self.check_tcp_payload(pkt) and
            (
                pkt[scapyall.Ether].dst in self.sent_pkt_dst_mac or
                pkt[scapyall.Ether].src in self.received_pkt_src_mac
            )
        ]
        logger.info("Number of filtered packets captured: {}".format(len(filtered_packets)))
        ...
        for server in server_to_packet_map.keys():
            server_to_packet_map[server].sort(
                key=lambda packet: (int(str(packet[scapyall.TCP].payload).replace('X','')), packet.time)
            )
    ...
    def examine_each_packet(self, server_ip, packets):
        ...
        for packet in packets:
            if packet[scapyall.Ether].dst in self.sent_pkt_dst_mac:
                # This is a sent packet
                num_sent_packets += 1
                continue
            if packet[scapyall.Ether].src in self.received_pkt_src_mac:
