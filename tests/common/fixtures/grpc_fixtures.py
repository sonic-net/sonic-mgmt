...
def setup_gnoi_tls_server(duthost, ptfhost):
    ...
    try:
        ...
        # 6. Cleanup: rollback configuration + remove generated certs
        ...
        try:
            ...
            logger.error("Failed to rollback configuration: %s", exc)

        try:
            _delete_gnoi_certs(cert_dir)
            logger.info("Certificate cleanup completed")
        except Exception as exc:
            logger.error("Failed to cleanup certificates: %s", exc)
