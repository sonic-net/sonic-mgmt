...
| ---- | -------------- | ---- |
| Amit Pawar | Nokia | Initial Draft |
| Amol Rawal | Nokia | Initial Draft |
...
3. **`kind == "top_all"`** (host-wide): **`parse_top_host_all(stdout)`** → one sample per process basename per tick (see README for dedupe).
4. **`kind == "top"`** (filtered): **`parse_top(stdout, proc_list)`** as before.
...
- Returns **`MemCpuMonitorResult`**: **`samples`**, **`events`**, **`timeline`** (merged, sorted by `(t_mono, seq)`), **`top_raw_log_path`** when a `top` raw file was created.
...
- **Snapshot markers**: vertical lines at each timeline event; the **event label** is repeated at the **top** of **each** of the three Y panels (not only the CPU panel).
...
- **JSON**: `nodeid`, `proc_subset`, **`plot_proc_subset_resolved`**, **`raw_command_log`** (path or null), **`top_raw_log`** (path or null), **`samples`**, **`events`** (serialized, ISO `t_wall`).
- **CSV**: one row per exported sample; same column set as JSON sample objects.
- Returns **`{"json": path, "csv": path}`** for files written.
...
- **`test_bgp_rib_route_optimztn_perf_v1`**: enables **`host_top_all_procs=True`**, **`include_host_free=True`**, **`capture_raw_stdout=True`**, then **`plot`/`export_samples`** with adaptive subsetting.
...
sequenceDiagram
    ...
    loop each iteration
        H->>M: snapshot(event=...)
        M->>D: top / free (periodic in parallel)
    end
    H->>M: stop() in finally
