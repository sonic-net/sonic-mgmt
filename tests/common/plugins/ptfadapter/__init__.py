...
CHECK_PORTS_DATA_READY_AFTER_NN_AGENT_START_INITIAL_DELAY = 5
...
def check_nn_agent_ready(adapter, sample_size=None):
    ...
    if not are_all_ports_ready:
        logger.warning(f"ptf_nn_agent not fully ready - {len(ports_to_check)} ports (out of {num_ports} ports) still not ready")
