...
def verify_packet_trimming(duthost, ptfadapter, ingress_port, egress_port, block_queue, send_pkt_size,
    ...
    try:
        if "PortChannel" in egress_port["name"]:
            logger.info("Using PortChannel members for trimming test")
            trimmed_ports = egress_port['dut_members']
            port_compute_buffer = egress_port['dut_members'][0]
        else:
            logger.info("Using physical interface for trimming test")
            trimmed_ports = egress_port['name']
            port_compute_buffer = egress_port['name']
...
def get_test_ports(upstream_links, downstream_links, peer_links, mg_facts):
    ...
    # Check if egress_port_1 is a PortChannel member and process if needed
    if is_portchannel_member(egress1_key, mg_facts):
        egress_port_1 = update_port_with_portchannel_info(egress_port_1, egress1_key, mg_facts)
    ...
    # Check if egress_port_2 is a PortChannel member and process if needed
    if is_portchannel_member(egress2_key, mg_facts):
        egress_port_2 = update_port_with_portchannel_info(egress_port_2, egress2_key, mg_facts)
...
def configure_tc_to_dscp_map(duthost, egress_ports):
    ...
    if "PortChannel" in egress_ports[0]["name"]:
        # For PortChannel interfaces, apply to all member interfaces
        for member_interface in egress_ports[0]['dut_members']:
            port_qos_map[member_interface] = {"tc_to_dscp_map": "spine_trim_map"}
        logger.info(f"Applied spine_trim_map to PortChannel members: {egress_ports[0]['dut_members']}")
    else:
        # For physical interfaces, apply directly
        port_qos_map[egress_ports[0]['name']] = {"tc_to_dscp_map": "spine_trim_map"}
        logger.info(f"Applied spine_trim_map to physical interface: {egress_ports[0]['name']}")

    if len(egress_ports) == 2:
        tc_to_dscp_map["host_trim_map"] = {ASYM_TC: ASYM_PORT_2_DSCP}

        # Handle second egress port (host_trim_map)
        if "PortChannel" in egress_ports[1]["name"]:
            # For PortChannel interfaces, apply to all member interfaces
            for member_interface in egress_ports[1]['dut_members']:
                port_qos_map[member_interface] = {"tc_to_dscp_map": "host_trim_map"}
            logger.info(f"Applied host_trim_map to PortChannel members: {egress_ports[1]['dut_members']}")
        else:
            # For physical interfaces, apply directly
            port_qos_map[egress_ports[1]['name']] = {"tc_to_dscp_map": "host_trim_map"}
            logger.info(f"Applied host_trim_map to physical interface: {egress_ports[1]['name']}")
...
def verify_queue_and_port_trim_counter_consistency(duthost, port):
    ...
    pytest_assert(total_queue_trim_packets == port_trim_packets)
