...
def vlan_ping_setup(duthosts, rand_one_dut_hostname, ptfhost, nbrhosts, tbinfo, lower_tor_host):
    """
...
def test_vlan_ping(vlan_ping_setup, duthosts, rand_one_dut_hostname, ptfadapter, tbinfo, toggle_all_simulator_ports_to_rand_selected_tor_m):
    ...
    for member in ptfhost_info:
        if tbinfo["topo"]["name"] in dualtor_topo:
            verify_icmp_packet(duthost.facts['router_mac'], ptfhost_info[member], vmhost_info, ptfadapter, tbinfo, vlan_mac, dtor_ul=True)
            verify_icmp_packet(duthost.facts['router_mac'], vmhost_info, ptfhost_info[member], ptfadapter, tbinfo, vlan_mac, dtor_dl=True)
        else:
