...
def create_lag_port(duthost, config_port_indices):
    ...
    """
    lag_port_map = {}
    portchannels = list(duthost.config_facts(host=duthost.hostname, source="running")['ansible_facts'].get('PORTCHANNEL', {}).keys())
    port_list_idx = 0
...
def get_port(duthost, ptfhost, interface_num, port_type, ports_to_exclude=None, exclude_sub_interface_ports=False):
    ...
    sub_interface_ports = set([_.split(constants.VLAN_SUB_INTERFACE_SEPARATOR)[0] for _ in list(cfg_facts.get('VLAN_SUB_INTERFACE', {}).keys())])
