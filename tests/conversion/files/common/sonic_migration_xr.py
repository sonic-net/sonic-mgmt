...
if on_sonic:
    ...
    from cisco.pacific.power import power
...
XR_INTERMEDIATE_FILENAME = "8000-golden-x86_64-7.5.41.04I-SONIC_CON_K9SEC_UPD.iso"
XR_INTERMEDIATE_MD5 = "517b6435a4b9992478fb11e26c7348da"
...
if not on_sonic:
    INVENTORY_FILE = "/mnt/mtd0/inventory.json"
else:
    INVENTORY_FILE = "/mnt/obfl/inventory.json"
...
def module_healthcheck_xr(interval=0, timeout=0, ignore_shut=False):
    ...
    This function validates the operational status of modules based on an inventory file
    if it exists, or checks all modules if no inventory file is present.
    ...
    while True:
        ...
        fail_state = 0
         # Case 1: Inventory file exists
        if os.path.exists(INVENTORY_FILE):
            with open(INVENTORY_FILE, "r") as f:
                inventory_modules = json.load(f)
            tracked_operational = [module["node"] for module in inventory_modules.get("operational", [])]
            for module in platform_modules:
                if module["node"] in tracked_operational:
                    if (
                        (module["state"] not in ["IOS XR RUN", "OPERATIONAL"])
...
def generate_inventory_file():
    """
    Generates an inventory file named 'inventory.json' that categorizes modules into operational and non-operational based on their current state.

    The function retrieves the platform information using the 'show platform' command, processes the output to determine the state of each module, and then writes this information into a JSON file.

    Returns:
        None. The function creates or overwrites the 'inventory.json' file in the current directory.
    """
    # Delete old inventory file if it exists
    if os.path.exists(INVENTORY_FILE):
        os.remove(INVENTORY_FILE)
        log_msg("Existing inventory file deleted")

    # Generate new inventory file
    platform_modules = parse_show_platform()
    operational_nodes = []
    non_operational_nodes = []

    for module in platform_modules:
        module_info = {
            "node": module.get("node", ""),
            "type": module.get("type", ""),
            "config_state": module.get("config_state", ""),
        }

        if module["state"] in ["IOS XR RUN", "OPERATIONAL"]:
            operational_nodes.append(module_info)
        else:
            non_operational_nodes.append(module_info)
    ...
    log_msg(
        "Inventory file created to track initial module state information for start of migration"
    )
...
def shutdown_unknown_lc():
    """
    Shuts down all nodes whose type is UNKNOWN and state is not 'IOS XR RUN' or 'OPERATIONAL'.
    """
    platform_modules = parse_show_platform()
    ...
    for module in platform_modules:
        ...
        if type == "UNKNOWN" and state not in ["IOS XR RUN", "OPERATIONAL"]:
            log_msg("Shutting down UNKNOWN module {node}".format(node=node))
            xr_cli_cmd("shutdown location {node} noprompt".format(node=node))
    time.sleep(20)
...
def execution_precheck():
    ...
    Prechecks:
    1. Shutdown Unknown LCs:
        - Identifies and shuts down any line cards with type "UNKNOWN" that are not operational to ensure that PID of LC is visible for PID precheck

    2. Generate Inventory File:
        - Creates an inventory file categorizing modules into operational and non-operational states.

    3. Validate Device Version:
    ...
    log_msg("Shutting down any UNKNOWN LCs that are not operational")
    shutdown_unknown_lc()

    log_msg("Creating inventory file")
    ...
    if not os.path.exists(INVENTORY_FILE):
        exit_script(
            return_code="053",
            exit_reason="Inventory file does not exist.",
        )
    ...
    modules = inventory.get("operational", []) + inventory.get("non_operational", [])
...
def ov_precheck():
    ...
    1. Verify BIOS, X86FPGA, and TAM Versions:
       ...
       - Skips modules in module_healthcheck_override or non-operational inventory
    ...
    if os.path.exists(INVENTORY_FILE):
        with open(INVENTORY_FILE, "r") as f:
            inventory = json.load(f)
            non_operational_nodes = [m["node"] for m in inventory.get("non_operational", [])]
    ...
    for i in range(0, len(result_items)):
        ...
        if split_result[0] in module_healthcheck_override:
            continue
        if non_operational_nodes and split_result[0] in non_operational_nodes:
            continue
        if (
...
def get_lc_powered_off(interval=0, timeout=0, is_migration_context=False):
    """
    Retrieves the list of Line Card (LC) slots that are Offline by parsing the output using platform library.
    - In normal (non-migration) context:
        Only LCs with oper_status == "Offline" are treated as powered off.
    - In migration context:
        Both "Online" and "Offline" LCs are treated as powered off, as iPXE LC can still appear as online.
        ...
        for module in modules:
            ...
            if "LINE-CARD" in name:
                ...
                if not is_migration_context:
                    ...
                    # In migration context, consider Online and Offline LCs as powered off
                    if oper_status == "Offline" or oper_status == "Online":
                        lc_powered_off.append(slot)
                        fail_state = 1
...
def determine_rma_or_migration(is_migration_context=False):
    """
    ...
    - If running in an explicit migration context (is_migration_context=True), immediately return "RMA" to handle LC conversion through LC RMA in case of migration failure.
    ...
    if is_migration_context:
        log_msg("In migration context, proceed with RMA workflow for migration")
        return "RMA"
    elif not os.path.exists("/mnt/obfl/.xr2sonic"):
...
def sonic_migration_precheck():
    ...
    3. Line Card iPXE Service Preparation:
        - Reads inventory file to determine LC availability and operational status
        - If operational LCs exist: Proceeds with operational LC with lowest slot number for iPXE service
        - If no operational LCs: Identifies target LC with lowest slot number for iPXE service
            - Reloads target LC with network boot media to enable iPXE service
            - Monitors LC state transition to "IOS XR RUN" with 30-second intervals and 600-second timeout
            - Updates inventory file to reflect new LC operational status

    4. Check Line Card Health:
    ...
    if os.path.exists(INVENTORY_FILE):
        log_msg("Check inventory file to determine target LC for iPXE service")
        with open(INVENTORY_FILE, "r") as f:
            inventory = json.load(f)
        operational = inventory.get("operational", [])
        non_operational = inventory.get("non_operational", [])

        if any("LC" in str(module.get("type", "")) for module in operational):
            log_msg("At least one LC is already operational.")

        else:
            lc_candidates = [module for module in non_operational if "LC" in str(module.get("type", ""))]
            slot_pattern = re.compile(r"^\S+/(\S+)/\S+$")
            def get_slot(mod):
                node = mod.get("node", "")
                match = slot_pattern.match(node)
                if match:
                    return int(match.group(1))
            target_lc = min(lc_candidates, key=get_slot)
            log_msg("Target LC for iPXE service: {lc}".format(lc=target_lc["node"]))

            log_msg("Reloading target LC {lc} to enable iPXE service".format(lc=target_lc["node"]))
            xr_cli_cmd("reload bootmedia network location {lc} noprompt".format(lc=target_lc["node"]))

            def wait_for_lc_operational(interval=0, timeout=0):
                start_time = time.time()
                while True:
                    platform_modules = parse_show_platform()
                    for module in platform_modules:
                        if module["node"] == target_lc["node"]:
                            if module["state"] in ["IOS XR RUN"]:
                                log_msg("LC {lc} is now operational.".format(lc=module["node"]))
                                operational.append(target_lc)
                                inventory["operational"] = operational
                                inventory["non_operational"] = [
                                    module for module in non_operational if module["node"] != target_lc["node"]
                                ]
                                # Write back to inventory file
                                log_msg("Updating inventory file.")
                                with open(INVENTORY_FILE, "w") as f:
                    ...
                    if elapsed_time >= timeout:
                        log_msg(
                            "Module {lc} did not become operational within the timeout period.".format(lc=target_lc["node"]),
                            "ERROR",
                            "054",
                        )
                        exit_script(
                            return_code="054",
                            exit_reason="Failure. Target LC for iPXE service did not become operational within the timeout period.",
                        )
                    time.sleep(interval)

            wait_for_lc_operational(interval=30, timeout=600)
...
def sonic_migration():
    ...
    Steps:
    1. Extracts information about RP and LC from both the inventory file and "show platform" output using the function 'parse_show_platform()'. If an inventory file exists, only processes modules listed as operational in the inventory. Based on this filtered information, it determines the active RP location, the LC location where iPXE service will be enabled, and the LC locations that will be temporarily disabled for migration.
    ...
    if not args.default_config:
        log_msg("Copying all minigraph files to RP")
        minigraph_files = minigraph_files_list()

        shell_cmd("rm -f /mnt/mtd0/minigraph* /mnt/mtd0/_minigraph*")
        ...
        for minigraph_file_name in minigraph_files:
            if "sup00" in minigraph_file_name:
                ...
                shell_cmd(
                    "cp /harddisk:/{minigraph_file_name} /mnt/mtd0/_{minigraph_file_name}".format(
                        minigraph_file_name=minigraph_file_name
                    )
                )
            log_msg(
                "Copied minigraph file {minigraph_file_name} to /mnt/mtd0/".format(
                    minigraph_file_name=minigraph_file_name
                )
            )
    ...
    if os.path.exists(INVENTORY_FILE):
        with open(INVENTORY_FILE, "r") as f:
            inventory = json.load(f)
            operational_nodes = [m.get("node") for m in inventory.get("operational", []) if "node" in m]
    ...
    lc_location_pxe = ""
    for module in platform_modules:
        if operational_nodes and module["node"] not in operational_nodes:
            continue
...
def sonic_check_lc_on_xr():
    """
    Determines whether RMA, Partial Migration, or Migration with RMA workflow is required during a SONiC migration, and exits the script with an appropriate return code.
    ...
    Depending on the returned state, it maps the result to a specific exit code:
        ...
        - "RMA / Partial Migration / Migration with RMA workflow is required. " - exit code 0
    ...
    log_msg("Check if RMA or Partial Migration or Migration with RMA workflow is required or not")
    if not os.path.exists("/mnt/obfl/.xr2sonic"):
        is_migration_context = False
    else:
        is_migration_context = True
    state = determine_rma_or_migration(is_migration_context)
    if state == "RMA":
        log_msg("RMA/partial migration/migration with RMA workflow is required")
        lc_powered_off = get_lc_powered_off(is_migration_context=is_migration_context)
        log_msg("LCs requiring RMA/Partial Migration/Migration with RMA workflow: {}".format(lc_powered_off))
...
def lc_recovery_on_xr():
    ...
    - If RMA:
        ...
        - Reload each powered-off line card.
        - Build and execute the RMA command: `lc_migration -s {}`
        - Trigger FPD upgrade checks and perform necessary upgrades for the powered-off LCs via `sonic_fpd_upgrade()`.
        - Remove the migration context(`/mnt/obfl/.xr2sonic`) if present.
        - Perform post-checks using `sonic_migration_postcheck()`.
    """
    global log_context
    ...
    state = determine_rma_or_migration(is_migration_context)
    if state == "RMA":
        log_msg("RMA/Partial Migration/Migration with RMA workflow case. Executing RMA/LC Migration Workflow")
        ...
        p = power()
        for lc in lc_powered_off:
            log_msg("Reloading LC in slot {lc}".format(lc=lc))
            p.reload_remote_card(lc)
        ...
        if not is_migration_context:
            time.sleep(60)
            sonic_fpd_upgrade(lc_powered_off)
        ...
        exit_script(return_code="052", exit_reason="Migration context is present. Not in RMA or partial migration LC or Migration with RMA workflow scenario")


def sonic_fpd_upgrade(linecards, postcheck=False):
    """
    Checks and optionally upgrades FPD (Field Programmable Device) components on specified SONiC linecards.
    For each linecard in the provided list, this function connects via SSH, retrieves the current and available
    FPD versions, and determines if an upgrade is required. If an upgrade is needed and `postcheck` is False,
    it performs the upgrade. If `postcheck` is True, it only checks and logs the status without performing upgrades.
    Args:
        linecards (list): List of linecard slot identifiers to check and/or upgrade.
        postcheck (bool, optional): If True, only checks and logs upgrade requirements without performing upgrades.
                                    If False (default), performs upgrades where necessary.
    Returns:
        bool or None: If `postcheck` is True, returns True if any upgrade is needed, otherwise False.
                      If `postcheck` is False, returns None.
    """
    overall_upgrade_needed = False

    for slot in linecards:
        host = "lc{slot}".format(slot=slot)
        output = sonic_ssh_command(host, SONiC_USER, SONiC_PASSWORD, "sudo fwutil show updates")
        log_msg("fwutil show updates output for host {host}. Output:\n{output}".format(host=host, output=output) , "DEBUG")
        if any(err in output for err in ["Error", "Aborting", "Aborted"]):
            exit_script(
                return_code="057",
                exit_reason="'fwutil show updates' returned error on {host}. Output:\n{output}".format(
                    host=host,
                    output=output
                )
            )
        fpd_modules = parse_sonic_fpd(output)
        log_msg(fpd_modules, "DEBUG")

        for item in fpd_modules:
            current = item["Current"]
            available = item["Available"]
            component = item["Component"]

            if component.strip() in ["IOFPGA", "Kenley"]:
                if is_fpd_upgrade_required(current, available):
                    overall_upgrade_needed = True
                    log_msg(
                        " Upgrade required for {component} of {host}, Current: {current}, Available: {available}".format(
                            component=component,
                            host=host,
                            current=current,
                            available=available
                    ...
                    if not postcheck:
                        command = "sudo fwutil update chassis component {component} fw".format(component=component)
                        result = sonic_ssh_command(host, SONiC_USER, SONiC_PASSWORD, command)
                        log_msg("{command} output for host {host}. Output:\n{result}".format(command=command, host=host, result=result) , "DEBUG")
                        success_line = None
                        for line in result.splitlines():
                            if "SUCCESS" in line:
                                ...
                                break
                        if success_line:
                            log_msg("Firmware update successful: {success_line}".format(success_line=success_line))
                        else:
                            exit_script(
                                return_code="057",
                                exit_reason="Firmware update did not report SUCCESS for {component} of {host}"
                                    .format(component=component, host=host)
                            )
                else:
                    log_msg(
                        " {component} of {host} is up-to-date".format(
                            component=component,
                            host=host,
                        )
                    )
    ...
    return overall_upgrade_needed


def is_fpd_upgrade_required(current, available):
    """
    Determines if an upgrade is required by comparing the current and available version strings.
    Args:
        current (str): The current version string, with components separated by hyphens (e.g., '1-2-3').
        available (str): The available version string, with components separated by hyphens (e.g., '1-2-4').
    Returns:
        bool: True if the available version is greater than the current version, False otherwise.
    """
    current_f = float(current.replace('-', '.'))
    available_f = float(available.replace('-', '.'))
    return available_f > current_f

def delete_sonic_and_onie_images(dir="/opt/cisco/var/tftp/"):
    """
    Deletes ONIE (*.pxe) and SONiC (*.bin) image files from the specified directory.

    Args:
        dir (str): The directory from which to delete the images. Default is '/opt/cisco/var/tftp/'.
    ...
    for file in os.listdir(dir):
        if (
            fnmatch.fnmatch(file, '*.pxe') or
            fnmatch.fnmatch(file, '*.bin') or
            file == "sonic-cisco-8000.bin.openssl.ipxcontainer"
        ):
    ...
    log_msg("Deleting SONiC and ONIE images from {dir}".format(dir=dir))
    for file_name in image_files:
        file_path = os.path.join(dir, file_name)
        os.remove(file_path)
        log_msg("Removed image file {file_path}".format(file_path=file_path))

def delete_minigraph_files(dir="/opt/cisco/var/tftp/"):
    """
    Deletes minigraph files from the specified directory.

    Args:
        dir (str): The directory from which to delete the minigraph files. Default is '/opt/cisco/var/tftp/'.
    ...
    log_msg("Deleting minigraph files from {dir}".format(dir=dir))
    for file_name in xml_files:
        file_path = os.path.join(dir, file_name)
        os.remove(file_path)
        log_msg("Removed minigraph file {file_path}".format(file_path=file_path))

def copy_and_rename_minigraph_files(src_dir="/mnt/obfl/", dest_dir="/opt/cisco/var/tftp/"):
    """
    Copies and renames minigraph files from the source directory to the destination directory.

    Args:
        src_dir (str): The source directory where the minigraph files are located. Default is '/mnt/mtd0/'.
        dest_dir (str): The destination directory where the minigraph files will be copied. Default is '/opt/cisco/var/tftp/'.

    Returns:
        None.
    """
    delete_minigraph_files()
    log_msg("Copying and renaming minigraph files from {src_dir} to {dest_dir}".format(src_dir=src_dir, dest_dir=dest_dir))
    pattern = re.compile(r"^_minigraph-.*-(lc\d{2})\.xml$", re.IGNORECASE)
    for file_name in os.listdir(src_dir):
        match = pattern.match(file_name)
        if match:
            suffix = match.group(1).lower()
            new_name = "minigraph-{suffix}.xml".format(suffix=suffix)
            src_file_path = os.path.join(src_dir, file_name)
            dest_file_path = os.path.join(dest_dir, new_name)
            shell_cmd("sudo cp {src} {dest}".format(src=src_file_path, dest=dest_file_path))
            log_msg("Copied and renamed {src} to {dest}".format(src=src_file_path, dest=dest_file_path))
...
def sonic_migration_lc():
    ...
    - If SONiC migration is required:
        - Copies and renames minigraph files from /mnt/obfl/ to /opt/cisco/var/tftp/
    ...
    if state == "RMA":
        ...
        copy_and_rename_minigraph_files()
        log_msg("Execute SONiC migration for required LCs")
        shell_cmd("rm /mnt/obfl/.xr2sonic")
...
def sonic_migration_postcheck():
    ...
    2. Cleanup Operations:
        - Removes minigraph files from '/opt/cisco/var/tftp/' directory
        - Removes ONIE (*.pxe) and SONiC (*.bin) image files from '/opt/cisco/var/tftp/'
        - Removes ONIE and SONiC image files from '/host/' directory
        - Removes minigraph files from '/host/' directory
        - Removes inventory file '/mnt/obfl/inventory.json' if it exists
    ...
    delete_minigraph_files()
    delete_sonic_and_onie_images()
    delete_sonic_and_onie_images(dir="/host/")
    delete_minigraph_files(dir="/host/")

    if os.path.exists(INVENTORY_FILE):
        log_msg("Removing inventory file {INVENTORY_FILE}".format(INVENTORY_FILE=INVENTORY_FILE))
        shell_cmd("rm /mnt/obfl/inventory.json")
...
def parse_sonic_fpd(output):
    """
    Parses the output of "sudo fwutil show updates" command.
    This function processes the given multiline string output, extracting information about
    FPD modules, including chassis, module, component, firmware, current and available versions,
    and status. It skips header and separator lines, and handles cases where the chassis or module
    fields may be missing or repeated.
    Args:
        output (str): The raw multiline string output from the SONiC FPD version command.
    Returns:
        list[dict]: A list of dictionaries, each representing an FPD module with the following keys:
            - 'Chassis' (str or None): The chassis identifier.
            - 'Module' (str or None): The module identifier.
            - 'Component' (str): The component name.
            - 'Firmware' (str): The firmware name.
            - 'Current' (str): The current firmware version.
            - 'Available' (str): The available firmware version.
            - 'Status' (str): The status of the firmware.
    """
    log_msg("Parsing SONiC 'sudo fwutil show updates' output")
    fpd_modules = []
    prev_chassis = None

    lines = output.splitlines()

    header_keywords = ["Chassis", "Firmware", "Version", "Status"]
    separator_line = re.compile(r"^[\s\-]+$")

    header_idx = None
    for i, line in enumerate(lines):
        if all(key in line for key in header_keywords):
            header_idx = i
            break

    log_msg("Header line index: {header_idx}".format(header_idx=header_idx), "DEBUG")
    if header_idx is None:
        exit_script(
                return_code="056",
                exit_reason="Header line not found in 'fwutil show updates' output. Output:\n{output}".format(
                    output=output
                )
            )
    header_line = lines[header_idx]
    chassis_start = header_line.find("Chassis")
    module_start = header_line.find("Module")
    chassis_end = module_start
    module_end = header_line.find("Component")

    regex_rest = re.compile(
        r"""
        \s*(?P<Component>\S+)
        \s+(?P<Firmware>\S+)
        \s+(?P<Current>[\d.-]+)\s*/\s*(?P<Available>[\d.-]+)
        \s+(?P<Status>.+)$
        """,
        re.VERBOSE
    )
    ...
    for line in lines[header_idx+2:]:
        ...
        chassis = line[chassis_start:chassis_end].strip()
        ...
        rest_match = regex_rest.match(line[module_end:])
        if not rest_match:
            continue

        entry = rest_match.groupdict()
        entry["Chassis"] = chassis
        entry["Module"] = module

        fpd_modules.append(entry)
...
def sonic_ssh_command(host, user, password, command, timeout=300):
    """
    Executes a command on a remote host via SSH using the provided credentials.
    This function uses `pexpect` to automate the SSH login process, handle password prompts,
    and optionally respond to confirmation prompts (such as "Continue? [y/N]:"). The command
    output is returned as a string. If any error occurs during the SSH session, the script
    ...
    Args:
        ...
        timeout (int, optional): Timeout in seconds for the SSH session. Defaults to 300.
    Returns:
        str: The output from the executed command on the remote host.
    Raises:
        SystemExit: Exits the script if the SSH command fails or an exception occurs.
    """
    ssh_cmd = "ssh -o StrictHostKeyChecking=no {user}@{host} '{command}'".format(
        user=user, host=host, command=command
    )
    log_msg("Executing SSH command: {}".format(ssh_cmd))
    ...
    try:
        ...
        exit_script(
            return_code="055",
            exit_reason="SSH command {command} to host {host} failed: {e}".format(
                command=command, host=host, e=str(e)
            )
        )
...
def minigraph_files_list():
    ...
    If the inventory file exists, it considers all operational and non-operational modules from it.
    Otherwise, this function parses the output of `parse_show_platform()` to identify modules with the
    ...
    if os.path.exists(INVENTORY_FILE):
        with open(INVENTORY_FILE, "r") as f:
            inventory = json.load(f)
        platform_modules = inventory.get("operational", []) + inventory.get("non_operational", [])
    else:
        platform_modules = [
            module for module in parse_show_platform() if module.get("state") == "IOS XR RUN"
        ]
    minigraph_files = []
    missing_files = []
    file_found = 0
