...
def test_voq_fabric_isolation_status(duthosts, enum_frontend_dut_hostname):
    ...
    cmd = "sonic-db-cli STATE_DB hget 'FABRIC_PORT_TABLE|PORT{}' STATUS".format(port)
    cmd_output = duthost.shell(cmd, module_ignore_errors=True)["stdout"].split("\n")
    logger.info(cmd_output)
    lnkStatus = cmd_output[0]
    pytest_assert(lnkStatus == "up", "Port {} is down and is expected to be up".format(port))

    pytest_assert(wait_until(1200, 60, 0, check_skip_poll, duthost, port, '20'),
    ...
    try:
        ...
        cmd = "sonic-db-cli STATE_DB hset 'FABRIC_PORT_TABLE|PORT{}' TEST_CRC_ERRORS 0".format(port)
        cmd_output = duthost.shell(cmd, module_ignore_errors=True)["stdout"].split("\n")
        cmd = "sonic-db-cli STATE_DB hset 'FABRIC_PORT_TABLE|PORT{}' TEST_CODE_ERRORS 0".format(port)
        cmd_output = duthost.shell(cmd, module_ignore_errors=True)["stdout"].split("\n")
        cmd = "sonic-db-cli STATE_DB hset 'FABRIC_PORT_TABLE|PORT{}' TEST 'TEST'".format(port)
        cmd_output = duthost.shell(cmd, module_ignore_errors=True)["stdout"].split("\n")

        pytest_assert(wait_until(600, 60, 0, check_fabric_link_status, duthost, port, '0'),
                      "auto_isolated state for port {} should be 0".format(port))

        # Update fake crc errors by setting TEST_CRC_ERRORS to <num>
        # Check auto_isolation status
        cmd = "sonic-db-cli STATE_DB hset 'FABRIC_PORT_TABLE|PORT{}' TEST_CRC_ERRORS 2".format(port)
        cmd_output = duthost.shell(cmd, module_ignore_errors=True)["stdout"].split("\n")

        pytest_assert(wait_until(360, 60, 0, check_fabric_link_status, duthost, port, '1'),
        ...
        cmd = "sonic-db-cli STATE_DB hset 'FABRIC_PORT_TABLE|PORT{}' TEST_CODE_ERRORS 0".format(port)
        cmd_output = duthost.shell(cmd, module_ignore_errors=True)["stdout"].split("\n")
        cmd = "sonic-db-cli STATE_DB hset 'FABRIC_PORT_TABLE|PORT{}' TEST 'product'".format(port)
        cmd_output = duthost.shell(cmd, module_ignore_errors=True)["stdout"].split("\n")


def check_skip_poll(host, port, skip_poll):
    cmd = "sonic-db-cli STATE_DB hget 'FABRIC_PORT_TABLE|PORT{}' 'SKIP_CRC_ERR_ON_LNKUP_CNT'".format(port)
...
def check_fabric_link_status(host, port, state):
    cmd = "sonic-db-cli STATE_DB hget 'FABRIC_PORT_TABLE|PORT{}' AUTO_ISOLATED".format(port)
    cmd_output = host.shell(cmd, module_ignore_errors=True)["stdout"].split("\n")
