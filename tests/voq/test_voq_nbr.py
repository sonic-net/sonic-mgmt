...
def test_neighbor_hw_mac_change(duthosts, enum_rand_one_per_hwsku_frontend_hostname, enum_rand_one_frontend_asic_index,
    ...
    * Verify that packets can be sent from local and remote linecards to the learned address.
    ...
    try:
        logger.info("Changing ethernet mac on intf %s, port %s, vm %s", nbrinfo['shell_intf'], nbrinfo['port'], nbrinfo['vm'])
        ...
        logger.info("-" * 60)
        logger.info("Will Restore ethernet mac on intf %s, port %s, vm %s", nbrinfo['shell_intf'], nbrinfo['port'], nbrinfo['vm'])
