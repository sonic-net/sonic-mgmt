...
def check_interface_status(duthost):
    if ":547" in duthost.shell("docker exec -i dhcp_relay ss -nlp | grep dhcp6relay")["stdout"]:
        return True
    return False
...
def test_interface_binding(duthosts, rand_one_dut_hostname, dut_dhcp_relay_data):
    ...
    output = duthost.shell("docker exec -i dhcp_relay ss -nlp | grep dhcp6relay")["stdout"]
    logger.info(output)
    for dhcp_relay in dut_dhcp_relay_data:
