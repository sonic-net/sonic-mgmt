...
def test_dhcp_relay_default(ptfhost, dut_dhcp_relay_data, validate_dut_routes_exist, testing_config,
    ...
    try:
        for dhcp_relay in dut_dhcp_relay_data:
            ...
            expected_agg_counter_message = r".*dhcp_relay#dhcpmon\[[0-9]+\]: \[\s*Agg-%s\s*-[\sA-Za-z0-9]+\s*rx/tx\] Discover: +1/ +4, Offer: +1/ +1, Request: +3/ +12, ACK: +1/ +1+" % dhcp_relay['downlink_vlan_iface']['name']
