...
def check_interface_status(duthost):
    if ":67" in duthost.shell("docker exec -i dhcp_relay ss -nlp | grep dhcrelay", module_ignore_errors=True)["stdout"]:
        return True
...
def test_interface_binding(duthosts, rand_one_dut_hostname, dut_dhcp_relay_data):
    ...
    output = duthost.shell("docker exec -i dhcp_relay ss -nlp | grep dhcrelay", module_ignore_errors=True)["stdout"]
    logger.info(output)
    for dhcp_relay in dut_dhcp_relay_data:
