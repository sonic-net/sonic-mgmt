...
# The SONiC image build pre-pulls and tags this pause image directly onto the
# device (files/build_templates/sonic_debian_extension.j2), so it is always
# present locally on the DUT. The test daemonset deploys it with
# imagePullPolicy=Never so kubelet only ever uses the image already on the sonic
# device and never pulls from the internet (the remote k8s.gcr.io registry).
