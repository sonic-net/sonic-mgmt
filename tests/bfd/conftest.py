...
def bfd_cleanup_db(request, duthosts, enum_supervisor_dut_hostname, bfd_base_instance, autouse=True):
    ...
    logger.info("Make Sure orchagent CPU utilization is less that %d before starting the test", orch_cpu_threshold)
    duts = duthosts.frontend_nodes
    for dut in duts:
        assert(wait_until(100, 2, 0, check_orch_cpu_utilization, dut, orch_cpu_threshold),
    ...
    for dut in duts:
        assert(wait_until(45, 2, 0, check_orch_cpu_utilization, dut, orch_cpu_threshold),
                    "Orch CPU utilization {} > orch cpu threshold {} after the test"
                    .format(dut.shell("show processes cpu | grep orchagent | awk '{print $9}'")["stdout"],
                            orch_cpu_threshold))
    ...
    if hasattr(request.config, 'rp_asic_ids'):
        for id in request.config.rp_asic_ids:
            docker_output = rp.shell("docker ps | grep swss{} | awk '{print $NF}'".format(id))['stdout']
            if len(docker_output) == 0:
                container_status = False
    if not container_status:
        config_reload(rp)

    logger.info("Clearing BFD configs on {}, {}".format(request.config.src_dut, request.config.dst_dut))
    ...
    if hasattr(request.config, 'portchannels_on_dut'):
        ...
        logger.info("None of the portchannels are selected to flap. So skipping portchannel interface check")
        selected_interfaces = []

    if selected_interfaces:
        dut = request.config.src_dut if portchannels_on_dut == "src" else request.config.dst_dut
        asic = request.config.src_asic if portchannels_on_dut == "src" else request.config.dst_asic
