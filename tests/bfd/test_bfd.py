...
def get_neighbors(duthost, tbinfo, ipv6=False, count=1):
    ...
    return vlan_addr, prefix_len, [neighbor_addrs[_] for _ in indices], [neighbor_devs[_] for _ in indices], [neighbor_interfaces[_] for _ in indices]
...
def create_bfd_sessions(ptfhost, duthost, local_addr, neighbor_addrs):
    # Create a tempfile for BFD sessions
    bfd_file_dir = duthost.shell('mktemp')['stdout']
    bfd_config = []

    for neighbor_addr in neighbor_addrs:
        duthost.shell("sonic-db-cli APPL_DB hmset 'BFD_SESSION_TABLE:default:default:{}' local_addr {}".format(neighbor_addr, local_addr))
        bfd_config.append({
        ...
        ptfhost.shell("bfdd-control connect local {} remote {}".format(neighbor_addr, local_addr))
...
def remove_bfd_sessions(duthost, local_addr, neighbor_addrs):
    # Create a tempfile for BFD sessions
    bfd_file_dir = duthost.shell('mktemp')['stdout']
    bfd_config = []
...
def test_bfd(rand_selected_dut, ptfhost, tbinfo, toggle_all_simulator_ports_to_rand_selected_tor_m):
    ...
    vlan_addr, prefix_len, neighbor_addrs, neighbor_devs, neighbor_interfaces = get_neighbors(duthost, tbinfo, count = bfd_session_cnt)
