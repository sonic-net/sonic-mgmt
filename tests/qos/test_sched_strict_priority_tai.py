...
class StrictPriorityRateLimitingDriverTAI:
    ...
    def _apply_strict_priority_rate_limiting_config(self):
        ...
        - create_scheduler: Automatically handles platform-specific requirements (e.g., DNX credit worth)
        """
        ...
        try:
            ...
            # credit_worth is consumed only by platforms that need it (DNX/Q3D
            # sets its per-port credit worth); Tomahawk ignores it.
            scheduler_config = {
                'type': 'STRICT',
    ...
    def _revert_configuration(self):
        ...
        revert_scheduler restores any platform-specific state (e.g. the DNX
        credit worth on Q3D) before unbinding the scheduler. If that restore
        fails it leaves the SCHEDULER state untouched, so the framework
        CONFIG_DB diff check fires a config reload and resets SDK state.
        """
