...
class QosParamMarvell(QosSaiBase):
    ...
    def __dump_generated_params_in_file(self):
        ...
        gen_file_path = r'qos/files/qos_gen_{}.yml'.format(self.dutConfig['dutAsic'])
    ...
    def __calculate_qos_parameters(self):
        ...
        if "pkts_num_trig_pfc_shp" not in self.qos_params_invm[self.speed_cable_len]['hdrm_pool_size']:
            logging.debug("Adding auto generated value for pkts_num_trig_pfc_shp in hdrm_pool_size")
            hdrm_pool_size['pkts_num_trig_pfc_shp'] = []
            shared_size = self.g_d_s_ll
            for i in range(0, hdrm_pool_size['pgs_num']):
               shared_size_accessible = int(math.ceil(float(ll_i_d_f * shared_size - \
                  self.asic_param_dic[self.dutConfig['dutAsic']]['e_b_tr_off']) / (ll_i_d_f + 1)))
        ...
        wm_pg_shared_lossless = self.qos_params_invm[self.speed_cable_len]['wm_pg_shared_lossless']
        if 'pkts_num_fill_min' not in wm_pg_shared_lossless:
            logging.debug("Adding auto generated value for pkts_num_fill_min in wm_pg_shared_lossless")
            wm_pg_shared_lossless['pkts_num_fill_min'] = ll_ip_r_s
        if 'pkts_num_trig_pfc' not in wm_pg_shared_lossless:
            logging.debug("Adding auto generated value for pkts_num_trig_pfc in wm_pg_shared_lossless")
            wm_pg_shared_lossless['pkts_num_trig_pfc'] = pkts_num_trig_pfc
        if 'cell_size' not in wm_pg_shared_lossless:
            logging.debug("Adding auto generated value for cell_size in wm_pg_shared_lossless")
            wm_pg_shared_lossless['cell_size'] = d_c_s
        self.qos_params_invm[self.speed_cable_len]['wm_pg_shared_lossless'].update(wm_pg_shared_lossless)

        wm_pg_shared_lossy = self.qos_params_invm[self.speed_cable_len]['wm_pg_shared_lossy']
        if 'pkts_num_fill_min' not in wm_pg_shared_lossy:
            logging.debug("Adding auto generated value for pkts_num_fill_min in wm_pg_shared_lossy")
            wm_pg_shared_lossy['pkts_num_fill_min'] = l_q_r_s
        if 'pkts_num_trig_egr_drp' not in wm_pg_shared_lossy:
            logging.debug("Adding auto generated value for pkts_num_trig_egr_drp in wm_pg_shared_lossy")
            wm_pg_shared_lossy['pkts_num_trig_egr_drp'] = pkts_num_trig_egr_drp
        if 'cell_size' not in wm_pg_shared_lossy:
            logging.debug("Adding auto generated value for cell_size in wm_pg_shared_lossy")
            wm_pg_shared_lossy['cell_size'] = d_c_s
        self.qos_params_invm[self.speed_cable_len]['wm_pg_shared_lossy'].update(wm_pg_shared_lossy)
					
        wm_pg_headroom = self.qos_params_invm[self.speed_cable_len]['wm_pg_headroom']
        if 'pkts_num_trig_pfc' not in wm_pg_headroom:
            logging.debug("Adding auto generated value for pkts_num_trig_pfc in wm_pg_headroom")
            wm_pg_headroom['pkts_num_trig_pfc'] = pkts_num_trig_pfc + b_s_t + b_r_s
        if 'pkts_num_trig_ingr_drp' not in wm_pg_headroom:
            logging.debug("Adding auto generated value for pkts_num_trig_ingr_drp in wm_pg_headroom")
            wm_pg_headroom['pkts_num_trig_ingr_drp'] = pkts_num_trig_ingr_drp
        if 'cell_size' not in wm_pg_headroom:
            logging.debug("Adding auto generated value for cell_size in wm_pg_headroom")
            wm_pg_headroom['cell_size'] = b_w_s
        self.qos_params_invm[self.speed_cable_len]['wm_pg_headroom'].update(wm_pg_headroom)
										
        wm_buf_pool_lossless = self.qos_params_invm[self.speed_cable_len]['wm_buf_pool_lossless']
        if 'pkts_num_fill_ingr_min' not in wm_buf_pool_lossless:
            logging.debug("Adding auto generated value for pkts_num_fill_ingr_min in wm_buf_pool_lossless")
            wm_buf_pool_lossless['pkts_num_fill_ingr_min'] = ll_ip_r_s
        if 'pkts_num_trig_pfc' not in wm_buf_pool_lossless:
            logging.debug("Adding auto generated value for pkts_num_trig_pfc in wm_buf_pool_lossless")
            wm_buf_pool_lossless['pkts_num_trig_pfc'] = pkts_num_trig_pfc
        if 'pkts_num_trig_ingr_drp' not in wm_buf_pool_lossless:
            logging.debug("Adding auto generated value for pkts_num_trig_ingr_drp in wm_buf_pool_lossless")
            wm_buf_pool_lossless['pkts_num_trig_ingr_drp'] = pkts_num_trig_ingr_drp
        if 'cell_size' not in wm_buf_pool_lossless:
            logging.debug("Adding auto generated value for cell_size in wm_buf_pool_lossless")
            wm_buf_pool_lossless['cell_size'] = d_c_s
        if 'extra_cap_margin' not in wm_buf_pool_lossless:
            logging.debug("Adding auto generated value for extra_cap_margin in wm_buf_pool_lossless")
            wm_buf_pool_lossless['extra_cap_margin'] = b_s_t + b_r_s
        self.qos_params_invm[self.speed_cable_len]['wm_buf_pool_lossless'].update(wm_buf_pool_lossless)	

        wm_buf_pool_lossy = self.qos_params_invm[self.speed_cable_len]['wm_buf_pool_lossy']
        if 'pkts_num_fill_egr_min' not in wm_buf_pool_lossy:
            logging.debug("Adding auto generated value for pkts_num_fill_ingr_min in wm_buf_pool_lossy")
            wm_buf_pool_lossy['pkts_num_fill_egr_min'] = l_q_r_s
        if 'pkts_num_trig_pfc' not in wm_buf_pool_lossy:
            logging.debug("Adding auto generated value for pkts_num_trig_pfc in wm_buf_pool_lossy")
            wm_buf_pool_lossy['pkts_num_trig_pfc'] = pkts_num_trig_pfc
        if 'pkts_num_trig_egr_drp' not in wm_buf_pool_lossy:
            logging.debug("Adding auto generated value for pkts_num_trig_egr_drp in wm_buf_pool_lossy")
            wm_buf_pool_lossy['pkts_num_trig_egr_drp'] = pkts_num_trig_egr_drp
        if 'cell_size' not in wm_buf_pool_lossy:
            logging.debug("Adding auto generated value for cell_size in wm_buf_pool_lossy")
            wm_buf_pool_lossy['cell_size'] = d_c_s
        if 'extra_cap_margin' not in wm_buf_pool_lossy:
            logging.debug("Adding auto generated value for extra_cap_margin in wm_buf_pool_lossy")
            wm_buf_pool_lossy['extra_cap_margin'] = b_s_t + b_r_s
        self.qos_params_invm[self.speed_cable_len]['wm_buf_pool_lossy'].update(wm_buf_pool_lossy)
