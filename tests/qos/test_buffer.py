...
def load_test_parameters(duthost):
    ...
    with open(param_file_name) as file:
        ...
        DEFAULT_CABLE_LENGTH_LIST = vendor_specific_param.get(
            'default_cable_length')
        TESTPARAM_HEADROOM_OVERRIDE = vendor_specific_param.get(
            'headroom-override')
        TESTPARAM_LOSSLESS_PG = vendor_specific_param.get('lossless_pg')
        TESTPARAM_SHARED_HEADROOM_POOL = vendor_specific_param.get(
        ...
        if TESTPARAM_ADMIN_DOWN is not None:
            ...
            if not check_qos_db_fv_reference_with_table(duthost):
                ...
                expected_queues = TESTPARAM_ADMIN_DOWN.get(
                    'BUFFER_QUEUE_TABLE')
                if expected_queues:
                    new_queues = {}
...
def get_pool_size_from_asic_db(duthost, ingress_lossless_pool_oid):
    pool_sai = _compose_dict_from_cli(duthost.shell(
        'redis-cli -n 1 hgetall ASIC_STATE:SAI_OBJECT_TYPE_BUFFER_POOL:{}'.format(ingress_lossless_pool_oid))['stdout'].split('\n'))
...
def ensure_pool_size(duthost, timeout, expected_pool_size, expected_shp_size, ingress_lossless_pool_oid=None):
    """
    Make sure the size of the buffer pool and shared headroom pool is as expected within a given time in APPL_DB and
    ASIC_DB (optional)
    ...
    Args:
        ...
        ingress_lossless_pool_oid: The SAI OID of ingress lossless buffer pool. If it is omitted, the ASIC DB won't be
                                   checked
    """
    ...
    return wait_until(timeout, delay, 0, _ensure_pool_size, duthost, expected_pool_size, expected_shp_size,
                      ingress_lossless_pool_oid)


def check_pg_profile(duthost, pg, expected_profile, fail_test=True):
    """
    Check whether the profile in BUFFER_PG match the expected value in a wait_until loop with maximum timeout as 10
    seconds
...
def check_pfc_enable(duthost, port, expected_pfc_enable_map):
    """
    Check whether the pfc_enable map in port table is correct in a wait_until loop with maximum timeoutas 10 seconds
    ...
    if m:
        ...
        if check_qos_db_fv_reference_with_table(duthost):
            ...
            if std_profiles_for_speed:
                ...
                if std_profile:
                    # This means it's a profile with std speed and cable length. We can check whether the headroom
                    # data is correct
                    pytest_assert(profile_appldb['xon'] == std_profile['xon'] and
                                  profile_appldb['xoff'] == std_profile['xoff'] and
                                  (profile_appldb['size'] == std_profile['size'] or
                                   DEFAULT_SHARED_HEADROOM_POOL_ENABLED),
                                  "Generated profile {} doesn't match the std profile {}".format(profile_appldb,
                                                                                                 std_profile))
                else:
            ...
            if ret:
                # This means it's a profile with std speed and cable length. We can check whether the headroom data is
                # correct
                pytest_assert(int(profile_appldb['xon']) == head_room_data['xon'] and
                              int(profile_appldb['xoff']) == head_room_data['xoff'] and
                              (int(profile_appldb['size']) == head_room_data['size'] or
                               DEFAULT_SHARED_HEADROOM_POOL_ENABLED),
                              "Generated profile {} doesn't match the std profile {}".format(profile_appldb,
                                                                                             head_room_data))
            else:
                logging.info(
                    "Skip headroom checking because headroom information is not able to be calculated for speed {}".format(speed))
...
def test_change_speed_cable(duthosts, rand_one_dut_hostname, conn_graph_facts, port_to_test, speed_to_test, mtu_to_test, cable_len_to_test):
    ...
    Change the variables of the port, including speed, mtu and cable length, in different ways and observe whether
    ...
    The flow of the test case:
        ...
        2. Determine whether the profile removing behavior can be verified:
           ...
           and the generated profile will be removed after the configuration change because the profile is referenced
           by this port only.
           For example:
               The mtu_to_test 1500 only applied on the port_to_test, thus the *_mtu1500_* profile is referenced by the
               port only.
               The *_mtu1500_* mtu will be removed after the mtu of the port is updated to default value.
               In this case, we are able to verify whether the buffer profile is removed after mtu reverted or all PGs
               are removed.
               On the other hand, if the mtu is 9100, the buffer profile can be referenced by many other ports and it's
               less possible for us to verify the removing behavior.
        ...
        4. Each time the PG on a port is added or removed, the following items will be checked:
            ...
            - Whether the profile is removed if all PGs are removed and we are able to check removing behavior (result
              of step 2).
    ...
    try:
        ...
        logging.info(
            '[Speed and/or cable-len and/or MTU updated] Checking whether new profile {} has been created and pfc_enable has been updated'.format(expected_profile))
        check_pg_profile(
        ...
        if check_profile_removed:
            logging.info('[Remove dynamic profile on PG removed] Checking whether the profile {} is removed on receiving all lossless PG removed'.format(
                expected_profile))
            ...
            logging.info(
                '[Remove dynamic profile on cable length and/or MTU updated] Checking whether the old profile is removed')
            check_lossless_profile_removed(
                duthost, expected_profile, profile_oid)
            expected_profile = make_expected_profile_name(
...
def test_shared_headroom_pool_configure(duthosts, rand_one_dut_hostname, conn_graph_facts, port_to_test):
    ...
    Test case to verify the variant commands of shared headroom pool configuration and how they affect the other buffer
    configurations
    ...
    if duthost.facts['asic_type'] == 'barefoot':
        pytest.skip(
            'Oversubscribe ratio is not supported for Barefoot platforms yet')
    ...
    try:
        ...
        if is_barefoot_device(duthost):
            expected_nondef_profile = make_expected_profile_name(
                original_speed, '15m', other_factors=['th7'])
        else:
            expected_nondef_profile = make_expected_profile_name(
...
def test_port_admin_down(duthosts, rand_one_dut_hostname, conn_graph_facts, port_to_test):
        ...
        if objects_in_configdb:
            for object_in_configdb in objects_in_configdb:
                ...
                pytest_assert(profile_in_appl_db == expected_profile_in_appldb,
                              "Buffer object {} contains {} which isn't expected ({})".format(key, profile_in_appl_db,
    ...
    def _check_buffer_object_aligns_with_expected_ones(port_to_test, table, expected_objects):
        ...
        if expected_objects:
            expected_object_keys = ['{}:{}:{}'.format(
                table, port_to_test, objectid) for objectid in expected_objects.keys()]
            pytest_assert(set(expected_object_keys) == set(objects_in_appl_db),
                          "Objects in {} on admin-down port is {} but should be {}".format(table, objects_in_appl_db,
            ...
            for objectid, expected_profile in expected_objects.items():
                ...
                pytest_assert(profile == expected_profile,
                              "Profile in {}:{}:{} should be {} but got {}".format(table, port_to_test, objectid,
                                                                                   expected_objects[objectid],
                                                                                   profile))
        else:
            pytest_assert(not objects_in_appl_db,
                          "There shouldn't be any object in {} on an administratively down port but we got {}".format(
                              table, objects_in_appl_db))
    ...
    def _check_buffer_object_list_aligns_with_expected_ones(port_to_test, table, expected_objects):
        ...
        if expected_objects:
            pytest_assert(set(expected_objects) == set(object_list_in_appl_db),
                          "Profile in {}:{} should be {} but got {}".format(table, port_to_test, expected_objects,
                                                                            object_list_in_appl_db))
        else:
            pytest_assert(not object_list_in_appl_db,
                          "There shouldn't be any object in {} on an administratively down port but we got {}".format(
                              table, object_list_in_appl_db))

    skip_traditional_model()

    if not TESTPARAM_HEADROOM_OVERRIDE:
        pytest.skip(
            'Shutdown port test skipped due to no headroom override parameter defined')
    param = TESTPARAM_HEADROOM_OVERRIDE.get("add")
    if not param:
        pytest.skip(
            'Shutdown port test skipped due to no headroom override parameters for add operation defined')
    ...
    if not TESTPARAM_EXTRA_OVERHEAD:
        pytest.skip(
            'Shutdown port test skipped due to no extra overhead parameter defined')
    ...
    """
        Each item is a tuple consisting of:
         ...
         - Whether we need to check whether the previous expected_profile has been removed after port is
    ...
    try:
        for scenario in scenarios:
            ...
            logging.info(
                'Check whether all queues are configured as zero profile or removed from port {}'.format(port_to_test))
            ...
            logging.info(
                'Check whether ingress profile list is configured as zero profile or removed from port {}'.format(port_to_test))
            expected_ingress_profile_list = TESTPARAM_ADMIN_DOWN.get(
                'BUFFER_PORT_INGRESS_PROFILE_LIST_TABLE')
            ...
            logging.info(
                'Check whether egress profile list is configured as zero profile or removed from port {}'.format(port_to_test))
            expected_egress_profile_list = TESTPARAM_ADMIN_DOWN.get(
                'BUFFER_PORT_EGRESS_PROFILE_LIST_TABLE')
            _check_buffer_object_list_aligns_with_expected_ones(
...
def test_port_auto_neg(duthosts, rand_one_dut_hostname, conn_graph_facts, port_to_test):
    ...
    The flow of the test case:
        ...
        3. Enable the port auto negotiation and then configure the advertised speed list and then disable it
           - The maximum supported speed should be taken into account for buffer calculation after port
    ...
    Note:
        ...
        - The buffer pool size depends on the buffer priority-groups and profiles but not directly on the effective
          speed.
          As buffer pool size has been verified in other test cases and checking it will consume more time, we don't
          repeat it here.
    """
    ...
    try:
        ...
        while True:
            ...
            if cable_length > 10000:
                pytest.skip(
                    "Not able to find the maximum headroom of port {} after cable length has been increased to 10km, skip the test".format(port_to_test))
        ...
        logging.info("Cable length {} can be applied but {} can't. Finding the exact maximum cable length".format(
            cable_length_lower, cable_length_upper))
        while True:
            ...
            if profile_applied:
                cable_length_lower = cable_length
                logging.info('Cable length {} has been applied successfully, moving forward. Range now is [{}, {})'.format(
                    cable_length, cable_length_lower, cable_length_upper))
            else:
                cable_length_upper = cable_length
                logging.info('Cable length {} has not been applied, moving backward. Range now is [{}, {})'.format(
                    cable_length, cable_length_lower, cable_length_upper))
        ...
        pytest_assert(not profile_applied,
                      "Profile {} applied on {}:5-7, which makes the accumulative headroom exceed the limit".format(
                          maximum_profile_name, port_to_test))
...
def test_buffer_deployment(duthosts, rand_one_dut_hostname, conn_graph_facts, tbinfo, dualtor_ports):
    ...
    1. For all ports in the config_db,
       ...
       - If name to oid maps exist for port and PG, check whether the information in ASIC_DB aligns with that in
         CONFIG_DB
    ...
    def _check_buffer_item_in_asic_db(duthost, port, buffer_item, name_map, buffer_profile_oid, asic_key_name,
                                      should_have_profile, use_assert):
        ...
        buffer_item_asic_key = duthost.shell(
            'redis-cli -n 1 keys *{}*'.format(buffer_item_asic_oid))['stdout']
        buffer_profile_oid_in_pg = duthost.shell(
            'redis-cli -n 1 hget {} {}'.format(buffer_item_asic_key, asic_key_name))['stdout']
    ...
    def _check_port_buffer_info_and_get_profile_oid(duthost, table, ids, port, expected_profile, use_assert=True):
        ...
        """
        profile_in_db = duthost.shell(
            'redis-cli hget "{}:{}:{}" profile'.format(table, port, ids))['stdout']
        ...
        if expected_profile:
            if not _check_condition(profile_in_db == expected_profile,
                                    "The profile of {}:{}:{} isn't the expected ({})".format(table, port, ids,
                                                                                             expected_profile),
                                    use_assert):
                return None, False
            ...
            if buffer_name_map:
                ...
                for item in id_list:
                    ...
                    buffer_profile_oid, success = _check_buffer_item_in_asic_db(
                        duthost, port, item, buffer_name_map, buffer_profile_oid, sai_field, True, use_assert)
                    if not success:
                        return None, False
        else:
            if not _check_condition(not profile_in_db,
                                    "{}:{}:{} configured on admin down port".format(table, port, ids), use_assert):
                return None, False
            if buffer_name_map:
                for item in id_list:
                    logging.info(
                        "Checking {}:{}:{} in ASIC_DB".format(table, port, item))
                    buffer_profile_oid, success = _check_buffer_item_in_asic_db(
                        duthost, port, item, buffer_name_map, None, sai_field, False, use_assert)
    ...
    def _check_port_buffer_info_and_return(duthost, table, ids, port, expected_profile):
        ...
        _, result = _check_port_buffer_info_and_get_profile_oid(
            duthost, table, ids, port, expected_profile, False)
        return result
    ...
    if not is_mellanox_device(duthost):
        ...
        if is_barefoot_device(duthost):
            buffer_table_up[KEY_2_LOSSLESS_QUEUE][1] = (
                'BUFFER_QUEUE_TABLE', '0-2', '[BUFFER_PROFILE_TABLE:]')
            buffer_table_up[KEY_2_LOSSLESS_QUEUE][3] = (
                'BUFFER_QUEUE_TABLE', '5-6', '[BUFFER_PROFILE_TABLE:]')
    ...
    for port in configdb_ports:
        ...
        port_config = _compose_dict_from_cli(duthost.shell(
        ...
        for table, ids, expected_profile in buffer_items_to_check:
            logging.info(
                "Checking buffer item {}:{}:{}".format(table, port, ids))

            if not expected_profile:
                continue

            buffer_profile_oid, _ = _check_port_buffer_info_and_get_profile_oid(
                duthost, table, ids, port, expected_profile)
            ...
            if expected_profile not in profiles_checked:
                profile_info = _compose_dict_from_cli(duthost.shell(
                    'redis-cli hgetall "{}"'.format(expected_profile_key))['stdout'].split())
                ...
                if is_ingress_lossless and not BUFFER_MODEL_DYNAMIC:
                    ...
                    if std_profiles_for_speed:
                        std_profile = std_profiles_for_speed.get(cable_length)
                        if std_profile:
                            # This means it's a profile with std speed and cable length. We can check whether the
                            # headroom data is correct
                            pytest_assert(profile_info['xon'] == std_profile['xon'] and
                                          profile_info['xoff'] == std_profile['xoff'] and
                                          (profile_info['size'] == std_profile['size'] or
                                           DEFAULT_SHARED_HEADROOM_POOL_ENABLED),
                                          "Buffer profile {} {} doesn't match default {}".format(expected_profile,
                                                                                                 profile_info,
                                                                                                 std_profile))
                ...
                if buffer_profile_oid:
                    ...
                    buffer_profile_asic_info = _compose_dict_from_cli(duthost.shell(
                    ...
                    pytest_assert(buffer_profile_asic_info.get('SAI_BUFFER_PROFILE_ATTR_XON_TH') == profile_info.get('xon') and
                                  ...
                                  "Buffer profile {} {} doesn't align with ASIC_TABLE {}".format(expected_profile,
                                                                                                 profile_info, buffer_profile_asic_info))
                ...
                if is_ingress_lossless:
                    if not lossless_pool_oid:
                        ...
                        pytest_assert(lossless_pool_oid == buffer_profile_asic_info['SAI_BUFFER_PROFILE_ATTR_POOL_ID'],
                                      "Buffer profile {} has different buffer pool id {} from others {}".format(
                                          expected_profile,
    ...
    if not BUFFER_MODEL_DYNAMIC:
        ...
        try:
            ...
            for pg_id_name in pg_id_names:
                wait_until(60, 5, 0, _check_port_buffer_info_and_return, duthost, 'BUFFER_PG_TABLE',
...
def mellanox_calculate_headroom_data(duthost, port_to_test):
    ...
    It intends to calculate the headroom size based on the input port attributes(speed, cable_length,
    number of lanes..., etc)
...
def barefoot_calculate_headroom_data(duthost, port_to_test):
    ...
    bytes_on_cable = 2 * (float(cable_length)) * port_speed * \
        1000000000 / speed_of_light / (8 * 1024)
    propagation_delay = port_mtu + bytes_on_cable + \
        MAC_PHY_DELAY + peer_response_time * 1024
