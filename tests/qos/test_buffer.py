...
def pg_to_test(request, duthost):
    ...
    Return:
        pg_to_test
    """
    pytest_require(request.param == 5 and not is_barefoot_device(duthost),
            "Skip test case for 5th PG")
    pytest_require(request.param == 6 and is_barefoot_device(duthost),
            "Skip test case for 6th PG")
    return request.param
...
def test_buffer_deployment(duthosts, rand_one_dut_hostname, conn_graph_facts):
    ...
    if not is_mellanox_device(duthost):
        if is_barefoot_device(duthost):
            buffer_items_to_check_dict["up"][3] = ('BUFFER_QUEUE_TABLE', '5-6', '[BUFFER_PROFILE_TABLE:]')
        else:
