...
def test_pfc_pause_threshold(ixia_api,
                             ...
                             on_exit):
    """
    Test PFC pause thresholds for the traditional buffer model
    ...
    Args:
        ...
        prio_dscp_map (pytest fixture): priority vs. DSCP map (key = priority).
        on_exit (pytest fixture): register callbacks for test cleanup

    Returns:
        N/A
    """
    duthost = duthosts[rand_one_dut_hostname]
    pytest_require(is_cisco_device(duthost) is True,
                   'Skip pause threshold test case for non-Cisco devices')
    ...
    dynamic_ths = [-2, -3, -4, -5]
    for dynamic_th in dynamic_ths:
        for lossless_prio in lossless_prio_list:
            run_pfc_test(api=ixia_api,
                         testbed_config=testbed_config,
                         port_config_list=port_config_list,
                         conn_data=conn_graph_facts,
