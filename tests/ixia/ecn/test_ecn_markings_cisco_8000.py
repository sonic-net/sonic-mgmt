#
# Script to record the ecn marking of packets by DUT during congestion.
#
# Usage:
#     ./run_tests.sh -n $tbname -d $dutname -O -u -l debug -e -s \
#     -e '--disable_ecn_test=False' -c \
#     'ixia/ecn/test_ecn_markings_cisco_8000.py'
import pytest
import time
import datetime
import os.path
...
from tests.common.ixia.qos_fixtures import (   # noqa: F401
    prio_dscp_map, lossless_prio_list)
...
def create_marking_data_file():
    if not os.path.exists(MARKING_DATA_FILE):
        with open(MARKING_DATA_FILE, 'a') as (fd):
            fd.write(
                "tx_ports,rate,deq,lat,data_pkt_size,kmin,kmax,pmax,"
                "xoff_quanta,total,marked,time_stamp\n")
...
@pytest.mark.parametrize(
    'dequeue_marking,latency_marking',
...
def test_ecn_markings(request,
    ...
    with open(MARKING_DATA_FILE, 'a') as (fd):
        fd.write((
            "{tx_ports},{rate},{deq},{lat},{data_pkt_size},{kmin},"
            "{kmax},{pmax},{xoff_quanta},{total},{marked},"
            "{time_stamp}\n").format(
