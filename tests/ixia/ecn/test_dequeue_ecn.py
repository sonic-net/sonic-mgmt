...
def test_dequeue_ecn(request,
    ...
    if cisco_platform:
        ...
        oq_cell_count = 100  # no of cells in OQ for this lossless priority

        cell_size = 384
        cell_per_pkt = (pkt_size + cell_size - 1) // cell_size
        margin_cells = 25
        margin = margin_cells // cell_per_pkt
        pkt_to_oq = (oq_cell_count//cell_per_pkt) + \
            margin  # Packets forwarded to OQ
    ...
    try:
        ...
        pytest_assert(
            is_ecn_marked(ip_pkts[pkt_to_check]),
