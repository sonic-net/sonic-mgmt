...
def test_gnoi_system_reboot_cold(duthosts, rand_one_dut_hostname, localhost, gnmi_tls):  # noqa: F811
    ...
    reboot_response = gnmi_tls.gnoi.system_reboot(method="COLD", message=REBOOT_MESSAGE)
    logging.info("System.Reboot API returned: {}".format(reboot_response))

    check_reboot_status(gnmi_tls, expected_active=True, expected_reason=REBOOT_MESSAGE, expected_method="COLD")
...
def test_gnoi_system_reboot_warm(duthosts, rand_one_dut_hostname, localhost, gnmi_tls):  # noqa: F811
    ...
    # Trigger reboot via gNOI using gnmi_tls fixture
    reboot_response = gnmi_tls.gnoi.system_reboot(method="WARM", message=REBOOT_MESSAGE)
    logging.info("System.Reboot API returned: {}".format(reboot_response))

    check_reboot_status(gnmi_tls, expected_active=True, expected_reason=REBOOT_MESSAGE, expected_method="WARM")
