...
# ---------------------------------------------------------------------------
# tests/gnmi/conftest.py can define module-scoped autouse fixtures that
# serve the dial-in tests; neither is needed for dial-out, and where present
# both are harmful here: download_gnmi_client docker-cps client binaries out
# of the gnmi container with no error tolerance (on a DUT without one it
# errors the whole module before dialout_setup can gracefully skip, and it
# picks its DUT via rand_one_dut_hostname, which need not match this suite's
# enum_rand_one_per_hwsku_hostname selection), and setup_vrf_configuration
# drags the vrf_config parametrization into every test id. Shadow both with
# no-ops.
# ---------------------------------------------------------------------------
...
def find_dialout_container(duthost):
    ...
    for container in ("gnmi", "telemetry"):
        res = duthost.shell("docker ps --filter name=%s --format '{{.Names}}'" % container,
                            module_ignore_errors=True)
        if res["rc"] != 0 or container not in res["stdout"]:
            continue
...
class CollectorHandle(object):
    ...
    def stop(self):
        self.ptfhost.shell("pkill -9 -f 'dialout_collector.py --port %d'" % self.port,
                           module_ignore_errors=True)
