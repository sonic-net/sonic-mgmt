...
def test_gnmi_queue_buffer_cnt(rand_one_dut_hostname, gnmi_tls):  # noqa: F811
    ...
    for i in uc_list:
        ...
        pytest_assert(response_has_update(result, "oid"), ("OID not found in result. " "Result: {}").format(result))
    ...
    admin_up_interfaces = [
        i for i, info in interfaces.items() if pattern.match(i) and info["admin"] == "up" and info["oper"] == "up"
    ]

    cfggen_namespace = "-n {} ".format(namespace_name) if namespace_name else ""
    data = json.loads(duthost.shell("sonic-cfggen {}-d --print-data".format(cfggen_namespace), verbose=False)["stdout"])
    ...
    try:
        ...
        pytest_assert(
            wait_until(120, 20, 0, _check_buffer_queues_cnt, duthost, client, interface_to_check),
        ...
        if duthost.facts.get("platform_asic") == "broadcom-dnx":
            pytest_assert(pre_del_cnt == post_del_cnt, "Queue counter count changed on static-queue Broadcom-DNX")
        else:
            pytest_assert(pre_del_cnt > post_del_cnt, "Number of queue counters count differs from expected")
    finally:
...
def _assert_responses_contain(result, text, expected):
    pytest_assert(len(result) == expected, ("Expected {} responses, got {}: {}").format(expected, len(result), result))
    pytest_assert(
        all(response_has_update(response, text) for response in result),
        ("Expected '{}' in every response: {}").format(text, result),
    )
...
def test_gnmi_output(gnmi_tls):  # noqa: F811
    ...
    assert "oid" in counter_key, ("Invalid oid: {}.").format(counter_key)
    ...
    logger.info(result)
    pytest_assert(
        response_has_update(result, "SAI_PORT_STAT_IF_IN_ERRORS"),
        ("SAI_PORT_STAT_IF_IN_ERRORS not found in gnmi_output: {}.").format(result),
    )
...
test_data_counters_port_name_map = [
    ...
    {"name": "Subscribe table field for COUNTERS_PORT_NAME_MAP", "port": "/Ethernet0"},
]
...
def test_gnmi_counterdb_polling_02(gnmi_tls):  # noqa: F811
    ...
    result = asic.run_sonic_db_cli_cmd("COUNTERS_DB hget COUNTERS_PORT_NAME_MAP Ethernet0")
    counter_key = result["stdout"].strip()
    assert "oid" in counter_key, ("Invalid oid: {}. " "Expected 'oid' in counter key").format(counter_key)
...
@pytest.mark.parametrize("test_data", test_data_counters_port_name_map)
def test_gnmi_counterdb_streaming_sample_01(gnmi_tls, test_data):  # noqa: F811
    """
    Verify GNMI subscribe API
