...
def test_gnmi_queue_buffer_cnt(duthosts, rand_one_dut_hostname, ptfhost):
    ...
    for i in uc_list:
        ...
        msg_list = gnmi_get(duthost, ptfhost, path_list)
        result = msg_list[0]
        pytest_assert("oid" in result, ("OID not found in result. " "Result: {}").format(result))
    ...
    try:
        ...
        assert "GRPC error" in str(e), ("Expected GRPC error, but got: {}. ").format(str(e))
    ...
    try:
        ...
        pytest_assert(
            wait_until(120, 20, 0, _check_buffer_queues_cnt, duthost, ptfhost, interface_to_check),
...
def test_gnmi_output(duthosts, rand_one_dut_hostname, ptfhost):
    ...
    logger.info(result)
    pytest_assert(
        "SAI_PORT_STAT_IF_IN_ERRORS" in result,
...
def test_gnmi_counterdb_polling_01(duthosts, rand_one_dut_hostname, ptfhost, test_data):
    ...
    assert msg.count("oid") >= exp_cnt, ("{}: {}. " "Expected count of 'oid': {}" "Actual count of 'oid': {}").format(
        test_data["name"], msg, exp_cnt, msg.count("oid")
    )
...
def test_gnmi_counterdb_polling_02(duthosts, rand_one_dut_hostname, ptfhost):
    ...
    assert msg.count("SAI_PORT_STAT_IF_IN_ERRORS") >= exp_cnt, (
        "Expected count of 'SAI_PORT_STAT_IF_IN_ERRORS' not met. " "Expected count: {}" "Actual count: {}" "Message: {}"
...
def test_gnmi_counterdb_streaming_sample_01(duthosts, rand_one_dut_hostname, ptfhost, test_data):
    ...
    assert msg.count("oid") >= exp_cnt, (
        "Expected count of 'oid' not met. " "Expected count: {}" "Actual count: {}" "Message: {}"
