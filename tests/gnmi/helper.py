...
from tests.common.helpers.ntp_helper import (  # noqa: F401
    NtpDaemon,
...
def gnmi_set(duthost, ptfhost, delete_list, update_list, replace_list, cert=None, ip=None):
    ...
    wait_until(120, 1, 5, lambda: len(duthost.shell(health_check_cmd, module_ignore_errors=True)["stdout_lines"]) > 0)
...
def gnmi_subscribe_streaming_sample(duthost, ptfhost, path_list, interval_ms, count, origin=None, target=None, ip=None):
    """
    Send GNMI subscribe request with GNMI client
...
def gnmi_subscribe_stream_connections(
    duthost, ptfhost, path_list, target, create_connections, update_count, namespace=None, ip=None
):
    """
