...
def apply_cert_config(duthost):
    ...
    dut_command = "docker exec %s bash -c \"/usr/bin/nohup /usr/sbin/telemetry -logtostderr --port %s --server_crt /etc/sonic/telemetry/gnmiserver.crt --server_key /etc/sonic/telemetry/gnmiserver.key --ca_crt /etc/sonic/telemetry/gnmiCA.pem -gnmi_native_write=true -v=10 >/root/gnmi.log 2>&1 &\""%(GNMI_CONTAINER_NAME, port)
    duthost.shell(dut_command)
...
def gnmi_capabilities(duthost, localhost):
    ip = duthost.mgmt_ip
    port = gnmi_port(duthost)
    cmd = "gnmi/gnmi_cli -client_types=gnmi -a %s:%s -logtostderr -client_crt ./gnmiclient.crt -client_key ./gnmiclient.key -ca_crt ./gnmiCA.pem -capabilities"%(ip, port)
...
def gnmi_set(duthost, localhost, delete_list, update_list, replace_list):
    ip = duthost.mgmt_ip
    port = gnmi_port(duthost)
    cmd = "gnmi/gnmi_set -target_addr %s:%s -alsologtostderr -cert ./gnmiclient.crt -key ./gnmiclient.key -ca ./gnmiCA.pem -time_out 60s"%(ip, port)
...
def gnmi_get(duthost, localhost, path_list):
    ip = duthost.mgmt_ip
    port = gnmi_port(duthost)
    cmd = "gnmi/gnmi_get -target_addr %s:%s -alsologtostderr -cert ./gnmiclient.crt -key ./gnmiclient.key -ca ./gnmiCA.pem"%(ip, port)
...
def gnoi_reboot(duthost, localhost, method, delay, message):
    ip = duthost.mgmt_ip
    port = gnmi_port(duthost)
    cmd = "gnmi/gnoi_client -target %s:%s -logtostderr -cert ./gnmiclient.crt -key ./gnmiclient.key -ca ./gnmiCA.pem -rpc Reboot"%(ip, port)
