...
def test_gnoi_os_verify(duthosts, rand_one_dut_hostname, gnmi_tls):  # noqa: F811
    ...
    pytest_assert("version" in response, "OS.Verify API did not return version field")
...
def test_gnoi_os_activate_invalid_image(gnmi_tls):  # noqa: F811
    ...
    pytest_assert(
        "activateError" in response, f"OS.Activate did not return activateError for invalid image: {response}"
    )
...
def test_gnoi_os_activate_valid_image(duthosts, rand_one_dut_hostname, gnmi_tls):  # noqa: F811
    ...
    pytest_assert(
        current_image in result.get("stdout", ""), f"Image {current_image} not found in sonic-installer list output"
    )
