...
def test_mimic_hwproxy_cert_rotation(duthosts, rand_one_dut_hostname, localhost, ptfhost):
    ...
    cmd_feature = 'bash -c "show feature status | awk \'$1==\\"gnmi\\" {print $1, $2}\'"'
    ...
    if get_image_type(duthost) != "public":
        pytest_assert(gnmi_enabled, "Internal image does not have the gnmi feature enabled")
    ...
    if gnmi_enabled:
        ...
        if result["stdout"].strip():
            ...
            port = env.gnmi_port
            set_table = (
                f'sonic-db-cli CONFIG_DB hset "GNMI|gnmi" ' f'client_auth "true" ' f'log_level "2" ' f'port "{port}"'
            )
