...
def test_gnoi_File_Get(duthosts, rand_one_dut_hostname, localhost):
    ...
    pytest_assert(ret == 0, "File.Get RPC failed: rc = {}, msg = {}".format(ret, msg))

    msg_json = extract_gnoi_response(msg)
    if not msg_json:
        pytest.fail("Failed to extract gnoi response for File.Get RPC")

    logging.info("File.Get Response: {}" .format(msg_json))
    pytest_assert("unimplemented" in msg_json, "Expected Method Unimplemented error")
...
def test_gnoi_File_Remove(duthosts, rand_one_dut_hostname, localhost):
    ...
    logging.info("File.Remove Response: {}" .format(msg_json))
    pytest_assert('File not found' in msg_json, "Failed to get the details - 'File not found'")
...
def test_gnoi_File_TransferToRemote(duthosts, rand_one_dut_hostname, localhost):
    ...
    pytest_assert(ret == 0, "File.TransferToRemote RPC failed: rc = {}, msg = {}".format(ret, msg))

    msg_json = extract_gnoi_response(msg)
    if not msg_json:
        pytest.fail("Failed to extract gnoi response for File.TransferToRemote RPC")

    logging.info("File.TransferToRemote Response: {}" .format(msg_json))
    pytest_assert('unimplemented' in msg_json, "Expected Method Unimplemented error")
...
def test_gnoi_File_Put(duthosts, rand_one_dut_hostname, localhost):
    ...
    ret, msg = gnoi_request(duthost, localhost, "File", "Put", request_json)
    pytest_assert(ret == 0, "File.Put RPC failed: rc = {}, msg = {}".format(ret, msg))

    msg_json = extract_gnoi_response(msg)
    if not msg_json:
        pytest.fail("Failed to extract gnoi response for File.Put RPC")

    logging.info("File.Put Response: {}" .format(msg_json))
    pytest_assert('unimplemented' in msg_json, "Expected Method Unimplemented error")
