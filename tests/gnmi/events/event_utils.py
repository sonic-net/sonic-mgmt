...
def listen_for_events(
    duthost, gnxi_path, ptfhost, filter_event_regex, op_file, timeout, update_count=1, match_number=0
):
...
def restart_monit(duthost):
    duthost.shell("systemctl restart monit")
    is_monit_running = wait_until(320, 5, 0, check_monit_running, duthost)
...
def restart_eventd(duthost):
    ...
    pytest_assert(
        wait_until(300, 10, 0, verify_counter_increase, duthost, 0, 2, PUBLISHED), "events_monit_test has not published"
    )
...
def find_test_vlan(duthost):
    ...
    for vlan in vlan_brief:
        ...
        dhcrelay_process = duthost.shell(
            "docker exec dhcp_relay supervisorctl status \
                                         | grep isc-dhcpv4-relay-%s | awk '{print $1}'"
            % vlan
        )["stdout"]
        dhcp6relay_process = duthost.shell(
            "docker exec dhcp_relay supervisorctl status \
                                           | grep dhcp6relay | awk '{print $1}'"
        )["stdout"]
