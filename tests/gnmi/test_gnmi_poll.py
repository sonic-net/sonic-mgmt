...
def test_poll_mode_no_table_or_key(duthosts, rand_one_dut_hostname, ptfhost, enum_rand_one_asic_index):
    ...
    result = gnmi_subscribe_polling_py(
        duthost,
        ptfhost,
        path_list,
        target="APPL_DB",
    ...
    assert len(sync_responses) == 5, "Expected 5 sync responses, got {}: {}".format(len(sync_responses), out)
...
def test_poll_mode_present_table_delayed_key(duthosts, rand_one_dut_hostname, ptfhost, enum_rand_one_asic_index):
    ...
    def poll_worker():
        holder["result"] = gnmi_subscribe_polling_py(
            duthost,
            ptfhost,
            path_list,
            target="APPL_DB",
...
def test_poll_mode_default_route(
    duthosts, rand_one_dut_hostname, ptfhost, enum_upstream_dut_hostname, tbinfo, enum_rand_one_asic_index
):
    """
    ...
    try:
        ...
        assert wait_until(
            60, 5, 0, _verify_route_table_status, duthost, namespace, "0", is_ipv6_only
        ...
        assert wait_until(
            60, 5, 0, _verify_route_table_status, duthost, namespace, "1", is_ipv6_only
