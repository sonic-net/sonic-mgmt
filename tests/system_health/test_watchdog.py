...
def pause_orchagent(duthost):
    # find orchagent pid
    pid = duthost.shell(r"ps -ef | grep orchagent | grep -v grep | awk '{print $2}'", module_ignore_errors=True)['stdout']
...
def test_orchagent_watchdog(duthosts, enum_rand_one_per_hwsku_hostname, pause_orchagent):
    ...
    while (True):
        ...
        alert = duthost.shell(r"sudo cat /var/log/syslog | grep 'is stuck in namespace'", module_ignore_errors=True)['stdout']
        ...
        if "orchagent" in str(alert):
            ...
            if current_attempt >= WATCHDOG_TIMEOUT/SLEEP_TIME:
                pytest_assert(False, "orchagent watchdog did not been trigger after {} seconds".format(WATCHDOG_TIMEOUT))
            else:
                current_attempt += 1
