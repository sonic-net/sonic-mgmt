...
def vlan_topology(duthosts, rand_one_dut_hostname, ptfhost):
    ...
    name = list(conf["VLAN"].keys())[0]
    vlan_id = int(conf["VLAN"][name]["vlanid"])
...
def test_fdb_repeated_learn_dedup(
    ...
    after = wait_for_stat_increase(
        ...
        before["lru_pushed"], timeout=STATS_PUBLISH_SEC + 10,
    )
...
def test_fdb_learn_delete_relearn(
    ...
    # LEARN #1
    inject_learn_frames(ptfadapter, topo["ptf_port_a"], mac, topo["inject_vlan_a"])
    pytest_assert(
        wait_until(
            10, 1, 0,
...
def test_fdb_port_move_not_deduped(
    ...
    inject_learn_frames(ptfadapter, topo["ptf_port_b"], mac, topo["inject_vlan_b"])
    pytest_assert(
        wait_until(
            10, 1, 0,
...
def test_fdb_flush_during_learn(
    ...
    flush_after = wait_for_stat_increase(
        ...
        flush_before["received"], timeout=STATS_PUBLISH_SEC + 10,
    )
    pytest_assert(
    ...
    # Re-LEARN one MAC
    inject_learn_frames(ptfadapter, topo["ptf_port_a"], macs[0], topo["inject_vlan_a"])
    pytest_assert(
        wait_until(
            10, 1, 0,
            lambda: fdb_table_has_mac_on_port(
