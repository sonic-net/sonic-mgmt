...
class TestDynamicNat(object):
    ...
    def test_nat_dynamic_basic(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type, protocol_type,
                                   default=True, handshake=True)
    ...
    def test_nat_dynamic_basic_icmp(self, tbinfo, duthost, ptfadapter, ptfhost, setup_test_env):
        ...
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, default=True)
        # Send ICMP traffic(host-tor -> leaf-tor) and check
        generate_and_verify_icmp_traffic(ptfadapter, setup_data, interface_type, direction, nat_type,
                                         icmp_id=POOL_RANGE_START_PORT)
    ...
    def test_nat_dynamic_entry_persist_icmp(self, tbinfo, ptfhost, duthost, ptfadapter, setup_test_env):
        ...
        # Send ICMP traffic(host-tor -> leaf-tor) and check
        for _ in range(0, 4):
            generate_and_verify_icmp_traffic(ptfadapter, setup_data, interface_type, direction, nat_type=nat_type,
                                             icmp_id=POOL_RANGE_START_PORT)
    ...
    def test_nat_dynamic_disable_nat(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        nat_type = 'dynamic'
        # Configure default rules for Dynamic NAT
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_info, interface_type, protocol_type,
                                   default=True, remove_bindings=False, handshake=True)
        # Disable NAT feature
        duthost.command("config nat feature disable")
        # Send traffic and check that traffic was L3 forwarded
        generate_and_verify_not_translated_traffic(ptfadapter, setup_info, interface_type, direction,
                                                   protocol_type, nat_type)
    ...
    @pytest.mark.nat_dynamic
    def test_nat_dynamic_disable_nat_icmp(self, tbinfo, duthost, ptfadapter, ptfhost, setup_test_env,
                                          enable_nat_feature):
        interface_type, setup_info = setup_test_env
        setup_data = copy.deepcopy(setup_info)
        direction = 'host-tor'
        ...
        generate_and_verify_not_translated_icmp_traffic(ptfadapter, setup_data, interface_type,
                                                        direction, nat_type=nat_type)
        # Enable NAT feature and send traffic to check that NAT happens
        duthost.command("config nat feature enable")
        # Send ICMP traffic(host-tor -> leaf-tor) and check
        generate_and_verify_icmp_traffic(ptfadapter, setup_data, interface_type, direction, nat_type=nat_type,
    ...
    def test_nat_dynamic_bindings(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type, protocol_type,
                                   port_range=pool, default=True, remove_bindings=True, handshake=True)
        # Send TCP/UDP bidirectional traffic(host-tor -> leaf-tor and vice versa) and check
        generate_and_verify_traffic_dropped(ptfadapter, setup_info, interface_type, 'leaf-tor', protocol_type,
                                            nat_type, src_port=dst_port, dst_port=POOL_RANGE_START_PORT,
                                            exp_src_port=dst_port, exp_dst_port=src_port)
        generate_and_verify_not_translated_traffic(ptfadapter, setup_info, interface_type, 'host-tor',
                                                   protocol_type, nat_type)
    ...
    def test_nat_dynamic_bindings_icmp(self, tbinfo, duthost, ptfadapter, ptfhost, setup_test_env):
        ...
        # Configure default rules for Dynamic NAT
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type, protocol_type,
                                   default=True, remove_bindings=True)
        # Send ICMP traffic(host-tor -> leaf-tor) and check
    ...
    def test_nat_dynamic_other_protocols(self, tbinfo, ptfhost, duthost, ptfadapter, setup_test_env):
        ...
        # Configure default rules for Dynamic NAT
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type="GRE", default=True)
        # Create packet
    ...
    def test_nat_dynamic_acl_rule_actions(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type, protocol_type,
                                   default=True, acl_rules=acl_rules, handshake=True)
        ...
        for path in DIRECTION_PARAMS:
            generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type, path, protocol_type,
                                        nat_type=nat_type)
        # Change rules ACL rule from "forward" to "do_not_nat" and check that NAT traffic was not NAT
        acl_rules = [{"priority": "10", "src_ip": acl_subnet, "action": "do_not_nat"}]
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type, protocol_type,
                                   default=True, remove_bindings=False, acl_rules=acl_rules, handshake=True)
        generate_and_verify_not_translated_traffic(ptfadapter, setup_info, interface_type, direction,
                                                   protocol_type, nat_type)
    ...
    def test_nat_dynamic_acl_rule_actions_icmp(self, tbinfo, duthost, ptfhost, ptfadapter, setup_test_env):
        ...
        acl_rules = [{"priority": "10", "src_ip": acl_subnet, "action": "do_not_nat"}]
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, acl_rules=acl_rules, default=True)
        generate_and_verify_not_translated_icmp_traffic(ptfadapter, setup_info, interface_type, direction, nat_type)
        # Change rules ACL rule from "do_not_nat" to "forward" and check that NAT traffic was NAT
        acl_rules = [{"priority": "10", "src_ip": acl_subnet, "action": "forward"}]
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, acl_rules=acl_rules, default=True)
        # Verify the behaviour when the ACL binding action changed from "do_not_nat" to "forward"
        generate_and_verify_icmp_traffic(ptfadapter, setup_data, interface_type, direction,
                                         nat_type=nat_type, icmp_id=POOL_RANGE_START_PORT)
        # Change rules ACL rule from "forward" to "do_not_nat" and check that NAT traffic was not NAT
        acl_rules = [{"priority": "10", "src_ip": acl_subnet, "action": "do_not_nat"}]
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, acl_rules=acl_rules, default=True)
    ...
    def test_nat_dynamic_acl_modify_rule(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        for path in DIRECTION_PARAMS:
            generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type,
                                        path, protocol_type, nat_type=nat_type)
        # Configure Dynamic NAT rule and set action to "do_not_nat"
        acl_subnet = "172.20.0.0/24"
        acl_rules = [{"priority": "10", "src_ip": acl_subnet, "action": "do_not_nat"}]
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, acl_rules=acl_rules, default=True, handshake=True)
        ...
        generate_and_verify_not_translated_traffic(ptfadapter, setup_data, interface_type, direction,
                                                   protocol_type, nat_type, ip_src=packet_source_ip,
                                                   exp_ip_src=packet_source_ip)
    ...
    def test_nat_dynamic_acl_modify_rule_icmp(self, tbinfo, duthost, ptfhost, ptfadapter, setup_test_env):
        ...
        # Configure default rules for Dynamic NAT
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, default=True, remove_bindings=False)
        # Send ICMP traffic(host-tor -> leaf-tor) and check
        generate_and_verify_icmp_traffic(ptfadapter, setup_data, interface_type,
                                         direction, nat_type, icmp_id=POOL_RANGE_START_PORT)
        # Configure Dynamic NAT rule and set action to "do_not_nat"
        acl_subnet = "172.20.0.0/24"
        acl_rules = [{"priority": "10", "src_ip": acl_subnet, "action": "do_not_nat"}]
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
        ...
        # Check that packet is L3 forwarded after rule was chenged from forward to do_not_nat
        generate_and_verify_not_translated_icmp_traffic(ptfadapter, setup_data, interface_type,
                                                        direction, nat_type, ip_src=packet_source_ip,
                                                        check_reply=False)
    ...
    def test_nat_dynamic_pool_threshold(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        pool = "{0}-{1}".format(POOL_RANGE_START_PORT, POOL_RANGE_START_PORT + 1)
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, port_range=pool, default=True, handshake=True)
        ...
        generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type, 'host-tor',
                                    protocol_type, nat_type=nat_type, src_port=src_port, dst_port=TCP_GLOBAL_PORT,
                                    exp_src_port=first_exp_src_port, exp_dst_port=TCP_GLOBAL_PORT)
        generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type, 'leaf-tor',
                                    protocol_type, nat_type=nat_type, src_port=TCP_GLOBAL_PORT,
                                    dst_port=first_exp_src_port, exp_src_port=TCP_GLOBAL_PORT, exp_dst_port=src_port)
        ...
        generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type, 'host-tor',
                                    protocol_type, nat_type=nat_type, src_port=src_port + 1,
                                    dst_port=TCP_GLOBAL_PORT, exp_src_port=second_exp_src_port,
                                    exp_dst_port=TCP_GLOBAL_PORT)
        generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type, 'leaf-tor',
                                    protocol_type, nat_type=nat_type, src_port=TCP_GLOBAL_PORT,
                                    dst_port=second_exp_src_port, exp_src_port=TCP_GLOBAL_PORT,
                                    exp_dst_port=src_port + 1)
        ...
        generate_and_verify_traffic_dropped(ptfadapter, setup_data, interface_type, 'host-tor',
                                            protocol_type, nat_type=nat_type, src_port=src_port + 2,
                                            dst_port=TCP_GLOBAL_PORT, exp_src_port=first_exp_src_port,
                                            exp_dst_port=TCP_GLOBAL_PORT)
        generate_and_verify_traffic_dropped(ptfadapter, setup_data, interface_type, 'host-tor',
                                            protocol_type, nat_type=nat_type, src_port=src_port + 2,
                                            dst_port=TCP_GLOBAL_PORT, exp_src_port=second_exp_src_port,
        ...
        generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type, 'leaf-tor',
                                    protocol_type, nat_type=nat_type, src_port=TCP_GLOBAL_PORT,
                                    dst_port=first_exp_src_port, exp_src_port=TCP_GLOBAL_PORT,
                                    exp_dst_port=src_port + 2)
    ...
    def test_nat_dynamic_crud(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, port_range=port_range, default=True)
        wait_timeout(protocol_type)
        ...
        exp_source_port = sorted(re.search(pattern.format(get_public_ip(setup_data, interface_type)), output).
                                 groups())[-1]
        pytest_assert(start_port <= int(exp_source_port) <= end_port,
                      "New entry was not added with l4 port from new port range")
        # Delete NAT rules
    ...
    def test_nat_dynamic_full_cone(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, port_range="100-5000", default=True, handshake=True)
        # Send TCP/UDP bidirectional traffic(host-tor -> leaf-tor and vice versa) and check
        generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type,
                                    'host-tor', protocol_type, nat_type=nat_type,
                                    dst_port=dst_port, exp_dst_port=dst_port)
        generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type,
                                    'leaf-tor', protocol_type, nat_type=nat_type,
    ...
    def test_nat_dynamic_zones(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        generate_and_verify_not_translated_traffic(ptfadapter, setup_info, interface_type,
                                                   direction, protocol_type, nat_type)
    ...
    def test_nat_dynamic_single_host(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env, protocol_type):
        ...
        # Configure default rules for Dynamic NAT
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_data, interface_type,
                                   protocol_type, port_range=p_range_conf, default=True, handshake=True)
        # Define network data and L4 ports
        network_data = get_network_data(ptfadapter, setup_data, direction, interface_type, nat_type=nat_type)
        # Set timeouts to max
    ...
    def test_nat_dynamic_binding_remove(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        direction = 'host-tor'
        # Configure default rules for Dynamic NAT
        configure_dynamic_nat_rule(duthost, ptfadapter, ptfhost, setup_info, interface_type,
                                   protocol_type, default=True, handshake=True)
        ...
        entries = re.findall(pattern.format(get_public_ip(setup_data, interface_type), "{0}-{1}"
                                            .format(POOL_RANGE_START_PORT, POOL_RANGE_END_PORT)),
                             output[0]['binding name'])
        pytest_assert(len(entries) == 1, "Binding has not been added properly, binding count: {} \n {} ; {}"
                      .format(len(entries), output[0]['binding name'], nat_pools_dump[0]['pool name']))
        ...
        pytest_assert(len(entries) == 0, "Binding has not been deleted properly, binding count: {} \n {} ; {}"
                      .format(len(entries), output, nat_pools_dump[0]['pool name']))
        # Send TCP/UDP traffic and check
        wait_timeout(protocol_type)
        generate_and_verify_not_translated_traffic(ptfadapter, setup_info, interface_type,
                                                   direction, protocol_type, nat_type)
    ...
    def test_nat_dynamic_iptable_snat(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        pytest_assert(len(entries) == 1, "Pool has not been added properly, pool count: {} \n {} ; {}"
                      .format(len(entries), output[0]['pool name'], nat_bindings_dump[0]['binding name']))
        # Send TCP/UDP traffic and check
        generate_and_verify_traffic(duthost, ptfadapter, setup_data, interface_type,
                                    direction, protocol_type, nat_type=nat_type)
        ...
        pytest_assert(len(entries) == 1, "IP Tables rules are not properly programmed: {} \n {} \n {} \n {} \n {}"
                      .format(len(entries), output, nat_pools_dump[0]['pool name'],
                      nat_bindings_dump[0]['binding name'], nat_translations_dump))
    ...
    def test_nat_dynamic_outside_interface_delete(self, ptfhost, tbinfo, duthost, ptfadapter, setup_test_env,
        ...
        ifname_to_disable = setup_data[interface_type]["outer_zone_interfaces"][0]
        dut_interface_control(duthost, "ip remove",
    ...
    def test_nat_dynamic_nat_pools(self, tbinfo, duthost, ptfhost, ptfadapter, setup_test_env, protocol_type):
        ...
        direction = 'host-tor'
        nat_type = 'dynamic'
        inner_interface = dict(setup_data["indices_to_ports_config"])[get_src_port(setup_info,
                                                                                   direction, interface_type)[0]]
