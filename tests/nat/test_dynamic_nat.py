...
class TestDynamicNat(object):
    ...
    def test_nat_dynamic_basic_icmp(self, tbinfo, duthost, ptfadapter, ptfhost, setup_test_env):
        ...
        generate_and_verify_icmp_traffic(ptfadapter, setup_data, interface_type, direction, nat_type, icmp_id_start=POOL_RANGE_START_PORT, icmp_id_end=POOL_RANGE_END_PORT)
    ...
    def test_nat_dynamic_entry_persist_icmp(self, tbinfo, ptfhost, duthost, ptfadapter, setup_test_env):
        ...
        # Send ICMP traffic(host-tor -> leaf-tor) and check
        for _ in range(0, 4):
            generate_and_verify_icmp_traffic(ptfadapter, setup_data, interface_type, direction, nat_type=nat_type, icmp_id_start=POOL_RANGE_START_PORT, icmp_id_end=POOL_RANGE_END_PORT)
    ...
    def test_nat_dynamic_crud_icmp(self, tbinfo, duthost, ptfhost, ptfadapter, setup_test_env):
        ...
        wait_timeout(protocol_type)
        # Send ICMP traffic(host-tor -> leaf-tor) and check
        generate_and_verify_icmp_traffic(ptfadapter, setup_data, interface_type, direction, nat_type, icmp_id_start=start_port, icmp_id_end=end_port)
