...
def generate_client_cli(duthost_ip, gnxi_path, method=METHOD_GET, xpath="COUNTERS/Ethernet0", target="COUNTERS_DB", subscribe_mode=SUBSCRIBE_MODE_STREAM, submode=SUBMODE_SAMPLE, intervalms=0, update_count=3):
    ...
    cmd = cmdFormat.format(duthost_ip, TELEMETRY_PORT, method, xpath, target, "ndastreamingservertest")
...
def test_telemetry_ouput(duthosts, enum_rand_one_per_hwsku_hostname, ptfhost, setup_streaming_telemetry, localhost, gnxi_path, add_route_to_ptf):
    ...
    dut_ip = duthost.facts['dut_ip_ptfnet_add']
...
def test_osbuild_version(duthosts, enum_rand_one_per_hwsku_hostname, ptfhost, localhost, gnxi_path, add_route_to_ptf):
    """ Test osbuild/version query.
    """
    duthost = duthosts[enum_rand_one_per_hwsku_hostname]
    skip_201911_and_older(duthost)
    dut_ip = duthost.facts['dut_ip_ptfnet_add']
    cmd = generate_client_cli(duthost_ip=dut_ip, gnxi_path=gnxi_path, method=METHOD_GET, target="OTHERS", xpath="osversion/build")
    show_gnmi_out = ptfhost.shell(cmd)['stdout']
    result = str(show_gnmi_out)
...
def test_sysuptime(duthosts, enum_rand_one_per_hwsku_hostname, ptfhost, localhost, gnxi_path, add_route_to_ptf):
    """
...
def test_virtualdb_table_streaming(duthosts, enum_rand_one_per_hwsku_hostname, ptfhost, localhost, gnxi_path, add_route_to_ptf):
    ...
    skip_201911_and_older(duthost)
    dut_ip = duthost.facts['dut_ip_ptfnet_add']
    cmd = generate_client_cli(duthost_ip=dut_ip, gnxi_path=gnxi_path, method=METHOD_SUBSCRIBE, update_count = 3)
    show_gnmi_out = ptfhost.shell(cmd)['stdout']
    result = str(show_gnmi_out)
