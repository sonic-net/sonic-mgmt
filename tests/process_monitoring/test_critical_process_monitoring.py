...
def _is_bmc_topology(tbinfo):
    """Return True when running against a BMC testbed topology."""
    if not tbinfo:
        return False
    topo = tbinfo.get("topo", {}) if isinstance(tbinfo, dict) else {}
    return "bmc" in str(topo.get("type", "")) or "bmc" in str(topo.get("name", ""))


def _filter_bmc_unsupported(tbinfo, container_name, critical_process_list):
    """Remove processes that are not supported on BMC topology from the given list.

    Only filters when running on BMC; other topologies are unchanged.
    """
    if not _is_bmc_topology(tbinfo):
    ...
    if filtered != critical_process_list:
        logger.info(
            "BMC topology: removing unsupported critical processes %s from container '%s'; remaining=%s",
...
def get_expected_alerting_messages_monit(duthost, containers_in_namespaces, tbinfo=None):
    ...
    for container_name in list(containers_in_namespaces.keys()):
        ...
        critical_process_list = _filter_bmc_unsupported(tbinfo, container_name, critical_process_list)
...
def get_expected_alerting_messages_supervisor(duthost, containers_in_namespaces, tbinfo=None):
...
def stop_critical_processes(duthost, containers_in_namespaces, tbinfo=None):
...
def test_monitoring_critical_processes(
    ...
    if "20191130" in duthost.os_version:
        expected_alerting_messages = get_expected_alerting_messages_monit(duthost, containers_in_namespaces, tbinfo)
    else:
        expected_alerting_messages = get_expected_alerting_messages_supervisor(
            duthost, containers_in_namespaces, tbinfo)

    loganalyzer.expect_regex.extend(expected_alerting_messages)
    marker = loganalyzer.init()

    stop_critical_processes(duthost, containers_in_namespaces, tbinfo)
