...
def pick_ctrl_lag_port(ctrl_links, portchannels, want_single, duthost=None):
    ...
    teamd-selected. Multi-member LAGs can lag MACsec session establishment after a
...
class TestFaultHandling():
    ...
    def test_eapol_block_brings_lag_member_down(self, duthost, ctrl_links,
        ...
        try:
            ...
            # (5) A teamsyncd APP_LAG_MEMBER_TABLE refresh with status=enabled must NOT
            #     re-enable the member while MACsec is down. teamd stays deselected
            #     regardless (LACP can't form), so inject status=enabled and assert
            #     orchagent leaves the ASIC member disabled.
            ...
            assert not wait_until(20, 2, 0, member_reenabled_in_asic), (
                ...
                "refresh while MACsec was down. ASIC state: {}".format(
                    port_name, asic_disable_state()))
    ...
    def test_macsec_rekey_keeps_lag_member_up(self, duthost, ctrl_links,
        ...
        # Snapshot the SA tables to confirm a rekey occurs.
