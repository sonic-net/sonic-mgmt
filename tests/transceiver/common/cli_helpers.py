...
def sfputil_show_presence(duthost, port=None):
    """Run ``sfputil show presence [-p <port>]`` → ``({port: status}, err)``."""
    cmd = sfputil_show_presence_cmd(port)
    label = "sfputil show presence" + (f" -p {port}" if port else "")
    return _run_and_parse(duthost, cmd, label, parse_presence)
...
def show_interfaces_transceiver_presence(duthost, port=None):
    """Run ``show interfaces transceiver presence [<port>]`` → ``({port: status}, err)``."""
    cmd = show_interfaces_transceiver_presence_cmd(port)
    label = "show interfaces transceiver presence" + (f" {port}" if port else "")
    return _run_and_parse(duthost, cmd, label, parse_presence)
