...
# ──────────────────────────────────────────────────────────────────────
# LogAnalyzer suppression.  TC13 deliberately exercises the I2C bus under
...
# Maximum tolerated background EEPROM read failure rate during the TC13 stress
...
# Stem for /tmp file names used by _RemoteBgReader.  Centralized here so the
# session-scoped cleanup fixture in this directory's conftest.py uses the
# same prefix for stale-file cleanup.  Update both call sites if the stem
# ever changes.
...
class _RemoteBgReader(object):
    ...
    def start(self):
        """Launch the background loop and capture its PGID."""
        # Sequence:
        #   1. setsid bash -c '...' &        — bash becomes a new session leader
        #      (PGID == bash's own PID), runs detached from the test runner's
        #      job-control group so 'kill -- -<pgid>' is safe.
        #   2. Bash writes its own PID ($$) to pgid_file so we can capture the
        #      real PGID — the captured ``$!`` would point at the setsid
        #      wrapper, not bash, since setsid forks before exec'ing.
        #   3. After spawning, poll pgid_file for up to ~2 s and echo its
        #      contents.  That value (bash's PID) is the PGID we use for
        #      signal delivery in join().
        cmd = (
            "setsid bash -c '"
            f"echo $$ > \"{self.pgid_file}\"; "
            "fail_count=0; total_count=0; "
            "write_counts() { "
            f"echo \"$fail_count\" > \"{self.fail_file}\"; "
            f"echo \"$total_count\" > \"{self.total_file}\"; "
            "}; "
            "trap \"write_counts; exit 0\" TERM INT EXIT; "
            "while true; do "
            "total_count=$((total_count + 1)); "
            f"{cli_helpers.SFPUTIL_READ_EEPROM} -p {self.port} -n 0 -o 0 -s 1 >/dev/null 2>&1 || "
            "fail_count=$((fail_count + 1)); "
            "done"
            f"' </dev/null >/dev/null 2>&1 & "
            # Wait up to ~2 s for bash to record its PID in pgid_file.
    ...
    def join(self, timeout=10):
        ...
        self.host.shell(
            f"rm -f {self.fail_file} {self.total_file} {self.pgid_file}",
            module_ignore_errors=True,
        )
        try:
...
# ──────────────────────────────────────────────────────────────────────
# Private helpers for the CDB background-mode stress test (TC13).
...
def test_cdb_background_mode_stress_test(duthost, port_attributes_dict):
    ...
    # Stem-detection mapping resolved once — see TC12 for full rationale.
    lport_to_first = get_lport_to_first_subport_mapping(duthost)
