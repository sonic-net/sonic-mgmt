...
def simulate_small_var_log_partition(rand_selected_dut):
    ...
    try:
        with allure.step('Create a small var log partition with size of {}'.format(SMALL_VAR_LOG_PARTITION_SIZE)):
            ...
            logger.info('Restart rsyslog service on the small var log partition')
            duthost.shell('sudo service rsyslog restart')
        ...
        with allure.step('Recovery var log'):
            ...
            logger.info('Restart rsyslog service on the restored var log partition')
            duthost.shell('sudo service rsyslog restart', module_ignore_errors=not setup_finished)
...
def test_logrotate_small_size(rand_selected_dut, simulate_small_var_log_partition):
    ...
    Create a temp device which is around 300MB large, then mount it to /var/log
