...
def get_threshold_based_on_memory(duthost):
    """
    Get the logrotate size threshold from the DUT.
    First tries to read the actual configured threshold from /etc/logrotate.d/rsyslog,
    which is dynamically generated by the logrotate-config service.
    Falls back to computing based on /var/log partition size if config cannot be parsed.
    :param duthost: DUT host object
