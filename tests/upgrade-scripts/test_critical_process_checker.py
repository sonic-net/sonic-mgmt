...
def test_critical_process(duthosts, enum_rand_one_per_hwsku_hostname):
    ...
    assert validate_result['stdout'] == "Package sonic_critical_process_checker is not installed", "Failed to validate if sonic-critical-process-checker is installed: {}".format(validate_result['stdout'])
    ...
    assert validate_exit_code == 0, "Failed to run installer.py --validate"
    assert validate_result['stdout'] == "Package sonic_critical_process_checker is installed", "Failed to validate if sonic-critical-process-checker is installed: {}".format(validate_result['stdout'])
    ...
    assert json_data['succeeded'] == True, "Critical Process Checker execution failed, succeeded = {}".format(json_data['succeeded'])
    ...
    for service, service_list in json_data["service_dict"].items():
        for service_info in service_list:
            assert service_info["status"] == True, "Service {} is not running".format(service)
            if "process_dict" in service_info:
                for process, process_info in service_info["process_dict"].items():
                    assert process_info["status"] == True, "Process {} is not running".format(process)
