...
ISSUE_URL = "https://github.com/sonic-net/sonic-buildimage/issues/28619"
PRIORITY_ISSUE_URL = "https://github.com/sonic-net/sonic-buildimage/issues/28626"
...
STANDING_ROUTES = {
    ...
    VRF_B: "198.51.100.128/25",
}
CHURN_BASE = "203.0.113"
CHURN_FIRST, CHURN_COUNT = 129, 32
...
CHURN_WINDOW = 25
CHURN_MARKER = "vrf_rehome_rif_consistency_churn"
...
def analyze_rehome_window(lines, port, target_route_key):
    ...
    churn_re = re.compile(r"^ROUTE_TABLE:{}\.(\d+)/32$".format(re.escape(CHURN_BASE)))
    result = {"intf_del_seen": False, "intf_del_ts": None, "intf_set_ts": None,
              "del_to_set_ms": None, "target_route_ts": None,
              "route_vs_intf_del_ms": None, "churn_ops": 0}
    set_ts_list = []
    for line in lines:
        ...
        churn = churn_re.match(table_key)
        ...
        elif churn and CHURN_FIRST <= int(churn.group(1)) < CHURN_FIRST + CHURN_COUNT:
            result["churn_ops"] += 1
...
def churn_prefixes():
    return ["{}.{}/32".format(CHURN_BASE, CHURN_FIRST + i) for i in range(CHURN_COUNT)]
...
def _start_churn_loop(duthost):
    prefixes = churn_prefixes()
    add_cmds = " ".join('-c "ip route {} blackhole"'.format(p) for p in prefixes)
    del_cmds = " ".join('-c "no ip route {} blackhole"'.format(p) for p in prefixes)
    # Single-quoted body so $SECONDS/$end expand on the DUT, not in the submitting
    # shell; the trailing '# {marker}' lets _stop_churn_loop pkill the loop by name.
    script = ('end=$((SECONDS+{window})); while [ $SECONDS -lt $end ]; do '
              'timeout 15 docker exec bgp vtysh -c "configure terminal" {add} '
              '>/dev/null 2>&1; '
              'timeout 15 docker exec bgp vtysh -c "configure terminal" {del_} '
              '>/dev/null 2>&1; done # {marker}').format(
                  window=CHURN_WINDOW, add=add_cmds, del_=del_cmds, marker=CHURN_MARKER)
    res = duthost.shell(
        "nohup setsid bash -c '{}' >/dev/null 2>&1 & "
        'PID=$!; echo $PID $(ps -o pgid= -p $PID | tr -d " ")'.format(script),
        module_ignore_errors=True)
    parts = res.get("stdout", "").strip().split()
    if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
        return parts[0], parts[1]
    logger.warning("Failed to start churn loop: %r", res.get("stdout", ""))
    return None, None


def _stop_churn_loop(duthost, pid, pgid):
    if pid:
        duthost.shell("kill -- -{} 2>/dev/null; kill {} 2>/dev/null".format(pgid, pid),
                      module_ignore_errors=True)
    # The [x]yz character-class trick stops this pkill from matching its own command line.
    duthost.shell("pkill -f '[{}]{}' 2>/dev/null".format(CHURN_MARKER[0], CHURN_MARKER[1:]),
                  module_ignore_errors=True)
    del_cmds = ["no ip route {} blackhole".format(p) for p in churn_prefixes()]
    _vtysh(duthost, del_cmds)


def _churn_alive(duthost, pid):
    if not pid:
        return False
    return duthost.shell("kill -0 {} 2>/dev/null".format(pid),
...
def rehome_env(rehome_duthost):
    ...
    try:
        ...
        for vrf_name, key in ((VRF_A, "vr_a"), (VRF_B, "vr_b")):
            known = _vr_oids(duthost)
            res = _run(duthost, "config vrf add {}".format(vrf_name))
            pytest_assert(res.get("rc") == 0, "config vrf add {} failed".format(vrf_name))
            found = {}

            def _vr_created(known=known, found=found):
            ...
            pytest_assert(wait_until(SETTLE_TIMEOUT, 2, 0, _vr_created),
                          "Virtual router for {} not created".format(vrf_name))
            env[key] = found["oid"]
        ...
        logger.info("rehome_env: port=%s port_oid=%s default_vr=%s vr_a=%s vr_b=%s",
                    port, env["port_oid"], env["default_vr"], env["vr_a"], env["vr_b"])
        yield env
    finally:
...
def _teardown(duthost, env):
    ...
    if port is None:
        return
    _stop_churn_loop(duthost, None, None)
    ...
    for vrf_name in (VRF_A, VRF_B):
        _run(duthost, "config vrf del {}".format(vrf_name))
    ...
    def _converged():
        ...
        testnet3_gone = not _asic_keys(
            duthost, '{}:*"dest":"{}.*'.format(ASIC_ROUTE_TBL, CHURN_BASE))
        return (rif_gone and routes_gone and testnet3_gone
...
def _expected_vr(env, vrf_name):
    ...
    return env["vr_a"] if vrf_name == VRF_A else env["vr_b"]
...
def _fail_with_evidence(duthost, env, label, details, analysis):
    ...
    pytest.fail(
        "{} ({})\n{}\nviolations={}\nport_rif={}\nordering analysis={}\n"
        "wedge logs:\n{}\nsairedis.rec:\n{}".format(
            label, ISSUE_URL, details, violations, rif, analysis, wedge, rec))
...
def _rehome_iteration(duthost, env, target_vrf, use_churn):
    port = env["port"]
    stats = {"target": target_vrf or "default", "churn_started": False, "churn_alive": False}
    ...
    pid = pgid = None
    if use_churn:
        pid, pgid = _start_churn_loop(duthost)
        stats["churn_started"] = pid is not None
    try:
        ok, detail = _rehome(duthost, port, target_vrf)
        if use_churn:
            stats["churn_alive"] = _churn_alive(duthost, pid)
        if not ok:
        ...
        if early:
            window = _swss_rec_window(duthost, rec_start, port)
            _fail_with_evidence(
                duthost, env, "VRF REHOME RACE OBSERVED (stale cross-VR route)",
        ...
        if use_churn:
            _stop_churn_loop(duthost, pid, pgid)
...
def _classify_anomaly(duthost, env, target_vrf, detail, window_lines, target_key):
    ...
    if violations:
        _fail_with_evidence(duthost, env,
                            "VRF REHOME RACE OBSERVED (stale cross-VR route)",
                            detail, analysis)
    if analysis["intf_del_seen"] is False and analysis["intf_set_ts"] is not None:
        _fail_with_evidence(
            duthost, env,
            "VRF REHOME RACE OBSERVED (rehome silently lost: INTF_TABLE DEL was "
    ...
    if rif is not None and rif[1] != expected_vr:
        _fail_with_evidence(
            duthost, env,
            "VRF REHOME RACE OBSERVED (rehome lost: RIF still in VR {} after the "
            "transition)".format(rif[1]), detail, analysis)
    pytest.fail("TEST-MACHINERY: rehome to {} {} without any race signature "
                "(analysis={}) — environment or harness problem, not the "
                "issue under test".format(_vrf_label(target_vrf), detail, analysis))


def test_vrf_rehome_baseline_converges_consistently(rehome_duthost, rehome_env):
    """Positive control: one churn-free rehome must converge fully route/RIF consistent."""
    duthost = rehome_duthost
    env = rehome_env
    verify_orchagent_running_or_assert(duthost)
    stats = _rehome_iteration(duthost, env, VRF_A, use_churn=False)
    logger.info("Baseline rehome telemetry: %s", stats)

    pytest_assert(len({env["default_vr"], env["vr_a"], env["vr_b"]}) == 3,
                  "Virtual routers are not distinct: {}".format(env))
    pytest_assert(not _route_consistent(duthost, env["vr_b"], STANDING_ROUTES[VRF_A]),
...
def test_vrf_rehome_bounded_stress_keeps_route_rif_vr_consistent(
    ...
    """Cycle the port A -> default -> B under bounded churn, asserting route/RIF VR
    ...
    logger.info("Running %d bounded rehome iterations (hard cap %d, "
                "2 producer workers)", iterations, MAX_ITERATIONS)

    cycle = [VRF_A, DEFAULT_VRF, VRF_B, DEFAULT_VRF]
    ...
    for index in range(1, iterations + 1):
        ...
        stats = _rehome_iteration(duthost, env, target, use_churn=True)
        stats["iteration"] = index
        all_stats.append(stats)
        if not stats["churn_started"] or not stats["churn_alive"]:
            degraded += 1
            logger.warning("Iteration %d ran without live churn — it did not "
                           "exercise concurrent producer load", index)
    ...
    summary = {
        ...
        "priority_caveat": PRIORITY_ISSUE_URL,
    }
    logger.info("Bounded stress summary (no stale cross-VR route observed): %s",
                summary)
    pytest_assert(degraded <= len(all_stats) // 2,
                  "Churn worker was dead in {} of {} iterations; the stress "
                  "did not exercise concurrency (machinery, not the bug): {}"
                  .format(degraded, len(all_stats), summary))
    verify_orchagent_running_or_assert(duthost)
