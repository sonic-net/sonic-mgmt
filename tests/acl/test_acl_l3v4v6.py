...
pytestmark = [
    pytest.mark.acl,
    pytest.mark.asic('marvell-teralynx'),
...
ACL_RULES_FULL_TEMPLATE = {
    "ipv4": "acltbv4v6_v4_test_rules.j2",
    "ipv6": "acltbv4v6_v6_test_rules.j2"
}
ACL_RULES_PART_TEMPLATES = {
    "ipv4": tuple("acltbv4v6_test_rules_part_{}.j2".format(i) for i in range(1, 3)),
    "ipv6": tuple("acltbv4v6_v6_test_rules_part_{}.j2".format(i) for i in range(1, 3))
}
...
def acl_table(duthosts, rand_one_dut_hostname, setup, stage, ip_version, tbinfo):
    ...
    for duthost in duthosts:
        if duthost.is_supervisor_node():
            continue
        statedb_capability = \
            'sudo sonic-db-cli STATE_DB HGET "ACL_STAGE_CAPABILITY_TABLE|{}" supported_L3V4V6'.format(stage.upper())
        supported = duthost.shell(statedb_capability)['stdout'].strip()
        logger.info("[{}]=>{}.".format(statedb_capability, supported))
        if supported != 'true':
            pytest.skip("L3V4V6 ACL not supported in stage {}.".format(stage))
