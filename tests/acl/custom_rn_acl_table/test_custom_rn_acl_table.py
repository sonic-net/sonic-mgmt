...
pytestmark = [
    pytest.mark.topology("t0-isolated-d2u510s2",
                         "t0-isolated-d2u254s1",
                         "t0-isolated-d32u32s2"),  # Only run on backend T0 testbed
    pytest.mark.asic("mellanox", "broadcom"),  # Only run on mellanox platform
...
def build_testing_pkts(router_mac):
    ...
    # Verify matching TCP source port and TCP flag
