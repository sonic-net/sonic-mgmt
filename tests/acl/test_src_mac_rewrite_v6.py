...
VNET_PRIMARY_ROUTE_IP = "2001:db8:150::3"  # Primary IPv6 VNET route destination
VNET_PRIMARY_ROUTE_PREFIX = f"{VNET_PRIMARY_ROUTE_IP}/128"
VNET_INGRESS_IP = "2001:db8:201::1/64"     # Routed IPv6 address on the VNET ingress interface
...
def fixture_setUp(request, rand_selected_dut, tbinfo, ptfadapter):
    ...
    for intf in loopback0_ips:
        if intf["name"] == "Loopback0" and "." in intf["addr"]:  # IPv4 address (outer VXLAN underlay)
...
def _send_and_verify_mac_rewrite(ptfadapter, ptf_port_1, ptf_ports, duthost,
    ...
    """
    Send one IPv6 test packet from the PTF host and verify whether the ACL rewrote
    the inner source MAC of the DUT-emitted VXLAN packet.
...
def _test_inner_src_mac_rewrite(setUp, scenario_name):
    ...
    inner_dst_ipv6 = VNET_PRIMARY_ROUTE_IP  # Route destination
    vni_id = str(VXLAN_VNI)  # VNI from configuration
    inner_src_ipv6 = "2001:db8:201::101"  # Source IPv6 address for test packets
    ...
    try:
        ...
        if scenario_name == "single_ipv6_test":
            # Use a specific source IPv6 address for ACL rule matching.
            ...
            # Match multiple source IPv6 addresses in the VNET ingress subnet.
        ...
        # Test with the configured source IPv6 address.
        ...
        # For the range test, also test other IPv6 addresses in the /64.
...
def test_partial_match(setUp):
    ...
    try:
        ...
        # Case 1: VNI matches but source IPv6 does not match.
        ...
        # Case 2: Source IPv6 matches but VNI does not. UNPROVISIONED_VNI is never provisioned
...
def test_multiple_acl_rules(setUp):
    ...
    try:
        ...
        # Test Rule 1: Send a packet matching the first source IPv6 address.
        ...
        # Test Rule 2: Send a packet matching the second source IPv6 address.
