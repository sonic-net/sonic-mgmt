...
class TestPfcXoffProbing:
    ...
    # ========================================================================
    # G. Bug Fix Validation (2 tests)
    # ========================================================================

    def test_pfc_xoff_threshold_at_one(self):
        """
        ...
        - Probing completes without crash (result may be failure for
          extreme threshold=1 since lower bound cannot go below 1)
        """
        actual_threshold = 1
        ...
        # threshold=1 is an extreme edge case — algorithm may not find a valid range
        # because lower-bound search cannot go below 1. The key validation is that
        # probing terminates and doesn't crash.
        if result.success:
            assert result.lower_bound <= actual_threshold <= result.upper_bound, \
    ...
    def test_pfc_xoff_threshold_at_two(self):
        ...
        - Lower-bound halving (2 -> 1 -> break) works as expected
        """
        actual_threshold = 2
    ...
    def test_pfc_xoff_point_probing_with_intermittent_failures(self):
        """
        ...
        - End-to-end: probing completes (may succeed or fail gracefully
          depending on noise severity)
        """
        actual_threshold = 500
        ...
        # Should complete without crash regardless of intermittent failures
        probe.runTest()
        result = probe.probe_result
