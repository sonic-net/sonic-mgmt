...
from probe_test_helper import create_pfc_xoff_probe_instance
...
class TestPfcXoffProbing:
    ...
    def test_pfc_xoff_normal_scenario(self):
        ...
        assert actual_range <= expected_precision * 2, \
            f"Range {actual_range} should be within precision {expected_precision * 2}"
        
        print(f"[PASS] Normal scenario: threshold={actual_threshold}, result=[{result.lower_bound}, {result.upper_bound}]")
    ...
    def test_pfc_xoff_point_probing_normal(self):
        ...
        assert result.lower_bound <= actual_threshold <= result.upper_bound, \
            f"Result [{result.lower_bound}, {result.upper_bound}] should contain actual {actual_threshold}"
        
        print(f"[PASS] Point Probing test: threshold={actual_threshold}, result=[{result.lower_bound}, {result.upper_bound}]")
    ...
    def test_pfc_xoff_noisy_hardware(self):
        ...
        result_range = result.upper_bound - result.lower_bound
        max_expected_range = actual_threshold * 0.5  # Allow up to 50% range for very noisy scenario
        assert result_range <= max_expected_range, \
            f"Noisy result range {result_range} should be reasonable (<= {max_expected_range})"
        
        print(f"[PASS] Noisy scenario: threshold={actual_threshold}, result=[{result.lower_bound}, {result.upper_bound}], range={result_range}")
        print(f"       Note: Noisy scenarios may have wider ranges or offset results")
    ...
    def test_pfc_xoff_intermittent(self):
        ...
        if result.success:
            assert result.lower_bound <= actual_threshold <= result.upper_bound, \
                f"Result [{result.lower_bound}, {result.upper_bound}] should contain {actual_threshold}"
            ...
            print(f"[PASS] Intermittent: Extreme case detected, probing failed as expected (success={result.success})")
    ...
    def test_pfc_xoff_point_probing_noisy(self):
        ...
        assert result_range < 150, \
            f"Point Probing with noise should still produce reasonable range, got {result_range}"
        
        print(f"[PASS] Point Probing (noisy): range={result_range}, result=[{result.lower_bound}, {result.upper_bound}]")
    ...
    def test_pfc_xoff_low_noise_few_attempts(self):
        ...
        assert result.lower_bound - tolerance <= actual_threshold <= result.upper_bound + tolerance, \
            f"Result [{result.lower_bound}, {result.upper_bound}] should roughly contain {actual_threshold} (tolerance={tolerance})"
    ...
    def test_pfc_xoff_zero_threshold(self):
        ...
        if result.success:
            ...
            assert result.upper_bound <= 100, \
                f"For zero threshold, upper bound {result.upper_bound} should be small"
            print(f"[PASS] Zero threshold: result=[{result.lower_bound}, {result.upper_bound}]")
        else:
            print(f"[PASS] Zero threshold: Edge case handled, probing failed as expected")
    ...
    def test_pfc_xoff_max_threshold(self):
        ...
        assert result.upper_bound <= pool_size, \
            f"Upper bound {result.upper_bound} should not exceed pool size {pool_size}"
        assert result.lower_bound <= actual_threshold <= result.upper_bound
    ...
    def test_pfc_xoff_always_pfc(self):
        ...
        if result.success:
            assert result.lower_bound <= 10, \
                f"If PFC always triggers, lower bound {result.lower_bound} should be very small"
            print(f"[PASS] Always PFC: result=[{result.lower_bound}, {result.upper_bound}]")
        else:
            print(f"[PASS] Always PFC: Edge case handled, probing failed as expected")
    ...
    def test_pfc_xoff_inconsistent_results(self):
        ...
        if result.success:
            assert result.lower_bound <= actual_threshold <= result.upper_bound, \
                f"Despite inconsistency, result [{result.lower_bound}, {result.upper_bound}] should bracket {actual_threshold}"
            print(f"[PASS] Inconsistent results: threshold={actual_threshold}, result=[{result.lower_bound}, {result.upper_bound}]")
        else:
            print(f"[PASS] Inconsistent results: Extreme inconsistency handled, probing failed as expected")
    ...
    def test_pfc_xoff_multi_verification_default_5_attempts(self):
        ...
        assert result_range <= expected_max * 2, \
            f"Precision should be reasonable: range={result_range} vs expected<={expected_max*2}"
        
        print(f"[PASS] Multi-verification default behavior validated:")
        print(f"      threshold={actual_threshold}, result=[{result.lower_bound}, {result.upper_bound}]")
        print(f"      range={result_range} cells")
        print(f"      -> Default max_attempts=5 mechanism working correctly")
