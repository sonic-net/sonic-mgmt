...
def create_mock_hardware_ops():
    ...
    def mock_port_tx_enable(client, asic_type, port_list, target='dst', last_port=True, enable_port_by_unblock_queue=True):
...
def create_probe_instance(probe_class, test_params):
    ...
    try:
        ...
        if hasattr(probing_base_module, 'send_packet'):
            original_send_packet = probing_base_module.send_packet
    ...
    original_assert_probing_result = probe.assert_probing_result
