...
from sim_ingress_drop_probing_executor import (
...
def test_ingress_drop_wrong_config(mock_observer):
    ...
    print(f"Configured threshold: 500, Offset: +100 -> Effective threshold: 600")
    print("\nTesting behavior:")
    
    # Below effective threshold (500+100=600) - should not drop
    result1 = executor.check(None, None, 500)[1]  # check() returns (success, detected)
    print(f"  Check at 500 packets (configured threshold): {'DROP' if result1 else 'NO drop'} (expected: NO drop due to offset)")
    assert not result1
    ...
    print(f"  At 500 packets: Normal={'DROP' if normal_result else 'NO drop'}, WrongConfig={'DROP' if wrong_result else 'NO drop'}")
