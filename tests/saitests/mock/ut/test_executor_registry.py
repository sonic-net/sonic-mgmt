...
def test_create_error_unregistered_executor():
    ...
    with pytest.raises(KeyError, match="not registered"):
        ExecutorRegistry.create('pfc_xoff', 'sim', scenario='nonexistent_variant',
...
def test_module_caching():
    ...
    assert executor1.check(None, None, 150)[1] == True
    assert executor2.check(None, None, 150)[1] == False  # noise might affect
...
def test_lazy_import_execution_order():
    ...
    pfc_entries = [k for k in ExecutorRegistry._registry.keys() if k[0] == 'pfc_xoff']
    print(f"   PFC executors in registry: {len(pfc_entries)}")
    print(f"   sim_pfc_xoff_probing_executor loaded: {'sim_pfc_xoff_probing_executor' in ExecutorRegistry._loaded_modules}")
    ...
    pfc_entries_after = [k for k in ExecutorRegistry._registry.keys() if k[0] == 'pfc_xoff']
