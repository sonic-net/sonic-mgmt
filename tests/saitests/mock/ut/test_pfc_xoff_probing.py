...
from probing_base import ProbeConfig
...
class TestPfcXoffProbingParameterParsing:
    ...
    def test_parse_param_different_pg_values(self):
        ...
        print(f"[OK] All PG values correctly processed")
...
class TestPfcXoffProbingAlgorithmCreation:
    ...
    def test_create_algorithms_with_point_probing(self):
        ...
        try:
            ...
            assert isinstance(algorithms, dict), "Should return dictionary"
            assert "threshold_point" in algorithms, "Should have threshold_point algorithm when precise detection enabled"
            assert len(algorithms) == 4, f"Should have 4 algorithms (including threshold_point), got {len(algorithms)}"
