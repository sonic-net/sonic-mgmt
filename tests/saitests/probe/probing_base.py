...
from sai_qos_tests import get_rx_port, log_message

# Import for runTest template
from ptf.testutils import send_packet
from buffer_occupancy_controller import BufferOccupancyController
...
class ProbingBase(sai_base_test.ThriftInterfaceDataPlane):
    ...
    def setUp(self):
        ...
        ProbingObserver.trace(f"[{self.__class__.__name__}] POINT_PROBING_STEP_SIZE before env check: {self.POINT_PROBING_STEP_SIZE}")
        ...
        if step_size.isdigit() and int(step_size) > 0:
            ...
            ProbingObserver.trace(f"[{self.__class__.__name__}] Applied env POINT_PROBING_STEP_SIZE={self.POINT_PROBING_STEP_SIZE}")
        else:
            ProbingObserver.trace(f"[{self.__class__.__name__}] Using class default POINT_PROBING_STEP_SIZE={self.POINT_PROBING_STEP_SIZE}")
        ...
        ProbingObserver.trace(f"[{self.__class__.__name__}] use_pg_drop_counter={self.use_pg_drop_counter} (env='{env_value}')")
