...
class ThresholdRangeProbingAlgorithm:
    ...
    def run(self, src_port: int, dst_port: int, 
        ...
        Returns:
            Tuple[lower_bound, upper_bound, phase_time]: Detected threshold range with phase time or (None, None, 0.0) on failure
        """
        try:
            # Prepare ports for threshold probing
            self.executor.prepare(src_port, dst_port)
            
            # Phase 3: Precision Range Detection using binary search with dynamic precision control
            ...
            while iteration < max_iterations and range_stack:
                ...
                if precision_reached:
                    ...
                    iteration_time, phase_time = self.observer.on_iteration_complete(iteration, candidate_threshold, IterationOutcome.SKIPPED)
                ...
                success, detected = self.executor.check(
                    src_port, dst_port, candidate_threshold, attempts=self.verification_attempts, iteration=iteration, **traffic_keys
                )
                
                iteration_time, phase_time = self.observer.on_iteration_complete(
                    iteration, candidate_threshold, IterationOutcome.from_check_result(detected, success)
                )
