...
class ThresholdPointProbingAlgorithm:
    ...
    def run(self, src_port: int, dst_port: int, lower_bound: int, upper_bound: int, **traffic_keys) -> Tuple[Optional[int], Optional[int], float]:
        """
        ...
        try:
            ...
            for iteration, current_packets in enumerate(range(lower_bound + 1, upper_bound + 1, step), start=1):
                ...
                # Pass current_packets as lower bound (current testing point) and upper_bound as window for markdown table display
                # This ensures the bounds column shows the current search range [current_value, upper_bound]
