        ...
        try:
            ...
            for cntr in ingress_counters:
                assert(recv_counters[cntr] == recv_counters_base[cntr]), \
                        'unexpectedly RX drop counter increased. Counter:{}, from {} to {}, {}'.format(
                                cntr, recv_counters_base[cntr], recv_counters[cntr], test_stage)
            # xmit port no egress drop
            for cntr in egress_counters:
                assert xmit_counters[cntr] == xmit_counters_base[cntr], \
                    'unexpectedly TX drop counter increase. Counter:{}, from {} to {}, {}'.format(
                        cntr, xmit_counters_base[cntr], xmit_counters[cntr], test_stage)
...
def get_multiple_flows_udp(dp, dst_mac, dst_id, dst_ip, src_vlan, dscp, ecn, ttl, pkt_len, src_details, packets_per_port=1):
    ...
    for src_tuple in src_details:
        ...
        while (num_of_pkts < packets_per_port):
            ...
            pkt_args = [
                pkt_len,
                dst_mac or dp.dataplane.get_mac(0, dst_id),
                dp.dataplane.get_mac(0, src_tuple[0]),
                ip_Addr,
                #src_tuple[1],
                dst_ip,
                dscp,
...
class LosslessVoq(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        def collect_counters():
            counter_details = []
            for src_tuple in src_details:
                counter_details.append(sai_thrift_read_port_counters(self.client, asic_type, port_list[src_tuple[0]]))
            counter_details.append(sai_thrift_read_port_counters(self.client, asic_type, port_list[dst_port_id]))
        ...
        try:
            ...
            for i in range(2):
                ...
                assert pfc_txd == 0, "Unexpected PFC TX {} on port {} for pg:{}".format(pfc_txd, src_details[i][0], pg)
                # recv port no ingress drop
                for cntr in ingress_counters:
            ...
            # recv port pfc
            for i in range(2):
                assert counter_details_3[i][0][pg] > counter_details_before[i][0][pg], "PFC TX counters did not increase for port {} for PG:{}".format(src_details[i][0], pg)

            for i in range(2):
                # recv port Starts PFC:
                pfc_txd = counter_details_3[i][0][pg] - counter_details_before[i][0][pg]
                assert pfc_txd > 0, "PFC TX didn't start on port {} for pg:{}".format(src_details[i][0], pg)
                # recv port no ingress drop
                for cntr in ingress_counters:
...
class DscpEcnSend(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            port_counters, queue_counters = sai_thrift_read_port_counters(self.client, self.asic_type, port_list[dst_port_id])
            print(port_counters)
...
class LossyQueueVoqTest(sai_base_test.ThriftInterfaceDataPlane):
    ...
    def runTest(self):
        ...
        try:
            ...
            fill_leakout_plus_one(self, self.src_port_id, self.dst_port_id, all_pkts[0][0],
            ...
            print("Addresses: ".format([all_pkts[i][0]['IP'].src for x in range(4)]))
