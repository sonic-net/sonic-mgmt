...
class LossyQueueVoqTest(sai_base_test.ThriftInterfaceDataPlane):
    ...
    def runTest(self):
        ...
        try:
            ...
            fill_leakout_plus_one(self, self.src_port_id, self.dst_port_id, first_pkt,
                                  int(self.test_params['pg']), self.asic_type)
            send_packet(self, self.src_port_id, first_pkt, self.pkts_num_trig_egr_drp)
            time.sleep(2)
            # Verify egress drop
            ...
            for index, (second_pkt, _) in enumerate(pkt_list):
                ...
                if second_pkt == first_pkt:
                    ...
                    if drop_count == 0:
                        print("Second flow detected on packet index {} in mode '{}'".format(index, self.flow_config), file=sys.stderr)
                        assert self.flow_config == "separate", "Identified a second flow despite being in mode '{}'".format(self.flow_config)
                        break
            else:
            ...
            xmit_counters_base, _ = sai_thrift_read_port_counters(self.dst_client, self.asic_type,
                                                                  port_list['dst'][self.dst_port_id])
            fill_leakout_plus_one(self, self.src_port_id, self.dst_port_id, second_pkt,
                                  int(self.test_params['pg']), self.asic_type)
