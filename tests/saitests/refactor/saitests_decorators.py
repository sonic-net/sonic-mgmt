...
def show_banner(decorator_instance, testcase_instance, testcase_method, param, result, *args, **kwargs):
    ...
    log_message(f'Entering {testcase_method.__name__} with args={summarized_args} kwargs={summarized_kwargs}', to_stderr=True)
        ...
        for port, port_rules in param.items():
            ...
            for counter_name, counter_rules in port_rules.items():
                ...
                for field_name, field_rules in counter_rules.items():
                    ...
                    delta = counter.get_counter_delta(testcase_method.__name__ + '.after',
                                                      testcase_method.__name__ + '.before',
                                                      port_id, field_name)
                    ops = {
                        '==': operator.eq,
                        '<=': operator.le,
