...
def convert_to_rsb(duthosts, duts_minigraph_facts):
    minigraph_xml = "/etc/sonic/minigraph.xml"
    local_xml = "/tmp/minigraph.xml"

    def convert_dut_to_rsb(node, duts_minigraph_facts, results):
        if node.is_supervisor_node():
            return
        node.shell(f"rm /etc/sonic/running_golden*", module_ignore_errors=True)
    ...
    for dut in duthosts:
        ...
        dut.shell(f"rm /etc/sonic/running_golden*", module_ignore_errors=True)
...
def prepare_gcu_patches(duthost, full_configs, rsb_configs, gcu_patches="gcu_patches"):
    ...
    for asic in range(3):
        cmds = [
            f'''jsondiff --indent 2 {rsb_configs}/config_db{asic}.json {full_configs}/config_db{asic}.json > /tmp/f; mv /tmp/f {gcu_patches}/patch{asic}.json''',
            f'''jq 'map(select(.op != "remove"))' {gcu_patches}/patch{asic}.json > /tmp/f; mv /tmp/f {gcu_patches}/patch{asic}.json''',
            f'''jq 'map(select(.op != "move"))' {gcu_patches}/patch{asic}.json > /tmp/f; mv /tmp/f {gcu_patches}/patch{asic}.json''',
            f'''jq 'map(select(.path | contains("BUFFER_PROFILE") | not))' {gcu_patches}/patch{asic}.json > /tmp/f; mv /tmp/f {gcu_patches}/patch{asic}.json''',
            f'''jq 'map(select(.path | contains("BUFFER_POOL") | not))' {gcu_patches}/patch{asic}.json > /tmp/f; mv /tmp/f {gcu_patches}/patch{asic}.json''',
            f'''jq 'map(select(.path | contains("BUFFER_PG") | not))' {gcu_patches}/patch{asic}.json > /tmp/f; mv /tmp/f  {gcu_patches}/patch{asic}.json''',
