...
def run_pfcwd_burst_storm_low_rate_test(
        api,
        testbed_config,
        port_config_list,
        conn_data,
    ...
    pause_flow_gap_sec = burst_cycle_sec - 5
...
def __gen_traffic(testbed_config,
    ...
    data_flow_rate_percent = 0.0000002 / len(prio_list)
...
def __run_traffic(api, config, all_flow_names, exp_dur_sec):
    ...
    for iteration in range(10):
        ...
        request = api.metrics_request()
        request.flow.flow_names = all_flow_names
        ret_rows.append(api.get_metrics(request).flow_metrics)
