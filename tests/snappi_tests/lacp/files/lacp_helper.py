...
def duthost_bgp_config(duthost,
    ...
    logger.info('Configuring PortChannel2 with IPs %s,%s' % (tgen_ports[1]['peer_ip'], tgen_ports[1]['peer_ipv6']))
...
def __tgen_bgp_config(snappi_api,
    ...
    for i in range(0, port_count):
        config.ports.port(name='Test_Port_%d' %
                          i, location=temp_tg_port[i]['location'])

    lag0 = config.lags.lag(name="lag0")[-1]
    lp = lag0.ports.port(port_name='Test_Port_0')[-1]
    lag0.protocol.lacp.actor_system_id = "00:10:00:00:00:01"
    lp.ethernet.name = "lag_Ethernet 1"
    lp.ethernet.mac = "00:10:01:00:00:01"
...
def get_lacp_add_remove_link_physically(snappi_api,
    ...
    all_port_names.insert(0, 'Test_Port_0')
