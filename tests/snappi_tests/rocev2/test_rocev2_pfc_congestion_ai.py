...
def test_rocev2_pfc_congestion_ai(
    ...
    try:
        ...
        pfc_fail_expr = " and ".join(f"{c} == 0" for c in pfc_cols) or "PFC_none == 0"
        ...
        checks = [
            ...
            # PFC pause must be generated by the DUT on at least one lossless priority.
            make_check(pfc_fail_expr, list(pfc_sum_df.columns),
                    f"DUT generated PFC on lossless priority {used_lossless}",
                    f"DUT Tx PFC expected on priority {used_lossless} under incast congestion",
