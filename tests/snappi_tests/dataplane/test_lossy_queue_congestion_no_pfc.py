...
def test_lossy_queue_congestion_no_pfc(
    ...
    for snappi_ports in congestion_ports:
        logger.info('\n')
        logger.info("Snappi ports used for the test:")
        for port in snappi_ports:
            logger.info('{}: {}'.format(port['peer_port'], port['location']))
        pytest_assert(len(snappi_ports) >= 3,
                      "Not enough ports for the test, Need at least 3 ports")
        # Two ingress (Tx) ports feed a single egress (Rx) port so the egress
        # lossy queue is driven at ~2x line rate.
        tx_ports = [snappi_ports[0], snappi_ports[1]]
