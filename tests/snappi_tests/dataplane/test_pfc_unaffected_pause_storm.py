...
# Traffic runs for ~60s before being explicitly stopped. The 1000s flow
# duration ensures flows never self-terminate before the stop command.
...
def test_pfc_unaffected_pause_storm(
    ...
    for snappi_ports in port_groups:
        ...
        traffic_flow_config.append({
            "is_pfc": True,
            "flow_name": "PFC_Storm_{}".format("_".join(str(p) for p in LOSSLESS_PRIO)),
            "frame_size": 64,
