...
from tests.snappi_tests.qos.files.qos_priority_helper import initiate_snappi_port_groups_dict, define_rx_tx_ports, read_dut_qos_configurations, create_snappi_flows, config_snappi_egress_tracking_wred, clear_dut_stat_counters, run_traffic,  map_scheduler_id_by_weight, save_dut_config, reload_dut, verify_dut_ports_up, backup_config_db, restore_config_db, stop_traffic, delete_flows, verify_line_rate, verify_line_rate_tc_5, verify_line_rate_tc_6, verify_frames 
...
from tests.snappi_tests.dataplane.files.helper import get_duthost_interface_details, set_primary_chassis, create_snappi_config
...
class Common_vars:
    ...
    backup_config_db_file = f'/tmp/{config_db_file.split("/")[-1]}.backup'
       

def execute_common_configs(duthosts, create_snappi_config, snappi_api, get_snappi_ports, subnet_type, 
                           flow_configs, config_egress_tracking=True, enable_pkt_sequence_checking=False):
    ...
    for duthost in duthosts:
        ...
        Common_vars.scheduler_to_weight_dict, Common_vars.weight_to_scheduler_dict = map_scheduler_id_by_weight(duthost)
    ...
    verify_dut_scheduler_obj = read_dut_qos_configurations(Common_vars, duthosts[0])
...
def test_dwrr_wred_with_extreme_weight_ratio_1(snappi_api,                 # noqa F811
    ...
    # TC 2.1: 
    # 2 Tx-Ports: 
    #       Note: Test case states to transmit 60% line rate, but this is incorrect. The CE bit won't change from ingressing bit2 to egressing bit 3.
    #             Line rate must be higher than the weight.
    #             Otherwise, the Sonic DUT doesn't transmit frames out of CE bit 3 and this script verifies packets on CE bit 3.
    Common_vars.flows_per_port = 'single'
    flow_configs = {'test_case': '2.1',
                    'total_tx_ports': 2,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 60, 'line_rate': 70, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 60},
                                      {'queue_id': 2, 'weight': 40, 'line_rate': 60, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 40}]}
    ...
    control_state_obj = execute_common_configs(duthosts, create_snappi_config, snappi_api, get_snappi_ports, subnet_type,
                                               flow_configs, config_egress_tracking=True, enable_pkt_sequence_checking=False)
...
def test_dwrr_wred_with_extreme_weight_ratio_2(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.2',
                    'total_tx_ports': 2,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 95, 'line_rate': 100, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 95},
                                      {'queue_id': 2, 'weight': 5,  'line_rate': 100, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 5}]} 
...
def test_dwrr_wred_with_extreme_weight_ratio_3(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.3',
                    'total_tx_ports': 3,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 30, 'line_rate': 50, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 30},
                                      {'queue_id': 2, 'weight': 40, 'line_rate': 50, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 40},
                                      {'queue_id': 3, 'weight': 30, 'line_rate': 50, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 30}]} 
...
def test_dwrr_wred_with_extreme_weight_ratio_4(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.4',
                    'total_tx_ports': 4,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 30, 'line_rate': 35, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 30}, 
                                      {'queue_id': 2, 'weight': 40, 'line_rate': 45, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 40}, 
                                      {'queue_id': 3, 'weight': 15, 'line_rate': 25, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 15}, 
                                      {'queue_id': 4, 'weight': 15, 'line_rate': 25, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 15}]}
...
def test_dwrr_wred_with_extreme_weight_ratio_6(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.6',
                    ...
                    'tx_port_flows': [{'queue_id': 1, 'weight': 30, 'line_rate': 70, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 30},
                                      {'queue_id': 2, 'weight': 40, 'line_rate': 70, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 40},
                                      {'queue_id': 3, 'weight': 15, 'line_rate': 70, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 15},
                                      {'queue_id': 4, 'weight': 15, 'line_rate': 70, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 15}]}

    # Avoid egress tracking for test case 2.6 because of flow name getting reset and need to be renamed
    control_state_obj = execute_common_configs(duthosts, create_snappi_config, snappi_api, get_snappi_ports, subnet_type,
                                               flow_configs, config_egress_tracking=False, enable_pkt_sequence_checking=False)  
...
def test_dwrr_wred_with_extreme_weight_ratio_7_phase_1(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.7 phase 1',
                    'total_tx_ports': 3,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 20, 'line_rate': 30, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 24},
                                      {'queue_id': 2, 'weight': 50, 'line_rate': 40, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 40},
                                      {'queue_id': 3, 'weight': 30, 'line_rate': 40, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 36}]}
...
def test_dwrr_wred_with_extreme_weight_ratio_7_phase_2(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.7 phase 2',
                    'total_tx_ports': 3,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 20, 'line_rate': 40, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 20},
                                      {'queue_id': 2, 'weight': 50, 'line_rate': 60, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 50},
                                      {'queue_id': 3, 'weight': 30, 'line_rate': 40, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 30}]} 
...
def test_dwrr_wred_with_extreme_weight_ratio_7_phase_3(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.7 phase 3',
                    'total_tx_ports': 3,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 20, 'line_rate': 50, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 20},
                                      {'queue_id': 2, 'weight': 50, 'line_rate': 50, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 50},
                                      {'queue_id': 3, 'weight': 30, 'line_rate': 50, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 30}]} 
...
def test_dwrr_wred_with_extreme_weight_ratio_7_phase_4(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.7 phase 4',
                    'total_tx_ports': 3,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 20, 'line_rate': 70, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 20},
                                      {'queue_id': 2, 'weight': 50, 'line_rate': 70, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 50},
                                      {'queue_id': 3, 'weight': 30, 'line_rate': 60, 'frame_size': Common_vars.frame_size, 'expected_line_rate': 30}]} 
...
def test_dwrr_wred_with_extreme_weight_ratio_8(snappi_api,                 # noqa F811
    ...
    flow_configs = {'test_case': '2.8',
                    'total_tx_ports': 4,
                    'weight_distribution_type': 'sequential',
                    'tx_port_flows': [{'queue_id': 1, 'weight': 25, 'line_rate': 24, 'frame_size': [64, 1, 512, 1, 1024, 1, 1500, 1], 'expected_line_rate': 24}, 
                                      {'queue_id': 2, 'weight': 25, 'line_rate': 24, 'frame_size': [64, 1, 512, 1, 1024, 1, 1500, 1], 'expected_line_rate': 24}, 
                                      {'queue_id': 3, 'weight': 25, 'line_rate': 24, 'frame_size': [64, 1, 512, 1, 1024, 1, 1500, 1], 'expected_line_rate': 24}, 
                                      {'queue_id': 4, 'weight': 25, 'line_rate': 25, 'frame_size': [64, 1, 512, 1, 1024, 1, 1500, 1], 'expected_line_rate': 25}]}

    Common_vars.subnet_type = subnet_type
    control_state_obj = execute_common_configs(duthosts, create_snappi_config, snappi_api, get_snappi_ports, subnet_type,
                                               flow_configs, config_egress_tracking=True, enable_pkt_sequence_checking=True)  
    verify_line_rate(Common_vars, duthosts, snappi_api, control_state_obj)
    stop_traffic(snappi_api, control_state_obj)
    verify_frames(Common_vars, duthosts, snappi_api, control_state_obj, pkt_sequence_checking_enabled=True)
    delete_flows(Common_vars, snappi_api, remove_egress_stat_view=True)
