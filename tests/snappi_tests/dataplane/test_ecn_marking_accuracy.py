...
@pytest.mark.parametrize("iteration", ["10"])
def test_ecn_marking_accuracy(duthosts,
    ...
    """
    Test to check if packets are getting ECN marked on congestion and see its accuracy
    """
    ...
    for i in range(1, int(iteration)+1):
        ...
        marking_percentage = (count / len(ip_pkts) * 100)
        logger.info("Percentage of packets marked: {}".format(marking_percentage))
        result_list.append(marking_percentage)
        ...
        api.set_config(config)  # resetting the config to clear packet capture
    iterations = list(range(1, int(iteration) + 1))
    logger.info(tabulate(
                list(zip(iterations, result_list)),
                headers=['Iteration', 'Packets Marked (%)'],
                tablefmt='psql'
                ))
