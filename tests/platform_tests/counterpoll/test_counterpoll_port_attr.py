...
def verify_port_attr_in_cli(duthost, expected_status):
    """
    Verify counterpoll show output for port-attr

    Args:
        duthost: DUT host object
        expected_status: Expected status ('enable' or 'disable')
    """
    with allure.step("Verifying 'counterpoll show' output for port-attr"):
        ...
        for entry in counterpoll_output:
            if PORT_ATTR_TYPE == entry.get(CounterpollConstants.TYPE, ''):
                ...
                pytest_assert(expected_status == actual_status,
                             "port-attr status is '{}', expected '{}'".format(actual_status, expected_status))
                logging.info("port-attr status verified: {}".format(actual_status))
                return

        pytest.fail("port-attr not found in counterpoll show output")


def verify_port_attr_in_config_db(duthost, expected_status, expected_interval=None):
    """
    Verify CONFIG_DB FLEX_COUNTER_TABLE for port-attr
    ...
    """
    with allure.step("Verifying CONFIG_DB FLEX_COUNTER_TABLE for port-attr"):
        for asic in duthost.asics:
            try:
...
def verify_port_attr_in_flex_counter_db(duthost, expected_interval=None):
    """
    Verify FLEX_COUNTER_DB group table and OID tables for port-attr
    ...
    with allure.step("Verifying FLEX_COUNTER_DB for port-attr"):
        for asic in duthost.asics:
            # Verify group table
            try:
...
def verify_counters_db_data(duthost, sample_ports):
    ...
    Args:
        ...
        sample_ports: Dictionary of sample ports with their metadata
    """
    with allure.step("Verifying COUNTERS_DB PORT_ATTR table data for port-attr"):
        for port_oid, port_info in sample_ports.items():
...
def test_port_attr_enable_and_validate(duthosts, enum_rand_one_per_hwsku_frontend_hostname, localhost):
    """
    Test 1: Configure port-attr and validate show output + all DBs
    ...
    with allure.step("Enabling port-attr"):
        for asic in duthost.asics:
            ConterpollHelper.enable_counterpoll(asic, [PORT_ATTR])

    # Validate CLI
    verify_port_attr_in_cli(duthost, ENABLE)

    # Validate CONFIG_DB
    verify_port_attr_in_config_db(duthost, ENABLE)

    # Validate FLEX_COUNTER_DB
    verify_port_attr_in_flex_counter_db(duthost)
    ...
    logging.info("Test 1 completed: port-attr enable and validate - PASSED")


def test_port_attr_interval_change(duthosts, enum_rand_one_per_hwsku_frontend_hostname):
    """
    ...
    verify_port_attr_in_cli(duthost, ENABLE)
    verify_port_attr_in_config_db(duthost, ENABLE, expected_interval=5000)
    verify_port_attr_in_flex_counter_db(duthost, expected_interval=5000)
    ...
    verify_port_attr_in_cli(duthost, ENABLE)
    verify_port_attr_in_config_db(duthost, ENABLE, expected_interval=10000)
    verify_port_attr_in_flex_counter_db(duthost, expected_interval=10000)

    logging.info("Test 2 completed: interval change validation - PASSED")


def test_port_attr_config_reload_persistence(duthosts, enum_rand_one_per_hwsku_frontend_hostname):
    """
    Test 3: Verify port-attr persists after config save + disable + reload

    Steps:
    1. Save config
    2. Disable port-attr
    3. Config reload
    4. Verify port-attr restored to enabled
    ...
    with allure.step("Disabling port-attr"):
        for asic in duthost.asics:
            ConterpollHelper.disable_counterpoll(asic, [PORT_ATTR])

    # Verify disabled
    verify_port_attr_in_cli(duthost, DISABLE)
...
def test_port_attr_reboot_persistence(duthosts, enum_rand_one_per_hwsku_frontend_hostname, localhost):
    """
    Test 4: Verify port-attr persists after disable + reboot

    Steps:
    1. Disable port-attr
    2. Reboot
    3. Verify port-attr restored to enabled
