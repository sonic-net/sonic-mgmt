...
def test_express_reboot(request, testing_config, get_advanced_reboot, verify_dut_health,        # noqa F811
    ...
    '''
    Express reboot test case is run using advacned reboot test fixture

    @param request: pytest commandline argument
    ...
    advancedReboot = get_advanced_reboot(rebootType='express-reboot',
                                         advanceboot_loganalyzer=advanceboot_loganalyzer)
    advancedReboot.runRebootTestcase()


def test_express_reboot_mac_jump(request, get_advanced_reboot, verify_dut_health,          # noqa F811
                                 advanceboot_loganalyzer, capture_interface_counters):
    '''
    Express reboot testcase with one MAC address (00-06-07-08-09-0A) jumping from
    all VLAN ports.
    Part of the express reboot handling is to ensure there are no MAC events reported
    while reboot is in progress. So at the beginning of reboot SONIC
    instructs SAI to disable MAC learning on all the ports.
    When the reboot completes, SAI is communicated again for each port to enable
    MAC learning. To ensure that this is properly handled by SAI, this test case
    purposely generates new MAC learn events or MAC move events during reboot
    and the expected results is to only see those MAC move events after reboot competed.
    If for some reason SAI is not adhering to this requirement, any MAC learn events
    generated during reboot will cause META checker failure resulting to Orchagent crash.
    '''
    advancedReboot = get_advanced_reboot(rebootType='express-reboot', allow_mac_jumping=True,
                                         advanceboot_loganalyzer=advanceboot_loganalyzer)
    advancedReboot.runRebootTestcase()
...
def test_express_reboot_sad(duthosts, rand_one_dut_hostname, nbrhosts, fanouthosts, vmhost, tbinfo,
...
def test_cancelled_express_reboot(request, add_fail_step_to_reboot,            # noqa F811
                                  verify_dut_health, get_advanced_reboot):     # noqa F811
    '''
    Negative express reboot test case to verify DUT is left in stable state
    when expressm reboot procedure abruptly ends.

    @param request: Pytest request instance
    @param get_advanced_reboot: advanced reboot test fixture
    '''
    add_fail_step_to_reboot('express-reboot')
    advancedReboot = get_advanced_reboot(
        rebootType='express-reboot', allow_fail=True)
    advancedReboot.runRebootTestcase()
