...
class SerTest(object):
    ...
    def test_memory(self, completeness='basic'):
        ...
        print(("SER Test failed for memories (%s): %s %s" % (len(self.mem_failed), self.mem_failed, list(self.mem_failed.keys()))))
        ...
        slow_injection = { k : v for k, v in list(self.mem_injection_speed.items()) if v['slow'] > 0 }
        print(("SER Test memory error injection too slow (%s): %s %s" % (len(slow_injection), slow_injection, list(slow_injection.keys()))))
