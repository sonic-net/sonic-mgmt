...
def test_show_platform_ssdhealth(duthosts, enum_supervisor_dut_hostname):
    ...
    for key in expected_fields:
        ...
        if key == "Health":
            ...
            pytest_assert(health_float_value > health_thr,
                          "SSD health is '{}', SSD replacement required".format(line_data))

        if key == "Temperature":
            temp_float_value = float(line_data.strip("C"))
            pytest_assert(temp_float_value < high_temp_thr,
                          "SSD temperature '{}' is too high, expected less than {} C".format(line_data, high_temp_thr))
