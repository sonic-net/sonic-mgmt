...
# Disable loganalyzer to ignore port state change notifications that are
# injected using a fake SAI OID. Disable route_check as the test flaps
# link status on DUT ports which causes BGP sessions to flap.
pytestmark = [
...
FAKE_PORT_OID = "oid:0x1000000000DEAD"
...
def get_consumer_stats(duthost, key=PORTSORCH_STATS_KEY):
...
def stats_available(duthost, key=PORTSORCH_STATS_KEY):
    out = duthost.command(
        "redis-cli -n {} exists '{}:{}'".format(COUNTERS_DB, STATS_TABLE_PREFIX, key),
        module_ignore_errors=True,
    )
    return out.get("rc") == 0 and out.get("stdout", "").strip() == "1"
...
def _pick_two_ports(duthost, tbinfo):
    mg_facts = duthost.get_extended_minigraph_facts(tbinfo)
    ports = sorted(mg_facts["minigraph_ports"].keys())
    pytest_assert(len(ports) >= 2, "Need at least 2 minigraph ports for end-state test")
...
def require_notification_consumer_stats(duthosts, rand_one_dut_hostname):
    """
    Skip the whole module when COUNTERS_DB doesn't expose the
    NOTIFICATION_CONSUMER_STATS:PortsOrch:port_state_change hash.
    ...
    if not wait_until(30, 5, 0, stats_available, duthost):
        pytest.skip(
            "DUT image does not publish {}:{} to COUNTERS_DB - LRU dedup fix not present".format(
                STATS_TABLE_PREFIX, PORTSORCH_STATS_KEY,
            )
        )
...
def test_link_flap_dedup_synthetic(
    ...
    stats_before = get_consumer_stats(duthost)
    ...
    pytest_assert(orchagent_is_running(duthost), "orchagent crashed during burst")

    # One publish cycle so the post-burst counters land.
    time.sleep(STATS_PUBLISH_SEC)
    stats_after = get_consumer_stats(duthost)
    ...
    pytest_assert(
        stats_after["queue_policy"] == "LruDedup",
        "PortsOrch:port_state_change queue_policy is {!r}; expected 'LruDedup'".format(
            stats_after["queue_policy"],
        ),
    )
    pytest_assert(
        received_delta >= burst_msgs * 0.9,
        "consumer received only {} of {} injected messages".format(received_delta, burst_msgs),
    )
...
def test_link_flap_dedup_paused_orchagent(
    ...
    """
    duthost = duthosts[enum_rand_one_per_hwsku_frontend_hostname]
    port_a, port_b = _pick_two_ports(duthost, tbinfo)
    ...
    try:
        ...
        duthost.shell("sudo config interface {ns} startup {p}".format(ns=ns_arg, p=port_a))
