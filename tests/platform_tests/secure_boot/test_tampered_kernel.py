...
def _start_console_recovery(
    ...
    return start_grub_entry_selection(
        vmhost,
        serial_port,
        target_index,
        original_index,
...
def _clear_next_boot(vmhost, duthost):
    command = r"""
set -eu
...
exec 9>/tmp/secure_boot_nbd.lock
flock 9
...
sudo mount "${{nbd}}p3" "$mount_dir"
sudo grub-editenv "$mount_dir/grub/grubenv" unset next_entry
""".format(vm_name=shlex.quote(duthost.hostname))
    result = vmhost.shell(command, module_ignore_errors=True)
    pytest_assert(result["rc"] == 0, "Failed to clear the KVM one-time boot entry: {}".format(
        result.get("stderr", "")
    ))


# Power-cycle the KVM so the one-time failed boot falls back to the unchanged default image.
def _recover_kvm(vmhost, duthost, localhost):
    ...
    _clear_next_boot(vmhost, duthost)
...
def _verify_kernel_is_rejected(
    ...
    recovery_tools = vmhost.shell(
        "command -v qemu-nbd && command -v grub-editenv && command -v flock",
        module_ignore_errors=True,
    )
    pytest_assert(recovery_tools["rc"] == 0, "The KVM host requires qemu-nbd, grub-editenv, and flock")
    ...
    try:
        ...
        duthost.command("sudo sonic-installer set-next-boot {}".format(shlex.quote(target_version)))

        capture_pid, console_log = _start_console_recovery(
            vmhost,
            duthost,
            serial_port,
            target_index,
