...
def skip_unsupported_warm_reboot(duthosts, enum_frontend_dut_hostname, tbinfo):
    ...
    for unsupported in ['dualtor', 'm0', 'm1', 'mx', 'smartswitch', 'isolated']:
        if unsupported in topo_name:
            pytest.skip("Warm reboot is not supported on the {} topology".format(topo_name))
