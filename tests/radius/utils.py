...
def load_radius_creds():
    ...
    """
    creds_file_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), RADIUS_CRED_FILE)
...
def check_group_output(result, creds, user_type):
    ...
    for line in result['stdout_lines']:
        objects = line.split(':')
        # userids are stored in object[3]d
        if "docker" == objects[0]:
...
def ssh_remote_run(localhost, remote_ip, username, password, cmd):
    """
    ssh to a remote host using the username and password passed into the functin
    """
...
def start_tcpdump_and_try_login(duthost, ptfhost_mgmt_ip, localhost, radius_creds):
    """
    The function serves as  worker function to test the source IP feature
    inside the RADIUS feature
    Order of operations:
     - Determine the correct egress interface based on the route to the RADIUS server
     - Start a tcpdump capture on this interface of the DUT
     - Run a test login to the DUT from a different session - forcing RADIUS traffic
     - Analyze packet capture
    """
    ptf_mgmt_prefix = ptfhost_mgmt_ip + "/32"
    route_json = json.loads(duthost.command("show ip route {} json".format(ptf_mgmt_prefix))["stdout"])
    assert ptf_mgmt_prefix in route_json.keys()
    tcpdump_int = route_json[ptf_mgmt_prefix][0]["nexthops"][0]["interfaceName"]

    tcpdump_command = "sudo timeout {tcpdump_time} tcpdump -i {tcpdump_int} port 1812 -w {dut_cap_file}"\
            .format(tcpdump_time=TCPDUMP_CAPTURE_TIME, tcpdump_int=tcpdump_int, dut_cap_file=DUT_CAPTURE_FILE) 

    # remove any lingering tcpdump files
    duthost.command("sudo rm -rf {}".format(DUT_CAPTURE_FILE))
    tcpdump_task, tcpdump_result = duthost.shell(tcpdump_command, module_async=True)
    #wait for tcpdump to fully start
    time.sleep(TCPDUMP_START_TIME)
    logging.debug("Radius Capture file started, begin login test")
    ssh_remote_run(localhost, duthost.mgmt_ip, radius_creds["invalid_user"],
    ...
    return os.path.join(DOCKER_TMP, duthost.hostname, DUT_CAPTURE_FILE.lstrip(os.path.sep))
