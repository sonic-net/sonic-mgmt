import logging
import pytest
from tests.common.fixtures.radius import radius_creds
...
def stop_radius_server(ptfhost):
    """
    stop the FreeRADIUS server running on the ptfhost by killing the process ID.
    """
    # Find the PID of the FreeRADIUS process
    find_pid_command = "ps aux | grep freeradius | grep -v grep | awk '{print $2}'"
    ...
    if pid_result["stdout_lines"]:
        for pid in pid_result["stdout_lines"]:
            ...
            kill_result = ptfhost.shell(kill_command)
...
def pytest_runtest_setup(item):
    ...
    if SKIP_RADIUS_TESTS:
        pytest.skip("Skipping RADIUS tests because RADIUS server not installed on PTF host")

@pytest.fixture(scope="module", autouse=True)
def setup_radius_server(ptfhost, duthosts, enum_rand_one_per_hwsku_hostname, radius_creds):
    ...
    ptfhost.template(src="radius/clients.conf.j2", dest="{}/clients.conf".format(FREE_RADIUS_CONF_DIR))

    ptfhost.template(src="radius/users.j2", dest="{}/mods-config/files/authorize".format(FREE_RADIUS_CONF_DIR))
...
@pytest.fixture(scope="module", autouse=True)
def setup_radius_client(ptfhost, duthosts, radius_creds, enum_rand_one_per_hwsku_hostname):
    duthost = duthosts[enum_rand_one_per_hwsku_hostname]
    ping_result = duthost.shell("ping {} -c 1 -W 3".format(ptfhost.mgmt_ip), module_ignore_errors=True)['stdout']
    ...
    duthost.shell("sudo config radius add {} --key {}".format(ptfhost.mgmt_ip, radius_creds["radius_secret"]))
...
@pytest.fixture(scope="module", autouse=True)
def routed_interfaces(duthosts, enum_rand_one_per_hwsku_hostname, enum_frontend_asic_index):
        """
