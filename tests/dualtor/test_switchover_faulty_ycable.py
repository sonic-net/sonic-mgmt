...
def setup_faulted_y_cable_driver(rand_unselected_dut, simulate_probe_unknown=False, simulate_peer_link_down=False):
    ...
    try:
        ...
        rand_unselected_dut.host.options['variable_manager'].extra_vars.update(extra_vars)
        rand_unselected_dut.template(
            src="dualtor/files/y_cable_simulated.py.j2",
            dest="/tmp/y_cable_simulated.py",
            force=True
        )
        find_path_res = rand_unselected_dut.shell("docker exec pmon find / -name y_cable_simulated.py")["stdout"]
        ...
        rand_unselected_dut.shell("docker exec pmon mv {path}/y_cable_simulated.py {path}/y_cable_simulated.py.orig".format(path=y_cable_simulated_path))
        rand_unselected_dut.shell("docker cp /tmp/y_cable_simulated.py pmon:%s/" % y_cable_simulated_path)
        rand_unselected_dut.shell("docker exec pmon supervisorctl restart ycabled")
        # Sleep 10 seconds for ycabled restart
        time.sleep(10)
        yield
    finally:
        rand_unselected_dut.shell("docker exec pmon mv {path}/y_cable_simulated.py.orig {path}/y_cable_simulated.py".format(path=y_cable_simulated_path))
        rand_unselected_dut.shell("docker exec pmon supervisorctl restart ycabled")
        # Sleep 10 seconds for ycabled restart
        time.sleep(10)
...
def select_mux_port(active_standby_ports):
    """Select an active-standby mux port to test."""
    pytest_require(active_standby_ports, "No active-standby mux ports, skip...")
...
def restore_pmon(rand_unselected_dut, select_mux_port):
    ...
    def _restart_pmon_container(duthost):
        ...
        restarted = wait_until(60, 5, 0, check_container_state, duthost, "pmon", True)
...
def test_switchover_probe_unknown(
    ...
    with setup_faulted_y_cable_driver(rand_unselected_dut, simulate_probe_unknown=True):
        ...
        pytest_assert(get_tor_mux_status(rand_unselected_dut, mux_port) == "active")
        pytest_assert(get_tor_mux_status(rand_selected_dut, mux_port) == "standby")
        ...
        try:
            ...
            verify_tor_states(rand_selected_dut, rand_unselected_dut, intf_names=[mux_port])
            pytest_assert(get_tor_mux_probe_state(rand_unselected_dut, mux_port) == "unknown")
        finally:
...
def test_switchover_peer_link_down(
    ...
    with setup_faulted_y_cable_driver(rand_unselected_dut, simulate_peer_link_down=True):
        ...
        try:
            ...
            verify_tor_states(rand_unselected_dut, rand_selected_dut, intf_names=[mux_port], verify_db_timeout=90)
            pytest_assert(get_tor_mux_cable_info(rand_unselected_dut, mux_port)["link_status_peer"] == "down")
        finally:
            rand_selected_dut.shell("config mux mode auto all")
