...
from tests.common.snappi.common_helpers import disable_packet_aging, stop_pfcwd
...
def __gen_data_flow(testbed_config,
    ...
    flow = testbed_config.flows.flow(name='{}{} {} -> {} Rate:{}'.format(index,
                                     flow_name_prefix, src_port_id, dst_port_id, flow_rate_percent))[-1]
...
def __verify_results(rows,
    ...
    for row in rows:
        ...
        pytest_assert(int(int(rx_frames)/int(tx_frames)) == 18,
                      'Rx did not receive 18 percent of trasnmitted frames in {}'.format(row.name))
