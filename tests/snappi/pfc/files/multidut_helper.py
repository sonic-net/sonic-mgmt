...
from tests.common.snappi.common_helpers import pfc_class_enable_vector,\
 get_egress_lossless_buffer_size, stop_pfcwd, disable_packet_aging                    # noqa: F401
...
def __verify_results(rows,
    ...
    logger.info('Flow Name : {} , Tx Frames : {}, Rx Frames : {}'.format(pause_flow_name, tx_frames, rx_frames))

    pytest_assert(tx_frames > 0 and rx_frames == 0,
                  'All the pause frames should be dropped')

    logger.info('Checking Background Flows')
    ...
    logger.info('Checking Test Flows')
    """ Check test flows """
    for row in rows:
        ...
        tx_frames = row.frames_tx
        rx_frames = row.frames_rx
        logger.info('Test Flow Name : {} ,  TX Frames : {} , RX Frames : {}'.format(row.name, tx_frames, rx_frames))
        if test_flow_pause:
            pytest_assert(tx_frames > 0 and rx_frames == 0,
