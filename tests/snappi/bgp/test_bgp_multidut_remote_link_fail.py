...
def test_bgp_multidut_remote_link_fail(cvg_api, duthosts, get_multidut_snappi_ports):
    ...
    bgp_config.rx_rate_threshold = 90/(len(tg_ports)-int(NO_OF_TX_PORTS)-1)
    ...
    table = []
    logger.info("Verify convergence with remote link failures")
    """ Iterating route withdrawal on all BGP peers """
    route_names = ['Network_Group%s'%i for i in range(2, len(tg_ports)+1)]
    for route in route_names:
        table.append(get_avg_cpdp_convergence_time(cvg_api, route, route_type=ROUTE_TYPE))
    ...
    pytest_assert(result == True, 'Test case test_bgp_multidut_remote_link_fail failed')
