...
def test_bgp_multidut_local_link_fail(cvg_api, duthosts, get_multidut_snappi_ports):
    ...
    # Step 8: Verify convergence with local link failures
    logger.info("Verify convergence with local link failures")

    table = []

    """ Iterating link flap test on all the rx ports """
    for i, port_name in enumerate(rx_port_names):
        table.append(get_avg_dpdp_convergence_time(cvg_api, port_name, route_type=ROUTE_TYPE))
    ...
    pytest_assert(result == True, 'Test case test_bgp_multidut_local_link_fail failed')
