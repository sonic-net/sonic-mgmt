...
def _resolve_test_params(request):
    ...
    Returns:
        tuple: (base_image, target_image, metadata_process, skip_postupgrade_actions)
    ...
    return base_image, target_image, metadata_process, skip_postupgrade_actions


def test_warmboot_data_consistency(localhost, duthosts, rand_one_dut_hostname, tbinfo, request,
                                    verify_dut_health, upgrade_strategy_fixture):  # noqa F811
    """
    ...
    from_image, to_image, metadata_process, _ = _resolve_test_params(request)
    ...
    if metadata_process:
        logger.info(f"Installing {to_image} on {duthost.hostname} using metadata scripts")
        sonic_update_firmware(duthost, localhost, to_image, REBOOT_TYPE_WARM, upgrade_strategy_fixture)
    else:
    ...
    # Postupgrade Actions
    # NOTE: That we explicitly don't run postupgrade actions here as that has a config save -y which
    #       would be reloaded on the subsequent coldboot and obstruct the comparison we're after.
