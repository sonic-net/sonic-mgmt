"""
This test is for verifying bgp session establishing, route announcing and traffic flow
from a 4 byte ASN DUT to 4 byte ASN bgp speakers.
The script is similar to test_bgp_speaker.py, except that bgp speaker ASN modification.
"""
...
configfilename = "/etc/sonic/config_db.json"
flag="false"
...
def dut_bgp_asn_update_status(duthost, bgp_speaker_asn, dut_asn):
...
#    duthost.command("sudo cp /tmp/bgp_config.json /tmp/bgp_config.json")


    # Load updated bgp_config config with sonic-cfggen
#    cmds = [
#        "sonic-cfggen -j {} -w".format(TMP_FILE),
#        "config save -y"
#    ]
    #duthost.command(cmds=cmds)
...
#    bgp_dut_asn="65100"
#    bgp_dut_asn_finder=duthost.shell("show ip bgp sum | awk 'NR==3{print $8}'")
#    for key, val in bgp_dut_asn_finder.items():
#        if "stdout" in key:
 #           bgp_dut_asn=val
#            logger.info("bgp_dut_asn=%s"%val)
#            break
#    if testtype=="4b4b":
    bgp_dut_asn = "65538"
    ...
    logger.info("inside common_teardown method")
    logger.info("lo_addr=%s"%lo_addr)
    ...
    bgpvacstatus = dut_bgp_asn_update_status(duthost, str(bgp_speaker_4byteasn), str(bgp_dut_asn))
    
    logger.info("bgpvacstatus=%s"%bgpvacstatus)
    
    
    if bgpvacstatus == "fourbyte":
        ...
        for i in range(0, 3):
            ...
            ptfhost.exabgp(name="bgps%d" % i,
                           ...
                           router_id=local_ip,
                           peer_ip=lo_addr,
                           local_asn=bgp_speaker_4byteasn,
    ...
    load_default_configuration(duthost, bgp_dut_asn, bgp_speaker_asn_default)
    dut_config_reset(duthost, bgp_dut_asn_default)
    logger.info("########### Done teardown for bgp speaker testing ###########")

def test_bgp_speaker_bgp_sessions(common_setup_teardown, duthosts, rand_one_dut_hostname, testtype="4b4b"):
    ...
    logger.info("end of test_bgp_speaker_bgp_Session test")
...
def load_default_configuration(duthost, dut_asn, default_bgp_asn):
    logger.info("Reverting the configuration to default")
    duthost.command("vtysh -c \"conf t\" \
            -c \"router bgp %s\" \
            -c \"neighbor BGPVac remote-as %s\" \
            -c \"neighbor BGPVac activate\" \
...
def bgp_speaker_announce_routes_common(common_setup_teardown,
    ...
    logger.info('enabled_ptf_ports={}, vlan_ptf_ports={}, vlan_mac={}'\
    ...
    logger.info(duthost.facts['router_mac'])
    logger.info("________")
    logger.info(PTF_TEST_PORT_MAP)
    ...
    time.sleep(20)
    logger.info("family={}".format(family))
    if family == "v4":
        ...
        logger.info(bgp_table_output["stdout_lines"])
        for values in bgp_table_output["stdout_lines"]:
            if prefix in values:
...
def test_bgp_speaker_announce_routes(common_setup_teardown, tbinfo, duthosts, rand_one_dut_hostname, ptfhost, ipv4, ipv6, mtu, vlan_mac, ptfadapter):
    """Setup bgp speaker on T0 topology and verify routes advertised by bgp speaker is received by T0 TOR

    """
    duthost = duthosts[rand_one_dut_hostname]
    nexthops = common_setup_teardown[3]
    bgp_speaker_announce_routes_common(common_setup_teardown, tbinfo, duthost, ptfhost, ipv4, ipv6, mtu, "v4", "10.10.10.0/26", nexthops, vlan_mac, ptfadapter)


@pytest.mark.parametrize("ipv4, ipv6, mtu", [pytest.param(False, True, 1514)])
def test_bgp_speaker_announce_routes_v6(common_setup_teardown, tbinfo, duthosts, rand_one_dut_hostname, ptfhost, ipv4, ipv6, mtu, vlan_mac, ptfadapter):
    """Setup bgp speaker on T0 topology and verify routes advertised by bgp speaker is received by T0 TOR

    """
    duthost = duthosts[rand_one_dut_hostname]
    logger.info(common_setup_teardown[4])
    nexthops = common_setup_teardown[4]
    bgp_speaker_announce_routes_common(common_setup_teardown, tbinfo, duthost, ptfhost, ipv4, ipv6, mtu, "v6", "fc00:10::/64", nexthops, vlan_mac, ptfadapter)
