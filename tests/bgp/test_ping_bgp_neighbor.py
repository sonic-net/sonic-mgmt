...
def test_ping_bgp_neighbor(duthosts, enum_frontend_dut_hostname,enum_asic_index):
    ...
    for neighbor_ip, neighbor_data in bgp_facts['bgp_neighbors'].items():
        ...
        assert neighbor_data['state'] == 'established', f"BGP session with {neighbor_ip} is not 'established'"
        ...
        if result['rc'] != 0:
            ...
            raise AssertionError(f"Ping failed to BGP neighbor {neighbor_ip} from {duthost.hostname} (ASIC {enum_asic_index})")
