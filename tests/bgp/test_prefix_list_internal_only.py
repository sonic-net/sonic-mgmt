...
def construct_neighbor_routes(ah_nbrs, rh_nbrs, community, duthost, anchor_prefixes, anchor_contributing_routes):
    ...
    #     ]
    # }
    config_facts = duthost.config_facts(host=duthost.hostname, source="running", namespace="asic{}"
                                        .format(duthost.get_frontend_asic_ids()[0]))["ansible_facts"]
    device_metadata = config_facts["DEVICE_METADATA"]
    is_azng = ("slice_type" in device_metadata["localhost"] and
               device_metadata["localhost"]["slice_type"] == "AZNG_Production")
    announced_result = {}
    not_announced_result = {}
    # In all scenarios, Anchor prefixes should be announced to RHs but not AHs
    for prefix in anchor_prefixes:
        ...
        not_announced_result.setdefault(prefix, [])
        not_announced_result[prefix].append({
            "neighbors": ah_nbrs
        })
    ...
    if not is_azng:
        # For non-azng, contributing prefixes shouldn't be announced to AHs
        for prefix in anchor_contributing_routes:
            not_announced_result.setdefault(prefix, [])
            not_announced_result[prefix].append({
                "neighbors": ah_nbrs
        ...
        # For azng, contributing prefixes should be announced to AHs without any Anchor communities
        for prefix in anchor_contributing_routes:
            announced_result.setdefault(prefix, [])
            announced_result[prefix].append({
                "expected_community": [],
                "unexpected_community": [community["anchor_contributing_community"],
                                         community["local_anchor_community"], community["anchor_community"]],
                "neighbors": ah_nbrs,
                "type": "contributing"
            })
...
def test_prefix_list_tsa_internal_only(common_setup_and_teardown, localhost, tbinfo, ptfhost, request):
...
def test_prefix_list_specific_routes_internal_only(common_setup_and_teardown, localhost, tbinfo, ptfhost, request):
    (anchor_prefixes, anchor_contributing_routes, announced_neighbors, not_announced_neighbors,
     downstream_nbr_names, ip_version, duthost) = common_setup_and_teardown
    neighbor_type = request.config.getoption("neighbor_type")
    for prefix in anchor_prefixes:
