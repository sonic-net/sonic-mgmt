...
This is distinct from:
  - tests/bgp/test_bgp_table_map.py + test_bgp_table_map_device_type.py:
    generic table-map / device-type-gating mechanism tests (community
    injected directly via ExaBGP, no CONFIG_DB PREFIX_LIST involvement).
...
exercised end-to-end. This mirrors the same fallback pattern already used
in test_bgp_table_map.py's setup_table_map_device_type fixture.

Assumes a single-ASIC DUT for the FRR-level checks, matching the existing
helpers in test_bgp_table_map.py.
"""
...
from tests.bgp.test_bgp_table_map import (  # noqa: F401
    set_device_type,
...
def skip_multi_asic(duthosts, enum_dut_hostname):
    ...
    This module assumes a single-ASIC DUT (matching test_bgp_table_map.py's
