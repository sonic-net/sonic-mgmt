...
The main pipeline tests in this module run only against the native URH KVM
topology ('urh_min', pytest topology 'urh'), so the DUT is a genuine
UpperRegionalHub.

The device-type gate negative test (test_urh_anchor_device_gate_negative) is
independent of the rest of the module: it runs on whatever topology it is
scheduled on (t0/t1/t2/lrh) and relies on that topology's own native device
type already being outside PrefixListMgr's ANCHOR_PREFIX allow list.

Assumes a single-ASIC DUT for the FRR-level checks, matching the shared
helpers in table_map_helpers.py.

Test cases:
  - urh-tc-single-anchor (test_urh_anchor_single_prefix): a single
    ANCHOR_PREFIX is rendered into prefix-list/aggregate-address config;
    once a contributing route is announced, the aggregate is community-
    tagged and suppressed from FIB while the contributing route installs
    normally.
  - urh-tc-multi-anchor (test_urh_anchor_multiple_prefixes): two
    ANCHOR_PREFIX entries configured simultaneously are each independently
    rendered and independently suppressed from FIB.
  - urh-tc-partial-delete-overlap (test_urh_anchor_partial_delete_overlap,
    xfail): deleting a child anchor nested inside a parent anchor removes
    only the child's own config/aggregate route, leaving the parent intact.
  - urh-tc-contributing-route-withdraw (test_urh_anchor_contributing_route_withdraw,
    xfail): withdrawing the last contributing route under an anchor causes
    the aggregate to disappear (standard aggregate-address behavior) while
    the CONFIG_DB/rendered config remain untouched; re-announcing brings it
    back correctly tagged and suppressed.
  - urh-tc-churn-idempotency (test_urh_anchor_churn_idempotency): rapid
    add/remove churn on the same ANCHOR_PREFIX converges idempotently, with
    no duplicate prefix-list/aggregate-address entries and no bgpcfgd crash.
  - urh-tc-reboot-persistence (test_urh_anchor_config_reload_persistence):
    ANCHOR_PREFIX config and its derived FRR config survive a
    'config save' + 'config reload' cycle, regenerated purely from
    CONFIG_DB with no CLI re-invocation.
  - urh-tc-device-gate-negative (test_urh_anchor_device_gate_negative): on a
    device type outside PrefixListMgr's ANCHOR_PREFIX allow list, both the
    CLI and a direct CONFIG_DB write are rejected/ignored - no rendering
    occurs.
"""
...
# Mirrors bgpcfgd's managers_prefix_list.py PREFIX_TYPE_CONFIG["ANCHOR_PREFIX"]
# ["allowed_devices"] - used only as a sanity guard in the negative test, to
# confirm the scheduled topology's native device type is actually disallowed
# (not to drive any device-type override).
ANCHOR_PREFIX_ALLOWED_DEVICES = [
...
def _is_anchor_prefix_device_allowed(device_type, device_subtype):
    for allowed_type, allowed_subtype in ANCHOR_PREFIX_ALLOWED_DEVICES:
        if device_type == allowed_type and (allowed_subtype is None or device_subtype == allowed_subtype):
            return True
    return False
...
def setup_urh_anchor(duthosts, enum_dut_hostname):
    ...
    pytest_assert(
        ...
        "test_urh_anchor_prefix requires a native UpperRegionalHub DUT, got {!r}".format(device_type)
    )
...
@pytest.mark.topology('t0', 't1', 't2', 'lrh')
def test_urh_anchor_device_gate_negative(duthosts, enum_dut_hostname):
    ...
    ANCHOR_PREFIX configuration must not produce any prefix-list/aggregate-address
    rendering:
    ...
    Runs against whichever topology it is scheduled on (t0/t1/t2/lrh) using
    that topology's own native DEVICE_METADATA type - no device-type override
    needed, since none of t0 (ToRRouter), t1 (LeafRouter), t2 (SpineRouter,
    no subtype), or lrh (LowerRegionalHub) are in the ANCHOR_PREFIX allow
    list to begin with.
    """
    duthost = duthosts[enum_dut_hostname]
    prefix = ANCHOR_A
    ...
    device_subtype = duthost.shell(
        "redis-cli -n 4 HGET 'DEVICE_METADATA|localhost' subtype",
        module_ignore_errors=True
    )["stdout"].strip() or None
    pytest_assert(
        not _is_anchor_prefix_device_allowed(device_type, device_subtype),
        "test_urh_anchor_device_gate_negative requires a DUT whose native device type "
        "({}/{}) is NOT in the ANCHOR_PREFIX allow list".format(device_type, device_subtype)
    )
