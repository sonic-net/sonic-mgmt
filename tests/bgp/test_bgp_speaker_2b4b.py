...
from a 2 byte ASN DUT to 4 byte ASN bgp speakers.
...
def common_setup_teardown(duthosts, rand_one_dut_hostname, ptfhost, localhost, tbinfo, toggle_all_simulator_ports):
    ...
    bgpvacstatus = "not4byte"
    bgpvacstatus = dut_bgp_asn_update_status(duthost, bgp_speaker_4byteasn, bgp_dut_asn)
    ...
    load_default_bgpvac(duthost, bgp_dut_asn, bgp_speaker_asn_default)
...
def load_default_bgpvac(duthost, dut_asn, default_bgp_asn):
...
#                    assert "true"
    ...
    for ip in nexthop_ips:
        checkneighbor = duthost.shell("ping {} -c 3".format(ip.ip))
        logger.info(checkneighbor)
