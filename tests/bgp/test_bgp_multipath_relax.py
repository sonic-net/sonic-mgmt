...
def get_downstream_neigh(tbinfo, topo_config):
    ...
    for vm in tbinfo['topo']['properties']['topology']['VMs'].keys():
        if NEIGHBOR_MAPPING[topo_type]["downstream"] in vm:
...
def get_vips_prefix_paths(downstream_duts_neighbor, vips_prefix, topo_config):
    vips_downstream = []
    vips_asn = []
...
def test_bgp_multipath_relax(tbinfo, duthosts, rand_one_dut_hostname):
    ...
    downstream_dut_neighbors = get_downstream_neigh(tbinfo, topo_config)

    # get t2 neighbors
    upstream_dut_neighbors = get_upstream_neigh(tbinfo)

    if not downstream_dut_neighbors:
        pytest.fail("Didn't find multipath downstream neighbor")
    ...
    vips_downstream, vips_asn = get_vips_prefix_paths(downstream_dut_neighbors, vips_prefix, topo_config)
