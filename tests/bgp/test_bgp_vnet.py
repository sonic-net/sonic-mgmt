...
def modify_dynamic_peer_cfg(duthost, cfg_facts, template):
    ...
    duthost.shell("cp {} /etc/sonic/{}.json".format(temp_location, template))
    duthost.shell("sonic-cfggen -j /etc/sonic/{}.json --write-to-db".format(template))
...
def dynamic_range_add_delete(duthost, cfg_facts, template):
    ...
    peer_uptime_before = bgp_summary['ipv4Unicast']['peers']['10.0.0.61']['peerUptimeMsec']
    time.sleep(10)
    ...
    peer_uptime_after = bgp_summary['ipv4Unicast']['peers']['10.0.0.61']['peerUptimeMsec']
    peer_uptime_before = peer_uptime_before + 2*10*1000  # add 20 seconds to the uptime to account for the sleep timer

    assert peer_uptime_after >= peer_uptime_before, \
...
def test_setup_vnet(duthosts, rand_one_dut_hostname, cfg_facts):
    '''
