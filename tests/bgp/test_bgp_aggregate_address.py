...
# Functions
from bgp_bbr_helpers import get_bbr_default_state, is_bbr_enabled, config_bbr_by_gcu
...
def dump_db(duthost, dbname, tablename):
    ...
    res = {}
    for k in keys:
        fields = duthost.shell(f"sonic-db-cli {dbname} hgetall '{k}'",
...
def verify_bgp_aggregate_consistence(duthost, bbr_enabled, cfg: AggregateCfg):
    ...
    pytest_assert(config_db[cfg.prefix].get('bbr-required') == ('true' if cfg.bbr_required else 'false'),
                  "bbr-required flag mismatch")
    pytest_assert(config_db[cfg.prefix].get('summary-only') == ('true' if cfg.summary_only else 'false'),
                  "summary-only flag mismatch")
    pytest_assert(config_db[cfg.prefix].get('as-set') == ('true' if cfg.as_set else 'false'),
    ...
    if cfg.bbr_required and not bbr_enabled:
        ...
        pytest_assert(cfg.prefix not in running_config, f"aggregate-address {cfg.prefix} should not present in FRR running-config when bbr is disabled")
    else:
...
def verify_bgp_aggregate_cleanup(duthost, prefix):
    ...
    pytest_assert(prefix.split('/')[0] not in running_config, f"aggregate-address {prefix} should not present in FRR running-config")
...
def test_bgp_aggregate_address_when_bbr_changed(duthosts, rand_one_dut_hostname,ip_version, bbr_required, summary_only, as_set):
    """
