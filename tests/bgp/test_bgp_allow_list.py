...
def checkout_bgpmon_allow_list_routes(ptfhost, setup_info, timeout=120):
    """Check bgpmon sees only the routes this test announces."""
    ...
    pytest_assert(ptfhost.stat(path=DUMP_FILE).get('stat', {}).get('exists', False),
                  "bgpmon dump file not found on PTF: {}".format(DUMP_FILE))

    time.sleep(20)  # wait for exabgp updates to be dumped
    bgpmon_routes = set([line.strip() for line in ptfhost.shell("cat {}".format(DUMP_FILE), verbose=False)['stdout_lines']
                         if line.strip()])
...
def test_default_allow_list_preconfig(duthosts, rand_one_dut_hostname, bgp_allow_list_setup, nbrhosts,  # noqa:F811
    ...
    checkout_bgpmon_allow_list_routes(ptfhost, bgp_allow_list_setup)
