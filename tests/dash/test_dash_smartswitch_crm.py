...
class TestDashCRM:
    ...
    @pytest.mark.parametrize("resource_name", ["dash_eni", "dash_eni_ether_address_map"])
    def test_dash_crm_eni(self, resource_name):
        ...
        if resource_name == "dash_eni":
            self.do_crm_validation(resource_name, self.eni_num, thr_verify_cmds=THR_VERIFY_CMDS)
        else:
            self.do_crm_validation(resource_name, self.eni_num)
