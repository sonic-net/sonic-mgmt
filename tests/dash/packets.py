...
def inbound_pl_packets(config, no_floatingnic=True, inner_packet_type='udp', vxlan_udp_dport=4789):
    ...
    gre_packet = testutils.simple_gre_packet(
        ...
        gre_key=int(pl.ENCAP_VNI) << 8 if no_floatingnic else int(pl.VNET1_VNI) << 8,
        inner_frame=inner_packet,
    )

    exp_inner_packet = generate_inner_packet(inner_packet_type)(
        eth_src=pl.REMOTE_MAC if no_floatingnic else pl.ENI_MAC,
        eth_dst=pl.ENI_MAC if no_floatingnic else pl.REMOTE_MAC,
        ip_src=pl.PE_CA,
        ip_dst=pl.VM1_CA,
        ip_id=0,
    )
...
def outbound_pl_packets(config, outer_encap, no_floatingnic=True,
                        inner_packet_type='udp', vxlan_udp_dport=4789, vxlan_udp_sport=1234):
    inner_packet = generate_inner_packet(inner_packet_type)(
        eth_src=pl.ENI_MAC if no_floatingnic else pl.REMOTE_MAC,
        eth_dst=pl.REMOTE_MAC if no_floatingnic else pl.ENI_MAC,
        ip_src=pl.VM1_CA,
        ip_dst=pl.PE_CA,
    )
    ...
    if outer_encap == 'vxlan':
        ...
        outer_packet = testutils.simple_gre_packet(
            ...
            ip_dst=pl.APPLIANCE_VIP,
            gre_key_present=True,
            gre_key=int(pl.ENCAP_VNI) << 8 if no_floatingnic else int(pl.VM_VNI) << 8,
            inner_frame=inner_packet
        )
    else:
