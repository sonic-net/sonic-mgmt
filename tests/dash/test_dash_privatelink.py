...
from packets import outbound_pl_packets, inbound_pl_packets, outbound_plnsg_packets, generate_fnic_packets
...
def test_pl_nsg(
    ...
    vm_to_dpu_pkt, exp_dpu_to_pe_pkt = outbound_plnsg_packets(
        dash_pl_config, outer_encap=encap_proto, inner_encap=encap_proto
    )
    pe_to_dpu_pkt, exp_dpu_to_vm_pkt = inbound_pl_packets(dash_pl_config)
    ptfadapter.dataplane.flush()
    testutils.send(ptfadapter, dash_pl_config[LOCAL_PTF_INTF], vm_to_dpu_pkt, 1)
...
def test_floating_nic(
    ptfadapter,
    dash_pl_config,
):
    num_packets = 1000
    packets = generate_fnic_packets(dash_pl_config, num_packets=1000)
    ptfadapter.dataplane.flush()

    tunnel_endpoint_counts = {ip: 0 for ip in pl.TUNNEL2_ENDPOINT_IPS}
    for pkt_set in packets:
        vm_to_fnic_pkt, exp_fnic_to_pe_pkt, pe_to_fnic_pkt, exp_fnic_to_vm_pkt = pkt_set
        testutils.send(ptfadapter, dash_pl_config[LOCAL_PTF_INTF], vm_to_fnic_pkt, 1)
        # testutils.verify_packet_any_port(ptfadapter, exp_fnic_to_pe_pkt, dash_pl_config[REMOTE_PTF_RECV_INTF])
        testutils.send(ptfadapter, dash_pl_config[REMOTE_PTF_SEND_INTF], pe_to_fnic_pkt, 1)

        result = testutils.dp_poll(ptfadapter, device_number=0, timeout=5)
        if isinstance(result, ptfadapter.dataplane.PollSuccess):
            if result.port == dash_pl_config[LOCAL_PTF_INTF] and \
               ptf.dataplane.match_exp_pkt(exp_fnic_to_vm_pkt, result.packet):
