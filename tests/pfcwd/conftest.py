...
def skip_pfcwd_higher_speeds(
    ...
    has_flakyness_pfc_gen_on_above_100gbps = duthost.get_facts().get("asic_type") == "cisco-8000" and \
        duthost.get_facts().get("modular_chassis")

    if has_flakyness_pfc_gen_on_above_100gbps:
        ...
        for test_port in test_ports:
            ...
            if int(speed_int) > 100:
                request.node.add_marker(
                    pytest.mark.xfail(
                        reason="The tests using pfc_gen or pfc_gen_t2 can be run " +
                        f"only for speeds less than 100Gbps(for:{enum_rand_one_per_hwsku_frontend_hostname})"
                    )
                )
