...
from tests.common.helpers.pfcwd_helper import send_background_traffic, manage_lag_config  # noqa: F401
...
class TestPfcwdAllPortStorm(object):
    ...
    def test_all_port_storm_restore(
            ...
            set_pfc_time_cisco_8000, tbinfo, manage_lag_config):  # noqa: F811
        """
        ...
        # manage_lag_config keeps a single active member per PortChannel; the shut-down
        # members carry no traffic and cannot storm, so exclude them from verification.
        shutdown_ports = manage_lag_config['shutdown_ports']
        if shutdown_ports:
            selected_test_ports = [p for p in selected_test_ports if p not in shutdown_ports]
