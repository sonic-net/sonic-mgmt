...
def common_setup_teardown(duthost, ptfhost, fib):
    ...
    try:
        ...
        # TODO: Convert reload_config.yml, use below command to do the recover for now

        if po1 is not None:
            duthost.shell('config portchannel member add {0} {1}'.format(po1, intf1))
            duthost.shell('config interface startup {0}'.format(intf1))
        
        if po2 is not None:
            duthost.shell('config portchannel member add {0} {1}'.format(po2, intf2))
            duthost.shell('config interface startup {0}'.format(intf2))
...
def test_arp_unicast_reply(common_setup_teardown):
    ...
    ptf_runner(ptfhost, '/tmp', "arptest.VerifyUnicastARPReply", '/root/ptftests', params=params, log_file=log_file)
...
def test_arp_expect_reply(common_setup_teardown):
    ...
    ptf_runner(ptfhost, '/tmp', "arptest.ExpectReply", '/root/ptftests', params=params, log_file=log_file)
...
def test_arp_no_reply_other_intf(common_setup_teardown):
    ...
    ptf_runner(ptfhost, '/tmp', "arptest.SrcOutRangeNoReply", '/root/ptftests', params=intf2_params, log_file=log_file)
...
def test_arp_no_reply_src_out_range(common_setup_teardown):
    ...
    clear_dut_arp_cache(duthost)
    log_file = "/tmp/arptest.SrcOutRangeNoReply.{0}.log".format(datetime.now().strftime("%Y-%m-%d-%H:%M:%S"))
    ptf_runner(ptfhost, '/tmp', "arptest.SrcOutRangeNoReply", '/root/ptftests', params=params, log_file=log_file)
...
def test_arp_garp_no_update(common_setup_teardown):
    ...
    ptf_runner(ptfhost, '/tmp', "arptest.GarpNoUpdate", '/root/ptftests', params=params, log_file=log_file)
    ...
    ptf_runner(ptfhost, '/tmp', "arptest.GarpUpdate", '/root/ptftests', params=params, log_file=log_file)
