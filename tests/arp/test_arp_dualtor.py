...
def test_arp_update_for_failed_standby_neighbor(
    ...
    pytest_assert(wait_until(5, 1, 0, lambda: verify_neighbor_status(rand_unselected_dut, neighbor_ip, FAILED)))
    ...
    rand_unselected_dut.shell(arp_update_cmd)
    pytest_assert(wait_until(
        5, 1, 0, lambda: verify_neighbor_status(rand_unselected_dut, neighbor_ip, expected_midpoint_state)))

    # Need to make sure the entry does not auto-transition to FAILED
    time.sleep(10)
    pytest_assert(verify_neighbor_status(rand_unselected_dut, neighbor_ip, expected_midpoint_state))
