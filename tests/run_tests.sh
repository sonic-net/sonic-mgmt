...
function get_dut_from_testbed_file() {
    if [[ -z ${DUT_NAME} ]]; then
        ...
        then
            content=$(python2 -c "from __future__ import print_function; import yaml; print('+'.join(str(tb) for tb in yaml.safe_load(open('$TESTBED_FILE')) if '$TESTBED_NAME'==tb['conf-name']))")
            if [[ -z ${content} ]]; then
                echo "Unable to find testbed '$TESTBED_NAME' in testbed file '$TESTBED_FILE'"
                show_help_and_exit 4
            fi
            ...
            tb_line=${tb_lines[0]}
            DUT_NAME=$(python2 -c "from __future__ import print_function; tb=eval(\"$tb_line\"); print(\",\".join(tb[\"dut\"]))")
        fi
    fi
}
...
function setup_test_options()
{
    ...
    FINAL_CASES=""
    includes=$(python2 -c "print '|'.join('''$INCLUDE_FOLDERS'''.split())")
...
function prepare_dut()
{
    ...
    echo Running: python2 -m pytest ${PYTEST_UTIL_OPTS} ${PRET_LOGGING_OPTIONS} ${UTIL_TOPOLOGY_OPTIONS} ${EXTRA_PARAMETERS} -m pretest
    python2 -m pytest ${PYTEST_UTIL_OPTS} ${PRET_LOGGING_OPTIONS} ${UTIL_TOPOLOGY_OPTIONS} ${EXTRA_PARAMETERS} -m pretest
}

function cleanup_dut()
{
    echo "=== Cleaning up DUT after tests ==="
    echo Running python2 -m pytest ${PYTEST_UTIL_OPTS} ${POST_LOGGING_OPTIONS} ${UTIL_TOPOLOGY_OPTIONS} ${EXTRA_PARAMETERS} -m posttest
    python2 -m pytest ${PYTEST_UTIL_OPTS} ${POST_LOGGING_OPTIONS} ${UTIL_TOPOLOGY_OPTIONS} ${EXTRA_PARAMETERS} -m posttest
}

function run_group_tests()
{
    echo "=== Running tests in groups ==="
    echo Running python2 -m pytest ${TEST_CASES} ${PYTEST_COMMON_OPTS} ${TEST_LOGGING_OPTIONS} ${TEST_TOPOLOGY_OPTIONS} ${EXTRA_PARAMETERS} --cache-clear
    python2 -m pytest ${TEST_CASES} ${PYTEST_COMMON_OPTS} ${TEST_LOGGING_OPTIONS} ${TEST_TOPOLOGY_OPTIONS} ${EXTRA_PARAMETERS} --cache-clear
}
...
function run_individual_tests()
{
    ...
    for test_script in ${TEST_CASES}; do
        ...
        echo Running python2 -m pytest ${test_script} ${PYTEST_COMMON_OPTS} ${TEST_LOGGING_OPTIONS} ${TEST_TOPOLOGY_OPTIONS} ${EXTRA_PARAMETERS} ${CACHE_CLEAR}
