...
def verify_port_speed_in_dbs(duthost, enum_rand_one_frontend_asic_index, cli_namespace_prefix, selected_random_port, verify=True):
    ...
    if verify:
        pytest_assert(port_speed_config_db == port_speed_asic, "Speed in ASIC SAI does not match CONFIG_DB and APPL_DB!")
...
def apply_patch_change_port_cluster(config_facts,
    ...
    for acl_table in ["DATAACL", "EVERFLOW", "EVERFLOWV6"]:
        if operation == "add":
            ...
            json_patch_acl.append({
                "op": "remove",
                "path": "{}/ACL_TABLE/{}/ports/{}".format(json_namespace, acl_table, get_port_index_in_acl_table(duthost, acl_table, selected_random_port))
            })
...
def apply_patch_change_port_speed(config_facts,
    ...
    # combine json patches
    json_patch = json_patch_queues_remove + json_patch_port + json_patch_queues_add
...
def setup_acl_config(duthost, ip_netns_namespace_prefix):
    ...
    #duthost.command("{} config acl add table {} {} -s {}".format(ip_netns_namespace_prefix, ACL_TABLE_NAME, ACL_TABLE_TYPE_L3, ACL_TABLE_STAGE_EGRESS))
    duthost.copy(src=ACL_RULE_FILE_PATH, dest=ACL_RULE_DST_FILE)
    #duthost.shell("{} acl-loader update full --table_name {} {}".format(ip_netns_namespace_prefix, ACL_TABLE_NAME, ACL_RULE_DST_FILE))
    cmds = [
...
def setup_port_speed_change(duthosts,
    ...
    with allure.step("Verify speed B updated in DBs"):
        verify_port_speed_in_dbs(duthost, enum_rand_one_frontend_asic_index, cli_namespace_prefix, selected_random_port, verify=False)
...
def test_port_speed_change(tbinfo,
    ...
    with allure.step("Verify speed A updated in DBs - ports should be up"):
        verify_port_speed_in_dbs(duthost, enum_rand_one_frontend_asic_index, cli_namespace_prefix, selected_random_port, verify=True)
        current_status_speed = get_port_speed(duthost, cli_namespace_prefix, selected_random_port)
    ...
    with allure.step("Verify new pg lossless profile created and assign to port"):
        ...
        # Pdb) current_buffer_pg_info == initial_buffer_pg_info
        # False
        # (Pdb) current_buffer_pg_info
        # {'Ethernet0|0': {'profile': 'ingress_lossy_profile'}, 'Ethernet0|3-4': {'profile': 'pg_lossless_100000_2000m_profile'}, 'Ethernet104|3-4': {'profile': 'pg_lossless_100000_2000m_profile'}, 'Ethernet64|0': {'profile': 'ingress_lossy_profile'}, 'Ethernet64|3-4': {'profile': 'pg_lossless_100000_2000m_profile'}}
        # (Pdb) initial_buffer_pg_info
        # {'Ethernet0': {'0': {'profile': 'ingress_lossy_profile'}, '3-4': {'profile': 'pg_lossless_100000_2000m_profile'}}, 'Ethernet104': {'0': {'profile': 'ingress_lossy_profile'}, '3-4': {'profile': 'pg_lossless_100000_2000m_profile'}}, 'Ethernet64': {'0': {'profile': 'ingress_lossy_profile'}, '3-4': {'profile': 'pg_lossless_100000_2000m_profile'}}}
...
def test_port_speed_change_only_traffic(tbinfo,
                                        duthosts,
                                        initialize_random_variables,
                                        initialize_facts,
                                        ptfadapter):
