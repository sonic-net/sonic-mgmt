...
def _get_profile_list_value(duthost, cli_namespace_prefix, port):
    key = f"BUFFER_PORT_INGRESS_PROFILE_LIST|{port}"
...
def test_gcu_buffer_port_ingress_profile_list_replace_succeeds(ensure_dut_readiness, rand_unselected_dut,
                                                               cli_namespace_prefix, operation):
    """Replace BUFFER_PORT_INGRESS_PROFILE_LIST and expect success."""
    duthost = ensure_dut_readiness[0]
    ports = _get_table_names(duthost, cli_namespace_prefix, "BUFFER_PORT_INGRESS_PROFILE_LIST")
    profiles = _get_table_names(duthost, cli_namespace_prefix, "BUFFER_PROFILE")
    if not ports or not profiles:
        pytest.skip("Insufficient BUFFER_PORT_INGRESS_PROFILE_LIST or BUFFER_PROFILE entries")
    ...
    json_patch = [{
        "op": operation,
        "path": f"/BUFFER_PORT_INGRESS_PROFILE_LIST/{port}/profile_list",
        "value": new_value
    }]
...
def test_gcu_buffer_port_ingress_profile_list_add_succeeds(ensure_dut_readiness, rand_unselected_dut,
                                                           cli_namespace_prefix, operation):
    """Add BUFFER_PORT_INGRESS_PROFILE_LIST profile_list and expect success."""
    duthost = ensure_dut_readiness[0]
    ports = _get_table_names(duthost, cli_namespace_prefix, "BUFFER_PORT_INGRESS_PROFILE_LIST")
    profiles = _get_table_names(duthost, cli_namespace_prefix, "BUFFER_PROFILE")
    if not ports or not profiles:
...
def test_gcu_buffer_port_ingress_profile_list_remove_succeeds(ensure_dut_readiness, rand_unselected_dut,
                                                              cli_namespace_prefix, operation):
    """Remove BUFFER_PORT_INGRESS_PROFILE_LIST profile_list and expect success."""
    duthost = ensure_dut_readiness[0]
    ports = _get_table_names(duthost, cli_namespace_prefix, "BUFFER_PORT_INGRESS_PROFILE_LIST")
    if not ports:
        pytest.skip("No BUFFER_PORT_INGRESS_PROFILE_LIST entries found")
...
@pytest.mark.parametrize("operation", ["replace"])
def test_gcu_buffer_port_ingress_profile_list_invalid_fails(ensure_dut_readiness, rand_unselected_dut,
                                                            cli_namespace_prefix, operation):
    """Replace BUFFER_PORT_INGRESS_PROFILE_LIST with invalid profile and expect failure."""
    duthost = ensure_dut_readiness[0]
    ports = _get_table_names(duthost, cli_namespace_prefix, "BUFFER_PORT_INGRESS_PROFILE_LIST")
    if not ports:
        pytest.skip("No BUFFER_PORT_INGRESS_PROFILE_LIST entries found")
