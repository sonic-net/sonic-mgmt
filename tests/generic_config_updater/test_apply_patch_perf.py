...
def format_json_patch_for_multiasic(duthost, json_data,
    ...
    - Case 1: is_host_specific=True → Apply only to /localhost namespace.
    - Case 2: is_asic_specific=True, no asic_namespaces → Apply to all ASICs.
    - Case 3: is_asic_specific=True, asic_namespaces=['asic0'] → Apply to specific ASICs.
    - Case 4: Neither flag → Apply to both /localhost and all ASICs.
    """
    json_patch = []
