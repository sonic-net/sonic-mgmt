...
def ensure_application_of_updated_config(duthost, value, pg_lossless_profiles, namespace=None):
    ...
    def _confirm_value_in_appl_db_and_asic_db():
        ...
        redis_namespace_prefix = '' if namespace is None else 'sudo ip netns exec {}'.format(namespace)
        for pg_lossless_profile in pg_lossless_profiles:
            # Retrieve dynamic_th from APPL_DB
            dynamic_th_in_appl_db = duthost.shell(
                "sonic-db-cli {} APPL_DB hget BUFFER_PROFILE_TABLE:{} dynamic_th".format(
                    namespace_prefix,
                    pg_lossless_profile
                )
            )["stdout"]
        ...
        buffer_pool_keys = duthost.shell("{} redis-cli -n 1 KEYS ASIC_STATE:SAI_OBJECT_TYPE_BUFFER_PROFILE:"
                                         "oid*".format(redis_namespace_prefix))["stdout_lines"]
    ...
    pytest_assert(
        wait_until(
            READ_APPL_DB_TIMEOUT,
            READ_APPL_DB_INTERVAL,
            0,
            _confirm_value_in_appl_db_and_asic_db
        ),
        "ASIC_DB or APPL_DB for namespace {} does not properly reflect new dynamic threshold expected value: {}".format(
            namespace,
            value
        )
    )
...
def get_pg_lossless_profiles(duthost, namespace=None):
    ...
    pg_lossless_profiles_str = duthost.shell(
        "sonic-db-cli {} APPL_DB KEYS *BUFFER_PROFILE_TABLE:pg_lossless*".format(
            namespace_prefix
        )
    )["stdout_lines"]
