...
def ensure_application_of_updated_config(duthost, configdb_field, values, namespace=None):
    ...
    Args:
        ...
        asic_index: Index id for the asic to be verified. Value is None for single asic platforms.
    """
        ...
        if len(wred_objects) > 1:
            for wred_object in wred_objects:
                wred_data = duthost.shell(
                    'sonic-db-cli {} ASIC_DB hgetall {}'.format(
                        namespace_prefix,
                        wred_object)
                )["stdout"]
            ...
            wred_data = duthost.shell('sonic-db-cli {} ASIC_DB hgetall {}'.format(
                namespace_prefix, wred_objects[0]))["stdout"]
            wred_data = ast.literal_eval(wred_data)
            for field, value in zip(configdb_field.split(','), values.split(',')):
                if value != wred_data[WRED_MAPPING[field]]:
                    return False
...
def test_ecn_config_updates(duthost, ensure_dut_readiness, configdb_field, operation, rand_asic_namespace):
    ...
    ecn_data = duthost.shell('sonic-db-cli {} CONFIG_DB hgetall "WRED_PROFILE|AZURE_LOSSLESS"'.format(
