...
def test_snmp_queue_counters(duthosts, enum_rand_one_per_hwsku_frontend_hostname, localhost, creds_all_duts):
    """
    Test SNMP queue counters
      ...
      - Using snmpwalk compare number of queue counters on Ethernet0, assuming there will be 8 less after
        removing the buffer. (Assuming unicast only, 4 counters for each queue in this case)
    This test covers the issue: 'The feature "polling only configured ports buffer queue" will break SNMP'
    ...
    get_bfr_queue_cntrs_cmd = "docker exec snmp snmpwalk -v2c -c {} {} {}".format(
        creds_all_duts[duthost.hostname]['snmp_rocommunity'], hostip, Ethernet0_queue_cntrs_oid
    )
    ...
    # Add create_only_config_db_buffers entry to device metadata to enable counters optimization
    # and get number of queue counters of Ethernet0 prior to removing buffer queues
    ...
    # Remove buffer queue and reload and get number of queue counters of Ethernet0 after removing
    # two buffer queues
    ...
    pytest_assert((queue_counters_cnt_pre - queue_counters_cnt_post) in [unicast_expected_diff, multicast_expected_diff],
