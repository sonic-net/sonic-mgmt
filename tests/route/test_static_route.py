...
SCENARIOS = [
    ...
    pytest.param(
        dict(name="v6_ecmp_cfg_reload", prefix="2000:2::/64", ipv6=True, count=3,
             save_routes=True, blackhole=False, config_reload_test=True, warm_reboot_test=False,
             summary="Test that an ipv6 static route to multiple nexthops is saved and can be used after config reload"),
...
def generate_and_verify_traffic(duthost, ptfadapter, tbinfo, ip_dst, expected_ports, ipv6=False, should_be_blocked=False):
    if ipv6:
    ...
    if should_be_blocked:
        testutils.verify_no_packet_any(ptfadapter, exp_pkt, ports=expected_ports)
    else:
...
def verify_traffic_over_static_route(duthost, ptfadapter, tbinfo,
    ...
    with allure.step("Verify traffic over static route, should_be_blocked={}".format(should_be_blocked)):
        ...
        with RouteFlowCounterTestContext(is_route_flow_counter_supported, duthost,
            ...
            generate_and_verify_traffic(duthost, ptfadapter, tbinfo, ip_dst, nexthop_devs, ipv6=ipv6, should_be_blocked=should_be_blocked)
...
def test_static_route_scenarios(rand_selected_dut, rand_unselected_dut, ptfadapter, ptfhost, tbinfo,
                                ...
                                is_route_flow_counter_supported, localhost, scenario):
