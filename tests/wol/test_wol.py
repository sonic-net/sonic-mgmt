...
def test_send_to_vlan(
    ...
    logging.info("Test with random vlan %s, members %s and member to remove %s"
                 % (random_vlan, connected_vlan_members, random_member_to_remove))
    ...
    duthost.shell("wol {} {}".format(random_vlan, target_mac))

    testutils.verify_packets(ptfadapter, pkt, list(map(lambda member: dut_ptf_int_map[member], connected_vlan_members)))
    testutils.verify_no_packet_any(ptfadapter, pkt,
                                   list(map(lambda member: dut_ptf_int_map[member], [random_member_to_remove])))
