...
def test_vxlan_scale_mac_vni(vxlan_scale_setup_teardown, ptfhost):
    ...
    # Wait for routes to get programmed programming delay
    ...
    if not ready_state:
        ...
        pytest.fail("ASIC_DB programming incomplete after {} sec".format(hw_delay))
    else:
    ...
    # Wait for routes to get programmed
    total_routes = setup["num_vnets"] * routes_per_vnet
    hw_delay = max(30, min(180, (total_routes // 500) * 10))
    ready_state = wait_until(
        hw_delay, 60, 0,
    ...
    if not ready_state:
        ...
        pytest.fail(f"ASIC_DB rewrite programming incomplete after {hw_delay} sec")
    else:
