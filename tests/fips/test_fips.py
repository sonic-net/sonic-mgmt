...
def test_fips(duthosts, enum_rand_one_per_hwsku_hostname):
    ...
    pytest_assert("sonic_fips=1" in kernel_cmdline, "Not sonic_fips enable parameter in kernel cmdline: {}".format(kernel_cmdline))
    ...
    pytest_assert(re.search("libk5crypto3.*fips", fips_packages), "libk5crypto3 missing: {}".format(fips_packages))
    pytest_assert(re.search("libssl-dev.*fips", fips_packages), "libk5crypto3 missing: {}".format(fips_packages))
    pytest_assert(re.search("libssl3.*fips", fips_packages), "libk5crypto3 missing: {}".format(fips_packages))
    pytest_assert(re.search("openssh-client.*fips", fips_packages), "libk5crypto3 missing: {}".format(fips_packages))
    pytest_assert(re.search("openssh-server.*fips", fips_packages), "libk5crypto3 missing: {}".format(fips_packages))
    pytest_assert(re.search("openssh-sftp-server.*fips", fips_packages), "libk5crypto3 missing: {}".format(fips_packages))
    pytest_assert(re.search("openssl.*fips", fips_packages), "libk5crypto3 missing: {}".format(fips_packages))
    pytest_assert(re.search("ssh.*fips", fips_packages), "libk5crypto3 missing: {}".format(fips_packages))
    ...
    pytest_assert(re.search("symcrypt-openssl.*", symcrypt_packages), "symcrypt-openssl missing: {}".format(symcrypt_packages))
    ...
    pytest_assert("(symcrypt) SCOSSL (SymCrypt engine for OpenSSL)" in openssl_engines, "Symcrypt engine missing: {}".format(openssl_engines))
    ...
    pytest_assert("libsymcrypt.so" in loaded_symcrypt_lib, "Symcrypt lib not load by ssh: {}".format(loaded_symcrypt_lib))
    ...
    pytest_assert("libsymcrypt.so" in loaded_symcrypt_lib, "Symcrypt lib not load by golang: {}".format(loaded_symcrypt_lib))
