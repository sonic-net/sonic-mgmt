...
def cli_sflow_drop_monitor_support(duthost):
    """
    Capability gate for the CLI-based drop-monitor-limit tests.

    Probes the sflow CLI help so the CLI tests self-skip on images where
    'config sflow drop-monitor-limit' is not wired in, instead of being
    unconditionally skipped (dead code) or erroring at setup.

    The command exits non-zero on unsupported images, and Click may emit
    the "no such command" text to stderr, so we check rc AND scan both
    stdout+stderr for the subcommand name to cover version differences.
    """
    res = duthost.shell(
        "config sflow drop-monitor-limit --help",
        module_ignore_errors=True,
    )
    supported = (
        res["rc"] == 0
        and "drop-monitor-limit" in (res["stdout"] + res["stderr"]).lower()
    )
    if not supported:
        pytest.skip(
            "CLI 'config sflow drop-monitor-limit' not implemented on this image"
        )
    return True
...
def preserve_sflow_drop_limit(duthost):
    """
    Snapshot/restore drop_monitor_limit around every test so PATCH/CLI writes
    don't leak into downstream tests on shared testbeds.
