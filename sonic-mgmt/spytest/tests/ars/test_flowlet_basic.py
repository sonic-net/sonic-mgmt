...
def initialize_globals():
    global vars, data_glob, tg1, tg2, tg_handle_1, tg_handle_2
...
def add_ars():
    ...
    st.config(data_glob.dut1, "sudo -s config ars add global --mode flowlet-quality --idle-time 1000")
    res = st.show(data_glob.dut1, "show ars")
    expected_values = {"ars_name": "global", "ars_mode": "flowlet-quality", "ars_idle_time": "1000"}
...
def check_ars(ars, expected_values ={"ars_name": "global", "ars_mode": "flowlet-quality", "ars_idle_time": "1000"}):
    ...
    for key in expected_values:
        if actual_values.get(key) != expected_values[key]:
            st.log(f"Value mismatch for {key}: expected {expected_values[key]}, got {actual_values.get(key)}")
            return False
    st.log("ARS Value Set Appropriately")
    return True

def del_ars():
    st.config(data_glob.dut1, "sudo -s config ars delete global")
    st.config(data_glob.dut1, "sudo -s config ars-profile del global")
    res = st.show(data_glob.dut1, "show ars")
    if not check_ars(res, expected_values = []):
        st.report_fail("test_case_failed_msg", "ARS Value Not SET Appropriately While Deleting")
    return

def reset_tg_interfaces():
...
def check_traffic_single_interface(counters, intfrecord, tolerance = 0.001, packetlosstolerance = 0.001):
    """
...
def check_traffic_balanced(counters, intfrecord, margin = 0.6, packetlosstolerance = 0.001):
    ...
    for pkt_count in interface_counts:
        if not (toleranceB <= pkt_count <= toleranceA):
            st.banner("Traffic is not evenly Distributed Across Interface")
            return False
...
def run_traffic_multiple_stream(stream, check_type='single'):
    """
    Run traffic for a given stream and check traffic based on the specified check type.

    Args:
    - stream: The traffic stream to be run.
    - check_type: The type of check to perform; either 'single' for single interface or 'balance' for balanced traffic.
                  Default is 'single'.
...
def verify_traffic_distribution(counter, intfrecord, tolerance_factor=0.5):
    total_count = 0
    interface_counts = []
