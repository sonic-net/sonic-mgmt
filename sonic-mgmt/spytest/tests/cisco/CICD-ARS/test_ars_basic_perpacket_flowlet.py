...
set_up_config = 'ars_basic_cfg_CICD_setup.yaml'
@pytest.fixture(scope='module', autouse=True, params=[set_up_config])
...
def initialize_globals(config_file):
    ...
    data_glob.dut3 = data_glob.nodes[2]
    data_glob.dut4 = data_glob.nodes[3]
    data_glob.interfaces = [vars.D1D3P1, vars.D1D3P2, vars.D1D4P1, vars.D1D4P2]
    data.dut_asn_list = {data_glob.dut1: "4234521651", data_glob.dut2: "65201", data_glob.dut3: "65203", data_glob.dut4: "65204"}
...
class Test_IPV6_config_ars():
    ...
    def test_continuous_traffic_various_idle_timegap_flowlet_v6(self):
        st.banner("Test Various idle Time GAP Single Continuous Traffic Flow Flowlet: Traffic Expected to Pass through single interface for various time gap")
        ...
        for i in range(0, 2):
            ...
            ars_common_utils.del_ars(data_glob.dut1)
        if test_failcount > 0:
            st.banner("Test Failed: Various idle Time GAP Single Continuous Traffic Flow")
    ...
    def test_bring_down_one_nhop_member_flowlet_v6(self):
        ...
        interface_obj.interface_shutdown(vars.D1, vars.D1D3P1, skip_verify=False)
        st.wait(2)
        ars_common_utils.run_traffic(stream, tg_handle_1, tg1, data_glob)
        ars_common_utils.del_ars(data_glob.dut1)
        counter = st.show(data_glob.dut1, "sudo -s show interface counters ")
        interface_obj.interface_noshutdown(vars.D1, vars.D1D3P1, skip_verify=False)
        st.wait(20)
        intfrecord =[vars.D1D3P2, vars.D1D4P1, vars.D1D4P2]
