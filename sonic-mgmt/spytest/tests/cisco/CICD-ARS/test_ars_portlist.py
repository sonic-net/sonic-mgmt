...
def setup_teardown_basic(request):
    ...
    interface_obj.interface_shutdown(vars.D1, vars.D1D4P1, skip_verify=False)
    ...
    interface_obj.interface_noshutdown(vars.D1, vars.D1D4P1, skip_verify=False)
...
def initialize_globals(config_file):
    ...
    data_glob.interfaces = [vars.D1D3P1, vars.D1D3P2, vars.D1D4P2]
...
class Test_IPV4_config_ars():
    def test_portlist_equal_ecmp_flowlet_withburst(self):
        st.banner("Test portlist member equal to ECMP for Flowlet with Burst: ARS expected to enable and traffic distributed")
    ...
    def test_portlist_less_ecmp_flowlet_withburst(self):
        st.banner("Test portlist member less ECMP for Flowlet with Burst: ARS expected to disable and traffic not distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="flowlet-quality", idle_time="1000")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
        ...
        if not ars_common_utils.check_traffic_single_interface(tg1, tg2, tg_handle_1, tg_handle_2, counter1, data_glob.interfaces):
            ...
            st.report_fail("test_case_failed_msg","Traffic does not flow through single interface")
        st.banner("Passed: Traffic passes through Single Interface")
        st.report_pass('test_case_passed')

    def test_portlist_greater_ecmp_flowlet_withburst(self):
        st.banner("Test portlist member greater than ECMP for Flowlet with Burst: ARS expected to enable and traffic distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="flowlet-quality", idle_time="1000")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
        st.config(data_glob.dut1, "sudo -s config ars-portlist-member add " + vars.D1D4P1 + " --ars-portlist non_global_port_list")
        ars_common_utils.run_traffic(stream,tg_handle_1, tg1, data_glob)
        counter1 = st.show(data_glob.dut1, "sudo -s show interface counters")
        st.config(data_glob.dut1, "sudo -s config ars-portlist-member del "+ vars.D1D4P1)
        ars_common_utils.del_nhg_equal_to_ecmp(data_glob, vars)
        ars_common_utils.del_ars(data_glob.dut1)
        if not ars_common_utils.check_traffic_balanced(tg1, tg2, tg_handle_1, tg_handle_2, counter1, data_glob.interfaces):
            st.banner("Test Failed: traffic load is not Distributed")
    ...
    def test_portlist_equal_ecmp_perpacket_withburst(self):
        st.banner("Test portlist member equal to ECMP for Perpacket with Burst: ARS expected to enable and traffic distributed")
        stream = trB1['stream_id']
    ...
    def test__portlist_less_ecmp_perpacket_withburst(self):
        st.banner("Test portlist member less ECMP for Perpacket With Burst: ARS expected to disable and traffic not distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
        st.config(data_glob.dut1, "sudo -s config ars-portlist-member del " + vars.D1D3P1)
    ...
    def test_portlist_greater_ecmp_perpacket_withburst(self):
        st.banner("Test portlist member greater than ECMP for perpacket With Burst: ARS expected to enable and traffic distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
        st.config(data_glob.dut1, "sudo -s config ars-portlist-member add " + vars.D1D4P1 +" --ars-portlist non_global_port_list")
    ...
    def test_portlist_equal_ecmp_flowlet_multiflow_with_and_without_burst(self):
        st.banner("Test portlist member equal to ECMP for MultiFlow Flowlet With Burst: ARS expected to enable and traffic distributed")
    ...
    def test_portlist_equal_ecmp_perpacket_multiflow_with_and_without_burst(self):
        st.banner("Test portlist member equal to ECMP for MultiFlow perpacket with Burst: ARS expected to enable and traffic distributed")
        stream = trB1M['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
        ars_common_utils.run_traffic(stream,tg_handle_1, tg1, data_glob)
    ...
    def test_portlist_equal_ecmp_flowlet_without_burst(self):
        st.banner("Test portlist member equal to ECMP Flowlet Without Burst: ARS expected to enable and traffic not distributed")
    ...
    def test_portlist_equal_ecmp_perpacket_without_burst(self):
        st.banner("Test portlist member equal to ECMP for Perpacket Without Burst: ARS expected to enable and traffic distributed")
        stream = trC1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
        ars_common_utils.run_traffic(stream,tg_handle_1, tg1, data_glob)
...
class Test_IPV6_config_ars():
    def test_portlist_equal_ecmp_flowlet_withburst_v6(self):
        st.banner("Test portlist member equal to ECMP for Flowlet With Burst: ARS expected to enable and traffic distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="flowlet-quality", idle_time="1000")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
    ...
    def test_portlist_less_ecmp_flowlet_withburst_v6(self):
        st.banner("Test portlist member less ECMP for Flowlet With Burst: ARS expected to disable and traffic not distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="flowlet-quality", idle_time="1000")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
    ...
    def test_portlist_greater_ecmp_flowlet_withburst_v6(self):
        st.banner("Test portlist member greater than ECMP for Flowlet with Burst: ARS expected to enable and traffic distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="flowlet-quality", idle_time="1000")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
    ...
    def test_portlist_equal_ecmp_perpacket_withburst_v6(self):
        st.banner("Test portlist member equal to ECMP for Perpacket with Burst: ARS expected to enable and traffic distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
    ...
    def test_portlist_less_ecmp_perpacket_withburst_v6(self):
        st.banner("Test portlist member less ECMP for Perpacket with Burst: ARS expected to disable and traffic not distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
    ...
    def test_portlist_greater_ecmp_perpacket_withburst_v6(self):
        st.banner("Test portlist member greater than ECMP for perpacket with Burst: ARS expected to enable and traffic distributed")
        stream = trB1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
    ...
    def test_portlist_equal_ecmp_flowlet_multiflow_with_and_without_burst_v6(self):
        st.banner("Test portlist member equal to ECMP for MultiFlow Flowlet with Burst: ARS expected to enable and traffic distributed")
        stream = trB1M['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="flowlet-quality", idle_time="1000")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
        ...
        if not ars_common_utils.check_traffic_balanced(tg1, tg2, tg_handle_1, tg_handle_2, counter1, data_glob.interfaces):
            st.banner("Test Failed: traffic load is not Distributed")
            st.report_fail("test_case_failed_msg", "Traffic does not pass through single interface")
        stream = trC1M['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="flowlet-quality", idle_time="1000")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
        ars_common_utils.run_traffic(stream,tg_handle_1, tg1, data_glob)
    ...
    def test_portlist_equal_ecmp_perpacket_multiflow_with_and_without_burst_v6(self):
        st.banner("Test portlist member equal to ECMP for MultiFlow perpacket with Burst: ARS expected to enable and traffic distributed")
        stream = trB1M['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
    ...
    def test_portlist_equal_ecmp_flowlet_without_burst_v6(self):
        st.banner("Test portlist member equal to ECMP Flowlet Without Burst: ARS expected to enable and traffic not distributed")
        stream = trC1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="flowlet-quality", idle_time="1000")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
    ...
    def test_portlist_equal_ecmp_perpacket_without_burst_v6(self):
        st.banner("Test portlist member equal to ECMP for Perpacket Without Burst: ARS expected to enable and traffic distributed")
        stream = trC1['stream_id']
        ars_common_utils.add_ars(data_glob.dut1, global_mode="false", mode="per-packet-quality")
        ars_common_utils.add_nhg_equal_to_ecmp(data_glob, vars)
