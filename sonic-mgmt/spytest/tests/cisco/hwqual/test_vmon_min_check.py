...
#!/usr/bin/env python3
"""
Enhanced Voltage Margin Percentage Validation Script

This script validates voltage margin percentage readings for both high and low 
margin testing to ensure they are within acceptable tolerance ranges (±10%) 
of their target percentage values.

Usage:
    python voltage_margin_validator_enhanced.py <input_file>
    or
    cat voltage_margin_output.txt | python voltage_margin_validator_enhanced.py
"""
...
def validate_margin_percentage(actual_percent: str, target_percent: str, margin_type: str, tolerance: float = 10.0) -> Tuple[bool, float, str]:
    """
    Validate if actual percentage is within tolerance of target percentage
    Handles both positive (high) and negative (low) margin testing
    
    Args:
        actual_percent (str): Actual margin percentage reading
        target_percent (str): Target margin percentage value
        margin_type (str): Type of margin test ('high', 'low', etc.)
    ...
    # For negative values, we need to handle the calculation properly
    if target != 0:
        deviation_percent = abs((actual - target) / abs(target)) * 100
    else:
        deviation_percent = 100.0  # Max deviation if target is zero
    ...
    if margin_type.lower() == 'high':
        margin_desc = "high margin"
    elif margin_type.lower() == 'low':
        margin_desc = "low margin"
    else:
        margin_desc = f"{margin_type} margin"
    
    if is_within_tolerance:
        status = f"PASS ({margin_desc}: {actual_diff:+.1f}% deviation, {deviation_percent:.1f}% relative)"
    else:
        status = f"FAIL ({margin_desc}: {actual_diff:+.1f}% deviation, {deviation_percent:.1f}% > {tolerance}%)"
...
def validate_all_margin_percentages(voltage_data: List[Dict[str, str]], tolerance: float = 10.0) -> Dict[str, any]:
    ...
    results = {
        ...
        'pass_count': 0,
        'fail_count': 0,
        'high_margin_devices': 0,
        'low_margin_devices': 0,
        'device_results': [],
        'failed_devices': [],
        'skipped_device_list': [],
        'margin_summary': {},
    ...
    margin_stats = {'high': {'total': 0, 'pass': 0, 'fail': 0}, 
                   'low': {'total': 0, 'pass': 0, 'fail': 0},
                   'other': {'total': 0, 'pass': 0, 'fail': 0}}
    ...
    for device in voltage_data:
        ...
        # Count margin types
        margin_type = margin.lower()
        if margin_type == 'high':
            results['high_margin_devices'] += 1
        elif margin_type == 'low':
            results['low_margin_devices'] += 1
        
        # Track margin statistics
        if margin_type in margin_stats:
            margin_stats[margin_type]['total'] += 1
        else:
            margin_stats['other']['total'] += 1
        
        # Validate margin percentage
        is_valid, deviation, status = validate_margin_percentage(
            actual_percent, target_percent, margin, tolerance
        )
        ...
        if is_valid:
            ...
            if margin_type in margin_stats:
                margin_stats[margin_type]['pass'] += 1
            else:
                margin_stats['other']['pass'] += 1
        else:
            results['fail_count'] += 1
            if margin_type in margin_stats:
                margin_stats[margin_type]['fail'] += 1
            else:
                margin_stats['other']['fail'] += 1
    ...
    results['margin_summary'] = margin_stats
    ...
    results['summary'] = {
        ...
        'high_margin_devices': results['high_margin_devices'],
        'low_margin_devices': results['low_margin_devices'],
...
def print_margin_validation_results(results: Dict[str, any], show_all: bool = True, tolerance: float = 10.0):
    ...
    print(f"High Margin Devices: {results['summary']['high_margin_devices']}")
    print(f"Low Margin Devices: {results['summary']['low_margin_devices']}")
    print(f"Pass Rate: {results['summary']['pass_rate']:.1f}%")
    print(f"Overall Status: {results['summary']['overall_status']}")
    print("-" * 100)
    
    # Show margin type breakdown
    print("MARGIN TYPE BREAKDOWN:")
    margin_summary = results['margin_summary']
    for margin_type, stats in margin_summary.items():
        if stats['total'] > 0:
            pass_rate = (stats['pass'] / stats['total'] * 100) if stats['total'] > 0 else 0
            print(f"  {margin_type.upper()} Margin: {stats['pass']}/{stats['total']} passed ({pass_rate:.1f}%)")
    print()
    ...
    if show_all:
        ...
        print(f"{'Device Name':<20} {'Margin':<8} {'Actual %':<9} {'Target %':<9} {'Status':<40}")
        print("-" * 100)
        ...
        for device in results['device_results']:
            ...
            actual_percent = device['actual_percent']
            target_percent = device['target_percent']
            status = device['status'][:39]  # Truncate long status
            
            print(f"{device_name:<20} {margin:<8} {actual_percent:<9} {target_percent:<9} {status:<40}")
    ...
    if results['failed_devices']:
        ...
        for failed in results['failed_devices']:
            actual_diff = failed['actual_percent'] - failed['target_percent']
            ...
            print(f"  Deviation: {actual_diff:+.1f}% from target")
...
def verify_vmon_min_state(output):
    """
    Function to validate reduced voltage condition 
    """
    try:
...
def check_vmon_min_data(CfgDataG, entity):
    '''
    '''
    cmd = "/opt/cisco/bin/voltage_margin.py margin min"
    output = st.config(CfgDataG.dut, cmd)
    st.tg_wait(90)
    cmd = "/opt/cisco/bin/voltage_margin.py ls -m"
    output = st.config(CfgDataG.dut, cmd)
    result = verify_vmon_min_state(output)
    if not result['overall_valid']: 
        report_fail(f"{CfgDataG.logprefix}: Validation of MIN voltaage margining failed")
        return False

    return True

def test_vmon_min_check(CfgDataG, vm_min_check, result):
    st.log(f"{CfgDataG.logprefix}: Executing {vm_min_check} check")

    for check_item in vm_min_check:
        if not check_vmon_min_data(CfgDataG, check_item):
