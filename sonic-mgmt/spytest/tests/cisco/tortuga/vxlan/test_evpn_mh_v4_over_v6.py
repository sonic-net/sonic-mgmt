...
def portchannel_fallback_ping_base_config():
    ...
    st.config(nodes['leaf0'], "sudo config portchannel member add {} {}".format(portchannel_name, vars.D2D6P1))
    ...
    st.config(nodes['leaf1'], "sudo config portchannel member add {} {}".format(portchannel_name, vars.D3D6P1))
    ...
    st.config(nodes['leaf0'], "sudo config portchannel member del {} {}".format(portchannel_name, vars.D2D6P1))
    ...
    st.config(nodes['leaf1'], "sudo config portchannel member del {} {}".format(portchannel_name, vars.D3D6P1))
...
def dhcp_server_base_config():
    ...
    st.config(nodes['leaf1'], "sudo config portchannel member add {} {}".format(portchannel_name, vars.D3D6P1))
    st.config(nodes['leaf1'], "sudo config vlan member add 10 {} -u".format(portchannel_name))
    st.config(nodes['leaf2'], 'sudo config vlan member add 10 {} -u'.format(vars.D4D5P1))
    st.config(nodes['leaf3'], 'sudo config vlan add 10')
    st.config(nodes['leaf3'], 'sudo config vlan member add 10 {} -u'.format(vars.D5D4P1))
    ...
    st.config(nodes['leaf1'], "sudo config portchannel member del {} {}".format(portchannel_name, vars.D3D6P1))
    st.config(nodes['leaf1'], "sudo config portchannel del {}".format(portchannel_name))
    st.config(nodes['leaf2'], 'sudo config vlan member del 10 {}'.format(vars.D4D5P1))
    ...
    st.config(nodes['leaf3'], 'sudo config vlan member del 10 {}'.format(vars.D5D4P1))
...
def test_inter_subnet_ping():
    ...
    try:
        get_cli_out()
        tg = handles[lag_name]["tg_handle"]
        tg.tg_emulation_lag_config(mode="modify", lag_handle=handles[lag_name]["lag_handle"], active= "0", lag_name= """LAG1""", protocol_type="lag_port_staticlag")
...
def test_portchannel_fallback_ping(portchannel_fallback_ping_base_config):
    ...
    try:
        ...
        if not portchannel_obj.verify_portchannel_member_state(nodes['leaf0'], portchannel_name, vars.D2D6P1, "up"):
            st.report_fail("Portchannel state verification failed on nodes['leaf0']")
        if not portchannel_obj.verify_portchannel_member_state(nodes['leaf1'], portchannel_name, vars.D3D6P1, "down"):
...
def test_portchannel_fallback_pxe_client(dhcp_server_base_config):
    """
    DHCP server is created on node SD5 connected to SD4(leaf2)
