...
CONFIGS_FILE = "vxlan_l2vni_ipv6_mac_move.yaml"
...
## Tgen Stream Config
move_host_gw_ip6_addr = "2004:db8:1::1"
move_host_ip6_addr = "2004:db8:1::5"
move_host_diff_ip6_addr = "2004:db8:1::6"
move_host_mac_addr = "00:0a:01:00:11:01"

data.d3tp1_ip6_addr = move_host_gw_ip6_addr
data.tp1d3_ip6_addr = move_host_ip6_addr
data.tp1d3_mac_addr = move_host_mac_addr

data.d4tp2_ip6_addr = move_host_gw_ip6_addr
data.tp2d4_ip6_addr = move_host_ip6_addr
data.tp2d4_mac_addr = move_host_mac_addr

data.d3tp3_ip6_addr = "2004:db8:1::1"
data.tp3d3_ip6_addr = "2004:db8:1::2"
data.tp3d3_mac_addr = "00:0a:01:00:11:02"

data.d4tp4_ip6_addr = "2004:db8:1::1"
data.tp4d4_ip6_addr = "2004:db8:1::3"
...
def test_l2vni_ipv6_basic_mac_move():
    vars = st.get_testbed_vars()
    nodes = {}
    nodes['leaf0'] = vars.D3
    nodes['leaf1'] = vars.D4
    ...
    if not result:
        ...
        st.report_fail("test_case_failed", "test_l2vni_ipv6_basic_mac_move traffic failed")
    ...
    count = 1
    st.show('leaf0', 'sudo ping -c {} {} -q'.format(count, move_host_ip6_addr), skip_tmpl=True, skip_error_check=True)

    result = vxlan_obj.check_traffic(streams_left, timeout=5)
    traffic_cleanup_left(streams_left, handles_left)
    if not result:
        traffic_cleanup_basic(streams_basic, handles_basic)
        st.report_fail("test_case_failed", "test_l2vni_ipv6_basic_mac_move traffic failed after connecting the host to the left leaf")
    ...
    leaf0_output = st.show(nodes['leaf0'], 'sudo ip neighbor show {}'.format(move_host_ip6_addr), skip_tmpl=True, skip_error_check=True)
    leaf1_output = st.show(nodes['leaf1'], 'sudo ip neighbor show {}'.format(move_host_ip6_addr), skip_tmpl=True, skip_error_check=True)
    ...
    count = 1
    st.show('leaf1', 'sudo ping -c {} {} -q'.format(count, move_host_ip6_addr), skip_tmpl=True, skip_error_check=True)
 
    result = vxlan_obj.check_traffic(streams_right, timeout=5)
    traffic_cleanup_right(streams_right, handles_right)
    if not result:
        traffic_cleanup_basic(streams_basic, handles_basic)
        st.report_fail("test_case_failed", "test_l2vni_ipv6_basic_mac_move traffic failed after moving the host to the right leaf")
    ...
    if not result:
        traffic_cleanup_basic(streams_basic, handles_basic)
        st.report_fail("test_case_failed", "test_l2vni_ipv6_basic_mac_move traffic failed after moving the host back to the left leaf")
    ...
    st.report_pass("test_case_passed", "test_l2vni_ipv6_basic_mac_move passed")

## same mac and ip address with shutdown interface
## MIGSOFTWAR-15520
def test_l2vni_ipv6_mac_move_with_intfdown():
    vars = st.get_testbed_vars()
    nodes = {}
    nodes['leaf0'] = vars.D3
    nodes['leaf1'] = vars.D4
    ...
    st.report_pass("test_case_passed", "test_l2vni_ipv6_mac_move_with_intfdown passed")


def traffic_setup_basic():
    ### Config tgen interface and get tg handle, port handle and interface handles ###
    int_dict = {"T1D3P2": {"host_ip": data.tp3d3_ip6_addr, "gateway": data.d3tp3_ip6_addr, "mac" : data.tp3d3_mac_addr},
...
def traffic_setup_right(streams_base, handles_base):
    ### Config tgen interface and get tg handle, port handle and interface handles ###
    int_dict = {"T1D4P1": {"host_ip": data.tp2d4_ip6_addr, "gateway": data.d4tp2_ip6_addr, "mac" : data.tp2d4_mac_addr }}
    handles = vxlan_obj.config_tgen_interface(int_dict, 'ipv6')
    handles.update(handles_base)
    ### Generate Traffic item and Ping test , get tg handle, stream id and port handles ###
    # T1D3P2 --- l2vni --- T1D4P1
