...
def get_xoff_rate(port_speed_gbps):
    ...
    Formula: rate = port_speed_gbps * 25 fps
        100G -> 1000 fps
        200G -> 2000 fps
        400G -> 4000 fps
        800G -> 8000 fps
...
def run_ecn_xoff_test(congestion_point, test_name, ect=ECN_ECT_10):
    ...
    try:
        ...
        try:
            ...
            dump_l2vni_diagnostics(nodes, "VTEP FAILED")
            raise
        ...
        # Pre-traffic diagnostics (from original test)
        dump_l2vni_diagnostics(nodes, "PRE-TRAFFIC")
        ...
        # Clear queue watermarks on all nodes
        ...
        # Step 7: Clear TGEN stats before starting traffic - Not needed per IXIA, done on apply
        #tg.tg_traffic_control(action='clear_stats', port_handle=[src_port_handle, dst_port_handle])
        ...
        # This starts both NGPF data traffic and raw XOFF stream together
        ...
        if capture_results and 'T1D4P1' in capture_results:
            ...
            if ect in (ECN_ECT_01, ECN_ECT_10):
                # ECT(0) or ECT(1): should see non-zero CE packets
                if ce_packets > 0:
                ...
                # Not-ECT: should see 0 CE packets (traffic stays Not-ECT)
                if ce_packets == 0:
                ...
                # CE: packets should remain CE (all CE or mostly CE)
