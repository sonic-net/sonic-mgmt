"""
PFC Watchdog (PFCWD) L2 (Bridged VLAN) Single-Node Test

Validates PFC Watchdog detection and recovery on a single DUT with
3 TGEN ports using IPv6 over an L2 bridged VLAN. All 3 DUT ports are
untagged members of the same VLAN; traffic is switched (bridged), not
routed.

This is the L2 incarnation of test_v6_pfcwd_l3_1node.py. Test bodies
are identical except they're suffixed with `_l2`. The only structural
difference is the module fixture, which configures a VLAN with all
3 ports as untagged members and exposes a single SVI gateway for NDP
resolution.

Topology:
    TGEN P1 (TX data 50%) --[VLAN 100]--> D3 --[VLAN 100]--> TGEN P3
                                                              |
                                                              v
...
Tests in this file (all suffixed _l2):
    ...
    test_pfcwd_forward_action_l2             -- T6: action='forward'; on supported platforms storm detected with NO tx_drop.
    test_pfcwd_burst_storm_l2                -- T7: N back-to-back single_burst storms; each must independently detect AND restore.
    test_pfcwd_many_to_one_trigger_l2        -- T9: P1+P2 incast onto P3 (lossless+lossy mix) with long XOFF; PFCWD MUST detect & restore.
    test_pfcwd_many_to_one_no_trigger_l2     -- T9: same M2O incast with sub-threshold XOFF burst; PFCWD MUST NOT trigger.
...
# IPv6 addressing -- ALL endpoints in same /64 subnet (L2 bridged).
PORT_SUBNETS = {
    1: {'tgen': '2001:db8:1::2'},
    2: {'tgen': '2001:db8:1::3'},
    3: {'tgen': '2001:db8:1::4'},
...
def pfcwd_module_setup():
    """
    Module fixture: create a single VLAN with all 3 DUT ports as
    untagged members (L2 bridged), configure TGEN NGPF IPv6 device
    groups in the same subnet, enable PFCWD, discover lossless
    priority config.
    """
    ...
    for idx, port in dut_ports.items():
        # Remove ALL existing IP addresses (IPv4 and IPv6) so the port is
        # no longer a router interface -- SONiC refuses VLAN membership
        # otherwise. Query CONFIG_DB INTERFACE table for this port.
    ...
    # Add SVI with IPv6 gateway for NDP resolution from each TGEN port.
    st.config(
        dut,
        f"sudo config interface ip add Vlan{VLAN_ID} {VLAN_GATEWAY}/64",
        skip_error_check=True,
    )
    st.wait(3)
    ...
    # Get MAC address of DUT port P3 for XOFF frames. PFC frames are
    # link-layer (dst=01:80:C2:00:00:01) so any valid local MAC works
    # as src; we keep parity with the L3 file by using the DUT P3 MAC.
    ...
    st.config(
        dut,
        f"sudo config interface ip rem Vlan{VLAN_ID} {VLAN_GATEWAY}/64",
        skip_error_check=True,
    )
    for idx, port in dut_ports.items():
        st.config(
            dut,
            f"sudo config vlan member del {VLAN_ID} {port}",
            skip_error_check=True,
        )
    ...
    for port, addrs in data.saved_port_addrs.items():
        for addr in addrs:
            ...
            st.config(
                dut,
                f"sudo config interface ip add {port} {addr}",
                skip_error_check=True,
            )
...
def test_pfcwd_detection_and_recovery_l2():
    """
    ...
    try:
        ...
        if not ok_ing:
            st.banner("SUMMARY: TEST FAILED: test_pfcwd_detection_and_recovery_l2")
        ...
        st.banner("SUMMARY: TEST PASSED: test_pfcwd_detection_and_recovery_l2")
        st.log(f"Summary:")
        ...
        st.report_pass("test_case_passed", "test_pfcwd_detection_and_recovery_l2 passed")
...
def test_pfcwd_xoff_only_no_data_l2():
    ...
    try:
        ...
        if expect_trigger and not triggered:
            st.banner("SUMMARY: TEST FAILED: test_pfcwd_xoff_only_no_data_l2")
            st.report_fail(
                'msg',
        ...
        st.banner("SUMMARY: TEST PASSED: test_pfcwd_xoff_only_no_data_l2")
        st.log(f"Summary:")
        st.log(f"  Platform: {platform}")
        st.log(f"  Expected trigger: {expect_trigger}, Got triggered: {triggered}")
        st.log(f"  storm_detected delta: {delta['storm_detected']}")
        st.report_pass("test_case_passed", "test_pfcwd_xoff_only_no_data_l2 passed")
...
def test_pfcwd_partial_xoff_no_trigger_l2():
    """
    ...
    try:
        ...
        st.log(f"Data ingress check: {msg_ing}")
        if not ok_ing:
            st.banner("SUMMARY: TEST FAILED: test_pfcwd_partial_xoff_no_trigger_l2")
            st.report_fail('msg', f"Data ingress check failed: {msg_ing}")
        ...
        st.banner("SUMMARY: TEST PASSED: test_pfcwd_partial_xoff_no_trigger_l2")
        st.log(f"Summary:")
        st.log(f"  Platform: {data.platform}")
        st.log(f"  Partial XOFF rate: {xoff_rate} fps ({partial_pct}% of full)")
        st.log(f"  storm_detected delta: {delta['storm_detected']} (expected 0)")
        st.report_pass("test_case_passed", "test_pfcwd_partial_xoff_no_trigger_l2 passed")
...
def test_pfcwd_no_trigger_l2():
    ...
    try:
        ...
        st.log(f"Data ingress check: {msg_ing}")
        if not ok_ing:
            st.banner("SUMMARY: TEST FAILED: test_pfcwd_burst_storm_l2")
            st.report_fail('msg', f"Data ingress check failed: {msg_ing}")
        ...
        if delta['storm_detected'] != 0:
            st.banner("SUMMARY: TEST FAILED: test_pfcwd_no_trigger_l2")
            st.report_fail(
                'msg',
        ...
        st.banner("SUMMARY: TEST PASSED: test_pfcwd_no_trigger_l2")
        st.log(f"Summary:")
        st.log(f"  Platform: {data.platform}")
        ...
        st.log(f"  storm_detected delta: {delta['storm_detected']} (expected 0)")
        st.report_pass("test_case_passed", "test_pfcwd_no_trigger_l2 passed")
...
def test_pfcwd_drop_action_l2():
    ...
    if not ok:
        st.banner("SUMMARY: TEST FAILED: test_pfcwd_drop_action_l2")
    ...
    try:
        ...
        st.banner("SUMMARY: TEST PASSED: test_pfcwd_drop_action_l2")
        st.log(f"Summary:")
        st.log(f"  Platform: {data.platform}")
        ...
        st.report_pass("test_case_passed", "test_pfcwd_drop_action_l2 passed")
...
def test_pfcwd_forward_action_l2():
    """
    ...
    if actual_action != 'forward':
        st.banner("SUMMARY: TEST FAILED: test_pfcwd_forward_action_l2")
        st.report_fail(
            'msg',
    ...
    try:
        ...
        st.banner("SUMMARY: TEST PASSED: test_pfcwd_forward_action_l2")
        st.log(f"Summary:")
        ...
        st.log(f"  tx_drop during storm: {delta_during['tx_drop']} "
               f"(expected == 0)")
        st.report_pass("test_case_passed", "test_pfcwd_forward_action_l2 passed")
...
def test_pfcwd_burst_storm_l2():
    ...
    try:
        ...
        st.banner("SUMMARY: TEST PASSED: test_pfcwd_burst_storm_l2")
        ...
        st.report_pass("test_case_passed", "test_pfcwd_burst_storm_l2 passed")
...
def _run_m2o(trigger_pfcwd):
    ...
    test_name = (
        "test_pfcwd_many_to_one_trigger_l2" if trigger_pfcwd
        else "test_pfcwd_many_to_one_no_trigger_l2"
    )
