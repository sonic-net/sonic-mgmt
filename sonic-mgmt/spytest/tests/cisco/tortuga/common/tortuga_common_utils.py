...
def convert_tc_to_dscp(dut, tc, asic_str=''):

    if dut not in dscp_to_tc:
        dscp_to_tc[dut] = {}

        # Get json output for dscp to tc map and reverse it to create 
        # the dictionary we need for DSCP values
        dscp_to_tc_map = show_cmd_to_dict(dut, 'dscp-to-tc-map')
        if dscp_to_tc_map == None:
            return None
        dscp_to_tc_map = dscp_to_tc_map['DSCP_TO_TC_MAP']
        dscp_to_tc[dut] = next(iter(dscp_to_tc_map.values()))
    for k, v in dscp_to_tc[dut].items():
        if v == str(tc):
            return k
    return None
