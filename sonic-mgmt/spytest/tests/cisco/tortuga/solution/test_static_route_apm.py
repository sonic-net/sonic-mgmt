...
def deploy_tcpserver(device_node, path="/home/cisco/tcpserver.py"):
    """Construct tcpserver.py on a device using echo commands."""
    st.config(device_node, "rm -f {}".format(path))
    st.config(device_node, "echo 'import socket' > {}".format(path))
    st.config(device_node, "echo 'import threading' >> {}".format(path))
    st.config(device_node, "echo 'import sys' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo 'def handle_client(conn, addr):' >> {}".format(path))
    st.config(device_node, "echo '    print(f\"Connected by {{addr}}\")' >> {}".format(path))
    st.config(device_node, "echo '    with conn:' >> {}".format(path))
    st.config(device_node, "echo '        while True:' >> {}".format(path))
    st.config(device_node, "echo '            data = conn.recv(1024)' >> {}".format(path))
    st.config(device_node, "echo '            if not data:' >> {}".format(path))
    st.config(device_node, "echo '                break' >> {}".format(path))
    st.config(device_node, "echo '            conn.sendall(data)  # Echo back' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo 'def start_server(host, port):' >> {}".format(path))
    st.config(device_node, "echo '    # Determine address family (IPv4 or IPv6)' >> {}".format(path))
    st.config(device_node, "echo '    family = socket.AF_INET6 if \":\" in host else socket.AF_INET' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo '    with socket.socket(family, socket.SOCK_STREAM) as s:' >> {}".format(path))
    st.config(device_node, "echo '        # Allow immediate reuse of the address after restart' >> {}".format(path))
    st.config(device_node, "echo '        s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo '        # Bind and listen' >> {}".format(path))
    st.config(device_node, "echo '        s.bind((host, port))' >> {}".format(path))
    st.config(device_node, "echo '        s.listen()' >> {}".format(path))
    st.config(device_node, "echo '        print(f\"Server listening on {{host}}:{{port}}\")' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo '        while True:' >> {}".format(path))
    st.config(device_node, "echo '            conn, addr = s.accept()' >> {}".format(path))
    st.config(device_node, "echo '            print(f\"Accepted connection from {{addr}}\")' >> {}".format(path))
    st.config(device_node, "echo '            client_thread = threading.Thread(target=handle_client, args=(conn, addr))' >> {}".format(path))
    st.config(device_node, "echo '            client_thread.daemon = True' >> {}".format(path))
    st.config(device_node, "echo '            client_thread.start()' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo 'if __name__ == \"__main__\":' >> {}".format(path))
    st.config(device_node, "echo '    if len(sys.argv) != 3:' >> {}".format(path))
    st.config(device_node, "echo '        print(f\"Usage: python3 {{sys.argv[0]}} <port> <ip>\")' >> {}".format(path))
    st.config(device_node, "echo '        sys.exit(1)' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo '    try:' >> {}".format(path))
    st.config(device_node, "echo '        PORT = int(sys.argv[1])' >> {}".format(path))
    st.config(device_node, "echo '    except ValueError:' >> {}".format(path))
    st.config(device_node, "echo '        print(\"Port must be an integer\")' >> {}".format(path))
    st.config(device_node, "echo '        sys.exit(1)' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo '    HOST = sys.argv[2]' >> {}".format(path))
    st.config(device_node, "echo '' >> {}".format(path))
    st.config(device_node, "echo '    start_server(HOST, PORT)' >> {}".format(path))
    st.config(device_node, "chmod +x {}".format(path))
    st.log("tcpserver.py deployed on device {}".format(device_node))
...
def vxlan_config_hooks():
    ...
    deploy_tcpserver(nodes['spine1'])
    deploy_tcpserver(nodes['spine2'])
