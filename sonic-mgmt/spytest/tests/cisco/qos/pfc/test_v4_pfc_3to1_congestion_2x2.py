...
# Test configuration
TRAFFIC_CLASS = 3
STREAM_FACTOR = 0.80  # 80% line rate
FRAME_SIZE = 1350
TRAFFIC_DURATION = 60  # seconds

# Stream definitions: 3 sources -> 1 destination (3:1 oversubscription)
# All 3 streams from Leaf0 (D3) converge to single port on Leaf1 (D4)
STREAM_DEFS = [
    ('T1D3P1', 'T1D4P1'),
    ('T1D3P2', 'T1D4P1'),
    ('T1D3P3', 'T1D4P1'),
...
def configure_spine1_only_100g():
    ...
    - Original: 360G×3 through 400G×2 spines
    - Scaled: 66G×3 through 100G×1 spine
    """
...
def setup_topo():
    ...
    st.log("Setup topology started - 3:1 congestion TC3 test (3 src -> 1 dst)")
...
def create_all_streams():
    """
    Create 3 traffic streams from Leaf0 (D3) converging to single port on Leaf1 (D4).
    This creates a 3:1 oversubscription scenario (285 Gbps into 100G egress port).
    """
    streams = []
...
def analyze_results(streams, stats):
    ...
    For 3:1 congestion with TC3 (lossless queue):
    ...
    st.banner("Traffic Results Summary (3:1 Congestion Test)")
...
def collect_counters(label):
    ...
    dut_interfaces = {
        ...
        vars.D3: [vars.D3T1P1, vars.D3T1P2, vars.D3T1P3, vars.D3D1P1, vars.D3D2P1],
...
def test_3to1_congestion_tc3():
    """
    Test 3:1 congestion scenario with TC3 (lossless queue).

    Scaled Jira Scenario:
        Original:  360G×3 streams through 400G×2 spines
        Scaled:    95G×3  streams through 100G×1 spine  (2.85:1 - bottleneck at spine)

    Traffic Paths:
        T1D3P1 (95 Gbps) -> D3 -> Spine1(100G) -> D4 -> T1D4P1
        T1D3P2 (95 Gbps) -> D3 -> Spine1(100G) -> D4 -> T1D4P1
        T1D3P3 (95 Gbps) -> D3 -> Spine1(100G) -> D4 -> T1D4P1
                                                     ^
                                                     |-- 285 Gbps through 100G spine

    Spine2 is shutdown. Spine1 link is 100G.
    All 3 streams use TC3 (lossless queue) at 95 Gbps each = 285 Gbps total.
    ...
    """
    st.banner("Test STARTED: 3:1 Congestion TC3 (PFC Lossless)")

    # Log the test configuration
    st.log(f"Stream rate: {test_info['gbps']} Gbps × 3 = {test_info['gbps'] * 3} Gbps total")
    st.log("Topology: Spine2 shutdown, Spine1 at 100G")
    st.log(f"Congestion point: {test_info['gbps'] * 3} Gbps through 100G spine link")
    
    # Create 3 streams converging to single destination
    streams = create_all_streams()
    if streams is None or len(streams) != 3:
        st.report_fail('msg', 'Failed to create all 3 traffic streams')
        return

    # Ingress ports on D3 facing IXIA sources (where PFC TX happens)
    pfc_intfs = [vars.D3T1P1, vars.D3T1P2, vars.D3T1P3]
    ...
    try:
        ...
        if passed:
            st.report_pass('msg',
                f'3:1 congestion test passed - {loss_info} PFC Tx: {pfc_str}')
        else:
            st.report_fail('msg',
                f'3:1 congestion test FAILED - {loss_info} PFC Tx: {pfc_str}')
