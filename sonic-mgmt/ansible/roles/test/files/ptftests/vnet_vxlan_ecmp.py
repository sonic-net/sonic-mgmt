...
# 2. 'FromServer' : Sends regular packets to Vlan member interface and expects to see the encapsulated packets on the corresponding PortChannel interface.
...
class VNET(BaseTest):
    ...
    def getSrvInfo(self, vnet, ifname=''):
        ...
        for item in self.serv_info[vnet]:
            ...
            if ifname == '' or item['ifname'] == ifname:
                ret_list.append((item['ip'], item['port'], item['vlan_id'], item['vni']))
    ...
    def checkPeer(self, test):
        ...
        for peers in self.peering:
            for key, peer in peers.items():
                if test['name'] == key:
                    for info in self.getSrvInfo(ptest['name']):
                      ptest = dict(test)
                      ptest['name'] = peer
                      ptest['src'], ptest['port'], ptest['vlan'], ptest['vni'] = info
                      if 'dst_vni' in test:
    ...
    def addTest(self, graph, name, entry):
        for info in self.getSrvInfo(name.split('_')[0]):
          test = {}
          ...
          test['src'], test['port'], test['vlan'], test['vni'] = info
          if 'vni' in entry:
    ...
    def addLocalTest(self, test, entry):
        nhtest = dict(test)
        for info in self.getSrvInfo(nhtest['name'], entry['ifname']):
          nhtest['src'], nhtest['port'], nhtest['vlan'], nhtest['vni'] = info
    ...
    def calculateTotalRoutes(self, graph):
        ...
        for routes in graph['vnet_routes']:
            for name, rt_list in routes.items():
                ...
                if graph['vnet_peers'] is not None:
        ...
        for idx, data in enumerate(graph['vnet_interfaces']):
            ...
            for nbr in graph['vnet_neighbors']:
                if nbr['ifname'] == data['ifname']:
                    ...
                    # Moved these to inside the block, since it is possible to have more than one route info per interface
                    serv_info = {}
                    serv_info['ifname'] = data['ifname']
                    serv_info['vlan_id'] = vlan_id
                    serv_info['ip'] = ip
                    serv_info['port'] = ports
                    serv_info['vni'] = data['vni']
