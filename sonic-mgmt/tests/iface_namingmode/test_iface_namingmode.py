...
class TestShowQueue():
    ...
    def test_show_queue_counters(self, setup, setup_config_mode):
        ...
        """
        dutHostGuest, mode, ifmode = setup_config_mode
        queue_counter = dutHostGuest.shell('SONIC_CLI_IFACE_MODE={} sudo show queue counters | grep "ALL"'.format(ifmode))['stdout']
        logger.info('queue_counter:\n{}'.format(queue_counter))

        if mode == 'alias':
            for alias in setup['port_alias']:
                assert (re.search(r'{}\s+ALL\d\s+\S+\s+\S+\s+\S+\s+\S+'.format(alias), queue_counter) is not None) and (setup['port_alias_map'][alias] not in queue_counter)
        elif mode == 'default':
            for intf in setup['default_interfaces']:
                assert (re.search(r'{}\s+ALL\d\s+\S+\s+\S+\s+\S+\s+\S+'.format(intf), queue_counter) is not None) and (setup['port_name_map'][intf] not in queue_counter)
    ...
    def test_show_queue_counters_interface(self, setup_config_mode, sample_intf):
        ...
        dutHostGuest, mode, ifmode = setup_config_mode
        test_intf = sample_intf[mode]
        queue_counter_intf = dutHostGuest.shell('SONIC_CLI_IFACE_MODE={} sudo show queue counters {} | grep "ALL"'.format(ifmode, test_intf))
        logger.info('queue_counter_intf:\n{}'.format(queue_counter_intf))

        for i in range(len(queue_counter_intf['stdout_lines'])):
            assert re.search(r'{}\s+ALL{}\s+\S+\s+\S+\s+\S+\s+\S+'.format(test_intf, i), queue_counter_intf['stdout']) is not None
