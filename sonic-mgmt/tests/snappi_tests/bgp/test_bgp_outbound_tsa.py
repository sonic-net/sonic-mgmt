...
from tests.snappi_tests.bgp.files.bgp_outbound_helper import run_bgp_outbound_tsa_tsb_test           # noqa: F401
...
def test_bgp_outbound_downlink_tsa(snappi_api,                                     # noqa: F811
    ...
    if topology_type == TOPOLOGY_T2_PIZZABOX:
        pytest.skip("Downlink TSA test not applicable to T2 pizzabox topology (no separate downlink LC)")
...
def test_bgp_outbound_supervisor_tsa(snappi_api,                                     # noqa: F811
    ...
    if topology_type == TOPOLOGY_T2_PIZZABOX:
        pytest.skip("Supervisor TSA test not applicable to T2 pizzabox topology (no supervisor card)")
