...
def test_mtu(tbinfo, vmhost, ptfhost, mtu, gather_facts):

    testbed_type = tbinfo['topo']['name']

    output = vmhost.shell("ifconfig {} | grep mtu".format(vmhost.external_port))['stdout']
    match = re.search(r'mtu (\d+)', output)
    if match:
        mtu_value_set = int(match.group(1))

    if mtu == 9114 and  mtu_value_set < EXPECTED_MTU_VALUE:
        pytest.fail(f'MTU on UCS interface is set to {mtu_value_set}, must be set to {EXPECTED_MTU_VALUE}')
