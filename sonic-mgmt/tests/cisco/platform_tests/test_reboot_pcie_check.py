...
from tests.common.reboot import REBOOT_TYPE_COLD, DUT_ACTIVE, wait_for_shutdown, wait_for_startup, perform_reboot, reboot_ctrl_dict
...
def get_pcie_skip_devices(platform):
    """
    Load PCIe devices to skip for the given platform from config file.

    @param platform: Platform PID string
    @return: List of PCIe addresses to skip (empty list if none configured)
    """
    config_path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)),
        "files", "pcie_skip_devices.yaml")
    try:
        with open(config_path) as f:
            config = yaml.safe_load(f) or {}
    except Exception as err:
        logger.warning("Failed to load PCIe skip devices config: %s", err)
        return []
    skip_devices = config.get(platform, [])
    if skip_devices:
        logger.info("Skipping PCIe devices for platform %s: %s",
                    platform, skip_devices)
    return skip_devices


def check_console_for_pcie_errors(console_output, skip_devices=None):
    """
    Check if PCIe Bus Error is present in console output.

    @param console_output: String containing console logs
    @param skip_devices: List of PCIe addresses to exclude from error check
    ...
    if skip_devices:
        error_lines = [line for line in error_lines
                       if not any(dev in line for dev in skip_devices)]
    return error_lines


def check_syslog_for_pcie_errors(duthost, reboot_time, skip_devices=None):
    """
...
def test_cold_reboot_pcie_check(duthosts, enum_rand_one_per_hwsku_hostname,
    ...
    platform = duthost.facts.get("platform", None)
    skip_devices = get_pcie_skip_devices(platform)
    ...
    try:
        ...
        try:
            ...
            console_error_lines = check_console_for_pcie_errors(console_output, skip_devices)
        ...
        syslog_error_lines = check_syslog_for_pcie_errors(duthost, dut_reboot_time, skip_devices)
