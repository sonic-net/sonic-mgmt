...
import os
import json
from tests.platform_tests.fwutil.fwutil_common import call_fwutil, show_firmware, upload_platform, find_pattern
...
def test_fwutil_install_file(duthost, localhost, pdu_controller, fw_pkg, random_component):
    ...
    if random_component not in ['SSD', 'IOFPGA', 'Kenley']:
        pytest.skip('Skip the test case for {} component for now due to a known failure'.format(random_component))
