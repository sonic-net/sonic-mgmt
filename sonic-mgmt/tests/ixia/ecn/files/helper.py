...
def run_ecn_test_latency_1x1(api,
                 testbed_config,
                 port_config_list,
                 conn_data,
                 fanout_data,
    ...
    Args:
        ...
        xoff_quanta: values of xoff quanta to iterate over
    ...
    # RR configure TGEN to capture count of ECN marked packets
    # RR find how to clear the packet counters and ECN marked packet counter for each iteration

    """ Generate traffic config """
    flows = __gen_traffic_latency_1x1(testbed_config=testbed_config,
    ...
    config.flows = flows
    #config.captures = None

    """ Run traffic and get counters """
    result = []

    __run_traffic_latency_1x1(api=api,
		  config=config,
		  all_flow_names=[PAUSE_FLOW_NAME, DATA_FLOW_NAME],
		  exp_dur_sec=EXP_DURATION_SEC)

    # read number of packets marked and add up for the value of quanta
    return result
...
def __gen_traffic_latency_1x1(testbed_config,
                  port_config_list,
                  port_id,
                  pause_flow_name,
                  data_flow_name,
    ...
    Args:
        ...
        exp_dur_sec (float): experiment duration in second
        prio_dscp_map (dict): Priority vs. DSCP map (key = priority).
        xoff_quanta: The quanta value to use in the Xoff packets.
...
def __run_traffic_latency_1x1(api,
                  config,
                  all_flow_names,
                  exp_dur_sec):
    """
    ...
    while attempts < max_attempts:
        ...
        if len(rows) == len(all_flow_names) and\
           list(set(transmit_states)) == ['started']:
...
def get_ecn_markings_dut(duthost):
    """
    ...
    original_values = {}
    original_values['dequeue'] = json_contents['devices'][i]['ecn_dequeue_marking']
    original_values['latency'] = json_contents['devices'][i]['ecn_latency_marking']
    return original_values

def setup_ecn_markings_dut(duthost, localhost, dequeue, latency):
    """
        Setup deque or latency depending on arguments.
        Applicable to cisco-8000 Platforms only.
    """
    config_file = get_markings_config_file(duthost)
    original_values = get_ecn_markings_dut(duthost)
    dest_file = "/tmp/"
    contents = duthost.fetch(src=config_file, dest = dest_file)
    local_file = contents['dest']
    with open(local_file) as fd:
    ...
    if original_values['dequeue'] != dequeue:
        reboot_required = True
    if original_values['latency'] != latency:
        reboot_required = True

    json_contents['devices'][required_entry]['ecn_dequeue_marking'] = dequeue
    json_contents['devices'][required_entry]['ecn_latency_marking'] = latency
