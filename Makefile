...
ut_t0:
	...
	echo $(TEST_LIST) | sed 's/,/\n/g' > sanity_scripts/$(TEMP_TESTFILE)
	cat infra/$(TESTFILE) >> sanity_scripts/$(TEMP_TESTFILE)
	echo "Running UT on T0 with ${TEMP_TESTFILE}"
	$(MAKE) TESTFILE=sanity_scripts/$(TEMP_TESTFILE) t0_run
	rm sanity_scripts/$(TEMP_TESTFILE)
