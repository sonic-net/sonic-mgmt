...
def generate_allure_report(build_id, current_result_file):
    ...
    result = subprocess.run(["allure", "generate", "-o" "/tmp/{}".format(report_folder_name), ALLURE_DIR])
    ...
    result = subprocess.run(["tar", "-cvzf", "/tmp/{}.tar.gz".format(report_folder_name), report_folder_name])
    if result.returncode != 0:
    ...
    print("Allure report tarball generated successfully!")
    current_result_file.write("Allure report tarball generated successfully!\n")
    current_result_file.flush()
