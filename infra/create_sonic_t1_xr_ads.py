...
def iosxr_inital_cfg(data,vEOS_count):
    ...
    for i in range (1,vEOS_count+1):
        ...
        base += 1
        add_iosxr_ssh_access(veos1_host,veos1_port, connection_timeout)
...
def add_iosxr_ssh_access(veos1_host,veos1_port, connection_timeout):
...
def upload_t1_files(data):
    ...
    ftp_client.put('testbed_add_vm_topology.yml','sonic-mgmt/ansible/testbed_add_vm_topology.yml')
    ftp_client.put('password.txt','sonic-mgmt/ansible/password.txt')
    ftp_client.put('testbed-sherman-t1.yaml','sonic-mgmt/ansible/testbed-sherman-t1.yaml')
    ftp_client.put('topo_t1.yml', 'sonic-mgmt/ansible/vars/topo_t1.yml')
    ftp_client.close()
...
def add_iosxr_cfg(data):
    ...
    chan.send('./testbed-cli.sh -t testbed.csv -m veos -k iosxr add-topo docker-ptf password.txt\n')
    chan.settimeout(180)
    buff = ''
    err_buff = ''
    rcv_timeout = 60
...
def main():
    ...
    # Create ssh access to iosxr
    print("****** Create admin user in vEOS vm *******")
    iosxr_inital_cfg(data,vEOS_count)
    ...
    for i in range (1,vEOS_count+1):
        print("XRV9k{}:  Tlnt: {}   Tlnt Port: {}  SSH: {}   SSH Port: {}".format(str(i-1), data['xrv9k'+ str(i)]['HostAgent'], data['xrv9k'+ str(i)]['serial0'], data['xrv9k'+ str(i)]['xr_mgmt_ip'], data['xrv9k'+ str(i)]['xr_redir22'] ))
