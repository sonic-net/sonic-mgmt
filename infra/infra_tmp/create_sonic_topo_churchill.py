...
def _create_parser():
    ...
    parser.add_argument('-d', '--device_type', type=str, help='options are sherman, mth32, crocodile, sfd',
                      required=False,default="mth64", choices=['sherman', 'mth32', 'mth64', 'crocodile', 'sfd', 'churchill'])
...
def repo_update(data):
    ...
    ssh.connect(data['sonic_mgmt']['HostAgent'], data['sonic_mgmt']['xr_redir22'], "vxr", "cisco123", allow_agent=False, look_for_keys=False)
...
def change_dut_passwd(device):
    ...
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        ssh.connect(host, port, user, passwd, allow_agent=False, look_for_keys=False)
    except paramiko.ssh_exception.AuthenticationException:
        ssh.connect(host, port, user, new_passwd, allow_agent=False, look_for_keys=False)
...
def upload_tb_files(data,topo_type,base_topo_file,device_type):
    ...
    if device_type == 'mth32' or device_type == 'churchill':
...
def replace_dut_mgmt_address(data):
    ...
    for dut_name in get_dut_names(data):
        ssh.connect(data[dut_name]['HostAgent'], data[dut_name]['xr_redir22'], data[dut_name]['uname'], data[dut_name]['passwd'], allow_agent=False, look_for_keys=False)
        ftp_client=ssh.open_sftp()
