...
def install_allure(data):
    ...
    allure_package_url = config['allure']['allure-debian-url']
    alure_package_name = os.path.basename(urllib.parse.urlparse(allure_package_url).path)

    # Download the allure package
    _run_cmd_in_channel(container_channel, f'wget {allure_package_url}', False)

    # Install the allure package
    _run_cmd_in_channel(container_channel, f'sudo dpkg -i {alure_package_name}', True)
