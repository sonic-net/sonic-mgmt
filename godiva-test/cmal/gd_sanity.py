...
"""
Script was developed for Godiva Project.
...
#from lib.Mar13_cmal.definitions import mal_pb2
#from lib.Mar13_cmal.definitions import mal_pb2_grpc
#from lib.protobuf.python.google.protobuf import json_format
...
def add_port(cmal_local_config, local_index):
    system_config = cmal_local_config.system_config
    system_config.chassis_id.name = "df1y022.mtv25.net.google.com"

    ports_config = cmal_local_config.ports_config

    ports_config.default_port_params_profile = "port_params_test"

    port = ports_config.ports.add()
    port.port_id.name = "df1y022.mtv25.net.google.com:eth-1/1/" + str(local_index)
    port.port_type = cmal_local_config_pb2.Port.TRUNK
    port.of_port_name = "eth-1/1/" + str(local_index)
    port.of_port_no = local_index
    port.admin_state = cmal_local_config_pb2.Port.DISABLED
    port.description = "df1y129.mtv25.net.google.com:eth-1/1/1 [(member of trk-df1y022s1i22.mtv25-1)]"
    port.local_index = local_index
    port.port_params_profile_name = "lan-phy-no-rate-cp"

    port.port_set_profile_name.append("uplinks-df1y022s1i22.mtv25")
    port.port_set_profile_name.append("all-uplinks")

    port.link_event_damp_control_profile_name = "fabric"
    port.card_slot_number = 1
    port.port_number = local_index
    port.channel_number = 1
    port.node_id.name = "df1y022s1i22.mtv25.net.google.com"
...
class Test_CMAL:
    ...
    def test_ConfigLock(self,stub,lock_status):
        ...
        try:
            ...
            tdData.lock_id = response.local_config_lock_id
    ...
    def test_ConfigUnlock(self,stub):
        ...
        try:
            response = stub.ConfigUnlock(mal_pb2.ConfigUnlockRequest(local_config_lock_id=tdData.lock_id))
            log.info(response)
        except grpc.RpcError as rpc_error_call:
            log.info(rpc_error_call.code())
            log.info(rpc_error_call.details())
    ...
    def test_ConfigVerifyAndCommit(self,stub):
        ...
        add_port(cmal_local_config, 1)
        add_port(cmal_local_config, 3)
    ...
    def test_ConfigGet(self,stub):
        try:
            response = stub.ConfigGet(mal_pb2.ConfigGetRequest(config_name=tdData.config_name))
            message = mal_pb2.ConfigGetResponse.Status.Name(response.status)
            if message == "NO_SUCH_CONFIG":
                log.error("No such config_name: %s exists on the server" % tdData.config_name)
    ...
    def test_ConfigCopy(self,stub):
        try:
            response = stub.ConfigGet(mal_pb2.ConfigCopyRequest(local_config_lock_id=tdData.lock_id,dst_config_name="cp_name"))
    ...
    def test_ConfigMove(self,stub):
        try:
            response = stub.ConfigMove(mal_pb2.ConfigMoveRequest(local_config_lock_id = tdData.lock_id,\
            src_config_name = tdData.config_name, dst_config_name = "move_name"))
            ...
            if tdData.copy_overwrite and message == "CANNOT_OVERWRITE_EXISTING_CONFIG":
                response = stub.ConfigMove(mal_pb2.ConfigMoveRequest(local_config_lock_id = tdData.lock_id,\
                src_config_name = tdData.config_name, dst_config_name = "move_name", do_overwrite=True))
    ...
    def test_ConfigDel(self,stub):
        try:
            response = stub.ConfigDel(mal_pb2.ConfigDelRequest(local_config_lock_id=tdData.lock_id,config_name="cp_name"))
