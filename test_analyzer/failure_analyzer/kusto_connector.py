...
class KustoConnector(object):
    ...
    def __init__(self, current_time):
        ...
        self.query_head = f'''
            ...
            let prod_branch_name_prefix_pattern = strcat("^(",
                strcat_array(prefix_match_os_list, "|"), @")\\d{{2}}$");
            let prod_os_version_prefix_pattern = strcat("^(",
                strcat_array(prefix_match_os_list, "|"), @")\\d{{2}}\\.\\d{{1,3}}$");
        ...
        logger.info("Select 7 days' start time: {}, 30 days' start time: {}, "
                    "current time: {}".format(self.search_start_time,
        ...
        if not ingest_cluster or not access_token:
            logger.error(
                "Could not load backup Kusto Credentials from environment, "
                "please check your environment setting.")
        ...
        if not icm_cluster:
            logger.error(
                "Could not load IcM cluster url from environment, "
                "please check your environment setting.")
            self._icm_client = None
        else:
            icm_kcsb = KustoConnectionStringBuilder.with_aad_application_token_authentication(
                icm_cluster, access_token)

            self.icm_client = KustoClient(icm_kcsb)
        if not ado_cluster:
            logger.error(
                "Could not load ADO cluster url from environment, "
                "please check your environment setting.")
            self._ado_client = None
        else:
            ado_kcsb = KustoConnectionStringBuilder.with_aad_application_token_authentication(
                ado_cluster, access_token)
    ...
    def query_common_summary_results(self):
        ...
        query_str += self.query_valid_condition + self.query_common_condition + '''
        | where Summary in (SummaryWhileList)
        | project UploadTimestamp, Feature,  ModulePath, FilePath, TestCase,
                  opTestCase, FullCaseName, Result, BranchName, OSVersion,
                  TestbedName, HardwareSku, Asic, AsicType, Topology,
                  TopologyType, Summary, BuildId, PipeStatus
        | distinct UploadTimestamp, Feature, ModulePath, OSVersion, BranchName,
                   Summary, BuildId, TestbedName, HardwareSku, Asic, AsicType,
    ...
    def query_consistent_failure(self):
        ...
        | project UploadTimestamp, Feature, ModulePath, FullTestPath, FullCaseName,
                  TestCase, opTestCase, Summary, Result, BranchName, OSVersion,
                  TestbedName, Asic, AsicType, TopologyType, Topology, HardwareSku,
                  BuildId, PipeStatus, minAttempt, maxAttempt
        | sort by UploadTimestamp desc
         '''.rstrip()
    ...
    def query_legacy_failure(self):
        ...
        | project UploadTimestamp, Feature, ModulePath, FullTestPath, FullCaseName,
                  TestCase, opTestCase, Summary, Result, BranchName, OSVersion,
                  TestbedName, Asic, AsicType, TopologyType, Topology, HardwareSku,
                  BuildId, PipeStatus, AttemptInt
        | sort by UploadTimestamp desc
        '''.rstrip()
        logger.info("Query 7 days's legacy failure cases:\n{}".format(query_str))
        return self.query(query_str)

    def query_history_results(self, testcase_name, module_path, is_common=False,
                              is_legacy=False, is_consistent=False, is_flaky=False):
        """
        ...
        project UploadTimestamp, Feature, ModulePath, FullTestPath, FullCaseName, TestCase, opTestCase, Summary,
        Result, BranchName, OSVersion, TestbedName, Asic, AsicType, TopologyType, Topology, HardwareSku, BuildId,
        PipeStatus
        """
        common_query_tail = '''
                | project UploadTimestamp, Feature, ModulePath, FullTestPath, FullCaseName,
                          TestCase, opTestCase, Summary, Result, BranchName, OSVersion,
                          TestbedName, Asic, AsicType, TopologyType, Topology,
                          HardwareSku, BuildId, PipeStatus
        ...
        if is_common:
            common_query_head = '''
                TestReportUnionData
                | where UploadTimestamp > datetime({}) and UploadTimestamp <= datetime({})
                '''.format(self.history_start_time, self.search_end_time).rstrip()
            common_query_head += self.query_valid_condition + self.query_common_condition
            query_str = self.query_head + common_query_head + '''
                | where ModulePath == "{}"
                | where Result in (ResultList)
                | where Summary in (SummaryWhileList)
                '''.format(module_path).rstrip() + common_query_tail
            logger.info("Query common history results:\n{}".format(query_str))
            return self.query(query_str)
        elif is_legacy:
            legacy_query_str = self.query_head + '''
                let buildsWithoutRetry = TestReportUnionData
                | where UploadTimestamp > datetime({}) and UploadTimestamp <= datetime({})
                '''.format(self.history_start_time, self.search_end_time).rstrip()
            legacy_query_str += self.query_valid_condition + '''
                | summarize maxAttempt = max(toint(Attempt)) by BuildId
                | where maxAttempt == 0 or isnull(maxAttempt)
                | project BuildId
                | extend BuildId = case(BuildId has':', split(BuildId, ':')[0], BuildId)
                ...
                | where UploadTimestamp > datetime({}) and UploadTimestamp <= datetime({})
                '''.format(self.history_start_time, self.search_end_time).rstrip()
            legacy_query_str += self.query_common_condition + '''
                | extend BuildId = case(BuildId has':', split(BuildId, ':')[0], BuildId)
                | where BuildId in (buildsWithoutRetry)
                | where Summary !in (SummaryWhileList)
                | where opTestCase == "{}" and ModulePath == "{}"
                | where Result in (ResultList)
                '''.format(testcase_name, module_path).rstrip()
            ...
            consistent_query_str = self.query_head + '''
                let buildsWithRetry = TestReportUnionData
                | where UploadTimestamp > datetime({}) and UploadTimestamp <= datetime({})
                '''.format(self.search_start_time, self.search_end_time).rstrip()
            consistent_query_str += self.query_valid_condition + '''
                | summarize maxAttempt = max(toint(Attempt)) by BuildId
                | where maxAttempt >= 1
                | project BuildId
                | distinct BuildId;
                ...
                | where UploadTimestamp > datetime({}) and UploadTimestamp <= datetime({})
                '''.format(self.search_start_time, self.search_end_time).rstrip()
            consistent_query_str += self.query_common_condition + '''
                | where BuildId in (buildsWithRetry)
                | extend AttemptInt = toint(Attempt)
                | where Summary !in (SummaryWhileList)
                | where opTestCase == "{}" and ModulePath == "{}"
        ...
        with tempfile.NamedTemporaryFile(mode="w+") as temp:
            ...
            if self._ingestion_client_backup:
                if configuration["upload"]:
                    ...
                    logger.info("Skip ingestion to backup cluster, "
                                "upload is set to False")
        return
