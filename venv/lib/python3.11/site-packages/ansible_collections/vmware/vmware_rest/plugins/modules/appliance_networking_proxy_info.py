...
options:
    protocol:
        description:
        - The protocol whose proxy configuration is requested. Required with I(state=['get'])
        type: str
    session_timeout:
        description:
...
"""
RETURN = r"""
# content generated by the update_return_section callback# task: Get the HTTP proxy configuration
value:
  description: Get the HTTP proxy configuration
  returned: On success
  sample:
    ftp:
      enabled: 0
...
def prepare_argument_spec():
    ...
    argument_spec["protocol"] = {"type": "str"}
...
def build_url(params):
    ...
    if params.get("protocol"):
        ...
        return yarl.URL(
            ("https://{vcenter_hostname}" "/api/appliance/networking/proxy/").format(
                **params
            )
            + params["protocol"]
...
async def entry_point(module, session):
    ...
    async with session.get(url, **session_timeout(module.params)) as resp:
        ...
        if module.params.get("protocol"):
            _json["id"] = module.params.get("protocol")
