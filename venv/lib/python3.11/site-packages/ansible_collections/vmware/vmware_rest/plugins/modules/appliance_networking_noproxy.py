...
options:
    servers:
        description:
        - List of strings representing servers to bypass proxy. A server can be a
            FQDN, IP address, FQDN:port or IP:port combinations. This parameter is
            mandatory.
        elements: str
        required: true
        type: list
...
def build_url(params):
    return ("https://{vcenter_hostname}" "/api/appliance/networking/noproxy").format(
        **params
    )
...
async def _set(params, session):
    ...
    subdevice_type = get_subdevice_type("/api/appliance/networking/noproxy")
    if subdevice_type and not params[subdevice_type]:
        _json = await exists(params, session, build_url(params))
        if _json:
            params[subdevice_type] = _json["id"]
    _url = ("https://{vcenter_hostname}" "/api/appliance/networking/noproxy").format(
        **params
