...
DOCUMENTATION = """
---
module: cp_mgmt_securemote_dns_server_facts
short_description: Get securemote-dns-server objects facts on Checkpoint over Web Services API
description:
  - Get securemote-dns-server objects facts on Checkpoint devices.
...
EXAMPLES = """
- name: show-securemote-dns-server
  cp_mgmt_securemote_dns_server_facts:
    name: TestSecuRemoteDNSSever

- name: show-securemote-dns-servers
  cp_mgmt_securemote_dns_server_facts:
    details_level: full
    limit: 5
    offset: 0
...
def main():
    ...
    api_call_object_plural_version = "securemote-dns-servers"
