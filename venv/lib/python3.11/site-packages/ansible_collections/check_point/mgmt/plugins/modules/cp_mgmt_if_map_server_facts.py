...
DOCUMENTATION = """
---
module: cp_mgmt_if_map_server_facts
short_description: Get if-map-server objects facts on Checkpoint over Web Services API
description:
  - Get if-map-server objects facts on Checkpoint devices.
...
EXAMPLES = """
- name: show-if-map-server
  cp_mgmt_if_map_server_facts:
    name: TestIfMapServer

- name: show-if-map-servers
  cp_mgmt_if_map_server_facts:
    details_level: full
    limit: 5
    offset: 0
...
def main():
    ...
    api_call_object_plural_version = "if-map-servers"
