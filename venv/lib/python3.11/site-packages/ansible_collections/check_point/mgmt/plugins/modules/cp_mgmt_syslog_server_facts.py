...
DOCUMENTATION = """
---
module: cp_mgmt_syslog_server_facts
short_description: Get syslog-server objects facts on Checkpoint over Web Services API
description:
  - Get syslog-server objects facts on Checkpoint devices.
...
- name: show-syslog-server
  cp_mgmt_syslog_server_facts:
    name: newSyslogServer

- name: show-syslog-servers
  cp_mgmt_syslog_server_facts:
    details_level: full
    limit: 5
    offset: 0
...
def main():
    ...
    api_call_object_plural_version = "syslog-servers"
