...
class NetworksWirelessSsidsIdentityPsks(object):
    ...
    def get_object_by_name(self, name):
        ...
        try:
            items = self.meraki.exec_meraki(
                family="wireless",
                function="getNetworkWirelessSsidIdentityPsks",
    ...
    def get_object_by_id(self, id):
        ...
        try:
            ...
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
            result = get_dict_result(items, 'identityPskId', id)
        except Exception as e:
            print("Error: ", e)
            result = None
        return result
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get(
            "identity_psk_id") or self.new_object.get("identityPskId")
        ...
        if name_exists:
            _id = prev_obj.get("id")
            _id = _id or prev_obj.get("identityPskId")
            if id_exists and name_exists and o_id != _id:
                raise InconsistentParameters(
                    "The 'id' and 'name' params don't refer to the same object")
            if _id:
                self.new_object.update(dict(id=_id))
                self.new_object.update(dict(identityPskId=_id))
            if _id:
    ...
    def update(self):
        ...
        id = id or self.new_object.get("identityPskId")
        name = self.new_object.get("name")
        result = None
        if not id:
            ...
            if prev_obj_name:
                ...
                id_ = id_ or prev_obj_name.get("identityPskId")
            if id_:
                self.new_object.update(dict(identityPskId=id_))
        result = self.meraki.exec_meraki(
            family="wireless",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = NetworksWirelessSsidsIdentityPsks(self._task.args, meraki)
