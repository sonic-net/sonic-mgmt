...
required_if = [
    ("state", "present", ["adminId", "name", "organizationId", "email"], True),
    ("state", "absent", ["adminId", "name", "organizationId", "email"], True),
]
required_one_of = []
mutually_exclusive = []
required_together = []
...
class OrganizationsAdmins(object):
    ...
    def delete_by_id_params(self):
        ...
        if self.new_object.get('adminId') is not None or self.new_object.get('admin_id') is not None:
            new_object_params['adminId'] = self.new_object.get('adminId') or \
                self.new_object.get('admin_id')
        return new_object_params
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get(
            "admin_id") or self.new_object.get("adminId")
    ...
    def delete(self):
        ...
        result = self.meraki.exec_meraki(
            family="organizations",
            function="deleteOrganizationAdmin",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = OrganizationsAdmins(self._task.args, meraki)
