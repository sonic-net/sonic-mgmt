...
argument_spec.update(dict(
    ...
    name=dict(type="str"),
    portSchedule=dict(type="dict"),
    networkId=dict(type="str"),
    portScheduleId=dict(type="str"),
))

required_if = [
    ("state", "present", ["name", "networkId", "portScheduleId"], True),
    ("state", "absent", ["name", "networkId", "portScheduleId"], True),
]
required_one_of = []
mutually_exclusive = []
required_together = []
...
class NetworksSwitchPortSchedules(object):
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(
            name=params.get("name"),
            portSchedule=params.get("portSchedule"),
    ...
    def get_object_by_name(self, name):
        ...
        try:
            items = self.meraki.exec_meraki(
                family="switch",
                function="getNetworkSwitchPortSchedules",
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get(
            "port_schedule_id") or self.new_object.get("portScheduleId")
        ...
        if name_exists:
            _id = prev_obj.get("id")
            _id = _id or prev_obj.get("portScheduleId")
            if id_exists and name_exists and o_id != _id:
                raise InconsistentParameters(
                    "The 'id' and 'name' params don't refer to the same object")
            if _id:
                self.new_object.update(dict(id=_id))
                self.new_object.update(dict(portScheduleId=_id))
    ...
    def create(self):
        result = self.meraki.exec_meraki(
            family="switch",
            function="createNetworkSwitchPortSchedule",
    ...
    def update(self):
        ...
        id = id or self.new_object.get("portScheduleId")
        name = self.new_object.get("name")
        result = None
        if not id:
            ...
            if prev_obj_name:
                ...
                id_ = id_ or prev_obj_name.get("portScheduleId")
            if id_:
                self.new_object.update(dict(portScheduleId=id_))
        result = self.meraki.exec_meraki(
            family="switch",
            function="updateNetworkSwitchPortSchedule",
    ...
    def delete(self):
        ...
        result = self.meraki.exec_meraki(
            family="switch",
            function="deleteNetworkSwitchPortSchedule",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = NetworksSwitchPortSchedules(self._task.args, meraki)
