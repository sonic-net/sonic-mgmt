...
class DevicesSensorRelationships(object):
    ...
    def update_all_params(self):
        new_object_params = {}
        if self.new_object.get('livestream') is not None or self.new_object.get('livestream') is not None:
            new_object_params['livestream'] = self.new_object.get('livestream') or \
                self.new_object.get('livestream')
        if self.new_object.get('serial') is not None or self.new_object.get('serial') is not None:
    ...
    def get_object_by_name(self, name):
        ...
        try:
            items = self.meraki.exec_meraki(
                family="sensor",
                function="getDeviceSensorRelationships",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = DevicesSensorRelationships(self._task.args, meraki)
