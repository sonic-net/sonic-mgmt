...
required_if = [
    ("state", "present", ["name", "networkId", "payloadTemplateId"], True),
    ("state", "absent", ["name", "networkId", "payloadTemplateId"], True),
]
required_one_of = []
mutually_exclusive = []
required_together = []
...
class NetworksWebhooksPayloadTemplates(object):
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get(
            "payload_template_id") or self.new_object.get("payloadTemplateId")
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = NetworksWebhooksPayloadTemplates(self._task.args, meraki)
