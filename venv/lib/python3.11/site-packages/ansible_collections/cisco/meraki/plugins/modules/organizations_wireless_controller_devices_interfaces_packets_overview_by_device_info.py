...
options:
  ...
  names:
    description:
      - >
        Names query parameter. Optional parameter to filter wireless LAN controller by its interface name. This filter uses multiple exact matches.
    elements: str
    type: list
  t0:
    description:
  ...
  timespan:
    description:
      - >
        ...
        and t1. The value must be in seconds and be less than or equal to 1 day. The default is 1 hour.
    type: float
  perPage:
    description:
...
EXAMPLES = r"""
- name: Get all organizations _wireless _controller _devices _interfaces _packets _overview _by _device
  cisco.meraki.organizations_wireless_controller_devices_interfaces_packets_overview_by_device_info:
