...
### 8. Application Token (appToken) - Default
Uses a pre-obtained application access token.

The internal version changed the default authentication method from appKey to appToken. It's because most internal pipelines are expecting an ACCESS_TOKEN of application. Change the default authentication to appToken can avoid updating the internal pipelines.

Another change is that the internal version is now expecting environment variable `ACCESS_TOKEN` instead of `TEST_REPORT_AAD_APP_TOKEN` for the same reason.

**Additional environment variable required:**
- `ACCESS_TOKEN`: A valid application access token
