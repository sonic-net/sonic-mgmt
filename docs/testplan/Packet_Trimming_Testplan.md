...
3. Packet trimming supports config reload, cold reboot and warm reboot (ISSU). Fast reboot is not supported. Warm reboot (ISSU) is only applicable on platforms that support ISSU.
...
## Test Case 14: Verify Trimming is Hitless During Warm Reboot (ISSU)
**Objective**: Validate that when packet trimming is enabled, a warm reboot (ISSU) is hitless: the trimmed traffic is not dropped during the warm reboot, and all forwarded packets remain trimmed.

**Applicability**: Only applicable on platforms that support ISSU (warm reboot). Skip the test otherwise.
