...
Add a new file `tests/bgp/test_prefix_list_suppress.py`. The structure
should mirror [tests/bgp/test_prefix_list.py](../../tests/bgp/test_prefix_list.py).
...
def op_prefix_with_cmd(duthost, ptype, prefix, action, ignore_error=False):
    cmd = "sudo prefix_list {} {} {}".format(action, ptype, prefix)
...
def get_device_metadata(duthost):
    ...
    md = cfg["DEVICE_METADATA"]["localhost"]
    return md.get("type", ""), md.get("subtype", "")


def get_suppress_pl_names(duthost):
    """Return (ipv4_name, ipv6_name) from constants.yml,
       falling back to registry defaults."""
    if duthost.stat(path=CONSTANTS_FILE)["stat"]["exists"]:
        constants = yaml.safe_load(
            duthost.shell("cat {}".format(CONSTANTS_FILE))["stdout"]
        )
        pl = constants.get("constants", {}) \
                       .get("bgp", {}) \
                       .get("prefix_list", {}) \
                       .get(SUPPRESS_TYPE, {})
        return (pl.get("ipv4_name", "SUPPRESS_IPV4_PREFIX"),
                pl.get("ipv6_name", "SUPPRESS_IPV6_PREFIX"))
    return ("SUPPRESS_IPV4_PREFIX", "SUPPRESS_IPV6_PREFIX")


def verify_prefix_list_in_db(duthost, ptype, prefix):
    out = duthost.shell("sudo prefix_list status")["stdout"]
    expected = len(duthost.get_frontend_asic_ids()) or 1
    tag = "('{}', '{}')".format(ptype, prefix)
    return out.count(tag) == expected


def verify_frr_prefix_list_entry(duthost, name, prefix, ipv, present=True):
    """ipv: 'ip' or 'ipv6'. Returns True iff present matches."""
    matches = []
    for asic in duthost.get_frontend_asic_ids() or [None]:
        ns = "-n {}".format(asic) if asic is not None else ""
...
def cleanup_suppress(rand_one_dut_hostname, duthosts):
    duthost = duthosts[rand_one_dut_hostname]
    yield duthost
    for ipv in ["ipv4", "ipv6"]:
        for p in SUPPRESS_PREFIXES[ipv]:
            op_prefix_with_cmd(duthost, SUPPRESS_TYPE, p,
                               "remove", ignore_error=True)
```

Each test case from the plan maps to a single `def test_...()` function.
For TC-A2, simply rely on running the existing tests as-is; no new code is
required.

### Skeleton for TC-S1 / TC-S2
...
def test_suppress_prefix_cli_and_frr(cleanup_suppress):
    ...
    pytest_assert(
        wait_until(30, 2, 0,
                   verify_prefix_list_in_db, duthost, SUPPRESS_TYPE, v4_pfx),
        "v4 SUPPRESS_PREFIX not present in CONFIG_DB via status",
    )
    pytest_assert(
        wait_until(30, 2, 0,
                   verify_prefix_list_in_db, duthost, SUPPRESS_TYPE, v6_pfx),
        "v6 SUPPRESS_PREFIX not present in CONFIG_DB via status",
    )
...
### Skeleton for TC-A3 (negative on non-spine)

```python
@pytest.mark.topology("t0", "t1")
def test_anchor_prefix_rejected_on_non_spine(rand_one_dut_hostname, duthosts):
    duthost = duthosts[rand_one_dut_hostname]
    dtype, dsub = get_device_metadata(duthost)
    pytest_require(
        not (dtype == "UpperSpineRouter"
             or (dtype == "SpineRouter" and dsub == "UpstreamLC")),
        "Need a non-spine device for this test",
    )
    res = op_prefix_with_cmd(duthost, ANCHOR_TYPE, "205.168.0.0/24",
                             "add", ignore_error=True)
    pytest_assert(res["rc"] != 0,
                  "CLI must reject ANCHOR_PREFIX on non-spine")
    pytest_assert(
        "is not supported on device type" in res["stderr"],
        "Stderr should explain device gating: {}".format(res["stderr"]),
    )

    # Direct CONFIG_DB write to bypass CLI
    duthost.shell(
        'sonic-db-cli CONFIG_DB hset '
        '"PREFIX_LIST|ANCHOR_PREFIX|205.168.0.0/24" NULL NULL'
    )
    pytest_assert(
        wait_until(30, 2, 0,
                   _syslog_contains, duthost,
                   "Device type .* not supported for ANCHOR_PREFIX"),
        "bgpcfgd should warn that ANCHOR_PREFIX is not allowed",
    )
    duthost.shell(
        'sonic-db-cli CONFIG_DB del "PREFIX_LIST|ANCHOR_PREFIX|205.168.0.0/24"'
    )


def _syslog_contains(duthost, pattern):
    cmd = "sudo grep -E '{}' /var/log/syslog | tail -20".format(pattern)
    return bool(duthost.shell(cmd,
                              module_ignore_errors=True)["stdout"].strip())
```

### Skeleton for TC-S4 (constants.yml override)

```python
@pytest.mark.topology("t0", "t1")
def test_suppress_prefix_constants_override(duthosts, rand_one_dut_hostname):
    duthost = duthosts[rand_one_dut_hostname]
    backup = "/tmp/constants.yml.bak"
    duthost.shell("sudo cp {} {}".format(CONSTANTS_FILE, backup))

    try:
        # Patch constants.yml via python-yaml in a one-shot script
        patch_script = r"""
import yaml
p = '/etc/sonic/constants.yml'
with open(p) as f:
    d = yaml.safe_load(f)
d.setdefault('constants', {}) \
 .setdefault('bgp', {}) \
 .setdefault('prefix_list', {})['SUPPRESS_PREFIX'] = {
...
with open(p, 'w') as f:
    yaml.safe_dump(d, f)
"""
        duthost.shell("sudo python3 -c \"{}\"".format(
            patch_script.replace('"', '\\"')))
        duthost.shell("sudo docker restart bgp")
        pytest_assert(
            wait_until(120, 5, 10,
                       duthost.is_service_running, "bgpcfgd", "bgp"),
            "bgpcfgd did not come back after restart",
        )
        ...
        pytest_assert(
            ...
            "Override name not applied in FRR",
        )
        pytest_assert(
            verify_frr_prefix_list_entry(
                duthost, "SUPPRESS_IPV4_PREFIX", v4, "ip", False),
            "Default name should not be present when override is set",
        )
        op_prefix_with_cmd(duthost, SUPPRESS_TYPE, v4, "remove",
                           ignore_error=True)

    finally:
        duthost.shell("sudo cp {} {}".format(backup, CONSTANTS_FILE))
        duthost.shell("sudo docker restart bgp")
```

### Loganalyzer

For every test, attach a `loganalyzer` `expect_regex` / `ignore_regex` list so
that intentional `log_warn` lines (TC-N2, TC-N3, TC-A3) are explicitly
expected, and any **new** error/traceback is caught as a failure.

```python
from tests.common.plugins.loganalyzer.loganalyzer import LogAnalyzer

@pytest.fixture
def suppress_loganalyzer(duthosts, rand_one_dut_hostname):
    duthost = duthosts[rand_one_dut_hostname]
    la = LogAnalyzer(ansible_host=duthost, marker_prefix="suppress_prefix")
    marker = la.init()
...
Inside the `sonic-mgmt` container:

```bash
# Existing ANCHOR_PREFIX regression (TC-A2) – physical t2
cd /data/sonic-mgmt/tests
pytest bgp/test_prefix_list.py \
    --inventory ../ansible/<inventory> \
    --host-pattern all \
    --testbed_file ../ansible/testbed.yaml \
    --testbed <t2-testbed-name> \
    --log-cli-level info \
    --show-capture no \
    -rav

# New SUPPRESS_PREFIX + negative cases
pytest bgp/test_prefix_list_suppress.py \
    --inventory ../ansible/<inventory> \
    --host-pattern all \
    --testbed_file ../ansible/testbed.yaml \
    --testbed <t0-or-t1-or-t2-testbed-name> \
    --log-cli-level info \
    --show-capture no \
    -rav
```

For VS smoke (CLI plumbing, registration only):
...
pytest bgp/test_prefix_list_suppress.py \
    ...
    -k "test_suppress_prefix_cli or test_unsupported_prefix"
```
