...
|------|------|------|------------------|
| 1 | Firmware download validation | 1. Run CDB abort command if `firmware_download_cdb_abort_support` flag is set. | 2. Download the next firmware specified in `firmware_versions` using the sfputil CLI<br>3. Wait until CLI execution completes | 1. If CDB abort command was executed, it should be successful. <br>2. CLI execution should finish within `firmware_download_timeout_minutes` minutes and return 0 <br>3. Active FW version should remain unchanged<br>4. Inactive FW version should reflect the downloaded firmware version<br>5. Running image bank, inactive image bank and committed image bank remain the same<br>6. Inactive image bank should show the downloaded fw version<br>7. No link flap should be seen<br>8. The kernel has no error messages in syslog if `monitor_kernel_errors` flag is set<br>9. Critical process such as `xcvrd`, `syncd`  `orchagent` does not crash/restart. |
...
| 5 | Firmware download interruption | 1. Start the firmware download and interrupt at the percentages specified by `firmware_download_interrupt_percentage`<br>2. Use the method specified in `firmware_download_interrupt_method` to interrupt the process:<br>"ctrl_c": Use CTRL+C or kill the download process<br> "sfputil_reset": reset the optics using sfputil reset<br>"optic_reinsert": remove the optics and re-insert | 1. Active firmware version remains unchanged<br>2. Inactive firmware version is invalid i.e. N/A or 0.0.0<br>3. No change in "Committed Image"<br>4. Critical process such as `xcvrd`, `syncd`  `orchagent` does not crash/restart. |
| 6 | CDB abort of firmware download | 1. Start the firmware download and interrupt using CDB abort command if `firmware_download_cdb_abort_support` flag is set | 1. Check that advertisement register for cdb abort is set if `firmware_download_cdb_abort_support` is set.<br> 2. Active firmware version remains unchanged<br>3. Inactive firmware version is invalid i.e. N/A or 0.0.0<br>4. No change in "Committed Image"<br>5. Critical process such as `xcvrd`, `syncd`  `orchagent` does not crash/restart. |
| 7 | Successful firmware download after interruption | 1. Perform steps in TC #4 followed by TC #1 | All the expectation of test case #4 and case #1 must be met |
| 8 | Firmware download validation post reset | 1. Perform steps in TC #1<br>2. Execute `sfputil reset PORT` and wait for it to finish | All the expectation of test case #1 must be met |
