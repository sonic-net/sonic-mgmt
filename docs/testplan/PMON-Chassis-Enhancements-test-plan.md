...
- [Platform Test Cases](#platform-test-cases)
    ...
    - [1.2 Configure platform chassis modules](#12-configure-platform-chassis-modules)
...
- [APIs for chassis base added for modular chassis](#APIs-for-chassis-base-added-for-modular-chassis)
...
- [APIs for power consumption and supply APIs for modular chassis](#APIs-for-power-consumption-and-supply-APIs-for-modular-chassis)
...
- [APIs for thermal added for modular chassis](#APIs-for-thermal-added-for-modular-chassis)
...
1. Use redis cli dump config db using ``` redis-dump -d 4 -y -k "*CHASSI* ``` after shutdown line card, for example
     ```
     ...
     ```
2. Use redis cli to dump state Db  ``` redis-dump -d 6 -y -k "*CHASSI* ```  
    ...
    ```
3. Use redis dump  `redis-dump  -d 6 -y -k "*MIDPLANE*"` to get database information
  for example:
  ```
