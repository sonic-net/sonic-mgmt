...
1a. As of now we are specifying BGP neighbor explicity. 
1b. If possible (provided Ixia supports), use dynamic BGP session and move to BGP unnumbered over Link Local IPv6, RFC 5549's v4 NLRI with v6 NH. 
...
1. Creates Source MAC Rewrite ACL Configuration on encap-leaf with rewrite action and counter creation.
...
- Ensure that traffic is ECMPed on Po100 and Po101 subinterfaces on decap leaf. 
- Traffic-distribution should be fair, i.e. ~50% 
---
...
16. Check that traffic is distributed fairly to all endpoints.
17, Check that same-flow (with same inner src/dst IPs, same src/dst UDP port) using same endpoint for all the packets.
...
1. Configure Vxlan Source-port as 128-range  on encap-Leaf
...
4. Revert back to same initial Vxlan src-port range on both the Leaf
...
- Check that same-flow (with same inner src/dst IPs, same src/dst UDP port) is using same VXLAN-UDP-src-port for all the packets.
- Check this different-flows, it should use single-but-different VXLAN-UDP-src-port and it should be one of port from the 128-range.
---
...
1. Configure Vxlan dst-port as DPORT-A (non-default) on both the Leaf
...
4. Revert back to Vxlan dst-port as DPORT-A on both the Leaf
...
2. Configure ingress-ACL to mirror PortChannel traffic.
3. Configure egress-ACL to mirrot PortChannel traffic.
...
- Verify that specified-ingress/egress traffic is mirrored as per the config.
---
