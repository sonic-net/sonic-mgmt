...
| Planned shutdown active (DPU-driven mode only) | Verify zero traffic loss in planned shutdown    | • Start sending traffic <br>• Issue shutdown on active side                                           | DPU-1 becomes standalone, DPU-2 is dead.     | T2 receives packets without disruption. |
