...
- [Test Components](#test-components)
  - [1. ICMPSessionManager Class](#1-icmpsessionmanager-class)
  - [2. Traffic Generation Functions](#2-traffic-generation-functions)
...
### 1. ICMPSessionManager Class

A helper class that manages ICMP sessions via Redis databases on the DUT.

**Responsibilities:**
- Session lifecycle management (create, delete, query)
- STATE_DB monitoring and polling
...
|--------|------------|--------|-------------|
| `initialize_icmp_util()` | None | None | Copies icmporch_util.py script to DUT |
| `create_session()` | `session_key`, `session_config` | `bool` | Creates ICMP session entry in APP_DB |
| `remove_session()` | `session_key` | `bool` | Removes session from APP_DB |
| `get_session_state()` | `session_key` | `str` or `None` | Queries current state from STATE_DB (Up/Down) |
| `wait_for_session_state()` | `session_key`, `expected_state`, `timeout_ms` | `bool` | Polls STATE_DB until expected state or timeout |
| `cleanup_sessions()` | None | None | Removes all test-created sessions |
...
session_config = {
    ...
    "src_ip": "192.16.2.1",
    "dst_ip": "192.16.2.100",
    "src_mac": "40:14:82:f7:55:90",
    "dst_mac": "02:42:f0:00:00:02"
}
session_manager.create_session(session_key, session_config)

# Monitor state
state = session_manager.get_session_state(session_key)
assert state == 'Up'
```

### 2. Traffic Generation Functions

#### configure_icmp_host()

Configures IXIA port as an emulated network host that automatically handles ARP and ICMP.

**Parameters:**
- `testbed_config`: Snappi testbed configuration object
...
**Behavior:**
- Creates IXIA device/host on specified port
- Configures Ethernet and IPv4 interfaces
- Automatically responds to ARP requests from DUT
- Automatically responds to ICMP Echo Requests from DUT
- **Used for NORMAL session testing**

#### create_rx_icmp_stream()

Creates a continuous ICMP Echo Reply stream for RX session testing.

**Parameters:**
- `testbed_config`: Snappi configuration
- `tx_port`, `rx_port`: IXIA port names
- `tx_mac`, `rx_mac`: Source and destination MACs
- `tx_ip`, `rx_ip`: Source and destination IPs
- `session_guid`: Session GUID to embed in payload

**Behavior:**
- Creates flow with ICMP Echo Reply packets (Type 0)
- Rate: 20 PPS (1 packet every 50ms)
- Embeds structured payload with session GUID
- **Used for RX session testing**

#### create_rx_icmp_stream_with_fps()

Similar to `create_rx_icmp_stream()` but with configurable FPS (frames per second).

**Additional Parameters:**
- `fps`: Configurable packet rate
- `stream_name`: Unique name for the stream

**Use Case:** Testing timeout detection with various packet intervals

### 3. Helper Functions

| Function | Purpose |
|----------|---------|
| `normalize_mac_address()` | Normalizes MAC addresses using netaddr library |
| `get_interface_config()` | Discovers DUT interface configuration (IP, MAC, connected port) |
| `setup_interface_for_test()` | Configures DUT interface with test IP address |
| `create_icmp_payload()` | Creates structured ICMP payload with session GUID |
| `traffic_manager()` | Context manager for traffic lifecycle (setup, ARP, cleanup) |
