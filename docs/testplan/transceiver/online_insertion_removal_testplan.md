...
| hot_swap_ports_under_test | List of tuples | [] | No | dut | A list of tuples under `dut.dut_name`. The first element of the tuple should be the physical port index (Int) under test and it must be from `ports_under_test`.<br> The second element of the tuple should be the string containing the type of optics that needs to be hot-swapped.<br>This attribute must exist only under `dut` field. |
...
| 5 | Optics hot-swap validation| 1. Physically remove the optical module under test.<br>2. Insert the optical module described by `hot_swap_ports_under_test`.<br>3. Remove the optical module.<br>4. Insert the original optical module.|Checks after step 2 if the optical module is supported:<br> 1. Show transceiver presence and sfputil presence commands should return "Present" with exit code 0.<br>2. Show transceiver eeprom, show transceiver info and sfputil eeprom commands should return the output in the expected format with the exit code as 0.<br>Checks after step 2 if the optical module is not supported:<br> 1. TRANSCEIVER_STATUS_SW table of STATE_DB is updated correctly with the status and the error .<br> Checks after step 4:<br> All the checks for TC#2. |
...
> Note for implementer: TRANSCEIVER_STATUS_SW table is the only transceiver state table that is not deleted after the optics removal.
