...
The existing transceiver parser already performs this merge.
...
predicate and is never interpreted as `false`. Capability overrides do not
bypass runtime CLI execution, field-presence checks, parsing, or correctness
validation. Supplied value attributes override their corresponding legacy
hardcoded defaults; omitted values retain those defaults.
...
- nonzero corrected codewords may not exceed symbol errors;
...
The two FEC test modules live outside `tests/transceiver/`, so they reuse the
attribute parser through common modules:

- `tests/common/port_attributes/builder.py` wraps `DutInfoLoader`,
  `AttributeManager`, and optional template validation.
...
- `tests/common/platform/fec_utils.py` owns legacy values, capability lookup,
  parsing, and shared runtime validation helpers.
...
| 2. Attribute resolution | `AttributeManager` | Apply the existing precedence rules to defaults and supplied scoped overrides. Optional capability keys may be absent. |
