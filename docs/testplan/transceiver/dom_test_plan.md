...
> **Why there is no datapath-state gate:** the suite's session-level link prerequisite requires every port under test to be admin-up and oper-up, so an activated datapath is already guaranteed for every lane this test examines. A lane whose laser is off or still ramping reports a Tx bias outside the warning range and is skipped by the range gate above. That gate is also the correctly-scoped instrument: `tx{lane}bias` is indexed by **media** lane, whereas `DP{lane}State` is indexed by **host** lane, so a datapath-state lookup using the media lane index is only valid for modules that map the two 1:1. If a future test case needs datapath state, it must perform an explicit media-to-host lane translation rather than assuming the indices coincide.
