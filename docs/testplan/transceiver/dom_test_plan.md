...
| tx_power_consistency_variation_threshold | float | 3.0 | O | transceivers or platform | Maximum allowed absolute delta between two consecutive per-lane Tx power samples in TC4 (`abs(curr_dBm - prev_dBm)`; threshold effectively in dB) |
| rx_power_consistency_variation_threshold | float | 3.0 | O | transceivers or platform | Maximum allowed absolute delta between two consecutive per-lane Rx power samples in TC4 (`abs(curr_dBm - prev_dBm)`; threshold effectively in dB) |
| tx_bias_consistency_variation_threshold | float | 10.0 | O | transceivers or platform | Maximum allowed per-lane Tx bias variation between two consecutive TC4 samples, expressed as percent change versus the previous sample |
| laser_temperature_consistency_variation_threshold | float | 3.0 | O | transceivers or platform | Maximum allowed absolute delta between two consecutive laser temperature samples in TC4, in Celsius |
| temperature_consistency_variation_threshold | float | 3.0 | O | transceivers or platform | Maximum allowed absolute delta between two consecutive module temperature samples in TC4, in Celsius |
| voltage_consistency_variation_threshold | float | 0.1 | O | transceivers or platform | Maximum allowed absolute delta between two consecutive module voltage samples in TC4, in volts |
