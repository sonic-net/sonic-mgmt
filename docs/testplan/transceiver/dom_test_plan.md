...
| S5 | DOM recovery after daemon/docker restart | Same as S1, iterating the shared `perform_daemon_restart(duthost, <daemon>, <settle_sec>)` over xcvrd / pmon / swss / syncd with the matching `*_restart_settle_sec`. | DOM data remains fresh and within operational range after each restart; deviation-from-baseline checks pass. |
