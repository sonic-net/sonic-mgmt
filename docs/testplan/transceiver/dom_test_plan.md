...
**Lane domain (Tx bias percentage check):** `tx{lane}bias` is indexed by **media** lane, whereas `DP{lane}State` is indexed by **host** lane. This test case is scoped to modules whose media and host lanes map 1:1, so the media lane index also selects the corresponding datapath state. Modules that require gearbox or inverse-gearbox lane translation are out of scope until that translation is implemented.
