...
| 3 | VDM alarm/warning threshold hierarchy and value validation | 1. Retrieve threshold data per [Threshold Validation](#threshold-validation).<br>2. Dynamically determine expected threshold fields based on attributes ending with `_operational_range` present in `vdm.json` using the [Dynamic Field Mapping Algorithm](#dynamic-field-mapping-algorithm).<br>3. For each determined field, verify the hierarchy from [Threshold Validation](#threshold-validation).<br>4. For any field that also has a `_threshold_range` attribute configured, additionally verify the configured values equal the live STATE_DB threshold values for each severity (per [Threshold Validation](#threshold-validation)); skip this additional check for fields without one.<br>5. Only validate fields derived from attributes present in `vdm.json`. | All four threshold tables are present in STATE_DB for the configured fields and follow the hierarchy in [Threshold Validation](#threshold-validation). For fields with a configured `_threshold_range`, the live values also match the configured values. Threshold data integrity is maintained in STATE_DB. Threshold validation is dynamically determined from the attribute table. |
...
Shut/no-shut is already covered by Advanced TC 1 (see above), not by S1 below. Low-power mode gets its
own expanded case (S2), since — unlike the operations below, which are purely "recover after" checks —
VDM's link-quality observables become unavailable *during* low power and that behavior needs its own
assertion. The remaining disruptive operations are covered by S1, substituting the matching
helper/settle-timer pair for the scenario under test:
