...
| 4 | Basic EEPROM content verification via show CLI | 1. Retrieve the BASE_ATTRIBUTES, EEPROM_ATTRIBUTES, and CDB_FIRMWARE_UPGRADE_ATTRIBUTES from `port_attributes_dict`.<br>2. Use `show interfaces transceiver info <port>` CLI.<br>3. Verify key fields against expected values, including active and inactive firmware versions (only `show interfaces transceiver info` reports firmware; `sfputil show eeprom` does not). | All key EEPROM fields from CLI output — including Active Firmware (`gold_firmware_version`) and Inactive Firmware (`inactive_firmware_version`) — match expected values from `port_attributes_dict`. |
