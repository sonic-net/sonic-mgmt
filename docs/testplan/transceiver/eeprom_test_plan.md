...
| 2 | CDB background mode stress test | 1. For transceivers with `cdb_background_mode_supported` = True, issue API to read CMIS CDB firmware version in a loop for `cdb_stress_iteration_count` iterations (required attribute; its default of 10 is supplied via eeprom.json `defaults`, and the test fails for a port if it is undefined).<br>2. Concurrently, keep accessing EEPROM using API and ensure that the kernel has no error logs throughout all iterations. | CDB background mode operations complete successfully for all `cdb_stress_iteration_count` iterations for supported transceivers without I2C errors in kernel logs. |
