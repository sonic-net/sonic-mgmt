...
This plan is intentionally scoped to margin characterization, fault injection, and coherent-OSNR validation. SI-settings validation remains the responsibility of the System test plan, which already defines and validates the `optics_si_settings` and `media_si_settings` attribute families, includes dedicated diagnostic cases for comparing configured values against EEPROM / APPL_DB, and re-verifies those settings as part of the shared Standard Port Recovery and Verification Procedure.
...
This plan covers all optics listed in the parent [Transceiver Onboarding Test Infrastructure and Framework](test_plan.md#scope).
...
Before starting tests, verify the parent framework prerequisites and health checks described in [Common Session-Level Prerequisites](test_plan.md#common-session-level-prerequisites) and [Common Per-Test Health Checks](test_plan.md#common-per-test-health-checks).

Then confirm the following transceiver-specific baseline conditions:

1. **Transceiver Baseline Verification**
...
|----------------|------|---------------|-----------|-----------------|-------------|
| tp0_tp1_fault_injection_method | string | - | O | transceivers or platform | Host-side SI setting name to perturb using the existing `optics_si_settings` attribute family during TP0-TP1 fault injection. This should identify a concrete SI parameter rather than a generic injection method. |
| tp4_tp5_fault_injection_method | string | - | O | transceivers or platform | Media-side SI setting name to perturb using the existing `media_si_settings` attribute family during TP4-TP5 fault injection. This should identify a concrete SI parameter rather than a generic injection method. |
...
- TP0 FIR Change Test (host-side): skip if no applicable SI setting is available through the `optics_si_settings` attribute family or if the selected parameter is empty/`-`.
- TP4 FIR Change Test (media-side): skip if no applicable SI setting is available through the `media_si_settings` attribute family or if the selected parameter is empty/`-`.
- ZR Coherent Optics Test Cases: skip if `zr_optics` is empty/False or the required coherent line-system setup is unavailable.

### Recovery and Disruptive Operation Conventions

- Any test that changes port state, optical conditions, or SI settings must restore the original state and then execute the shared [Standard Port Recovery and Verification Procedure](system_test_plan.md#standard-port-recovery-and-verification-procedure) for the affected ports.
- For tests that perturb SI settings, the original values should be captured and restored through the shared [State Preservation and Restoration](system_test_plan.md#state-preservation-and-restoration) procedure rather than via ad-hoc per-test restore steps.
- Disruptive actions such as shut/no-shut, reset, low-power toggles, and reseat operations should be invoked through the shared transceiver operation primitives (for example, port state-change helpers and low-power/reset/reseat helpers such as `perform_lpm_toggle`) rather than described as ad-hoc inline commands.
...
|------|------|------|------------------|
| 1 | Link soak test | 1. Verify all links are operationally up.<br>2. Record baseline: link status, post-FEC error counters, link flap counts, and traffic counters (if `traffic_validation_enabled`) for all ports.<br>3. Monitor continuously for `soak_duration_hr` hours (default 24 hrs):<br>   a. Periodically check link status (no link flaps).<br>   b. Periodically record post-FEC error counters.<br>   c. Periodically record DOM sensor values (TX/RX power).<br>   d. If `traffic_validation_enabled`, verify transmitted byte/packet counts match received counts.<br>4. After the soak duration completes, execute the shared [Standard Port Recovery and Verification Procedure](system_test_plan.md#standard-port-recovery-and-verification-procedure) for the affected ports and verify the final state:<br>   a. All links remain operationally up.<br>   b. Post-FEC error increment = 0 (or within `post_fec_error_threshold`).<br>   c. Link flap count unchanged (or within `link_flap_tolerance`).<br>   d. Traffic counters: transmitted = received (if enabled). | 1. Link flap count increment = 0 (or within `link_flap_tolerance`).<br>2. Post-FEC error increment for any link = 0 (or within `post_fec_error_threshold`).<br>3. Transmitted byte/packet count = received byte/packet count (if traffic validation enabled).<br>4. All DOM values remain within operational ranges throughout the test, and the shared recovery procedure passes for the affected ports. |
| 2 | Receiver Sensitivity Test | 1. Skip test if `rx_power_reduction_method` is empty or `-`.<br>2. Testbed: use a uni-directional optical path (DR8 one optic pair — 8 links, or 2xLR4-6 two optic pairs — 4 links).<br>3. Record baseline: link status, post-FEC error counters, and RX power levels for all lanes.<br>4. Reduce RX optical power using `rx_power_reduction_method` (e.g., variable optical attenuator).<br>5. Monitor and record:<br>   a. Link status per lane<br>   b. Post-FEC error counter increments<br>   c. RX power levels from DOM<br>   d. RX LOS/LOL alarm flags<br>   e. VDM values (if supported)<br>6. Optionally introduce reflection between the TX and RX paths.<br>7. Restore original optical power levels.<br>8. Execute the shared [Standard Port Recovery and Verification Procedure](system_test_plan.md#standard-port-recovery-and-verification-procedure) for the affected ports. | RX power reduction triggers the appropriate alarm thresholds. Link behavior at the margin boundary is characterized, and the link recovers after optical power is restored and passes the shared recovery procedure. |
| 3 | Crosstalk Penalty Test | 1. Skip test if `crosstalk_injection_method` is empty or `-`.<br>2. Testbed: use a uni-directional optical path (DR8 one optic pair — 8 links, or 2xLR4-6 two optic pairs — 4 links).<br>3. Record baseline: link status, post-FEC error counters, and DOM values for all lanes.<br>4. Couple an aggressor TX to an existing link using `crosstalk_injection_method`.<br>5. Monitor and record:<br>   a. Link status per lane<br>   b. Post-FEC error counter increments<br>   c. Crosstalk penalty impact on RX signal quality<br>   d. DOM alarm/warning flags<br>   e. VDM values (if supported)<br>6. Remove the crosstalk source.<br>7. Execute the shared [Standard Port Recovery and Verification Procedure](system_test_plan.md#standard-port-recovery-and-verification-procedure) for the affected ports. | The crosstalk penalty impact on link quality is characterized, the appropriate alarm thresholds are triggered, and the link recovers after the crosstalk source is removed and passes the shared recovery procedure. |
...
| TC No. | Test | Steps | Expected Results |
|------|------|------|------------------|
| 1 | TP0 FIR Change Test (host-side) | 1. Skip test if no applicable SI setting is available through the `optics_si_settings` attribute family or if the selected parameter is empty/`-`.<br>2. Record baseline: link status, post-FEC error counters, and DOM values for all ports under test.<br>3. Execute the shared [State Preservation and Restoration](system_test_plan.md#state-preservation-and-restoration) procedure to capture the current state and the relevant SI-setting values for the affected ports.<br>4. Apply host-side SI fault injection by perturbing the selected setting from the `optics_si_settings` attribute family at TP0-TP1.<br>5. Monitor link behavior for `si_fault_stress_iterations` iterations.<br>6. For each iteration, record:<br>   a. Link status (up/down)<br>   b. Post-FEC error counter increments<br>   c. DOM alarm/warning flags (TX LOS, TX LOL, RX LOS, RX LOL)<br>   d. VDM values (if supported)<br>7. Restore the original SI-setting values using the shared [State Preservation and Restoration](system_test_plan.md#state-preservation-and-restoration) procedure.<br>8. Execute the shared [Standard Port Recovery and Verification Procedure](system_test_plan.md#standard-port-recovery-and-verification-procedure) for the affected ports. | TP0–TP1 alarm behavior is validated. Link behavior under SI degradation is characterized, alarm flags are raised when SI thresholds are crossed, and the link recovers after fault removal and passes the shared recovery procedure. |
| 2 | TP4 FIR Change Test (media-side) | 1. Skip test if no applicable SI setting is available through the `media_si_settings` attribute family or if the selected parameter is empty/`-`.<br>2. Record baseline: link status, post-FEC error counters, and DOM values for all ports under test.<br>3. Execute the shared [State Preservation and Restoration](system_test_plan.md#state-preservation-and-restoration) procedure to capture the current state and the relevant SI-setting values for the affected ports.<br>4. Apply media-side SI fault injection by perturbing the selected setting from the `media_si_settings` attribute family at TP4-TP5.<br>5. Monitor link behavior for `si_fault_stress_iterations` iterations.<br>6. For each iteration, record:<br>   a. Link status (up/down)<br>   b. Post-FEC error counter increments<br>   c. DOM alarm/warning flags (TX power, TX bias, RX power on the remote side)<br>   d. VDM values (if supported)<br>7. Restore the original SI-setting values using the shared [State Preservation and Restoration](system_test_plan.md#state-preservation-and-restoration) procedure.<br>8. Execute the shared [Standard Port Recovery and Verification Procedure](system_test_plan.md#standard-port-recovery-and-verification-procedure) for the affected ports. | TP4–TP5 alarm behavior is validated. Link behavior under media-side SI degradation is characterized, alarm flags are raised when SI thresholds are crossed, and the link recovers after fault removal and passes the shared recovery procedure. |
...
| TC No. | Test | Steps | Expected Results |
|------|------|------|------------------|
| 1 | Neighboring Channel Disturbance Test | 1. Skip test if `zr_optics` is empty/False or coherent line-system support is unavailable.<br>2. Turn up all channels and verify all links are healthy.<br>3. For each interface/channel under test:<br>   a. Identify neighboring channels (i-1 and i+1).<br>   b. Record baseline flap counts, BER, and power levels on neighboring channels.<br>   c. Clear FCS error counters on neighboring channels.<br>4. Invoke the shared disruptive operation primitive for the selected disturbance on the target channel (for example, the shut/no-shut helper, reset helper, low-power toggle helper such as `perform_lpm_toggle`, or reseat helper) rather than an ad-hoc inline operation.<br>5. While the operation is in progress, monitor neighboring channels for link flaps, BER / TX-RX power spikes or degradation, and FCS errors.<br>6. Restore the original channel state and execute the shared [Standard Port Recovery and Verification Procedure](system_test_plan.md#standard-port-recovery-and-verification-procedure) for the affected ports.<br>7. Repeat for the configured number of iterations.<br>8. Log and aggregate results across all channels. | 1. Zero link flaps on neighboring channels.<br>2. No DOM spikes or degradation on neighboring channels.<br>3. Zero FCS errors on neighboring channels.<br>4. Pass criteria met for all tested interfaces, and the affected ports complete the shared recovery procedure successfully. |
...
3. **Parent Health Checks**: Re-run the parent framework health checks from [Common Per-Test Health Checks](test_plan.md#common-per-test-health-checks) and confirm no new issues remain
