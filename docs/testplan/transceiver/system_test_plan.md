...
| swss_restart_settle_sec | integer | 180 | O | hwsku | Time to wait after swss restart before verification. Default sized for a typical ~128-port DUT; recovery scales with port count, so high-radix DUTs override this per platform (a ~512-port DUT needs ~540s). |
| syncd_restart_settle_sec | integer | 180 | O | hwsku | Time to wait after syncd restart before verification. Same recovery profile as swss; shares its default and per-platform override. |
| syncd_restart_supported | boolean | True | O | platform or hwsku | Whether restarting syncd in isolation is cleanly supported. True by default; set False on platforms where a standalone syncd restart does not recover cleanly, which skips the syncd-restart scenario there. |
| expect_xcvrd_restart_with_swss_or_syncd | boolean | False | O | platform | Whether the xcvrd process inside pmon is expected to restart during swss/syncd restart |
...
| prefec_ber_check_supported | boolean | False | O | transceivers or platform_hwsku_overrides | Whether to run the Pre-FEC BER Guard (Standard Port Recovery sub-check 8) and the peer-side flap BER test. Enable only for SKUs whose modules report a Pre-FEC BER (`fec_pre_ber` in `show interfaces counters fec-stats`) |
| prefec_ber_measure_sec | integer | 30 | O | transceivers or platform_hwsku_overrides | Measurement-window length. `fec_pre_ber` accumulates since the last counter clear, so baseline and post-flap reads both clear counters, wait this long, then read |
| prefec_ber_max | float | 1e-8 | O | transceivers or platform_hwsku_overrides | Absolute Pre-FEC BER ceiling; a post-recovery reading above this fails the Pre-FEC BER Guard regardless of baseline |
| prefec_ber_degradation_factor | float | 10 | O | transceivers or platform_hwsku_overrides | Max allowed ratio of post-recovery Pre-FEC BER to baseline; catches order-of-magnitude degradation under the `prefec_ber_max` ceiling (e.g. 1E-10 → 1E-9). Applied only when the baseline is non-zero |
...
- **Gating.** Sub-checks 2, 3, 5, 6, and 8 run for a port only if that port's sub-check 1 (Link Status) passed — stability, LLDP, CMIS state, SI settings, and Pre-FEC BER are meaningless on a port that never came up. Sub-check 7 (Docker and Process Health) runs **unconditionally**: if a link failed to recover, knowing whether a critical service died on the way is exactly the diagnostic wanted. Sub-check 8 (Pre-FEC BER Guard) is additionally **opt-in** — it runs only where `prefec_ber_check_supported` is True and a Pre-FEC BER baseline exists for the port.
...
8. **Pre-FEC BER Guard** (optional — per port, only if the port linked up, `prefec_ber_check_supported` is True, and a Pre-FEC BER baseline was recorded for the port in [Common Setup](#common-test-setup-and-teardown))
   - `fec_pre_ber` accumulates since the last counter clear, so measure over a bounded window: once the port is oper-up (sub-check 1), call [`clear_interface_counters_and_wait`](../../../tests/common/platform/interface_utils.py) with `wait_time=prefec_ber_measure_sec` (it runs `sonic-clear counters` then sleeps that window), then read `fec_pre_ber` from `show interfaces counters fec-stats`. The baseline in Common Setup uses the same helper. Only ports the CLI reports as up (`state` == `U`) are measured, corroborated by `fec_corr` always reading `0` for a down port; a down or `N/A` port is skipped.
   - **Fail** if the reading exceeds `prefec_ber_max`, **or** — when the baseline is non-zero — if it exceeds `prefec_ber_degradation_factor` × baseline. Log baseline, reading, and ratio at `info`.
   - Rationale: a link can stay up and pass every other check while its Pre-FEC BER silently degrades after a flap (e.g. ~1E-10 → ~1E-8), observed on some LPO/AOC optics where the peer TX driver over-amplifies the returning signal and the DUT receiver locks onto that poor transient. This is the only sub-check that inspects post-recovery signal quality.
...
2. **Pre-FEC BER baseline** (only where `prefec_ber_check_supported` is True): record the steady-state Pre-FEC BER per port using the same helper the guard uses (`clear_interface_counters_and_wait` with `wait_time=prefec_ber_measure_sec`, then read `fec_pre_ber`). This is the reference the [Standard Port Recovery](#standard-port-recovery-and-verification-procedure) **Pre-FEC BER Guard** (sub-check 8) compares against. A port that isn't up, is absent from the CLI output, or reads `N/A`, has no baseline and is skipped by the guard.
...
| 3 | Pre-FEC BER stability after peer-side link flap | 1. Skip test if `prefec_ber_check_supported` is False.<br>2. For each transceiver port with a Pre-FEC BER baseline (recorded in [Common Setup](#common-test-setup-and-teardown); a down, CLI-absent, or `N/A` port has no baseline and is skipped):<br>   a. Resolve the peer device and port via [Remote-Side Port Resolution](test_plan.md#remote-side-port-resolution).<br>   b. Flap the **peer** end (the trigger; a DUT-side flap will not reproduce the defect): `config interface shutdown <peerport>` on the peer, wait `port_shutdown_wait_sec`, then `config interface startup <peerport>`, wait `port_startup_wait_sec`. The DUT port stays administratively up.<br>3. Execute the **Standard Port Recovery and Verification Procedure** (incl. the **Pre-FEC BER Guard**, sub-check 8) for the affected DUT ports.<br>4. Teardown: leave every flapped peer port administratively up. | The DUT ports return to oper-up **and** their post-flap Pre-FEC BER stays within tolerance of the baseline (below `prefec_ber_max`, and — when baseline is non-zero — within `prefec_ber_degradation_factor` of baseline). A peer-side flap that leaves the link up but degrades DUT-side Pre-FEC BER is caught by the guard rather than silently passing. |
...
| 5 | swss docker restart impact | 1. Verify current link states to be up for all transceivers.<br>2. Restart swss container.<br>3. Monitor link state transitions and recovery.<br>4. Wait for `swss_restart_settle_sec` before verification.<br>5. Check if `expect_xcvrd_restart_with_swss_or_syncd` is True and verify the xcvrd PID changes accordingly.<br>6. Execute **Standard Port Recovery and Verification Procedure** for all ports. | Ensure `xcvrd` restarts (based on `expect_xcvrd_restart_with_swss_or_syncd`) and all ports pass verification checks. |
...
| TC No. | Test | Steps | Expected Results |
|------|------|------|------------------|
| 1 | Transceiver reset validation | 1. Skip test if `transceiver_reset_supported` is False.<br>2. Execute **State Preservation and Restoration** (capture phase).<br>3. Reset the transceiver using appropriate CLI command.<br>4. Wait for `transceiver_reset_i2c_recover_sec` to allow I2C recovery.<br>5. Confirm the reset generated no transceiver OIR event: assert (via `LogAnalyzer`) that syslog contains no `<port>: Got SFP removed event` or `<port>: Got SFP inserted event` (xcvrd `log_notice`) for the port over the reset→recovery window — a reset is an in-place re-init, not an OIR.<br>6. Verify port is linked down after reset and transceiver is in low power mode (if `low_power_mode_supported` is True).<br>7. If `low_pwr_request_hw_asserted` is True:<br>   a. Check DataPath is in DPDeactivated state.<br>   b. Verify LowPwrAllowRequestHW (page 0h, byte 26.6) is set to 1.<br>8. Issue `config interface shutdown <port>` and wait for `port_shutdown_wait_sec`.<br>9. Issue `config interface startup <port>` and wait for `port_startup_wait_sec`.<br>10. Execute **Standard Port Recovery and Verification Procedure**.<br>11. Execute **State Preservation and Restoration** (restoration phase). | Ensure that the port is linked down after reset and is in low power mode (if transceiver supports it). The reset generates no transceiver OIR event — no xcvrd `Got SFP removed event` / `Got SFP inserted event` is logged for the port. If `low_pwr_request_hw_asserted` is True, verify DataPath is in DPDeactivated state and LowPwrAllowRequestHW signal is asserted (set to 1). The shutdown and startup commands should re-initialize the port and bring the link up with all verification checks passing. |
...
1. **Configuration files**: Ensure any temporary configuration changes (e.g., `pmon_daemon_control.json` modifications from Transceiver Event Handling TC 3) have been reverted to their original state.
2. **C-CMIS settings**: Confirm frequency and tx power values are restored to their defaults (first entry in `frequency_values` and `tx_power_values` respectively) for all ports tested by Configuration Validation test cases.
3. **Transceiver power state**: If any transceiver remains in low power mode, restore it to high power mode using the appropriate CLI command.
4. **Interface state**: If any port in `port_attributes_dict` remains administratively down (e.g., due to a mid-test failure), issue a batched interface startup. After restoring administrative state, make one batched shutdown/startup recovery attempt for ports that remain administratively up but operationally down, using `port_shutdown_wait_sec` and `port_startup_wait_sec` as the recovery budgets.

### Post-Session Checks

1. **Database consistency**: Verify STATE_DB contains expected transceiver information for all modeled ports. Validate `TRANSCEIVER_INFO` content for every logical port and poll `TRANSCEIVER_DOM_SENSOR` for DOM-capable primary subports for up to `dom_info_recover_sec`, because DOM publication can lag link recovery. This is a session-level check not covered by per-test teardown.
2. **Final link and LLDP verification**: After all restoration and database consistency checks complete, confirm all ports are operationally up and LLDP neighbors are discovered (if LLDP is enabled). This is the final end-to-end confirmation after the full disruptive test sequence.
