...
6. **SI Settings Verification** (if applicable)
   ...
   <!-- TODO: Once the Signal Integrity test plan merges, move the concrete DB keys (NPU_SI_SETTINGS_SYNC_STATUS_KEY / PORT_TABLE APPL_DB) into its shared SI verification procedure and reference it here, so the keys live in one place. -->

7. **Docker and Process Health Check**
   - **Inherited (not restated here):** the [Common Per-Test Health Checks](test_plan.md#common-per-test-health-checks) already verify every framework-monitored process is `RUNNING` with an unchanged PID since the pre-test baseline — unless the caller declares an expected PID change for a specific process (see the restart guidance below), in which case that process's restart is permitted while all others must still show an unchanged PID — and that no new core files appeared in `/var/core`, before and after each test. Runs once per invocation regardless of port count.
   ...
   - For an operation that does **not** restart any monitored process, the caller passes no uptime requirement; the inherited PID-unchanged check already proves no crash-restart occurred.
