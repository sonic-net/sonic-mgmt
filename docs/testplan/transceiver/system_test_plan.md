...
| require_npu_si_settings_done | boolean | True | O | dut | Whether the media SI settings check requires `NPU_SI_SETTINGS_SYNC_STATUS` (`PORT_TABLE` of `STATE_DB`) to be `NPU_SI_SETTINGS_DONE` before comparing. Set False for deployments that program media SI settings by a path other than the xcvrd/media_settings.json sync workflow (e.g. directly from config_db.json at boot), where this key sits at `NPU_SI_SETTINGS_DEFAULT` permanently by design. |
