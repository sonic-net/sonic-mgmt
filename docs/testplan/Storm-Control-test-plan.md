...
| **Document** | **Link** |
|--------------|----------|
| SONiC storm-control CLI | [storm-control CLI](https://github.com/sonic-net/sonic-utilities/blob/master/doc/Command-Reference.md#storm-control) |
...
- Resilience across triggers (interface flap, cold/warm/fast reboot)
...
Out of scope (tracked under [Open Items](#6-open-items--future-work)):

- Storm control on LAG **parent** interface (negative path is in scope; member-port path is now in scope)
- Breakout port behavior
- QoS / ACL interop
- Reserved multicast addresses (STP BPDUs etc.)
...
| `RATE_TOLERANCE_PCT` | `10` | Forwarded-rate tolerance: when policing is expected the forwarded rate may exceed the ceiling by up to this percent; when not expected, the forwarded count must be within this percent of everything sent. |
...
**Steps**
For each invalid value in `{"abc", "-1"}`:
...
#### TC 4.4.2 — Invalid storm-control type rejected
...
#### TC 4.4.3 — Missing kbps argument rejected
...
#### TC 4.4.4 — Storm-control on PortChannel rejected
...
#### TC 4.4.5 — Re-apply identical configuration is idempotent
...
#### TC 4.4.6 — `del` with extra kbps argument rejected
