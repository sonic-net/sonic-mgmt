# Testbed Setup with Keysight IxANVL container and SONIC Virtual DUT
 ...
 - Run the host setup script to install required packages and initialize the management bridge network
  ...
  - Download Keysight IxANVL container image from the below location
    - [IxANVL](https://downloads.ixiacom.com/support/downloads_and_updates/anvl/builds/9.30.EA/VM-Image/Non-Automotive/docker/keysight_ixanvl_docker_image.tar). The path is not of ixia webserver as IxANVL container is not released yet. Once released the path will be properly updated. 
 
     Downloaded files would be like below - 
     - `keysight_ixanvl_docker_image.tar` 
...
- Now that the testbed has been fully setup and configured, let's run a simple test to make sure everything is functioning as expected.
	...
	- Before running the tests follow the steps inside host as mentioned below(This is to setup IxANVL with correct vtysh scripts):
		...
		- Copy `BGP4` directory inside the already up ixanvl container named `ptf_anvl`. Use scp or docker copy. e.g.
            ...
            ```
            $ scp -r BGP4 root@10.250.0.100:/opt/Ixia/IxANVL/DocScript
            ```

	- Run the following command to execute the `ixanvl_bgp_conformance` tests (including the pre/post setup steps):
