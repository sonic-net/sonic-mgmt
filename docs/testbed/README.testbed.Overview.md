...
The M0 type topology is to simulate a SONiC DUT running as a M0 device. For this type of topology, a set of DUT ports are connected to VMs simulating upstream M1 neighbors. Another set of the ports are connected to a PTF docker simulating downstream servers. Rest of the ports are connected to VMs simulating downstream Mx neighbors.
