...
one call (GoBGP has no `DeletePathStream`, but `AddPathStream` honors each
...
- **Per-prefix protobuf build:** GoBGP's gRPC API carries one route per message
...
- **Re-measure trigger:** re-evaluate **RustyBGP** only if it ships a stable release
  with a GoBGP-parity gRPC API **and** we push far beyond the ≤64-neighbor / 100k target.
...
|---|---|
| **RustyBGP** (Rust GoBGP re-impl) | only **~1.3–1.4×** over GoBGP (per-core edge, not an order of magnitude); experimental, nightly-binary; its gRPC API diverges from the GoBGP API the shim targets — **not drop-in**. Re-measure only on a stable GoBGP-compatible release + >64-neighbor scale. |
