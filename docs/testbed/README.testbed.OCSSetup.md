...
3. Run the host setup script to install required packages

```
sudo apt install python3-j2cli -y
git clone https://github.com/sonic-net/sonic-mgmt /data/sonic-mgmt
...
`testbed_ocs_new.yaml` is a specialized testbed topology configuration file designed for OCS testing environments. It should be placed in the `/data/sonic-mgmt/ansible` directory.This file defines the overall structure of the OCS test environment and contains the following key configuration blocks:
...
- `vm_type`: Define the type of virtual machines used in the test environment, determining which specific configuration tasks the system will execute
...
- `sonic_ocs`: Contains all SONiC OCS devices with hardware SKU `DLXA64B64HNLA1MS`
  - `mgmt141`: OCS devices with management IPs in the 10.250.0.x range
...
- `vm_host`: Contains the virtual machine host configuration
  - `svt`: VM host server with IP 10.250.0.188 and svt/123456 credentials
...
## Add/update configurations for ansible to access test server
...
topology:
  host_interfaces:
   ...
   - 63
```
Now we're finally ready to deploy the topology for our testbed! Run the following command in the directory /data/sonic-mgmt/ansible/:
...
```
If you want to configure the network for PTF, you can use the following command:

```
./config_ptf_network_for_ocs.sh --server-interface enp0s3 --vm-set-name ptf_2 --ptf-ip 10.250.0.189/24 --mgmt-gw 10.250.0.228
```
