// Package pinstesthelper contains APIs that help in writing GPINs Ondatra tests.
...
var (
	pinstesthelperIntfOperStatusGet = func(t *testing.T, d *ondatra.DUTDevice, port string) oc.E_Interface_OperStatus {
		return gnmi.Get(t, d, gnmi.OC().Interface(port).OperStatus().State())
	}

	pinstesthelperAllIntfNameGet = func(t *testing.T, d *ondatra.DUTDevice) []string {
		return gnmi.GetAll(t, d, gnmi.OC().InterfaceAny().Name().State())
	}

	pinstesthelperDUTNameGet = func(d *ondatra.DUTDevice) string {
		return d.Name()
	}

	pinstesthelperDUTPortGet = func(t *testing.T, d *ondatra.DUTDevice, id string) *ondatra.Port {
		return d.Port(t, id)
	}

	pinstesthelperDUTPortsGet = func(d *ondatra.DUTDevice) []*ondatra.Port {
		return d.Ports()
	}

	pinstesthelperConfigIntfAggregateIDGet = func(t *testing.T, d *ondatra.DUTDevice, port string) string {
		return gnmi.Get(t, d, gnmi.OC().Interface(port).Ethernet().AggregateId().Config())
	}

	pinstesthelperIntfAggregateIDReplace = func(t *testing.T, d *ondatra.DUTDevice, port string, ID string) {
		gnmi.Replace(t, d, gnmi.OC().Interface(port).Ethernet().AggregateId().Config(), ID)
	}

	pinstesthelperIntfPhysicalChannelsGet = func(t *testing.T, d *ondatra.DUTDevice, port string) []uint16 {
		return gnmi.Get(t, d, gnmi.OC().Interface(port).PhysicalChannel().State())
	}

	pinstesthelperIntfOperStatusAwait = func(t *testing.T, d *ondatra.DUTDevice, port string, expectedOperSatus oc.E_Interface_OperStatus, timeout time.Duration) (oc.E_Interface_OperStatus, bool) {
	...
	pinstesthelperIntfDelete = func(t *testing.T, d *ondatra.DUTDevice, port string) {
		gnmi.Delete(t, d, gnmi.OC().Interface(port).Config())
	}

	pinstesthelperIntfLookup = func(t *testing.T, d *ondatra.DUTDevice, port string) *ygnmi.Value[*oc.Interface] {
		return gnmi.Lookup(t, d, gnmi.OC().Interface(port).State())
	}

	pinstesthelperIntfHardwarePortGet = func(t *testing.T, d *ondatra.DUTDevice, port string) string {
		return gnmi.Get(t, d, gnmi.OC().Interface(port).HardwarePort().State())
	}

	pinstesthelperConfigPortSpeedGet = func(t *testing.T, d *ondatra.DUTDevice, portName string) oc.E_IfEthernet_ETHERNET_SPEED {
		return gnmi.Get(t, d, gnmi.OC().Interface(portName).Ethernet().PortSpeed().Config())
	}

	pinstesthelperStatePortSpeedGet = func(t *testing.T, d *ondatra.DUTDevice, portName string) oc.E_IfEthernet_ETHERNET_SPEED {
		return gnmi.Get(t, d, gnmi.OC().Interface(portName).Ethernet().PortSpeed().State())
	}

	pinstesthelperOndatraPortNameGet = func(p *ondatra.Port) string {
		return p.Name()
	}

	pinstesthelperOndatraPortIDGet = func(p *ondatra.Port) string {
		return p.ID()
	}
	...
	pinstesthelperBreakoutModeGet = func(t *testing.T, d *ondatra.DUTDevice, physicalPort string) *oc.Component_Port_BreakoutMode {
		return gnmi.Get(t, d, gnmi.OC().Component(physicalPort).Port().BreakoutMode().State())
	}

	pinstesthelperPortPmdTypeGet = func(t *testing.T, d *ondatra.DUTDevice, port string) (string, error) {
	...
	pinstesthelperTransceiverEmpty = func(t *testing.T, d *ondatra.DUTDevice, port string) bool {
...
func RandomInterface(t *testing.T, dut *ondatra.DUTDevice, params *RandomInterfaceParams) (string, error) {
	...
	if len(interfaces) == 0 {
		if params == nil {
			return "", errors.Errorf("no operationally UP interfaces found in %v", pinstesthelperDUTNameGet(dut))
		}
		return "", errors.Errorf("no interface found in %v with params: %+v", pinstesthelperDUTNameGet(dut), *params)
	}
...
func FetchPortsOperStatus(t *testing.T, d *ondatra.DUTDevice, ports ...string) (*OperStatusInfo, error) {
	...
	for _, port := range ports {
		switch operStatus := pinstesthelperIntfOperStatusGet(t, d, port); operStatus {
...
func FrontPanelPortListForDevice(t *testing.T, dut *ondatra.DUTDevice) ([]string, error) {
	...
	for _, port := range pinstesthelperAllIntfNameGet(t, dut) {
...
func DUTPortNames(dut *ondatra.DUTDevice) []string {
	...
	for _, port := range pinstesthelperDUTPortsGet(dut) {
		portNames = append(portNames, pinstesthelperOndatraPortNameGet(port))
	}
	return portNames
}
