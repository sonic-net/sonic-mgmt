...
func PeerPortsBySpeed(t *testing.T, host *ondatra.DUTDevice, peer *ondatra.DUTDevice) map[oc.E_IfEthernet_ETHERNET_SPEED][]PeerPorts {
	...
	for _, hostPort := range pinstesthelperDUTPortsGet(host) {
		peerPort := pinstesthelperDUTPortGet(t, peer, pinstesthelperOndatraPortIDGet(hostPort))

		// Verify the host port is UP. Otherwise, LACPDU packets will never be transmitted.
		if got, want := pinstesthelperIntfOperStatusGet(t, host, pinstesthelperOndatraPortNameGet(hostPort)), oc.Interface_OperStatus_UP; got != want {
			log.Warningf("Port %v:%v oper state will not work for LACP testing: want=%v got=%v", pinstesthelperDUTNameGet(host), pinstesthelperOndatraPortNameGet(hostPort), want, got)
			continue
		}

		// Verify we are not already part of an existing PortChannel since one port cannot belong to
		// multiple PortChannels.
		if got, want := pinstesthelperConfigIntfAggregateIDGet(t, host, pinstesthelperOndatraPortNameGet(hostPort)), ""; got != want {
			log.Warningf("Port %v:%v cannot be used since it is already assigned to a PortChannel: want=%v got=%v", pinstesthelperDUTNameGet(host), pinstesthelperOndatraPortNameGet(hostPort), want, got)
			continue
		}

		// Check that the host port has a valid peer port.
		if peerPort == nil {
			log.Warningf("Port %v:%v does not have a peer on %v.", pinstesthelperDUTNameGet(host), pinstesthelperOndatraPortNameGet(hostPort), pinstesthelperDUTNameGet(peer))
			continue
		}

		log.Infof("Found peer ports: %v:%v and %v:%v", pinstesthelperDUTNameGet(host), pinstesthelperOndatraPortNameGet(hostPort), pinstesthelperDUTNameGet(peer), pinstesthelperOndatraPortNameGet(peerPort))
		portSpeed := pinstesthelperConfigPortSpeedGet(t, host, pinstesthelperOndatraPortNameGet(hostPort))
		peerPortsBySpeed[portSpeed] = append(peerPortsBySpeed[portSpeed], PeerPorts{pinstesthelperOndatraPortNameGet(hostPort), pinstesthelperOndatraPortNameGet(peerPort)})
	}
...
func RemovePortChannelFromDevice(t *testing.T, timeout time.Duration, dut *ondatra.DUTDevice, portChannelName string) error {
	...
	pinstesthelperIntfDelete(t, dut, portChannelName)

	stopTime := time.Now().Add(timeout)
	for time.Now().Before(stopTime) {
		if !pinstesthelperIntfLookup(t, dut, portChannelName).IsPresent() {
			return nil
		}
...
func AssignPortsToAggregateID(t *testing.T, dut *ondatra.DUTDevice, portChannelName string, portNames ...string) {
	for _, portName := range portNames {
		log.Infof("Assigning %v:%v to %v", pinstesthelperDUTNameGet(dut), portName, portChannelName)
		pinstesthelperIntfAggregateIDReplace(t, dut, portName, portChannelName)
	}
}
