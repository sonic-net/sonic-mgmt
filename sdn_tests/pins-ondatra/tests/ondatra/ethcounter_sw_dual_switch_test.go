...
func TestGNMIEthernetInErrors(t *testing.T) {
	...
	defer pinstesthelper.NewTearDownOptions(t).WithID("4b8a5e02-7389-474a-a677-efde088667b0").WithID("fb11ecf4-6e74-4255-b150-4a30c2493c86").Teardown(t)

	dut := ondatra.DUT(t, "DUT")
	control := ondatra.DUT(t, "CONTROL")

	params := pinstesthelper.RandomInterfaceParams{
		PortList: []string{
	...
	dutIntf, err := pinstesthelper.RandomInterface(t, dut, &params)
	if err != nil {
	...
	for i = 1; i <= 10; i++ {
		...
		packetOut := &pinstesthelper.PacketOut{
		...
		p4rtClient, err := pinstesthelper.FetchP4RTClient(t, control, control.RawAPIs().P4RT(t), nil)
		if err != nil {
