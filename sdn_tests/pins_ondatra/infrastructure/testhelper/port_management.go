...
// Breakout mode is in the format "numBreakouts1 x breakoutSpeed1 + numBreakouts2 x breakoutSpeed2 + ...
// Eg: "1x400G", 2x100G(4) + 1x200G(4)"
func ConfigFromBreakoutModeWithSkipLane(t *testing.T, dut *ondatra.DUTDevice, breakoutMode, port string, prevSkippedPorts []string, includeNewSkippdPorts bool) (*oc.Root, []string, error) {
	if len(breakoutMode) == 0 {
	...
	for _, mode := range modes {
		...
		if includeNewSkippedPorts {
			...
			for {
				randomLen = rand.Intn(len(allInterfaces) + 1)
				if randomLen > 0 {
					break
				}
			}

			randomIndex := rand.Perm(len(allInterfaces))
