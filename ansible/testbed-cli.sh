...
# Credential variables that can be injected via environment
CRED_VARS=(
  ansible_user ansible_password ansible_ssh_pass ansible_become_pass
...
{
  local _envvar _envval _cred_options=""

  # Build credential options fresh each call (avoids double-injection)
  for _envvar in "${CRED_VARS[@]}"; do
    _envval="${!_envvar}"
    if [ -n "$_envval" ]; then
