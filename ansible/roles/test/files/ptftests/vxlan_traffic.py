...
# ptf --test-dir ptftests vxlan_traffic.VXLAN --platform-dir ptftests --qlen=1000 --platform \
#    remote -t 't2_ports=[16, 17, 0, 1, 4, 5, 21, 20];dut_mac=u"64:3a:ea:c1:73:f8";\
...
#    "/tmp/vxlan-config-TC1-v6_in_v4.json";t0_ports=[u"Ethernet42"];random_src_ip=False;\
#    random_dport=True;random_dport=False' --relax --debug info\
#    --log-file /tmp/vxlan-tests.TC1.v6_in_v4.log

'''
     The test checks vxlan encapsulation:
     'test_encap' : Sends regular packets to T0-facing interface and expects to see the
                    encapsulated packets on the T2-facing interfaces.

    The test has the following parameters:
    config_file          : is a filename of a file which contains all necessary information to run
                           the test. The file is populated by ansible. This parameter is mandatory.
    t2_ports             : The list of PTF port indices facing T2 Neighbors, AKA ports to expect
                           the encapsulated packets to come in.
    dut_mac              : The MAC address of the dut, given by "show platform summary".
    expect_encap_success : Is the encapsulation expected to succeed ? True/False.
    packet_count         : NUmber of packets per endpoint to try. Default 10
    downned_endpoints    : The list of IP addresses which are down, due to BFD being disabled.
    vxlan_port           : The global VxLAN port setup in the DUT. Default: 4789
    topo_file            : The file that contains the topology information, like minigraph
                           data, connections, and so on.
    t0_ports             : The DUT intf into which we will inject payload packets.
    random_src_ip        : Should we use random src IP addresses for the payload packets?
                           Default:False
    random_dport         : Should we use random dest port for the payload packets? Default:True
    random_sport         : Should we use random src port for the payload packets? Default:True
'''
...
def get_ip_address(af, hostid=1, netid=100):
    ...
    if af == 'v4':
        return unicode("{}.{}.{}.{}".format(first_octet, second_octet, third_octet, hostid))
    if af == 'v6':
        # :0: gets removed in the IPv6 addresses. Adding a to octets, to avoid it.
        return unicode("fddd:a{}:a{}::a{}:{}".format(
            first_octet, second_octet, third_octet, hostid))

def get_incremental_value(key):
    '''
        Global function to keep track of the tcp/udp port numbers used in payload.
    '''
    global VARS
...
class VXLAN(BaseTest):
    ...
    def setUp(self):
        ...
        if self.packet_count < MIN_PACKET_COUNT:
            Logger.warning("Packet_count is below minimum, resetting to %s", MIN_PACKET_COUNT)
    ...
    def runTest(self):
        ...
        for t0_intf in self.test_params['t0_ports']:
            ...
            for addr in neighbors:
                for destination, nexthops in self.config_data['dest_to_nh_map'][vnet].iteritems():
                    self.test_encap(
                        ptf_port,
                        vni,
                        addr,
    ...
    def verify_all_addresses_used_equally(
            self, nhs, returned_ip_addresses, packet_count, downed_endpoints=[]):
        '''
           ...
           Params:
               ...
               returned_ip_addresses : The dict containing the nh addresses and corresponding
                   packet counts.
        '''
        ...
        if downed_endpoints:
            for down_endpoint in downed_endpoints:
                ...
                if down_endpoint in returned_ip_addresses:
                    raise RuntimeError(
                        "We received traffic with a downed endpoint({}), unexpected.".format(
                            down_endpoint))
        ...
        if set(nhs) - set(returned_ip_addresses.keys()) == set([]):
            ...
            for nh_address in returned_ip_addresses.keys():
                Logger.info("      %s : %s", nh_address, returned_ip_addresses[nh_address])
            # Check #2 : The packets are almost equally distributed.
            # Every next-hop should have received within {tolerance}% of the packets that we sent
            # per nexthop(which is packet_count). This check is valid only if there are large
            # enough number of packets(300). Any lower number will need higher
            # tolerance(more than 2%).
            if packet_count > MINIMUM_PACKETS_FOR_ECMP_VALIDATION:
                for nh_address in returned_ip_addresses.keys():
                    if (1.0-self.tolerance) * packet_count <= \
                        returned_ip_addresses[nh_address] <= (1.0+self.tolerance) * packet_count:
                        pass
                    else:
                        raise RuntimeError(
                            "ECMP nexthop address: {} received too less or too many"
                            " of the packets expected. Expected:{}, received on that"
                            " address:{}".format(
                                nh_address,
                                packet_count,
            ...
            raise RuntimeError(
                "Not all addresses were used. Here are the unused ones:{}, expected:{}, got:{}".\
                format(set(nhs) - set(returned_ip_addresses.keys()), nhs, returned_ip_addresses))
    ...
    def test_encap(
        ...
        '''
           ...
           3. Optionally: Perform if the ECMP is working(all nexthops are used equally).
        '''
        try:
            pkt_len = 100
            if 'vlan' != 0:
            ...
            if not working_nhs:
                ...
                test_nhs = nhs
                # Also reduce the packet count, since this script has to wait 1 second per
                # packet(1000 packets is 20 minutes).
                packet_count = 4
            ...
            for host_address in test_nhs:
                ...
                Logger.info("Sending %s packets from port %s to %s", packet_count, str(ptf_port), destination)
                for _ in range(packet_count):
                    ...
                    if isinstance(ip_address(destination), IPv4Address) and \
                        ...
                        raise RuntimeError("Invalid mapping of destination and PTF address.")
                ...
                if expect_success:
                    ...
                    while (datetime.now() - start_time).total_seconds() * 1000 < loop_timeout and \
                            vxlan_count < packet_count:
                        result = dp_poll(
                            self, timeout=wait_timeout
                        )
                        if isinstance(result, self.dataplane.PollSuccess):
                            if not isinstance(result, self.dataplane.PollSuccess) or \
                                    not result.port in self.t2_ports or \
                                    "VXLAN" not in Ether(result.packet):
                                continue
                            else:
                                vxlan_count += 1
                            ...
                            Logger.info("No packet came in %s seconds", wait_timeout)
                            break
                    if not vxlan_count or not returned_ip_addresses:
                        raise RuntimeError("Didnot get any reply for this destination:{}"
                            "its active endpoints:{}".format(destination, test_nhs))
                    Logger.info("Vxlan packets received:%s, loop time:%s seconds",
                                vxlan_count, (datetime.now() - start_time).total_seconds())
