...
class SadTest(object):
    def __init__(self, oper_type, vm_list, portchannel_ports, vm_dut_map, test_args, vlan_ports, ports_per_vlan):
        ...
        self.fails_dut = set()
        self.log = []
        self.shandle = SadOper(self.oper_type, self.vm_list, self.portchannel_ports,
                               self.vm_dut_map, self.test_args, self.vlan_ports, self.ports_per_vlan)
...
class SadPath(object):
    ...
    def select_vm(self):
        ...
        # neigh_vms list will contain cnt number of VMs starting from the start VM.
        # vm_list will have the rest of the VMs
        ...
        if exceed_len <= 0:
            ...
            self.vm_list = self.vm_list[exceed_len:exceed_len +
                                        vm_len - self.cnt]
        ...
        for key in self.vm_dut_map:
            for neigh_vm in self.neigh_vms:
                if self.vm_dut_map[key]['mgmt_addr'] == neigh_vm:
                    # VM address to name mapping
                    self.neigh_names[neigh_vm] = key
                    break
    ...
    def select_vlan_ports(self):
        ...
        if exceed_len <= 0:
            ...
            self.if_port = self.if_port[exceed_len:exceed_len +
                                        vlan_len - self.cnt]
    ...
    def down_vlan_ports(self):
        ...
        for item in self.down_vlan_info:
            self.vlan_ports = [
                port for port in self.vlan_ports if port != item[1]]
    ...
    def setup(self):
        ...
        if 'member' in self.oper_type:
            self.tot_memb_cnt = len(
                self.vm_dut_map[self.neigh_names.values()[0]]['dut_ports'])
            if self.memb_cnt == 0:
...
class SadOper(SadPath):
    def __init__(self, oper_type, vm_list, portchannel_ports, vm_dut_map, test_args, vlan_ports, ports_per_vlan):
        super(SadOper, self).__init__(oper_type, vm_list, portchannel_ports,
                                      vm_dut_map, test_args, vlan_ports, ports_per_vlan)
    ...
    def sad_setup(self, is_up=True):
            ...
            if 'vlan' in self.oper_type:
                ...
                if 'lag' in self.oper_type:
                    ...
                    if self.bp_ip and self.bp_ip6:
                        ...
                        neigh_rt_v4_info, ret = self.get_bgp_route_cnt(
                            is_up=is_up)
                        neigh_rt_v6_info, ret1 = self.get_bgp_route_cnt(
                            is_up=is_up, v4=False)
                        if not ret and not ret1:
                            self.build_neigh_rt_map(
                                neigh_rt_v4_info + neigh_rt_v6_info)
        ...
        if 'routing' in self.oper_type:
                ...
                for vm in self.neigh_vms:
                    if not is_up:
                        ...
                        if 'routing_del' in self.oper_type:
                            self.log.append(
                                'Adding %d routes from VM %s' % (2 * self.ip_cnt, vm))
                            self.vm_handles[vm].change_bgp_route(
                                self.route_cfg)
                    else:
                        self.log.append(
                            'Removing %d routes from VM %s' % (2 * self.ip_cnt, vm))
                        self.vm_handles[vm].change_bgp_route(self.no_route_cfg)

        elif 'bgp' in self.oper_type:
            self.log.append('BGP state change will be for %s' %
                            ", ".join(self.neigh_vms))
            if self.oper_type == 'neigh_bgp_down':
                for vm in self.neigh_vms:
            ...
            self.log.append('LAG %s state change will be for %s' %
                            (self.memb_str, ", ".join(self.neigh_vms)))
            if 'neigh_lag' in self.oper_type:
                for vm in self.neigh_vms:
                    ...
                    if 'member' in self.oper_type:
                        ...
                        down_intfs = [
                            self.vm_dut_map[self.neigh_names[vm]]['neigh_portchannel']]

                    self.log.append('Changing state of LAG %s %s to shut' % (
                        self.memb_str, ", ".join(down_intfs)))
                    self.vm_handles[vm].change_neigh_intfs_state(
                        down_intfs, is_up=is_up)
    ...
    def generate_ips(self):
        ...
        self.ip_pfx_list = list(ipaddress.ip_network(
            u'%s' % self.start_ip_pfx).hosts())[0:self.ip_cnt]
        self.ip_pfx_list = [str(ip) for ip in self.ip_pfx_list]
        self.ip6_pfx_list = list(ipaddress.IPv6Network(
            u'%s' % self.start_ip6_pfx).hosts())[0:self.ip_cnt]
    ...
    def build_route_config(self):
        ...
        for cnt, ip in enumerate(zip(self.ip_pfx_list, self.ip6_pfx_list)):
            ...
            self.route_cfg.append('ipv6 route %s/128 %s' %
                                  (ip[1], self.bp_ip6))
            # remove route cfg
            self.no_route_cfg.append(
                'no ip route %s/32 %s' % (ip[0], self.bp_ip))
            self.no_route_cfg.append(
                'no ipv6 route %s/128 %s' % (ip[1], self.bp_ip6))
        self.route_cfg.append('router bgp %s' %
                              self.neigh_bgps[self.neigh_vms[-1]]['asn'])
        self.route_cfg.append('redistribute static')
        self.route_cfg.append('exit')
        self.no_route_cfg.append('router bgp %s' %
                                 self.neigh_bgps[self.neigh_vms[-1]]['asn'])
    ...
    def get_bgp_route_cnt(self, is_up=True, v4=True):
        ...
        if v4:
            cmd = (r'show ip bgp summary | sed \'1,/Neighbor/d;/^$/,$d;/^-/d\' '
                   r'| sed \'s/\s\s*/ /g\' | cut -d\' \' -f 1,10')
        else:
            cmd = (r'show ipv6 bgp summary | sed \'1,/Neighbor/d;/^$/,$d;/^-/d\' '
                   r'| sed \'s/\s\s*/ /g\' | cut -d\' \' -f 1,10')

        stdout, stderr, return_code = self.dut_connection.execCommand(cmd)
        if return_code != 0:
            self.fails['dut'].add(
                '%s: Failed to retreive BGP route info from DUT' % self.msg_prefix[1 - is_up])
            self.fails['dut'].add('%s: Return code: %d' %
                                  (self.msg_prefix[1 - is_up], return_code))
            self.fails['dut'].add('%s: Stderr: %s' %
                                  (self.msg_prefix[1 - is_up], stderr))
        ...
        if not ret:
            for line in neigh_rt_info:
                ...
                if int(rt_cnt) != exp_cnt:
                    self.fails['dut'].add('%s: Route cnt incorrect for neighbor %s Expected: %d Obtained: %d' % (
                        self.msg_prefix[is_up], neigh_ip, exp_cnt, int(rt_cnt)))
                else:
                    self.log.append(
                        'Route cnt as expected for neighbor %s: %d' % (neigh_ip, exp_cnt))
        ...
        if self.bp_ip:
            ...
            for vm in neigh_vms:
                if 'routing_add' in self.oper_type:
                    ...
                    self.log.append('Removing %d routes from VM %s' %
                                    (2 * self.ip_cnt, vm))
                    self.vm_handles[vm].change_bgp_route(self.no_route_cfg)
    ...
    def change_vlan_port_state(self, is_up=True):
        ...
        for intf, port in self.down_vlan_info:
            if not re.match(r'Ethernet\d+', intf):
                continue
            self.log.append(
                'Changing state of %s from DUT side to %s' % (intf, state[is_up]))
            stdout, stderr, return_code = self.dut_connection.execCommand(
                'sudo config interface %s %s' % (state[is_up], intf))
            if return_code != 0:
                self.fails['dut'].add('%s: State change not successful from DUT side for %s' % (
                    self.msg_prefix[1 - is_up], intf))
                self.fails['dut'].add('%s: Return code: %d' % (
                    self.msg_prefix[1 - is_up], return_code))
                self.fails['dut'].add('%s: Stderr: %s' %
                                      (self.msg_prefix[1 - is_up], stderr))
    ...
    def verify_vlan_port_state(self, state='down', pre_check=True):
        ...
        pat = re.compile(r'(\S+\s+){7}%s' % state)
        for intf, port in self.down_vlan_info:
            stdout, stderr, return_code = self.dut_connection.execCommand(
                'show interfaces status %s' % intf)
            if return_code == 0:
                for line in stdout:
                    if intf in line:
                        ...
                        if is_match:
                            self.log.append(
                                'Interface state is down as expected on the DUT for %s' % intf)
                            self.log.append('Pattern check: %s' % line)
                            break

                        else:
                            self.fails['dut'].add('%s: Interface state is not down on the DUT for %s' % (
                                self.msg_prefix[pre_check], intf))
                            self.fails['dut'].add('%s: Obtained: %s' % (
                                self.msg_prefix[pre_check], line))
            else:
                self.fails['dut'].add('%s: Retreiving interface %s info from DUT side failed' % (
                    self.msg_prefix[pre_check], intf))
                self.fails['dut'].add('%s: Return code: %d' % (
                    self.msg_prefix[pre_check], return_code))
                self.fails['dut'].add('%s: Stderr: %s' %
                                      (self.msg_prefix[pre_check], stderr))
    ...
    def change_bgp_dut_state(self, is_up=True):
        ...
        for vm in self.neigh_vms:
            for key in self.neigh_bgps[vm].keys():
                ...
                self.log.append('Changing state of BGP peer %s from DUT side to %s' % (
                    self.neigh_bgps[vm][key], state[is_up]))
                stdout, stderr, return_code = self.dut_connection.execCommand(
                    'sudo config bgp %s neighbor %s' % (state[is_up], self.neigh_bgps[vm][key]))
                if return_code != 0:
                    self.fails['dut'].add(
                        'State change not successful from DUT side for peer %s' % self.neigh_bgps[vm][key])
    ...
    def verify_bgp_dut_state(self, state='Idle'):
        ...
        for vm in self.neigh_vms:
            ...
            for key in self.neigh_bgps[vm].keys():
                if key not in ['v4', 'v6']:
                    continue
                self.log.append('Verifying if the DUT side BGP peer %s is %s' % (
                    self.neigh_bgps[vm][key], states))
                stdout, _, _ = self.dut_connection.execCommand(
                    'sudo sonic_installer list | grep Current | cut -f2 -d " "')
                if key == 'v4':
                ...
                if return_code == 0:
                    for line in stdout:
                        if 'BGP state' in line:
                            curr_state = re.findall(r'BGP state = (\w+)', line)[0]
                            bgp_state[vm][key] = (curr_state in states)
                            break
                else:
                    self.fails['dut'].add(
                        'Retreiving BGP info for peer %s from DUT side failed' % self.neigh_bgps[vm][key])
                    self.fails['dut'].add('Return code: %d' % return_code)
                    self.fails['dut'].add('Stderr: %s' % stderr)
        return bgp_state
    ...
    def sad_bgp_verify(self):
        self.log = []
        for vm in self.neigh_vms:
            fails_vm, bgp_state = self.vm_handles[vm].verify_bgp_neigh_state(
                dut=self.dut_needed[vm], state=self.neigh_bgps['changed_state'])
        ...
        bgp_state = self.verify_bgp_dut_state(
            state=self.dut_bgps['changed_state'])
        state = True
        for vm in self.neigh_vms:
    ...
    def populate_lag_member_down(self, neigh_name):
        ...
        if self.tot_memb_cnt != self.memb_cnt:
            self.lag_members_down[po_name] = [
                self.vm_dut_map[neigh_name]['dut_ports'][self.memb_index]]
            self.neigh_lag_members_down[neigh_name] = [
                self.vm_dut_map[neigh_name]['neigh_ports'][self.memb_index]]
        else:
    ...
    def change_dut_lag_state(self, is_up=True):
        ...
        for neigh_name in self.neigh_names.values():
            ...
            for intf in down_intfs:
                if not re.match(r'(PortChannel|Ethernet)\d+', intf):
                    continue
                self.log.append(
                    'Changing state of %s from DUT side to %s' % (intf, state[is_up]))
                stdout, stderr, return_code = self.dut_connection.execCommand(
                ...
                if return_code != 0:
                    ...
                    self.log.append(
                        'State change successful on DUT for %s' % intf)
    ...
    def verify_dut_lag_member_state(self, match, pre_check=True):
        ...
        for member in self.vm_dut_map[neigh_name]['dut_ports']:
            ...
            if lag_memb_output.find(search_str) != -1:
                ...
                success = False
                self.fails['dut'].add('%s: Lag member %s state not as expected' % (
                    self.msg_prefix[pre_check], member))
        return success

    def verify_dut_lag_state(self, pre_check=True):
        # pattern match eg: '0001  PortChannel0001  LACP(A)(Up)  Ethernet0(S) Ethernet4(S)'.
        # extract the portchannel name and members
        pat = re.compile(r"\s+\d+\s+(\w+\d+)\s+LACP\(A\)\(Dw\)\s+(.*)")
                ...
                for po_name in po_list:
                    if po_name in line:
                        ...
                        if is_match and self.verify_dut_lag_member_state(is_match, pre_check=pre_check):
                            self.log.append(
                                'Lag state is down as expected on the DUT for %s' % po_name)
                            self.log.append('Pattern check: %s' % line)
                        else:
                            self.fails['dut'].add('%s: Lag state is not down on the DUT for %s' % (
                                self.msg_prefix[pre_check], po_name))
                            self.fails['dut'].add('%s: Obtained: %s' % (
                                self.msg_prefix[pre_check], line))
        else:
            self.fails['dut'].add(
                '%s: Retreiving LAG info from DUT side failed' % self.msg_prefix[pre_check])
            self.fails['dut'].add('%s: Return code: %d' %
                                  (self.msg_prefix[pre_check], return_code))
            self.fails['dut'].add('%s: Stderr: %s' %
                                  (self.msg_prefix[pre_check], stderr))
    ...
    def sad_lag_verify(self, pre_check=True):
        for vm in self.neigh_vms:
            fails_vm, lag_state = self.vm_handles[vm].verify_neigh_lag_state(
                self.vm_dut_map[self.neigh_names[vm]]['neigh_portchannel'],
                state=self.neigh_lag_state, pre_check=pre_check)
            self.fails[vm] |= fails_vm
            if lag_state:
                self.log.append('LAG state down as expected for %s' % vm)
            else:
                self.fails[vm].add('%s: LAG state not down for %s' %
                                   (self.msg_prefix[pre_check], vm))
