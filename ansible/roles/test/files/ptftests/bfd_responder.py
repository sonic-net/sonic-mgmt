...
class BFD_Responder(BaseTest):
    ...
    def setUp(self):
        ...
        for ipaddr in self.dut_loop_ips:
            if isinstance(ip_address(unicode(ipaddr)), IPv4Address):
                self.dut_loop_ipv4 = ipaddr
            if isinstance(ip_address(unicode(ipaddr)), IPv6Address):
    ...
    def respond_to_packet(self, port_number, received_pkt):
       ...
       if count == 0:
           raise RuntimeError("send_packet failed args:port_number{}, dp_tuple:{}".format(port_number, port_number, str(pkt)))
    ...
    def runTest(self):
        while True:
            ...
            if not isinstance(result, self.dataplane.PollSuccess) or not result.port in ports_to_monitor or "UDP" not in Ether(result.packet):
                continue
            if Ether(result.packet)['UDP'].dport != 4784:
                continue
            received_pkt = result.packet
            port_number = result.port
            mac_src, mac_dst, ip_src, ip_dst,  bfd_remote_disc, bfd_state = self.extract_bfd_info(received_pkt)
            ...
            if bfd_state == 3L:
                ...
                if count == 0:
                    raise RuntimeError("send_packet failed args:port_number{}, dp_tuple:{}".format(port_number, port_number, str(session['pkt'])))
            ...
            bfd_pkt_init = self.craft_bfd_packet(session['my_disc'], received_pkt, mac_src, mac_dst, ip_src, ip_dst, bfd_remote_disc, 2L)
            count = send_packet(self, session['intf'], str(bfd_pkt_init))
            if count == 0:
                raise RuntimeError("send_packet failed args:port_number{}, dp_tuple:{}".format(port_number, port_number, str(bfd_pkt_init)))
    ...
    def craft_bfd_packet(self, my_discriminator, data, mac_src, mac_dst, ip_src, ip_dst, bfd_remote_disc, bfd_state):
        ethpart = Ether(data)
