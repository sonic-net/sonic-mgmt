...
class HashTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.src_ip_range = [x for x in self.test_params['src_ip_range'].split(',')]
        self.dst_ip_range = [x for x in self.test_params['dst_ip_range'].split(',')]
    ...
    def check_hash(self, hash_key):
        ...
        if hash_key == 'ingress-port':
            ...
            assert len(list(hit_count_map.keys())) == 1
        else:
    ...
    def check_ip_route(self, hash_key, src_port, dst_ip, dst_port_list):
        if ip_network(dst_ip).version == 4:
