...
class LpmDict():
    ...
    def __init__(self, ipv4=True):
        ...
        self._boundaries = { ip_address('0.0.0.0') : 1} if ipv4 else { ip_address('::') : 1}

    def __setitem__(self, key, value):
        prefix = ip_network(key)
        ...
        if prefix.prefixlen and key not in self._prefix_set:
            ...
            if prefix[-1] != ip_address('255.255.255.255') and prefix[-1] != ip_address('ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff'):
    ...
    def ranges(self):
        ...
        for index, boundary in enumerate(sorted_boundaries):
            if index != len(sorted_boundaries) - 1:
                ...
                if self._ipv4:
                    interval = self.IpInterval(sorted_boundaries[index], ip_address('255.255.255.255'))
                else:
                    interval = self.IpInterval(sorted_boundaries[index], ip_address('ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff'))
