"""
('ptf --test-dir ptftests fast-reboot --qlen=1000 --platform remote -t 'verbose=True;dut_username="admin";
  dut_hostname="10.0.0.243";reboot_limit_in_seconds=30;portchannel_ports_file="/tmp/portchannel_interfaces.json";
  vlan_ports_file="/tmp/vlan_interfaces.json";ports_file="/tmp/ports.json";dut_mac="4c:76:25:f5:48:80";
  default_ip_range="192.168.0.0/16";vlan_ip_range="{\"Vlan100\": \"172.0.0.0/22\"}";
  arista_vms="[\"10.0.0.200\",\"10.0.0.201\",\"10.0.0.202\",\"10.0.0.203\"]"'
  --platform-dir ptftests --disable-vxlan --disable-geneve --disable-erspan --disable-mpls --disable-nvgre')
"""
#
...
class ReloadTest(BaseTest):
    ...
    def get_neigh_port_info(self):
        ...
        for key in content.keys():
            if content[key]['name'] in self.vm_dut_map.keys():
                self.vm_dut_map[content[key]['name']
                                ]['dut_ports'].append(str(key))
                self.vm_dut_map[content[key]['name']]['neigh_ports'].append(
                    str(content[key]['port']))
                self.vm_dut_map[content[key]['name']]['ptf_ports'].append(
    ...
    def setUp(self):
        ...
        self.vlan_ip_range = ast.literal_eval(
            self.test_params['vlan_ip_range'])
        ...
        self.from_server_src_addr = random.choice(
            self.vlan_host_map[self.random_vlan].keys())
        self.from_server_src_mac = self.hex_to_mac(
            self.vlan_host_map[self.random_vlan][self.from_server_src_addr])
    ...
    def peer_state_check(self, ip, queue):
        ...
        self.log('lacp_pdu_down_times: IP:{}: {}'.format(
            ip, lacp_pdu_down_times))
        ...
        if ('warm-reboot' in self.reboot_type or 'service-warm-restart' in self.reboot_type)\
                and lacp_pdu_before_reboot and lacp_pdu_after_reboot:
