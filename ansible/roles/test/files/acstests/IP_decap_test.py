...
Description:    This file contains the Decapasulation test for SONIC,
                to test Decapsulation of IPv4 with double and triple encapsulated packets

                Design is available in https://github.com/sonic-net/SONiC/wiki/IPv4-Decapsulation-test

Precondition:   Before the test start, all routes need to be defined as in the route_info.txt file,
                in addition to the decap rule that need to be set as the dspc_mode
topology:         The test need to run on non-lag systems with at least 31 active ports

Usage:          Examples of how to start the test
                ptf  --test-dir /root/dor/ ip_decap_test_red --platform remote -t \
                "verbose=True;route_info='/tmp/route_info.txt';lo_ip='10.1.0.32';router_mac='00:02:03:04:05:00';\
                dscp_mode='pipe'" --log-dir /tmp/logs --verbose
Parameters:    route_info - The route_info file location
                       ...
                       dscp_mode - The rule for the dscp parameter in the decap packet that is configured
                                   in the JSON file ('pipe' for inner and 'uniform' for outer)
...
from ptf.testutils import simple_ip_only_packet
...
class DecapPacketTest(BaseTest, RouterUtility):
    ...
    def send_and_verify(self, dst_ip, expected_ports, src_port, triple_encap=False):
        ...
        @summary: This function builds encap packet, send and verify their arrival,
         When a packet will not arrived as expected an exeption will be throwen
        ...
        if (not triple_encap):
            ...
            # after the decap process the retuning packet will be normal tcp packet,
            # The TTL is taked from the inner layer and redused by one
            ...
            # Building triple encap packet with SCAPY, because there is no PTF function for it,
            # I use the defualt values for the TCP header
            ...
            inner_pkt = scapy.IP(src=inner_src_ip, dst=dst_ip, tos=tos_in,
                                 ttl=64, id=1, ihl=None, proto=4) / inner_pkt2
            inner_pkt = inner_pkt/("".join([chr(x) for x in range(100 - len(inner_pkt))]))
            # The expected packet is also built by scapy,
            # and the TTL is taked from the inner layer and redused by one
