...
class VMTopology(object):
    ...
    def init(self, vm_set_name, vm_base, duts_fp_ports, duts_name, ptf_exists=True):
        ...
        if 'devices_interconnect_interfaces' in self.topo:
            self.devices_interconnect_interfaces = self.topo['devices_interconnect_interfaces']
        else:
    ...
    def bind_devices_interconnect(self):
        for link_index, vlans in self.devices_interconnect_interfaces.items():
            ...
            (dut_index, vlan_index, ptf_index) = VMTopology.parse_vm_vlan_port(vlans[0])
            (dut_index_1, vlan_index_1, ptf_index_1) = VMTopology.parse_vm_vlan_port(vlans[-1])
            vlan1_iface = self.duts_fp_ports[self.duts_name[dut_index]][str(vlan_index)]
            vlan2_iface = self.duts_fp_ports[self.duts_name[dut_index_1]][str(vlan_index_1)]
    ...
    def bind_fp_ports_ovs(self, topo, duts_fp_ports, duts_name):
        ...
        """
        for ovs_int in topo['ovs_interfaces']:
            m = re.match("(\d+)\.(\d+)@(\d+)\.(\d+)", ovs_int)
            (dut_index, int_index, ovs_index, vlan_tag) = (int(m.group(1)), int(m.group(2)), int(m.group(3)), int(m.group(4)))
            br_name = "br-sonic-ovs" + str(ovs_index)
            VMTopology.cmd('ifconfig %s mtu 9200' % (duts_fp_ports[duts_name[dut_index]][str(int_index)]))
            if vlan_tag == 0:
                VMTopology.cmd('ovs-vsctl --may-exist add-port %s %s' % (br_name, duts_fp_ports[duts_name[dut_index]][str(int_index)]))
            else:
                VMTopology.cmd('ovs-vsctl --may-exist add-port %s %s tag=%d' % (br_name, duts_fp_ports[duts_name[dut_index]][str(int_index)], vlan_tag))
    ...
    def bind_fp_ports(self, disconnect_vm=False):
        ...
        for attr in self.VMs.values():
            for idx, vlan in enumerate(attr['vlans']):
                ...
                VMTopology.cmd('ifconfig %s mtu 9200' % (vm_iface))
                VMTopology.cmd('ifconfig %s mtu 9200' % (self.duts_fp_ports[self.duts_name[dut_index]][str(vlan_index)]))
...
def check_devices_interconnect(topo, is_mutli_dut=False):
    ...
    if 'devices_interconnect_interfaces' in topo:
        ...
        for key, vlans in links.items():
            for vlan in vlans:
                if is_mutli_dut:
                    condition = (isinstance(vlan, str) and re.match(r"^\d+\.\d+(@\d+)?$", vlan))
                    _assert(condition, ValueError,
                            "topo['devices_interconnect_interfaces'][%s] should be a "
                            "list of strings of format '<dut>.<dut_intf>' or '<dut>.<dut_intf>,<dut>.<dut_intf>'")
                else:
                    _assert(isinstance(vlan, int) and vlan >= 0, ValueError, "topo['devices_interconnect_interfaces'][%s] should be a list of integers" % key)
                _assert(vlan not in all_vlans, ValueError, "topo['devices_interconnect_interfaces'][%s] double use of vlan: %s" % (key, vlan))
                all_vlans.add(vlan)
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            cmd=dict(required=True, choices=['create', 'create_ovs', 'bind', 'bind_ovs', 'bind_keysight_api_server_ip', 'renumber', 'unbind', 'destroy', "connect-vms", "disconnect-vms"]),
    ...
    try:
        ...
        if cmd == 'create':
            ...
            net.create_bridges_ovs(duts_name, fp_mtu)
        elif cmd == 'destroy':
            net.destroy_bridges()
        elif cmd == 'bind_ovs':
            duts_name = module.params['duts_name']
            duts_fp_ports = module.params['duts_fp_ports']
            net.bind_fp_ports_ovs(topo, duts_fp_ports, duts_name)
        elif cmd == 'bind':
            ...
            devices_interconnect_exists = check_devices_interconnect(topo, is_multi_duts)
