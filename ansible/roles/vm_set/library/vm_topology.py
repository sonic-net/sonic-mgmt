...
class VMTopology(object):
    ...
    def bind_ovs_ports(self, br_name, dut_iface, injected_iface, vm_iface, disconnect_vm=False, **kwargs):
        ...
        if disconnect_vm:
            ...
            bind_helper("ovs-ofctl add-flow %s table=0,priority=10,udp,in_port=%s,udp_src=11364,action=output:%s" %
                        (br_name, dut_iface_id, vm_iface_id))
            bind_helper("ovs-ofctl add-flow %s table=0,priority=10,tcp,in_port=%s,tcp_src=11362,action=output:%s" %
                        (br_name, dut_iface_id, vm_iface_id))
