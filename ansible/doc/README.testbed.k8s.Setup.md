...
8. From `docker-sonic-mgmt` container, `cd` into `sonic-mgmt/ansible` directory and run `./testbed-cli.sh -m k8s_ubuntu [additional OPTIONS] create-master <k8s-server-name> ~/password.txt`
...
`./testbed-cli.sh -m k8s_ubuntu create-master k8s_server_19 ~/password.txt` 
...
- Run `./testbed-cli.sh -m k8s_ubuntu [additional OPTIONS] destroy-master <k8s-server-name> ~/password.txt`
- For HA Kubernetes master set 1 running on server 19 shown above, the proper command would be: 
`./testbed-cli.sh -m k8s_ubuntu destroy-master k8s_server_19 ~/password.txt` 
