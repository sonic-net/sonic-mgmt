...
def _resolve_localhost_python_interpreter():
    ...
    (PyYAML, ansible, etc.) available on the controller.

    Historically the localhost interpreter was hardcoded to
    ``/usr/bin/python3``. The new ``docker-sonic-mgmt`` image installs every
    dependency into a uv-managed venv at ``/opt/venv`` and leaves
