...
def gather_lldp(module, lldpctl_docker_cmd, skip_interface_pattern_list):
    ...
    skip_interface_pattern_str = "(?:% s)" % '|'.join(
        skip_interface_pattern_list) if skip_interface_pattern_list else None
...
def main():
    ...
    lldpctl_docker_cmd = "docker exec -i {} lldpctl -f keyvalue".format("lldp" + (
