...
def parse_pfc_counters(output, module):
    counters = dict()
...
def get_pfc_counters(module):
    ...
    while True:
        ...
        if rc != 0:
            module.fail_json(
                msg="Command pfcstat failed rc=%d, out=%s, err=%s" % (rc, out, err))
...
def clear_pfc_counters(module):
    ...
    if rc != 0:
        module.fail_json(
            msg="Command pfcstat -c failed rc=%d, out=%s, err=%s" % (rc, out, err))


def main():
    module = AnsibleModule(argument_spec=dict(
        method=dict(required=True)), supports_check_mode=False)

    method = module.params['method']
    if method == "get":
        counters = parse_pfc_counters(output=get_pfc_counters(module), module=module)
