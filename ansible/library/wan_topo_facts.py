...
class ParseTestbedTopoinfo():
    ...
    def get_topo_config(self, topo_name, hwsku, asic_name):
        ...
        # analyze wan dut port-channel configuration
        if 'wan_dut_configuration' in topo_definition:
            vm_topo_config['wan_dut_config'] = {}
            for dut in topo_definition['wan_dut_configuration']:
                vm_topo_config['wan_dut_config'][dut] = {}
                for p_intf in topo_definition['wan_dut_configuration'][dut]['interfaces']:
                    vm_topo_config['wan_dut_config'][dut][p_intf] = {}
                    vm_topo_config['wan_dut_config'][dut][p_intf]['attach_to'] = topo_definition['wan_dut_configuration'][dut]['interfaces'][p_intf]['attach_to']
                    vm_topo_config['wan_dut_config'][dut][p_intf]['ipv4'] = topo_definition['wan_dut_configuration'][dut]['interfaces'][p_intf]['ipv4']
                    vm_topo_config['wan_dut_config'][dut][p_intf]['ipv6'] = topo_definition['wan_dut_configuration'][dut]['interfaces'][p_intf]['ipv6']
                
                vm_topo_config['wan_dut_config'][dut]['connect'] = {}
                for dev in topo_definition['wan_dut_configuration'][dut]['connection']:
                    vm_topo_config['wan_dut_config'][dut]['connect'][dev] = topo_definition['wan_dut_configuration'][dut]['connection'][dev]

        # analyze wan bgp peer configuration
        if 'wan_bgp_config' in topo_definition:
            vm_topo_config['wan_bgp_config'] = {}
            for dut in topo_definition['wan_bgp_config']:
                vm_topo_config['wan_bgp_config'][dut] = {}
                for peer in topo_definition['wan_bgp_config'][dut]:
                   vm_topo_config['wan_bgp_config'][dut][peer] = {}
                   vm_topo_config['wan_bgp_config'][dut][peer]['asn'] = topo_definition['wan_bgp_config'][dut][peer]['asn']
                   vm_topo_config['wan_bgp_config'][dut][peer]['start_router'] = topo_definition['wan_bgp_config'][dut][peer]['start_router']
                   vm_topo_config['wan_bgp_config'][dut][peer]['start_peer'] = topo_definition['wan_bgp_config'][dut][peer]['start_peer']
                   vm_topo_config['wan_bgp_config'][dut][peer]['end_router'] = topo_definition['wan_bgp_config'][dut][peer]['end_router']
                   vm_topo_config['wan_bgp_config'][dut][peer]['end_peer'] = topo_definition['wan_bgp_config'][dut][peer]['end_peer']
        # analyze wan  isis configuration
        if 'wan_isis_config' in topo_definition:
            vm_topo_config['wan_isis_config'] = topo_definition['wan_isis_config']
