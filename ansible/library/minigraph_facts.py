...
def parse_dpg(dpg, hname):
    for child in dpg:
        ...
        for ipintf in ipintfs.findall(str(QName(ns, "IPInterface"))):
            ...
            (ipaddr, prefix_len) = ipprefix.split('/')
            mask_bytes = struct.pack("I", socket.htonl((0xffffffff << (32 - int(prefix_len))) & 0xffffffff))
            ipmask = socket.inet_ntoa(mask_bytes)

            ipaddr_bytes = socket.inet_aton(ipaddr)
            ipaddr_val = struct.unpack("I", ipaddr_bytes)
            ipmask_val = struct.unpack("I", mask_bytes)
            subnet_bytes = struct.pack("I", ipaddr_val[0] & ipmask_val[0])
            subnet = socket.inet_ntoa(subnet_bytes) + "/" + str(prefix_len)
            if intfname[0:4] == "Vlan":
                vlan_map.update({intfname: {'addr': ipaddr, 'mask': ipmask, 'subnet': subnet}})
            else:
                intf = {'name': intfname, 'addr': ipaddr, 'mask': ipmask, 'prefixlen': int(prefix_len),
                        'subnet': subnet}
                if port_alias_map.has_key(intfname):
                    intf['alias'] = port_alias_map[intfname]
                else:
                    intf['alias'] = intfname
                peer_addr_bytes = None
                if int(prefix_len) == 30:
                    if socket.ntohl(ipaddr_val[0]) & 0x3 == 1:
                        peer_addr_bytes = struct.pack("I", socket.htonl(socket.ntohl(ipaddr_val[0]) + 1))
                    else:
                        peer_addr_bytes = struct.pack("I", socket.htonl(socket.ntohl(ipaddr_val[0]) - 1))
                if int(prefix_len) == 31:
                    if socket.ntohl(ipaddr_val[0]) & 0x1 == 0:
                        peer_addr_bytes = struct.pack("I", socket.htonl(socket.ntohl(ipaddr_val[0]) + 1))
                    else:
                        peer_addr_bytes = struct.pack("I", socket.htonl(socket.ntohl(ipaddr_val[0]) - 1))
                if peer_addr_bytes:
                    intf['peer_addr'] = socket.inet_ntoa(peer_addr_bytes)
                intfs.append(intf)
        ...
        for lointf in lointfs.findall(str(QName(ns1, "LoopbackIPInterface"))):
            ...
            (ipaddr, prefix_len) = ipprefix.split('/')
            lo_intf = {'addr': ipaddr, 'prefixlen': prefix_len}
        ...
        for mgmtintf in mgmtintfs.findall(str(QName(ns1, "ManagementIPInterface"))):
            ...
            ipaddr_val = struct.unpack("I", ipaddr_bytes)
            ipmask_val = struct.unpack("I", mask_bytes)
            gwaddr_bytes = struct.pack("I", socket.htonl(socket.ntohl(ipaddr_val[0] & ipmask_val[0]) + 1))
            gwaddr = socket.inet_ntoa(gwaddr_bytes)
        ...
        for vintf in vlanintfs.findall(str(QName(ns, "VlanInterface"))):
            ...
            if vintfname in vlan_map:
                vlan_attributes.update(vlan_map[vintfname])
            vlan_intfs.append(vlan_attributes)

    return intfs, lo_intf, mgmt_intf, vlan_intfs, pc_intfs
    ...
    for child in cpg:
        ...
        if tag == str(QName(ns, "PeeringSessions")):
            for session in child.findall(str(QName(ns, "BGPSession"))):
                ...
                if end_router == hname:
                    bgp_sessions.append({'name': start_router, 'addr': start_peer, 'peer_addr': end_peer})
                else:
                    bgp_sessions.append({'name': end_router, 'addr': end_peer, 'peer_addr': start_peer})
...
def reconcile_mini_graph_locations(filename, hostname):
    ...
    if filename is not None:
        ...
        if os.path.exists(mini_graph_path) and file_age(mini_graph_path) < ANSIBLE_USER_MINIGRAPH_MAX_AGE:
            ...
            # No recent cached graph found -- initiate a connection to NGS and download the mini-graph.
            mini_graph_string = ngs_lib.NGSConnection().ReadDeviceMiniGraph(hostname=hostname)
            root = ET.fromstring(mini_graph_string)
            # perform the additional work of writing the mini-graph to cache.
            root.getroottree().write(mini_graph_path, pretty_print=True)
...
def parse_xml(filename, hostname):
    ...
    for child in root:
        if child.tag == str(QName(ns, "DpgDec")):
            (intfs, lo_intf, mgmt_intf, vlan_intfs, pc_intfs) = parse_dpg(child, hostname)
        elif child.tag == str(QName(ns, "CpgDec")):
            (bgp_sessions, bgp_asn) = parse_cpg(child, hostname)
        elif child.tag == str(QName(ns, "PngDec")):
            (neighbors, devices, console_dev, console_port) = parse_png(child, hostname)
        elif child.tag == str(QName(ns, "UngDec")):
            (u_neighbors, u_devices, _, _) = parse_png(child, hostname)
    ...
    results['minigraph_lo_interface'] = lo_intf
...
def main():
    ...
    except ngs_lib.exceptions.NgsConnectionError as e:
        # all attempts to find a minigraph failed.
        module.fail_json(msg=e.message)


def debug_main():
    hostname = 'CO4SCH03001BBLF'
