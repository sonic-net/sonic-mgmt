...
#    host = 'str-s6000-on-4'
    ...
    results['device_conn'] = lab_graph.get_host_connections(host)
    if lab_graph.get_host_vlan(host):
        results['device_vlan_range'] = lab_graph.get_host_vlan(host)['VlanRange']
        results['device_vlan_list'] = lab_graph.get_host_vlan(host)['VlanList']
    results['device_port_vlans'] = lab_graph.get_host_port_vlans(host)
    import json
    print(json.dumps(results['device_port_vlans'], indent=2))
