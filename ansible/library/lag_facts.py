...
class LagModule(object):
    ...
    def run(self):
        ...
        self.module.exit_json(
            ansible_facts={'lag_facts': {'names': self.lag_names, 'lags': self.lags}})
        return
    ...
    def get_po_status(self, po_name, ns):
        ...
        '''
        rt, out, err = self.module.run_command(
            "docker exec -i teamd{} teamdctl ".format(ns) + po_name+" state dump")
        if rt != 0:
            fail_msg = "failed dump port channel %s status return=%d, out=%s, err=%s" % (
                po_name, rt, out, err)
            self.module.fail_json(msg=fail_msg)
    ...
    def get_po_config(self, po_name, ns):
        ...
        '''
        rt, out, err = self.module.run_command(
            "docker exec -i teamd{} teamdctl ".format(ns) + po_name+" config dump")
        if rt != 0:
            fail_msg = "failed dump port channel %s config return=%d, out=%s, err=%s" % (
                po_name, rt, out, err)
            self.module.fail_json(msg=fail_msg)
        json_info = json.loads(out)
        return json_info
    ...
    def get_po_intf_stat(self, po_name, ns):
        ...
        '''
        rt, out, err = self.module.run_command(
            "sudo ip {} link show ".format('-n ' + ns if ns else '') + po_name)
        if rt != 0:
            fail_msg = "failed show interface status of %s return=%d, out=%s, err=%s" % (
                po_name, rt, out, err)
            self.module.fail_json(msg=fail_msg)
